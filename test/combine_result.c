...
static int combine_stat_overall(int *ptotal_count, int *psuccess_count,
                                int *ptime_used);
static int combine_stat_by(const char *file_prefix, EntryStat *stats,
                           const int max_entries, int *entry_count);
...
int main(int argc, char **argv) {
    ...
    printf(
        "total_count=%d, success_count=%d, success ratio: %.2f%% "
        "time_used=%ds, avg time used: %dms, QPS=%.2f\n\n",
        total_count, success_count,
        total_count > 0 ? 100.00 * success_count / total_count : 0.00,
        time_used, total_count > 0 ? time_used * 1000 / total_count : 0,
        time_used == 0 ? 0 : (double)success_count / time_used);

    if (combine_stat_by(STAT_FILENAME_BY_FILE_TYPE, stats, FILE_TYPE_COUNT,
                        &entry_count) == 0) {
        printf(
            "file_type total_count success_count time_used(s) avg(ms) QPS "
            "success_ratio\n");
    ...
    if (combine_stat_by(STAT_FILENAME_BY_STORAGE_IP, stats, FILE_TYPE_COUNT,
                        &entry_count) == 0) {
        printf(
            "ip_addr  total_count success_count time_used(s) avg(ms) QPS "
            "success_ratio\n");
        print_stat_by(stats, entry_count);
        printf("\n");
    }
...
static void print_stat_by(EntryStat *stats, const int entry_count) {
    ...
    for (pEntry = stats; pEntry < pEnd; pEntry++) {
        ...
        printf("%s %d %d %d %d %.2f %.2f\n", pEntry->id, pEntry->total_count,
               ...
               pEntry->total_count == 0 ? 0 : (int)(pEntry->time_used /
                                                    pEntry->total_count),
               seconds == 0 ? 0 : (double)pEntry->success_count / seconds,
               pEntry->total_count > 0
                   ? 100.00 * pEntry->success_count / pEntry->total_count
                   : 0.00);
    }
}
...
static int combine_stat_by(const char *file_prefix, EntryStat *stats,
    ...
    *entry_count = 0;
    ...
    for (proccess_index = 0; proccess_index < proccess_count;
         proccess_index++) {
        ...
        while (fgets(buff, sizeof(buff), fp) != NULL) {
            ...
            if (sscanf(buff, "%s %d %d %" PRId64, id, &total_count,
                       &success_count, &time_used) != 4) {
                if (*buff == ' ')  // empty id (eg. storage ip)
                {
                    *id = '\0';
                    if (sscanf(buff + 1, "%d %d %" PRId64, &total_count,
                               &success_count, &time_used) != 3) {
    ...
    *ptime_used = 0;
    ...
    for (proccess_index = 0; proccess_index < proccess_count;
        ...
        while (fgets(buff, sizeof(buff), fp) != NULL) {
            ...
            if (sscanf(buff, "%d %d %d", &total_count, &success_count,
                       &time_used) != 3) {
                printf("sscanf %s fail, errno: %d, error info: %s\n", filename,
                       errno, STRERROR(errno));
                return errno != 0 ? errno : EINVAL;
