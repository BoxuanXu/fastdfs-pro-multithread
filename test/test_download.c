...
int main(int argc, char **argv) {
    ...
    file_entries[file_count-1].file_id);
    */
    ...
    while (time(NULL) - start_time < TOTAL_SECONDS) {
        ...
        *storage_ip = '\0';
        result = download_file(file_entries[file_index].file_id, &file_size,
                               storage_ip);
...
static int save_stats_by_file_type() {
    ...
    for (k = 0; k < FILE_TYPE_COUNT; k++) {
        fprintf(fp, "%s %d %d %" PRId64 "\n", files[k].filename,
                files[k].download_count, files[k].success_count,
                files[k].time_used);
    }
...
static int load_file_ids() {
    ...
    if (file_entries == NULL) {
        printf(
            ...
            ", line: %d, "
            "malloc %d bytes fail\n",
            __LINE__, (int)sizeof(FileEntry) * file_count);
        free(file_buff);
        return ENOMEM;
    }
