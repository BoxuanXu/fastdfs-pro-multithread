...
static int add_to_storage_stat(const char *storage_ip, const int result,
                               const int time_used);
    ...
    while (total_count < upload_count) {
        ...
        *storage_ip = '\0';
        ...
        result = upload_file(files[file_index].file_buff,
                             files[file_index].bytes, file_id, storage_ip);
        ...
        if (result == 0)  // success
        {
            ...
            fprintf(fpSuccess, "%d %d %s %s %d\n", (int)tv_end.tv_sec,
                    files[file_index].bytes, file_id, storage_ip, time_used);
        } else  // fail
        {
...
static int save_stats_by_file_type() {
    ...
    for (k = 0; k < FILE_TYPE_COUNT; k++) {
        fprintf(fp, "%s %d %d %" PRId64 "\n", files[k].filename,
                files[k].upload_count, files[k].success_count,
                files[k].time_used);
    }
    ...
    for (i = 0; i < FILE_TYPE_COUNT; i++) {
        ...
        if (files[i].fd < 0) {
            fprintf(stderr,
                    ...
                    __LINE__, files[i].filename, errno, STRERROR(errno));
        ...
        if (file_size != files[i].bytes) {
            fprintf(stderr,
                    ...
                    ", line: %d, "
                    "%s file size: %d != %d\n",
                    __LINE__, files[i].filename, (int)file_size,
                    files[i].bytes);
        ...
        files[i].file_buff =
            mmap(NULL, file_size, PROT_READ, MAP_SHARED, files[i].fd, 0);
