...
static ConnectionInfo *getConnectedStorageServer(ConnectionInfo *pStorageServer,
                                                 int *err_no) {
    ConnectionInfo *pEnd;
    ...
    for (pServer = storage_servers; pServer < pEnd; pServer++) {
        if (strcmp(pStorageServer->ip_addr, pServer->ip_addr) == 0) {
            if (pServer->sock < 0) {
                *err_no =
                    conn_pool_connect_server(pServer, g_fdfs_connect_timeout);
                if (*err_no != 0) {
                    return NULL;
                }
            } else {
    ...
    pServer->sock = -1;
    if ((*err_no = conn_pool_connect_server(pServer, g_fdfs_connect_timeout)) !=
        0) {
        return NULL;
    }
...
int upload_file(const char *file_buff, const int file_size, char *file_id,
    ...
    *group_name = '\0';
    ...
    if ((pStorageServer = getConnectedStorageServer(&storageServer, &result)) ==
        NULL) {
        return result;
    }
...
int download_file(const char *file_id, int *file_size, char *storage_ip) {
    ...
    if ((result = tracker_query_storage_fetch1(pTrackerServer, &storageServer,
                                               file_id)) != 0) {
        return result;
    }
    ...
    strcpy(storage_ip, storageServer.ip_addr);
    result =
        storage_download_file_ex1(pTrackerServer, pStorageServer, file_id, 0, 0,
                                  downloadFileCallback, NULL, &file_bytes);
...
int delete_file(const char *file_id, char *storage_ip) {
    ...
    if ((result = tracker_query_storage_update1(pTrackerServer, &storageServer,
                                                file_id)) != 0) {
        return result;
    }
