...
#define CHECK_CONNECTION(pTrackerServer, conn, result, new_connection)      \
    do {                                                                    \
        if (pTrackerServer->sock < 0) {                                     \
            if ((conn = tracker_connect_server(pTrackerServer, &result)) != \
                NULL) {                                                     \
                return result;                                              \
            }                                                               \
            new_connection = true;                                          \
...
#define tracker_query_storage_store(pTrackerServer, pStorageServer,           \
    ...
    tracker_query_storage_store_without_group(pTrackerServer, pStorageServer, \
                                              group_name, store_path_index)
/**
...
#define tracker_query_storage_store_list_without_group(                       \
    pTrackerServer, storageServers, nMaxServerCount, storage_count,           \
    group_name, store_path_index)                                             \
...
* return: 0 success, !=0 fail, return the error code
**/
#define tracker_query_storage_update(pTrackerServer, pStorageServer, \
                                     group_name, filename)           \
    tracker_do_query_storage(pTrackerServer, pStorageServer,         \
...
* return: 0 success, !=0 fail, return the error code
**/
#define tracker_query_storage_fetch(pTrackerServer, pStorageServer,     \
                                    group_name, filename)               \
    tracker_do_query_storage(pTrackerServer, pStorageServer,            \
