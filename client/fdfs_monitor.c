...
static void usage(char *argv[]) {
    printf(
        "Usage: %s <config_file> [-h <tracker_server>] "
        "[list|delete|set_trunk_server <group_name> "
        "[storage_id]]\n",
        argv[0]);
}
...
int main(int argc, char *argv[]) {
    ...
    if (tracker_server == NULL) {
        if (g_tracker_group.server_count > 1) {
            ...
            g_tracker_group.server_index =
                (int)((g_tracker_group.server_count * (double)rand()) /
                      (double)RAND_MAX);
        }
    } else {
        int i;
        ...
        *ip_addr = '\0';
        if (getIpaddrByName(tracker_server, ip_addr, sizeof(ip_addr)) ==
            INADDR_NONE) {
            printf(
                "resolve ip address of tracker server: %s "
                "fail!\n",
                tracker_server);
            return 2;
        }
        ...
        for (i = 0; i < g_tracker_group.server_count; i++) {
            if (strcmp(g_tracker_group.servers[i].ip_addr, ip_addr) == 0) {
    ...
    if (strcmp(op_type, "list") == 0) {
        ...
        if (arg_index >= argc) {
            if ((result = tracker_delete_group(&g_tracker_group, group_name)) ==
                0) {
            ...
            if ((result = tracker_delete_storage(&g_tracker_group, group_name,
                                                 storage_id)) == 0) {
        ...
        if ((result = tracker_set_trunk_server(&g_tracker_group, group_name,
                                               storage_id,
                                               new_trunk_server_id)) == 0) {
            printf(
                "set trunk server %s::%s success, "
                "new trunk server: %s\n",
                group_name, storage_id, new_trunk_server_id);
        } else {
            printf(
                "set trunk server %s::%s fail, "
                "error no: %d, error info: %s\n",
                group_name, storage_id, result, STRERROR(result));
        }
    } else {
...
static int list_storages(FDFSGroupStat *pGroupStat) {
    ...
    printf(
        "group name = %s\n"
        "disk total space = %" PRId64
        " MB\n"
        "disk free space = %" PRId64
        " MB\n"
        "trunk free space = %" PRId64
        " MB\n"
        ...
        pGroupStat->group_name, pGroupStat->total_mb, pGroupStat->free_mb,
        pGroupStat->trunk_free_mb, pGroupStat->count, pGroupStat->active_count,
        pGroupStat->storage_port, pGroupStat->storage_http_port,
        pGroupStat->store_path_count, pGroupStat->subdir_count_per_path,
        pGroupStat->current_write_server, pGroupStat->current_trunk_file_id);

    result = tracker_list_servers(pTrackerServer, pGroupStat->group_name, NULL,
    ...
    for (pStorage = storage_infos; pStorage < pStorageEnd; pStorage++) {
        ...
        for (p = storage_infos; p < pStorageEnd; p++) {
            if (p != pStorage &&
                p->stat.last_source_update > max_last_source_update) {
            ...
            *szSyncedDelaySeconds = '\0';
            ...
            if (pStorageStat->last_synced_timestamp == 0) {
                ...
                if (day != 0) {
                    ...
                    sprintf(szDelayTime, "%02dh:%02dm:%02ds", hour, minute,
                            second);
                } else if (minute != 0) {
        ...
        if (pStorage->up_time != 0) {
            formatDatetime(pStorage->up_time, "%Y-%m-%d %H:%M:%S", szUpTime,
                           sizeof(szUpTime));
        } else {
            *szUpTime = '\0';
        }
        ...
        printf(
            ...
            ++k, pStorage->id, pStorage->ip_addr, szHostnamePrompt,
            get_storage_status_caption(pStorage->status), pStorage->domain_name,
            pStorage->version,
            formatDatetime(pStorage->join_time, "%Y-%m-%d %H:%M:%S", szJoinTime,
                           sizeof(szJoinTime)),
            szUpTime, pStorage->total_mb, pStorage->free_mb,
            pStorage->upload_priority, pStorage->store_path_count,
            pStorage->subdir_count_per_path, pStorage->storage_port,
            pStorage->storage_http_port, pStorage->current_write_path,
            pStorage->src_id, pStorage->if_trunk_server,
...
static int list_all_groups(const char *group_name) {
    ...
    result = tracker_list_groups(pTrackerServer, group_stats, FDFS_MAX_GROUPS,
                                 &group_count);
    if (result != 0) {
