...
int tracker_get_all_connections_ex(TrackerServerGroup *pTrackerGroup) {
    ...
    for (pServer = pTrackerGroup->servers; pServer < pEnd; pServer++) {
        if (pServer->sock >= 0) {
            success_count++;
        } else if (conn_pool_connect_server(pServer, g_fdfs_connect_timeout) ==
                   0) {
...
ConnectionInfo *tracker_get_connection_ex(TrackerServerGroup *pTrackerGroup) {
    ...
    do {
        ...
        for (pServer = pTrackerGroup->servers; pServer < pCurrentServer;
             pServer++) {
            if ((conn = tracker_connect_server(pServer, &result)) != NULL) {
                pTrackerGroup->server_index = pServer - pTrackerGroup->servers;
                break;
...
ConnectionInfo *tracker_get_connection_r_ex(TrackerServerGroup *pTrackerGroup,
    ...
    do {
        ...
        for (pServer = pCurrentServer + 1; pServer < pEnd; pServer++) {
            ...
            pTrackerServer->sock = -1;
            if ((conn = tracker_connect_server(pTrackerServer, err_no)) !=
                NULL) {
                pTrackerGroup->server_index = pServer - pTrackerGroup->servers;
                break;
            }
...
int tracker_list_servers(ConnectionInfo *pTrackerServer,
    ...
    if ((result = tcpsenddata_nb(
        ...
        pInBuff = (char *)stats;
        result = fdfs_recv_response(conn, &pInBuff, sizeof(stats), &in_bytes);
        if (result != 0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
    ...
    *storage_count = in_bytes / sizeof(TrackerStorageStat);
    ...
    for (pSrc = stats; pSrc < pEnd; pSrc++) {
        ...
        memcpy(pDest->src_id, pSrc->src_id, FDFS_STORAGE_ID_MAX_SIZE - 1);
        ...
        pDest->subdir_count_per_path =
            buff2long(pSrc->sz_subdir_count_per_path);
        pDest->storage_port = buff2long(pSrc->sz_storage_port);
        pDest->storage_http_port = buff2long(pSrc->sz_storage_http_port);
        pDest->current_write_path = buff2long(pSrc->sz_current_write_path);
        ...
        pStorageStat->last_synced_timestamp =
            buff2long(pStatBuff->sz_last_synced_timestamp);
        ...
        pStorageStat->last_heart_beat_time =
            buff2long(pStatBuff->sz_last_heart_beat_time);
...
int tracker_list_one_group(ConnectionInfo *pTrackerServer,
    ...
    if ((result = tcpsenddata_nb(conn->sock, out_buff, sizeof(out_buff),
        ...
        pInBuff = (char *)&src;
        result = fdfs_recv_response(conn, &pInBuff, sizeof(TrackerGroupStat),
                                    &in_bytes);
        if (result != 0) {
            logError(
                "file: "__FILE__
    ...
    *group_count = in_bytes / sizeof(TrackerGroupStat);
    if (*group_count > max_groups) {
        logError(
            ...
            __LINE__, pTrackerServer->ip_addr, pTrackerServer->port, max_groups,
            *group_count);
        *group_count = 0;
        return ENOSPC;
    }
    ...
    for (pSrc = stats; pSrc < pEnd; pSrc++) {
        ...
        pDest->subdir_count_per_path =
            buff2long(pSrc->sz_subdir_count_per_path);
        pDest->current_trunk_file_id =
            buff2long(pSrc->sz_current_trunk_file_id);
...
int tracker_do_query_storage(ConnectionInfo *pTrackerServer,
    ...
    if ((result = tcpsenddata_nb(
        ...
        pInBuff = in_buff;
        result = fdfs_recv_response(conn, &pInBuff, sizeof(in_buff), &in_bytes);
        if (result != 0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
    ...
    memcpy(pStorageServer->ip_addr, in_buff + FDFS_GROUP_NAME_MAX_LEN,
           IP_ADDRESS_SIZE - 1);
    pStorageServer->port =
        (int)buff2long(in_buff + FDFS_GROUP_NAME_MAX_LEN + IP_ADDRESS_SIZE - 1);
    return 0;
}
...
int tracker_query_storage_list(ConnectionInfo *pTrackerServer,
                               ...
                               char *group_name, const char *filename) {
    TrackerHeader *pHeader;
    ConnectionInfo *pServer;
    ...
    *server_count = 1 +
                    (in_bytes - TRACKER_QUERY_STORAGE_FETCH_BODY_LEN) /
                        (IP_ADDRESS_SIZE - 1);
...
int tracker_query_storage_store_without_group(ConnectionInfo *pTrackerServer,
    ...
    *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    pStorageServer->port =
        (int)buff2long(in_buff + FDFS_GROUP_NAME_MAX_LEN + IP_ADDRESS_SIZE - 1);
    *store_path_index = *(in_buff + FDFS_GROUP_NAME_MAX_LEN + IP_ADDRESS_SIZE -
...
int tracker_query_storage_store_with_group(ConnectionInfo *pTrackerServer,
    ...
    if ((result =
             tcpsenddata_nb(conn->sock, out_buff,
...
int tracker_query_storage_store_list_with_group(ConnectionInfo *pTrackerServer,
                                                const char *group_name,
                                                ConnectionInfo *storageServers,
                                                const int nMaxServerCount,
                                                int *storage_count,
                                                int *store_path_index) {
    ConnectionInfo *pStorageServer;
    ConnectionInfo *pServerEnd;
        ...
        if ((result =
                 tcpsenddata_nb(conn->sock, out_buff,
                                sizeof(TrackerHeader) +
                                    FDFS_GROUP_NAME_MAX_LEN + storage_id_len,
                                g_fdfs_network_timeout)) != 0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
...
int tracker_set_trunk_server(TrackerServerGroup *pTrackerGroup,
    ...
    *new_trunk_server_id = '\0';
    ...
    for (pServer = pTrackerGroup->servers; pServer < pEnd; pServer++) {
        ...
        pInBuff = in_buff;
        result =
            fdfs_recv_response(conn, &pInBuff, sizeof(in_buff) - 1, &in_bytes);
...
int tracker_get_storage_status(ConnectionInfo *pTrackerServer,
    ...
    if ((result = tcpsenddata_nb(conn->sock, out_buff, p - out_buff,
                                 g_fdfs_network_timeout)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
        ...
        result = fdfs_recv_response(conn, &pInBuff, sizeof(FDFSStorageBrief),
                                    &in_bytes);
        if (result != 0) {
            logError(
                "file: "__FILE__
...
int tracker_get_storage_id(ConnectionInfo *pTrackerServer,
    ...
    if ((result = tcpsenddata_nb(conn->sock, out_buff, p - out_buff,
        ...
        result = fdfs_recv_response(conn, &storage_id, FDFS_STORAGE_ID_MAX_SIZE,
                                    &in_bytes);
        if (result != 0) {
            logError(
                "file: "__FILE__
...
int tracker_get_storage_max_status(TrackerServerGroup *pTrackerGroup,
    ...
    *status = -1;
    ...
    for (pServer = pTrackerGroup->servers; pServer < pEnd; pServer++) {
        ...
        result = tracker_get_storage_status(conn, group_name, ip_addr,
                                            &storage_brief);
        tracker_disconnect_server_ex(conn, result != 0);
