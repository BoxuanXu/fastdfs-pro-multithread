...
static int storage_cmp_by_ip_and_port(const void *p1, const void *p2) {
    ...
    return ((ConnectionInfo *)p1)->port - ((ConnectionInfo *)p2)->port;
}
...
static void insert_into_sorted_servers(TrackerServerGroup *pTrackerGroup,
    ...
    for (pDestServer = pTrackerGroup->servers + pTrackerGroup->server_count;
        ...
        if (storage_cmp_by_ip_and_port(pInsertedServer, pDestServer - 1) > 0) {
...
static int copy_tracker_servers(TrackerServerGroup *pTrackerGroup,
    ...
    for (ppSrc = ppTrackerServers; ppSrc < ppEnd; ppSrc++) {
        ...
        if (bsearch(&destServer, pTrackerGroup->servers,
                    pTrackerGroup->server_count, sizeof(ConnectionInfo),
...
int fdfs_load_tracker_group_ex(TrackerServerGroup *pTrackerGroup,
    ...
    if ((pTrackerGroup->server_count =
             iniGetValues(NULL, "tracker_server", pIniContext, ppTrackerServers,
                          FDFS_MAX_TRACKERS)) <= 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if (pTrackerGroup->servers == NULL) {
        logError(
            ...
            "malloc %d bytes fail",
            __LINE__,
            (int)sizeof(ConnectionInfo) * pTrackerGroup->server_count);
        pTrackerGroup->server_count = 0;
...
int fdfs_load_tracker_group(TrackerServerGroup *pTrackerGroup,
    ...
    result =
        fdfs_load_tracker_group_ex(pTrackerGroup, conf_filename, &iniContext);
...
static int fdfs_get_params_from_tracker(bool *use_storage_id) {
    ...
    if ((result = fdfs_get_ini_context_from_tracker(
             &g_tracker_group, &iniContext, &continue_flag, false, NULL)) !=
        0) {
        return result;
    }
    ...
    *use_storage_id =
    ...
    if (*use_storage_id) {
        result = fdfs_get_storage_ids_from_tracker_group(&g_tracker_group);
    }
...
static int fdfs_client_do_init_ex(TrackerServerGroup *pTrackerGroup,
    ...
    g_fdfs_connect_timeout = iniGetIntValue(NULL, "connect_timeout", iniContext,
                                            DEFAULT_CONNECT_TIMEOUT);
    if (g_fdfs_connect_timeout <= 0) {
        g_fdfs_connect_timeout = DEFAULT_CONNECT_TIMEOUT;
    }

    g_fdfs_network_timeout = iniGetIntValue(NULL, "network_timeout", iniContext,
                                            DEFAULT_NETWORK_TIMEOUT);
    if (g_fdfs_network_timeout <= 0) {
        g_fdfs_network_timeout = DEFAULT_NETWORK_TIMEOUT;
    }

    if ((result = fdfs_load_tracker_group_ex(pTrackerGroup, conf_filename,
                                             iniContext)) != 0) {
        return result;
    }

    g_anti_steal_token =
        iniGetBoolValue(NULL, "http.anti_steal.check_token", iniContext, false);
    if (g_anti_steal_token) {
        char *anti_steal_secret_key;

        anti_steal_secret_key =
            iniGetStrValue(NULL, "http.anti_steal.secret_key", iniContext);
    ...
    g_tracker_server_http_port =
        iniGetIntValue(NULL, "http.tracker_server_port", iniContext, 80);
    ...
    load_fdfs_parameters_from_tracker = iniGetBoolValue(
        NULL, "load_fdfs_parameters_from_tracker", iniContext, false);
    if (load_fdfs_parameters_from_tracker) {
        ...
        use_storage_id =
            iniGetBoolValue(NULL, "use_storage_id", iniContext, false);
        if (use_storage_id) {
            result = fdfs_load_storage_ids_from_file(conf_filename, iniContext);
        }
    }
    ...
    logDebug(
        ...
        g_anti_steal_secret_key.length, g_use_connection_pool,
        g_connection_pool_max_idle_time, use_storage_id, g_storage_id_count);
#endif
...
int fdfs_client_init_ex(TrackerServerGroup *pTrackerGroup,
    ...
    result = fdfs_client_do_init_ex(pTrackerGroup, conf_filename, &iniContext);
    iniFreeContext(&iniContext);
    return result;
}
...
int fdfs_copy_tracker_group(TrackerServerGroup *pDestTrackerGroup,
    ...
    pDestServerEnd =
        pDestTrackerGroup->servers + pDestTrackerGroup->server_count;
    for (pDestServer = pDestTrackerGroup->servers; pDestServer < pDestServerEnd;
         pDestServer++) {
        pDestServer->sock = -1;
    }
