...
int main(int argc, char *argv[]) {
    ...
    printf(
        "This is FastDFS client test program v%d.%02d\n"
        "\nCopyright (C) 2008, Happy Fish / YuQing\n"
        "\nFastDFS may be copied only under the terms of the GNU General\n"
        "Public License V3, which may be found in the FastDFS source kit.\n"
    ...
    if (strcmp(operation, "upload") == 0) {
...

        {
            ...
            if ((result = tracker_query_storage_store_list_without_group(
                     ...
                     FDFS_MAX_SERVERS_EACH_GROUP, &storage_count, group_name,
                     &store_path_index)) == 0) {
                printf("tracker_query_storage_store_list_without_group: \n");
                pServerEnd = storageServers + storage_count;
                for (pServer = storageServers; pServer < pServerEnd;
                     pServer++) {
                    printf(
                        "\tserver %d. group_name=%s, "
                        "ip_addr=%s, port=%d\n",
                        (int)(pServer - storageServers) + 1, group_name,
        ...
        *group_name = '\0';
        ...
        if (upload_type == FDFS_UPLOAD_BY_FILE) {
            ...
            char *file_content;
            if ((result = getFileContent(local_filename, &file_content,
                                         &file_size)) == 0) {
                result = storage_upload_by_filebuff(
                    pTrackerServer, pStorageServer, store_path_index,
                    file_content, file_size, file_ext_name, meta_list,
            ...
            if (stat(local_filename, &stat_buf) == 0 &&
                ...
                result = storage_upload_by_callback(
                    pTrackerServer, pStorageServer, store_path_index,
                    uploadFileCallback, local_filename, file_size,
        ...
        url_len = sprintf(file_url, "http://%s%s/%s", pStorageServer->ip_addr,
                          szPortPart, file_id);
        if (g_anti_steal_token) {
            ts = time(NULL);
            fdfs_http_gen_token(&g_anti_steal_secret_key, file_id, ts, token);
        ...
        *remote_filename = '\0';
        if (upload_type == FDFS_UPLOAD_BY_FILE) {
            ...
            result = storage_upload_slave_by_filename(
                pTrackerServer, NULL, local_filename, master_filename,
            ...
            if (stat(local_filename, &stat_buf) == 0 &&
                ...
                result = storage_upload_slave_by_callback(
                    pTrackerServer, NULL, uploadFileCallback, local_filename,
        ...
        if (fdfs_gen_slave_filename(master_filename, prefix_name, file_ext_name,
        ...
        snprintf(remote_filename, sizeof(remote_filename), "%s", argv[4]);
        if (strcmp(operation, "setmeta") == 0 ||
            strcmp(operation, "delete") == 0) {
            result = tracker_query_storage_update(
                pTrackerServer, &storageServer, group_name, remote_filename);
        } else if (strcmp(operation, "query_servers") == 0) {
            ConnectionInfo storageServers[FDFS_MAX_SERVERS_EACH_GROUP];
            int server_count;

            result = tracker_query_storage_list(
                pTrackerServer, storageServers, FDFS_MAX_SERVERS_EACH_GROUP,
                &server_count, group_name, remote_filename);

            if (result != 0) {
                printf(
                    "tracker_query_storage_list fail, "
            ...
            result = tracker_query_storage_fetch(pTrackerServer, &storageServer,
                                                 group_name, remote_filename);
        }
        ...
        if (result != 0) {
            fdfs_client_destroy();
            printf(
                "tracker_query_storage_fetch fail, "
                "group_name=%s, filename=%s, "
                "error no: %d, error info: %s\n",
                group_name, remote_filename, result, STRERROR(result));
            return result;
        ...
        if (strcmp(operation, "download") == 0) {
            if (argc >= 6) {
                ...
                if (strcmp(local_filename, "CALLBACK") == 0) {
                    ...
                    if (fp == NULL) {
                        ...
                        result = storage_download_file_ex(
                            pTrackerServer, pStorageServer, group_name,
                            remote_filename, 0, 0, writeToFileCallback, fp,
                            &file_size);
                        fclose(fp);
                    }
                } else {
                ...
                if ((result = storage_download_file_to_buff(
                         pTrackerServer, pStorageServer, group_name,
                         remote_filename, &file_buff, &file_size)) == 0) {
                    ...
                    result = writeToFile(local_filename, file_buff, file_size);
            ...
            if ((result = storage_get_metadata(pTrackerServer, pStorageServer,
                                               group_name, remote_filename,
                                               &pMetaList, &meta_count)) == 0) {
                printf(
                    "get meta data success, "
                    "meta count=%d\n",
                    meta_count);
            ...
            if (argc < 7) {
                printf(
                    ...
                    "name2=value2,...\n",
                    argv[0], operation, STORAGE_SET_METADATA_FLAG_OVERWRITE,
            ...
            pMetaList = fdfs_split_metadata_ex(meta_buff, ',', '=', &meta_count,
                                               &result);
            if (pMetaList == NULL) {
                printf("Out of memory!\n");
                free(meta_buff);
            ...
            if ((result = storage_set_metadata(pTrackerServer, NULL, group_name,
                                               remote_filename, pMetaList,
                                               meta_count, *argv[5])) == 0) {
            ...
            if ((result = storage_delete_file(pTrackerServer, NULL, group_name,
                                              remote_filename)) == 0) {
