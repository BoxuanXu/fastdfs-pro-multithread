...
int main(int argc, char *argv[]) {
    ...
    *group_name = '\0';
    ...
    if (upload_type == FDFS_UPLOAD_BY_FILE) {
        if (stat(local_filename, &stat_buf) == 0 && S_ISREG(stat_buf.st_mode)) {
        ...
        if ((result = getFileContent(local_filename, &file_content,
            ...
            result = storage_upload_appender_by_filebuff(
                pTrackerServer, pStorageServer, store_path_index, file_content,
                file_size, file_ext_name, meta_list, meta_count, group_name,
                remote_filename);
            free(file_content);
        }
        ...
        if (stat(local_filename, &stat_buf) == 0 && S_ISREG(stat_buf.st_mode)) {
            ...
            result = storage_upload_appender_by_callback(
                pTrackerServer, pStorageServer, store_path_index,
                uploadFileCallback, local_filename, file_size, file_ext_name,
    ...
    if (storage_truncate_file(pTrackerServer, pStorageServer, group_name,
                              appender_filename, file_size / 2) != 0) {
        printf(
            "truncate file fail, "
            "error no: %d, error info: %s\n",
            result, STRERROR(result));
    ...
    if (file_info.file_size != file_size / 2) {
        fprintf(stderr, "file size: %" PRId64 " != %" PRId64 "!!!",
    ...
    if (upload_type == FDFS_UPLOAD_BY_FILE) {
        ...
        if (stat(local_filename, &stat_buf) == 0 && S_ISREG(stat_buf.st_mode)) {
            ...
            result = storage_append_by_callback(
                pTrackerServer, pStorageServer, uploadFileCallback,
                local_filename, file_size, group_name, appender_filename);
        } else {
            result = errno != 0 ? errno : ENOENT;
        }
    ...
    if (file_info.file_size != file_size + file_size / 2) {
        fprintf(stderr, "file size: %" PRId64 " != %" PRId64 "!!!",
                file_info.file_size, file_size + file_size / 2);
    }
    ...
    if (upload_type == FDFS_UPLOAD_BY_FILE) {
        ...
        if ((result = getFileContent(local_filename, &file_content,
            ...
            result = storage_modify_by_filebuff(
                pTrackerServer, pStorageServer, file_content, file_offset,
        ...
        if (stat(local_filename, &stat_buf) == 0 && S_ISREG(stat_buf.st_mode)) {
            ...
            result = storage_modify_by_callback(
                pTrackerServer, pStorageServer, uploadFileCallback,
                local_filename, file_offset, file_size, group_name,
                appender_filename);
        } else {
            result = errno != 0 ? errno : ENOENT;
        }
    ...
    if (file_info.file_size != 2 * file_size + file_size / 2) {
        fprintf(stderr, "file size: %" PRId64 " != %" PRId64 "!!!",
                file_info.file_size, 2 * file_size + file_size / 2);
    }
