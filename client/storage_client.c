...
#define storage_get_read_connection(pTrackerServer, ppStorageServer,   \
                                    group_name, filename, pNewStorage, \
                                    new_connection)                    \
    storage_get_connection(pTrackerServer, ppStorageServer,            \
                           TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ONE,  \
                           group_name, filename, pNewStorage, new_connection)

#define storage_get_update_connection(pTrackerServer, ppStorageServer,         \
                                      group_name, filename, pNewStorage,       \
                                      new_connection)                          \
    storage_get_connection(pTrackerServer, ppStorageServer,                    \
                           TRACKER_PROTO_CMD_SERVICE_QUERY_UPDATE, group_name, \
                           filename, pNewStorage, new_connection)

static int storage_get_connection(ConnectionInfo *pTrackerServer,
                                  ConnectionInfo **ppStorageServer,
                                  const byte cmd, const char *group_name,
                                  const char *filename,
    ...
    if (*ppStorageServer == NULL) {
        ...
        if (cmd == TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ONE) {
            result = tracker_query_storage_fetch(pNewTracker, pNewStorage,
                                                 group_name, filename);
        } else {
            result = tracker_query_storage_update(pNewTracker, pNewStorage,
                                                  group_name, filename);
        }
        ...
        if ((*ppStorageServer = tracker_connect_server(pNewStorage, &result)) ==
            NULL) {
            return result;
        }
        ...
        *new_connection = true;
        ...
        if ((*ppStorageServer)->sock >= 0) {
            *new_connection = false;
            ...
            if ((*ppStorageServer = tracker_connect_server(*ppStorageServer,
                                                           &result)) == NULL) {
                return result;
            }
    ...
    if (*ppStorageServer == NULL) {
        ...
        if (*group_name == '\0') {
            result = tracker_query_storage_store_without_group(
                pNewTracker, pNewStorage, group_name, store_path_index);
        } else {
            result = tracker_query_storage_store_with_group(
                pNewTracker, group_name, pNewStorage, store_path_index);
        }
...
int storage_get_metadata1(ConnectionInfo *pTrackerServer,
    ...
    return storage_get_metadata(pTrackerServer, pStorageServer, group_name,
                                filename, meta_list, meta_count);
}

int storage_get_metadata(ConnectionInfo *pTrackerServer,
                         ConnectionInfo *pStorageServer, const char *group_name,
                         const char *filename, FDFSMetaData **meta_list,
                         int *meta_count) {
    ...
    *meta_count = 0;
    ...
    if ((result = storage_get_update_connection(
             pTrackerServer, &pStorageServer, group_name, filename,
    ...
    do {
        ...
        **/
        ...
        snprintf(out_buff + sizeof(TrackerHeader),
                 sizeof(out_buff) - sizeof(TrackerHeader), "%s", group_name);
        ...
        if ((result = tcpsenddata_nb(
                 pStorageServer->sock, out_buff,
        ...
        if ((result = fdfs_recv_response(pStorageServer, &file_buff, 0,
                                         &in_bytes)) != 0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
...
int storage_query_file_info_ex1(ConnectionInfo *pTrackerServer,
                                ...
                                const char *file_id, FDFSFileInfo *pFileInfo,
                                const bool bSilence) {
    FDFS_SPLIT_GROUP_NAME_AND_FILENAME(file_id)
...
int storage_query_file_info_ex(ConnectionInfo *pTrackerServer,
    ...
    if ((result = storage_get_read_connection(
             pTrackerServer, &pStorageServer, group_name, filename,
             &storageServer, &new_connection)) != 0) {
        return result;
    }
    ...
    do {
        ...
        **/
        ...
        pInBuff = in_buff;
        if ((result = fdfs_recv_response(pStorageServer, &pInBuff,
        ...
        if (in_bytes != sizeof(in_buff)) {
            logError(
                ...
                "recv bytes: %" PRId64 " != %d",
                __LINE__, pStorageServer->ip_addr, pStorageServer->port,
        ...
        if (filename_len >= FDFS_LOGIC_FILE_PATH_LEN +
            ...
            base64_decode_auto(&the_base64_context,
                               (char *)filename + FDFS_LOGIC_FILE_PATH_LEN,
...
int storage_delete_file1(ConnectionInfo *pTrackerServer,
    ...
    return storage_delete_file(pTrackerServer, pStorageServer, group_name,
                               filename);
}
...
int storage_truncate_file1(ConnectionInfo *pTrackerServer,
    ...
    return storage_truncate_file(pTrackerServer, pStorageServer, group_name,
                                 filename, truncated_file_size);
}
...
int storage_delete_file(ConnectionInfo *pTrackerServer,
    ...
    do {
        ...
        **/
        ...
        pBuff = in_buff;
        if ((result = fdfs_recv_response(pStorageServer, &pBuff, 0,
                                         &in_bytes)) != 0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
...
int storage_do_download_file1_ex(ConnectionInfo *pTrackerServer,
                                 ...
                                 const int64_t download_bytes, char **file_buff,
                                 void *arg, int64_t *file_size) {
    FDFS_SPLIT_GROUP_NAME_AND_FILENAME(file_id)

    return storage_do_download_file_ex(
        pTrackerServer, pStorageServer, download_type, group_name, filename,
        file_offset, download_bytes, file_buff, arg, file_size);
}

int storage_do_download_file_ex(ConnectionInfo *pTrackerServer,
                                ConnectionInfo *pStorageServer,
                                const int download_type, const char *group_name,
    ...
    *file_size = 0;
    ...
    do {
        ...
        **/
        ...
        filename_len = snprintf(p, sizeof(out_buff) - (p - out_buff), "%s",
                                remote_filename);
        p += filename_len;
        ...
        if ((result = tcpsenddata_nb(pStorageServer->sock, out_buff, out_bytes,
                                     g_fdfs_network_timeout)) != 0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
        ...
        if (download_type == FDFS_DOWNLOAD_TO_FILE) {
            ...
            if ((result = tcprecvfile(pStorageServer->sock, *file_buff,
            ...
            *file_buff = NULL;
            ...
            while (remain_bytes > 0) {
                ...
                if ((result =
                         tcprecvdata_nb(pStorageServer->sock, buff, recv_bytes,
                                        g_fdfs_network_timeout)) != 0) {
                    logError(
                        "file: "__FILE__
                        ", line: %d, "
...
int storage_download_file_to_file1(ConnectionInfo *pTrackerServer,
    ...
    return storage_download_file_to_file(pTrackerServer, pStorageServer,
                                         group_name, filename, local_filename,
                                         file_size);
}
...
int storage_download_file_to_file(ConnectionInfo *pTrackerServer,
    ...
    return storage_do_download_file(
        pTrackerServer, pStorageServer, FDFS_DOWNLOAD_TO_FILE, group_name,
        remote_filename, &pLocalFilename, NULL, file_size);
}

int storage_upload_by_filename1_ex(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    const int store_path_index, const char cmd, const char *local_filename,
    ...
    result = storage_upload_by_filename_ex(
        pTrackerServer, pStorageServer, store_path_index, cmd, local_filename,
        file_ext_name, meta_list, meta_count, new_group_name, remote_filename);
    if (result == 0) {
        sprintf(file_id, "%s%c%s", new_group_name, FDFS_FILE_ID_SEPERATOR,
                remote_filename);
    } else {
        file_id[0] = '\0';
    }
...
int storage_do_upload_file1(ConnectionInfo *pTrackerServer,
                            ConnectionInfo *pStorageServer,
                            const int store_path_index, const char cmd,
                            const int upload_type, const char *file_buff,
                            void *arg, const int64_t file_size,
    ...
    result = storage_do_upload_file(
        pTrackerServer, pStorageServer, store_path_index, cmd, upload_type,
        file_buff, arg, file_size, NULL, NULL, file_ext_name, meta_list,
...
int storage_do_upload_file(ConnectionInfo *pTrackerServer,
    ...
    *remote_filename = '\0';
    ...
    if (bUploadSlave) {
        if ((result = storage_get_update_connection(
                 pTrackerServer, &pStorageServer, group_name, master_filename,
                 &storageServer, &new_connection)) != 0) {
            return result;
        }
    } else if ((result = storage_get_upload_connection(
                    pTrackerServer, &pStorageServer, group_name, &storageServer,
    ...
    */
    ...
    do {
        ...
        if ((result = tcpsenddata_nb(pStorageServer->sock, out_buff,
                                     p - out_buff, g_fdfs_network_timeout)) !=
            0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
        ...
        if (upload_type == FDFS_UPLOAD_BY_FILE) {
            ...
            if ((result =
                     tcpsenddata_nb(pStorageServer->sock, (char *)file_buff,
        ...
        } else  // FDFS_UPLOAD_BY_CALLBACK
        {
            ...
            if ((result = callback(arg, file_size, pStorageServer->sock)) !=
                0) {
                break;
            }
        }
    ...
    if (result == 0 && meta_count > 0) {
        result = storage_set_metadata(
            pTrackerServer, pStorageServer, group_name, remote_filename,
            meta_list, meta_count, STORAGE_SET_METADATA_FLAG_OVERWRITE);
        if (result != 0)  // rollback
        {
            storage_delete_file(pTrackerServer, pStorageServer, group_name,
                                remote_filename);
            *group_name = '\0';
            *remote_filename = '\0';
        }
...
int storage_upload_by_callback_ex(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    const int store_path_index, const char cmd, UploadCallback callback,
    void *arg, const int64_t file_size, const char *file_ext_name,
    const FDFSMetaData *meta_list, const int meta_count, char *group_name,
    char *remote_filename) {
    return storage_do_upload_file(
        pTrackerServer, pStorageServer, store_path_index, cmd,
        FDFS_UPLOAD_BY_CALLBACK, (char *)callback, arg, file_size, NULL, NULL,
        file_ext_name, meta_list, meta_count, group_name, remote_filename);
}

int storage_upload_by_callback1_ex(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    const int store_path_index, const char cmd, UploadCallback callback,
    void *arg, const int64_t file_size, const char *file_ext_name,
    const FDFSMetaData *meta_list, const int meta_count, const char *group_name,
    char *file_id) {
    char new_group_name[FDFS_GROUP_NAME_MAX_LEN + 1];
    char remote_filename[128];
    int result;
...
int storage_upload_by_filename_ex(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    const int store_path_index, const char cmd, const char *local_filename,
    const char *file_ext_name, const FDFSMetaData *meta_list,
    ...
    return storage_do_upload_file(
        pTrackerServer, pStorageServer, store_path_index, cmd,
        FDFS_UPLOAD_BY_FILE, local_filename, NULL, stat_buf.st_size, NULL, NULL,
        file_ext_name, meta_list, meta_count, group_name, remote_filename);
}
...
int storage_set_metadata1(ConnectionInfo *pTrackerServer,
    ...
    return storage_set_metadata(pTrackerServer, pStorageServer, group_name,
                                filename, meta_list, meta_count, op_flag);
}
...
int storage_set_metadata(ConnectionInfo *pTrackerServer,
                         ConnectionInfo *pStorageServer, const char *group_name,
                         const char *filename, const FDFSMetaData *meta_list,
                         const int meta_count, const char op_flag) {
    TrackerHeader *pHeader;
    int result;
    ConnectionInfo storageServer;
    ...
    do {
        ...
        if (meta_count > 0) {
            meta_buff =
                fdfs_pack_metadata(meta_list, meta_count, NULL, &meta_bytes);
            if (meta_buff == NULL) {
                result = ENOMEM;
                break;
            }
        ...
        *p++ = op_flag;
        ...
        pHeader = (TrackerHeader *)out_buff;
        long2buff((int)(p - (out_buff + sizeof(TrackerHeader))) + meta_bytes,
                  pHeader->pkg_len);
        ...
        if (meta_bytes > 0 &&
            (result = tcpsenddata_nb(pStorageServer->sock, meta_buff,
                                     meta_bytes, g_fdfs_network_timeout)) !=
                0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
...
int storage_download_file_ex1(ConnectionInfo *pTrackerServer,
                              ConnectionInfo *pStorageServer,
                              const char *file_id, const int64_t file_offset,
                              const int64_t download_bytes,
                              DownloadCallback callback, void *arg,
                              int64_t *file_size) {
    FDFS_SPLIT_GROUP_NAME_AND_FILENAME(file_id)

    return storage_download_file_ex(pTrackerServer, pStorageServer, group_name,
                                    filename, file_offset, download_bytes,
                                    callback, arg, file_size);
}

int storage_download_file_ex(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    ...
    return storage_do_download_file_ex(
        pTrackerServer, pStorageServer, FDFS_DOWNLOAD_TO_CALLBACK, group_name,
        remote_filename, file_offset, download_bytes, &pCallback, arg,
        file_size);
}
...
int tracker_query_storage_fetch1(ConnectionInfo *pTrackerServer,
    ...
    return tracker_query_storage_fetch(pTrackerServer, pStorageServer,
                                       group_name, filename);
}
...
int tracker_query_storage_update1(ConnectionInfo *pTrackerServer,
    ...
    return tracker_query_storage_update(pTrackerServer, pStorageServer,
                                        group_name, filename);
}
...
**/
int storage_client_create_link(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    ...
    *remote_filename = '\0';
    ...
    if ((result = storage_get_update_connection(
             pTrackerServer, &pStorageServer, group_name, src_filename,
             &storageServer, &new_connection)) != 0) {
        return result;
    }
...
int tracker_query_storage_list1(ConnectionInfo *pTrackerServer,
                                ConnectionInfo *pStorageServer,
                                const int nMaxServerCount, int *server_count,
                                const char *file_id) {
    FDFS_SPLIT_GROUP_NAME_AND_FILENAME(file_id)
    return tracker_query_storage_list(pTrackerServer, pStorageServer,
                                      nMaxServerCount, server_count, group_name,
                                      filename);
}

int storage_upload_slave_by_filename(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    const char *local_filename, const char *master_filename,
    ...
    return storage_do_upload_file(
        pTrackerServer, pStorageServer, 0, STORAGE_PROTO_CMD_UPLOAD_SLAVE_FILE,
        FDFS_UPLOAD_BY_FILE, local_filename, NULL, stat_buf.st_size,
        master_filename, prefix_name, file_ext_name, meta_list, meta_count,
        group_name, remote_filename);
}

int storage_upload_slave_by_callback(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    ...
    const int meta_count, char *group_name, char *remote_filename) {
    if (master_filename == NULL || *master_filename == '\0' ||
        prefix_name == NULL || *prefix_name == '\0' || group_name == NULL ||
        *group_name == '\0') {
        return EINVAL;
    }
    ...
    return storage_do_upload_file(
        pTrackerServer, pStorageServer, 0, STORAGE_PROTO_CMD_UPLOAD_SLAVE_FILE,
        FDFS_UPLOAD_BY_CALLBACK, (char *)callback, arg, file_size,
        master_filename, prefix_name, file_ext_name, meta_list, meta_count,
        group_name, remote_filename);
}

int storage_upload_slave_by_filebuff(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    const char *file_buff, const int64_t file_size, const char *master_filename,
    const char *prefix_name, const char *file_ext_name,
    const FDFSMetaData *meta_list, const int meta_count, char *group_name,
    char *remote_filename) {
    if (master_filename == NULL || *master_filename == '\0' ||
    ...
    return storage_do_upload_file(
        pTrackerServer, pStorageServer, 0, STORAGE_PROTO_CMD_UPLOAD_SLAVE_FILE,
        FDFS_UPLOAD_BY_BUFF, file_buff, NULL, file_size, master_filename,
        prefix_name, file_ext_name, meta_list, meta_count, group_name,
        remote_filename);
}

int storage_upload_slave_by_filename1(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    const char *local_filename, const char *master_file_id,
    const char *prefix_name, const char *file_ext_name,
    const FDFSMetaData *meta_list, const int meta_count, char *file_id) {
    int result;
    char new_group_name[FDFS_GROUP_NAME_MAX_LEN + 1];
    char remote_filename[128];
    ...
    result = storage_upload_slave_by_filename(
        pTrackerServer, pStorageServer, local_filename, filename, prefix_name,
        file_ext_name, meta_list, meta_count, new_group_name, remote_filename);
    if (result == 0) {
        sprintf(file_id, "%s%c%s", new_group_name, FDFS_FILE_ID_SEPERATOR,
                remote_filename);
...
int storage_upload_slave_by_filebuff1(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    const char *file_buff, const int64_t file_size, const char *master_file_id,
    const char *prefix_name, const char *file_ext_name,
    const FDFSMetaData *meta_list, const int meta_count, char *file_id) {
    int result;
    char new_group_name[FDFS_GROUP_NAME_MAX_LEN + 1];
    ...
    result = storage_upload_slave_by_filebuff(
        pTrackerServer, pStorageServer, file_buff, file_size, filename,
...
int storage_upload_slave_by_callback1(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    UploadCallback callback, void *arg, const int64_t file_size,
    ...
    result = storage_upload_slave_by_callback(
        pTrackerServer, pStorageServer, callback, arg, file_size, filename,
        prefix_name, file_ext_name, meta_list, meta_count, new_group_name,
        remote_filename);
    if (result == 0) {
        sprintf(file_id, "%s%c%s", new_group_name, FDFS_FILE_ID_SEPERATOR,
...
int storage_do_modify_file(ConnectionInfo *pTrackerServer,
                           ConnectionInfo *pStorageServer,
                           const int upload_type, const char *file_buff,
                           void *arg, const int64_t file_offset,
...
int storage_append_by_filename(ConnectionInfo *pTrackerServer,
    ...
    return storage_do_append_file(
        pTrackerServer, pStorageServer, FDFS_UPLOAD_BY_FILE, local_filename,
...
int storage_append_by_callback(ConnectionInfo *pTrackerServer,
    ...
    return storage_do_append_file(
        pTrackerServer, pStorageServer, FDFS_UPLOAD_BY_CALLBACK,
        (char *)callback, arg, file_size, group_name, appender_filename);
}
...
int storage_append_by_filename1(ConnectionInfo *pTrackerServer,
    ...
    return storage_append_by_filename(pTrackerServer, pStorageServer,
                                      local_filename, group_name, filename);
}
...
int storage_append_by_filebuff1(ConnectionInfo *pTrackerServer,
    ...
    return storage_append_by_filebuff(pTrackerServer, pStorageServer, file_buff,
                                      file_size, group_name, filename);
}
...
int storage_append_by_callback1(ConnectionInfo *pTrackerServer,
    ...
    return storage_append_by_callback(pTrackerServer, pStorageServer, callback,
                                      arg, file_size, group_name, filename);
}
...
int storage_modify_by_filename(ConnectionInfo *pTrackerServer,
    ...
    return storage_do_modify_file(
        pTrackerServer, pStorageServer, FDFS_UPLOAD_BY_FILE, local_filename,
        NULL, file_offset, stat_buf.st_size, group_name, appender_filename);
}
...
int storage_modify_by_callback(ConnectionInfo *pTrackerServer,
    ...
    return storage_do_modify_file(pTrackerServer, pStorageServer,
                                  FDFS_UPLOAD_BY_CALLBACK, (char *)callback,
                                  arg, file_offset, file_size, group_name,
                                  appender_filename);
}
...
int storage_modify_by_filebuff(ConnectionInfo *pTrackerServer,
    ...
    return storage_do_modify_file(
        pTrackerServer, pStorageServer, FDFS_UPLOAD_BY_BUFF, file_buff, NULL,
...
int storage_modify_by_filename1(ConnectionInfo *pTrackerServer,
    ...
    return storage_modify_by_filename(pTrackerServer, pStorageServer,
                                      local_filename, file_offset, group_name,
                                      filename);
}
...
int storage_modify_by_filebuff1(ConnectionInfo *pTrackerServer,
    ...
    return storage_modify_by_filebuff(pTrackerServer, pStorageServer, file_buff,
...
int storage_modify_by_callback1(ConnectionInfo *pTrackerServer,
    ...
    return storage_modify_by_callback(pTrackerServer, pStorageServer, callback,
                                      arg, file_offset, file_size, group_name,
                                      filename);
}
...
int fdfs_get_file_info_ex(const char *group_name, const char *remote_filename,
    ...
    if (filename_len < FDFS_NORMAL_LOGIC_FILENAME_LENGTH) {
        logError(
            ...
            __LINE__, filename_len, FDFS_NORMAL_LOGIC_FILENAME_LENGTH);
        return EINVAL;
    }
    ...
    if (fdfs_get_server_id_type(ip_addr.s_addr) == FDFS_ID_TYPE_SERVER_ID) {
        ...
        if (g_storage_ids_by_id != NULL && g_storage_id_count > 0) {
            ...
            if (pStorageId != NULL) {
                strcpy(pFileInfo->source_ip_addr, pStorageId->ip_addr);
            } else {
    ...
    if (IS_SLAVE_FILE(filename_len, pFileInfo->file_size) ||
        ...
        (*(pFileInfo->source_ip_addr) == '\0' &&
         get_from_server)) {  // slave file or appender file
        if (get_from_server) {
            ConnectionInfo *conn;
            ...
            result = storage_query_file_info(conn, NULL, group_name,
                                             remote_filename, pFileInfo);
    ...
    } else  // master file (normal file)
    {
        if ((pFileInfo->file_size >> 63) != 0) {
            ...
            pFileInfo->file_size =
                FDFS_TRUNK_FILE_TRUE_SIZE(pFileInfo->file_size);
        }
...
int storage_file_exist(ConnectionInfo *pTrackerServer,
    ...
    return storage_query_file_info_ex(pTrackerServer, pStorageServer,
                                      group_name, remote_filename, &file_info,
                                      true);
}
...
int storage_file_exist1(ConnectionInfo *pTrackerServer,
                        ConnectionInfo *pStorageServer, const char *file_id) {
    FDFS_SPLIT_GROUP_NAME_AND_FILENAME(file_id)
    return storage_file_exist(pTrackerServer, pStorageServer, group_name,
                              filename);
}
