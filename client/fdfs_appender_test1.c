...
int main(int argc, char *argv[]) {
    ...
    *group_name = '\0';
    ...
    if (storage_truncate_file1(pTrackerServer, pStorageServer, appender_file_id,
                               0) != 0) {
        printf(
            "truncate file fail, "
            "error no: %d, error info: %s\n",
    ...
    if (file_info.file_size != 0) {
        fprintf(stderr, "file size: %" PRId64 " != 0!!!", file_info.file_size);
    }

    // sleep(70);
    if (upload_type == FDFS_UPLOAD_BY_FILE) {
        result = storage_append_by_filename1(pTrackerServer, pStorageServer,
                                             local_filename, appender_file_id);
        ...
        if (stat(local_filename, &stat_buf) == 0 && S_ISREG(stat_buf.st_mode)) {
            file_size = stat_buf.st_size;
            result = storage_append_by_callback1(
                pTrackerServer, pStorageServer, uploadFileCallback,
                local_filename, file_size, appender_file_id);
        } else {
            result = errno != 0 ? errno : ENOENT;
        }
    ...
    if (upload_type == FDFS_UPLOAD_BY_FILE) {
        ...
        if (stat(local_filename, &stat_buf) == 0 && S_ISREG(stat_buf.st_mode)) {
            ...
            result = storage_modify_by_callback1(
                pTrackerServer, pStorageServer, uploadFileCallback,
                local_filename, file_offset, file_size, appender_file_id);
        } else {
            result = errno != 0 ? errno : ENOENT;
        }
    ...
    if (file_info.file_size != 2 * file_size) {
        fprintf(stderr, "file size: %" PRId64 " != %" PRId64 "!!!",
                file_info.file_size, 2 * file_size);
    }
