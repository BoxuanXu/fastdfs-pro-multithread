...
#define TRACKER_PROTO_CMD_STORAGE_SYNC_REPORT \
    89  // report src last synced time as dest server
#define TRACKER_PROTO_CMD_STORAGE_SYNC_DEST_QUERY \
    79  // dest storage query sync src storage server
#define TRACKER_PROTO_CMD_STORAGE_REPORT_IP_CHANGED \
    78  // storage server report it's ip changed
#define TRACKER_PROTO_CMD_STORAGE_CHANGELOG_REQ \
    77  // storage server request storage server's changelog
#define TRACKER_PROTO_CMD_STORAGE_REPORT_STATUS \
    76  // report specified storage server status
#define TRACKER_PROTO_CMD_STORAGE_PARAMETER_REQ \
    75  // storage server request parameters
#define TRACKER_PROTO_CMD_STORAGE_REPORT_TRUNK_FREE \
    74  // storage report trunk free space
#define TRACKER_PROTO_CMD_STORAGE_REPORT_TRUNK_FID \
    73  // storage report current trunk file id
#define TRACKER_PROTO_CMD_STORAGE_FETCH_TRUNK_FID \
    72  // storage get current trunk file id
#define TRACKER_PROTO_CMD_STORAGE_GET_STATUS \
    71  // get storage status from tracker
#define TRACKER_PROTO_CMD_STORAGE_GET_SERVER_ID \
    70  // get storage server id from tracker
#define TRACKER_PROTO_CMD_STORAGE_FETCH_STORAGE_IDS \
    69  // get all storage ids from tracker
#define TRACKER_PROTO_CMD_STORAGE_GET_GROUP_NAME \
    109  // get storage group name from tracker

#define TRACKER_PROTO_CMD_TRACKER_GET_SYS_FILES_START \
    61  // start of tracker get system data files
#define TRACKER_PROTO_CMD_TRACKER_GET_SYS_FILES_END \
    62  // end of tracker get system data files
#define TRACKER_PROTO_CMD_TRACKER_GET_ONE_SYS_FILE \
    63  // tracker get a system data file
#define TRACKER_PROTO_CMD_TRACKER_GET_STATUS \
    64  // tracker get status of other tracker
#define TRACKER_PROTO_CMD_TRACKER_PING_LEADER 65  // tracker ping leader
#define TRACKER_PROTO_CMD_TRACKER_NOTIFY_NEXT_LEADER \
    66  // notify next leader to other trackers
#define TRACKER_PROTO_CMD_TRACKER_COMMIT_NEXT_LEADER \
    67  // commit next leader to other trackers
#define TRACKER_PROTO_CMD_TRACKER_NOTIFY_RESELECT_LEADER \
    68  // storage notify reselect leader when split-brain
...
#define FDFS_PROTO_CMD_ACTIVE_TEST \
    111  // active test, tracker and storage both support since V1.28
...
#define STORAGE_PROTO_CMD_FETCH_ONE_PATH_BINLOG \
    26  // fetch binlog of one store path
...
#define STORAGE_PROTO_CMD_TRUNK_ALLOC_SPACE \
    27  // since V3.00, storage to trunk server
#define STORAGE_PROTO_CMD_TRUNK_ALLOC_CONFIRM \
    28  // since V3.00, storage to trunk server
#define STORAGE_PROTO_CMD_TRUNK_FREE_SPACE \
    29  // since V3.00, storage to trunk server
#define STORAGE_PROTO_CMD_TRUNK_SYNC_BINLOG \
    30  // since V3.00, trunk storage to storage
#define STORAGE_PROTO_CMD_TRUNK_GET_BINLOG_SIZE \
    31  // since V3.07, tracker to storage
#define STORAGE_PROTO_CMD_TRUNK_DELETE_BINLOG_MARKS \
    32  // since V3.07, tracker to storage
#define STORAGE_PROTO_CMD_TRUNK_TRUNCATE_BINLOG_FILE \
    33  // since V3.07, trunk storage to storage
...
#define TRACKER_QUERY_STORAGE_FETCH_BODY_LEN \
    (FDFS_GROUP_NAME_MAX_LEN + IP_ADDRESS_SIZE - 1 + FDFS_PROTO_PKG_LEN_SIZE)
#define TRACKER_QUERY_STORAGE_STORE_BODY_LEN                                   \
    (FDFS_GROUP_NAME_MAX_LEN + IP_ADDRESS_SIZE - 1 + FDFS_PROTO_PKG_LEN_SIZE + \
     1)

#define STORAGE_TRUNK_ALLOC_CONFIRM_REQ_BODY_LEN \
    (FDFS_GROUP_NAME_MAX_LEN + sizeof(FDFSTrunkInfoBuff))
...
int fdfs_recv_response(ConnectionInfo *pTrackerServer, char **buff,
                       const int buff_size, int64_t *in_bytes);
...
int fdfs_deal_no_body_cmd_ex(const char *ip_addr, const int port,
                             const int cmd);
...
FDFSMetaData *fdfs_split_metadata_ex(char *meta_buff,
                                     ...
                                     const char filedSeperator, int *meta_count,
                                     int *err_no);

int fdfs_get_ini_context_from_tracker(TrackerServerGroup *pTrackerGroup,
                                      IniContext *iniContext,
                                      bool *volatile continue_flag,
                                      const bool client_bind_addr,
