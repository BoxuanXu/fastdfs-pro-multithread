...
FDFSStorageIdInfo *g_storage_ids_by_ip =
    NULL;  // sorted by group name and storage IP
FDFSStorageIdInfo **g_storage_ids_by_id = NULL;  // sorted by storage ID
static FDFSStorageIdInfo **g_storage_ids_by_ip_port =
    NULL;  // sorted by storage ip and port

int g_storage_id_count = 0;

int fdfs_get_tracker_leader_index_ex(TrackerServerGroup *pServerGroup,
                                     const char *leaderIp,
                                     const int leaderPort) {
...
int fdfs_parse_storage_reserved_space(
    IniContext *pIniContext, FDFSStorageReservedSpace *pStorageReservedSpace) {
    int result;
    int len;
    ...
    pReservedSpaceStr =
        iniGetStrValue(NULL, "reserved_storage_space", pIniContext);
...
const char *fdfs_storage_reserved_space_to_string(
    FDFSStorageReservedSpace *pStorageReservedSpace, char *buff) {
...
int fdfs_get_storage_reserved_space_mb(
    const int total_mb, FDFSStorageReservedSpace *pStorageReservedSpace) {
    if (pStorageReservedSpace->flag == TRACKER_STORAGE_RESERVED_SPACE_FLAG_MB) {
...
bool fdfs_check_reserved_space(
    FDFSGroupInfo *pGroup, FDFSStorageReservedSpace *pStorageReservedSpace) {
    if (pStorageReservedSpace->flag == TRACKER_STORAGE_RESERVED_SPACE_FLAG_MB) {
...
bool fdfs_check_reserved_space_trunk(
    FDFSGroupInfo *pGroup, FDFSStorageReservedSpace *pStorageReservedSpace) {
    if (pStorageReservedSpace->flag == TRACKER_STORAGE_RESERVED_SPACE_FLAG_MB) {
...
bool fdfs_check_reserved_space_path(
    const int64_t total_mb, const int64_t free_mb, const int avg_mb,
...
FDFSStorageIdInfo *fdfs_get_storage_id_by_ip(const char *group_name,
    ...
    return (FDFSStorageIdInfo *)bsearch(
        &target, g_storage_ids_by_ip, g_storage_id_count,
...
FDFSStorageIdInfo *fdfs_get_storage_by_id(const char *id) {
    ...
    pTarget = &target;
    ppFound = (FDFSStorageIdInfo **)bsearch(
        &pTarget, g_storage_ids_by_id, g_storage_id_count,
...
static int fdfs_init_ip_port_array() {
    ...
    if (g_storage_ids_by_ip_port == NULL) {
        ...
        logError(
            ...
            __LINE__, alloc_bytes, result, STRERROR(result));
        return result;
    }
    ...
    if (port_count > 0 && port_count != g_storage_id_count) {
        logError(
            ...
            "must be the same format as host:port",
            __LINE__);
    ...
    for (i = 1; i < g_storage_id_count; i++) {
        if (fdfs_cmp_ip_and_port(&g_storage_ids_by_ip_port[i], &previous) ==
            0) {
            char szPortPart[16];
            if (previous->port > 0) {
            ...
            logError(
                ...
                "duplicate storage: %s%s",
                __LINE__, previous->ip_addr, szPortPart);
...
FDFSStorageIdInfo *fdfs_get_storage_id_by_ip_port(const char *pIpAddr,
    ...
    for (i = 0; i < 2; i++) {
        target.port = ports[i];
        ppFound = (FDFSStorageIdInfo **)bsearch(
            &pTarget, g_storage_ids_by_ip_port, g_storage_id_count,
            sizeof(FDFSStorageIdInfo *), fdfs_cmp_ip_and_port);
        if (ppFound != NULL) {
            return *ppFound;
        }
...
int fdfs_load_storage_ids(char *content, const char *pStorageIdsFilename) {
        ...
        for (i = 0; i < line_count; i++) {
            ...
            if (*group_name == '\0') {
                logError(
                    ...
                    __LINE__, pStorageIdsFilename, i + 1, line);
                result = EINVAL;
                break;
            }
            ...
            *pHost = '\0';
            ...
            if (!fdfs_is_server_id_valid(id)) {
                logError(
                    ...
                    "integer, such as 100001",
                    __LINE__, id);
                result = EINVAL;
                break;
            }

            snprintf(pStorageIdInfo->id, sizeof(pStorageIdInfo->id), "%s", id);
            snprintf(pStorageIdInfo->group_name,
                     sizeof(pStorageIdInfo->group_name), "%s", group_name);
            pStorageIdInfo++;
        }
    } while (0);
    ...
    logDebug(
        "file: "__FILE__
        ", line: %d, "
        "g_storage_id_count: %d",
        __LINE__, g_storage_id_count);
    pStorageIdInfo = g_storage_ids_by_ip;
    for (i = 0; i < g_storage_id_count; i++) {
        char szPortPart[16];
        ...
        logDebug("%s  %s  %s%s", pStorageIdInfo->id, pStorageIdInfo->group_name,
    ...
    pStorageIdInfo = g_storage_ids_by_ip;
    for (ppStorageIdInfo = g_storage_ids_by_id;
         ppStorageIdInfo < ppStorageIdEnd; ppStorageIdInfo++) {
        *ppStorageIdInfo = pStorageIdInfo++;
    }

    qsort(g_storage_ids_by_ip, g_storage_id_count, sizeof(FDFSStorageIdInfo),
          fdfs_cmp_group_name_and_ip);
    qsort(g_storage_ids_by_id, g_storage_id_count, sizeof(FDFSStorageIdInfo *),
          fdfs_cmp_server_id);
        ...
        do {
            ...
            if (content == NULL) {
                ...
                logError(
                    ...
                    "malloc %d bytes fail, "
                    "errno: %d, error info: %s",
                    __LINE__, total_length + 1, result, STRERROR(result));
                break;
            }
            ...
            *p = '\0';
            ...
            result = fdfs_load_storage_ids(content, "storage-ids-from-tracker");
            free(content);
        } while (0);
    }
...
int fdfs_load_storage_ids_from_file(const char *config_filename,
    ...
    pStorageIdsFilename =
        iniGetStrValue(NULL, "storage_ids_filename", pItemContext);
    ...
    {
        result = getFileContent(pStorageIdsFilename, &content, &file_size);
    } else {
        ...
        if (lastSlash == NULL) {
            ...
            *(filepath + len) = '\0';
            snprintf(full_filename, sizeof(full_filename), "%s/%s", filepath,
                     pStorageIdsFilename);
...
int fdfs_connection_pool_init(const char *config_filename,
    ...
    g_use_connection_pool =
        iniGetBoolValue(NULL, "use_connection_pool", pItemContext, false);
    if (!g_use_connection_pool) {
        return 0;
    }

    g_connection_pool_max_idle_time = iniGetIntValue(
        NULL, "connection_pool_max_idle_time", pItemContext, 3600);
    if (g_connection_pool_max_idle_time <= 0) {
        logError(
            ...
            __LINE__, g_connection_pool_max_idle_time, config_filename);
        return EINVAL;
    }

    return conn_pool_init(&g_connection_pool, g_fdfs_connect_timeout, 0,
                          g_connection_pool_max_idle_time);
}

void fdfs_connection_pool_destroy() { conn_pool_destroy(&g_connection_pool); }

void fdfs_set_log_rotate_size(LogContext *pContext,
                              const int64_t log_rotate_size) {
