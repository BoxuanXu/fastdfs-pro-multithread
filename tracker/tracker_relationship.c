...
static int fdfs_ping_leader(ConnectionInfo *pTrackerServer) {
    ...
    for (p = in_buff; p < pEnd;
         p += FDFS_GROUP_NAME_MAX_LEN + FDFS_STORAGE_ID_MAX_SIZE) {
        ...
        pGroup->pTrunkServer = tracker_mem_get_storage(pGroup, trunk_server_id);
        if (pGroup->pTrunkServer == NULL) {
            logWarning(
                ...
                __LINE__, pTrackerServer->ip_addr, group_name, trunk_server_id);
        }
        snprintf(pGroup->last_trunk_server_id,
                 sizeof(pGroup->last_trunk_server_id), "%s", trunk_server_id);
        success_count++;
    }
...
static int relationship_get_tracker_leader(
...
static int do_notify_leader_changed(ConnectionInfo *pTrackerServer,
    ...
    *bConnectFail = false;
    ...
    do {
        ...
        pHeader->cmd = cmd;
        sprintf(out_buff + sizeof(TrackerHeader), "%s:%d", pLeader->ip_addr,
                pLeader->port);
...
static int relationship_notify_leader_changed(ConnectionInfo *pLeader) {
    ...
    for (pTrackerServer = g_tracker_servers.servers;
         pTrackerServer < pTrackerEnd; pTrackerServer++) {
        if ((result = relationship_notify_next_leader(pTrackerServer, pLeader,
                                                      &bConnectFail)) != 0) {
            if (!bConnectFail) {
                return result;
            }
    ...
    for (pTrackerServer = g_tracker_servers.servers;
         pTrackerServer < pTrackerEnd; pTrackerServer++) {
        if ((result = relationship_commit_next_leader(pTrackerServer, pLeader,
                                                      &bConnectFail)) != 0) {
            if (!bConnectFail) {
                return result;
            }
    ...
    if (trackerStatus.pTrackerServer->port == g_server_port &&
        ...
        if (trackerStatus.if_leader) {
            g_tracker_servers.leader_index =
                trackerStatus.pTrackerServer - g_tracker_servers.servers;
        ...
        if (g_tracker_servers.servers != NULL) {
            if (g_tracker_servers.leader_index < 0) {
                if (relationship_select_leader() != 0) {
                    sleep_seconds =
                        1 + (int)((double)rand() * (double)MAX_SLEEP_SECONDS /
                                  RAND_MAX);
                }
            } else {
