...
int fdfs_recv_response(ConnectionInfo *pTrackerServer, char **buff,
    ...
    if ((result = tcprecvdata_nb(pTrackerServer->sock, *buff, *in_bytes,
                                 g_fdfs_network_timeout)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
...
int fdfs_quit(ConnectionInfo *pTrackerServer) {
    ...
    result = tcpsenddata_nb(pTrackerServer->sock, &header, sizeof(header),
...
int fdfs_deal_no_body_cmd(ConnectionInfo *pTrackerServer, const int cmd) {
    ...
    if (in_bytes == 0) {
        ...
        logError(
            ...
            "but received: %" PRId64,
            __LINE__, pTrackerServer->ip_addr, in_bytes);
        return EINVAL;
    }
}
...
FDFSMetaData *fdfs_split_metadata_ex(char *meta_buff,
    ...
    *meta_count = splitEx(meta_buff, recordSeperator, rows, *meta_count);
...
ConnectionInfo *tracker_connect_server_ex(ConnectionInfo *pTrackerServer,
                                          ...
                                          int *err_no) {
    if (g_use_connection_pool) {
        return conn_pool_get_connection(&g_connection_pool, pTrackerServer,
                                        err_no);
    } else {
        *err_no = conn_pool_connect_server(pTrackerServer, connect_timeout);
        if (*err_no != 0) {
            return NULL;
        } else {
...
int tracker_connect_server_no_pool(ConnectionInfo *pTrackerServer) {
    ...
    return conn_pool_connect_server(pTrackerServer, g_fdfs_connect_timeout);
}

static int fdfs_do_parameter_req(ConnectionInfo *pTrackerServer, char *buff,
                                 const int buff_size) {
...
int fdfs_get_ini_context_from_tracker(TrackerServerGroup *pTrackerGroup,
    ...
    do {
        for (pGlobalServer = pServerStart; pGlobalServer < pServerEnd;
            ...
            result = fdfs_do_parameter_req(pTServer, in_buff, sizeof(in_buff));
            if (result == 0) {
...
int fdfs_get_tracker_status(ConnectionInfo *pTrackerServer,
    ...
    do {
        ...
        pStatus->restart_interval =
            buff2long(in_buff + 1 + FDFS_PROTO_PKG_LEN_SIZE);
