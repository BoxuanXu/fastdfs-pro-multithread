...
#define ALLOC_CONNECTIONS_ONCE 1024
    ...
    if ((result = free_queue_init_ex(
             ...
             g_min_buff_size, g_max_buff_size, sizeof(TrackerClientInfo))) !=
        0) {
        return result;
    }
    ...
    for (pThreadData = g_thread_data; pThreadData < pDataEnd; pThreadData++) {
        if (ioevent_init(&pThreadData->ev_puller, g_max_connections + 2, 1000,
                         0) != 0) {
...
#else
        if ((result = fd_add_flags(pThreadData->pipe_fds[0], O_NONBLOCK)) !=
            0) {
            break;
        }
#endif
        ...
        if ((result = pthread_create(&tid, &thread_attr, work_thread_entrance,
            ...
            logError(
                ...
                "errno: %d, error info: %s",
                __LINE__, g_tracker_thread_count, result, STRERROR(result));
            break;
        } else {
...
static void *accept_thread_entrance(void *arg) {
    ...
    while (g_continue_flag) {
        ...
        if (write(pThreadData->pipe_fds[1], &incomesock, sizeof(incomesock)) !=
            sizeof(incomesock)) {
            close(incomesock);
            logError(
                "file: "__FILE__
            ...
            current_connections =
                __sync_add_and_fetch(&g_connection_stat.current_count, 1);
        ...
        if (pClientInfo->chg_count.tracker_leader !=
            ...
            *pFlags |= FDFS_CHANGE_FLAG_TRACKER_LEADER;
            ...
            if (leader_index >= 0) {
                ...
                snprintf(pDestServer->id, FDFS_STORAGE_ID_MAX_SIZE, "%s",
                         pTServer->ip_addr);
        ...
        if (pClientInfo->pStorage->trunk_chg_count !=
            ...
            *pFlags |= FDFS_CHANGE_FLAG_TRUNK_SERVER;
            ...
            if (pServer != NULL) {
                ...
                memcpy(pDestServer->id, pServer->id, FDFS_STORAGE_ID_MAX_SIZE);
                memcpy(pDestServer->ip_addr, pServer->ip_addr, IP_ADDRESS_SIZE);
                int2buff(pClientInfo->pGroup->storage_port, pDestServer->port);
            }
            pDestServer++;
...
static int tracker_deal_changelog_req(struct fast_task_info *pTask) {
    ...
    do {
        if (pClientInfo->pGroup != NULL &&
            pClientInfo->pStorage != NULL) {  // already logined
            if (pTask->length - sizeof(TrackerHeader) != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
                    "cmd=%d, client ip: %s, package size " PKG_LEN_PRINTF_FORMAT
                    " is not correct, "
                    "expect length = %d",
                    __LINE__, TRACKER_PROTO_CMD_STORAGE_CHANGELOG_REQ,
                    pTask->client_ip,
                    pTask->length - (int)sizeof(TrackerHeader), 0);
            ...
            *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
            ...
            storage_id =
                pTask->data + sizeof(TrackerHeader) + FDFS_GROUP_NAME_MAX_LEN;
            *(storage_id + FDFS_STORAGE_ID_MAX_SIZE - 1) = '\0';
            ...
            if (pStorage == NULL) {
                logError(
                    ...
                    __LINE__, pTask->client_ip, group_name, storage_id);
                result = ENOENT;
                break;
            }
...
static int tracker_deal_get_trunk_fid(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) != 0) {
        logError(
            ...
            " is not correct, "
            "expect length = %d",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_FETCH_TRUNK_FID,
            pTask->client_ip, pTask->length - (int)sizeof(TrackerHeader), 0);
...
static int tracker_deal_parameter_req(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) != 0) {
        logError(
            ...
            " is not correct, "
            "expect length = %d",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_PARAMETER_REQ, pTask->client_ip,
            pTask->length - (int)sizeof(TrackerHeader), 0);
    ...
    pTask->length =
        ...
        sprintf(
            ...
            g_storage_ip_changed_auto_adjust, g_storage_sync_file_max_delay,
            g_groups.store_path,
            fdfs_storage_reserved_space_to_string(&g_storage_reserved_space,
                                                  reserved_space_str),
            g_if_use_trunk_file, g_slot_min_size, g_slot_max_size,
            g_trunk_file_size, g_trunk_create_file_advance,
            g_trunk_create_file_time_base.hour,
            g_trunk_create_file_time_base.minute, g_trunk_create_file_interval,
            g_trunk_create_file_space_threshold, g_trunk_init_check_occupying,
            g_trunk_init_reload_from_binlog,
            g_trunk_compress_binlog_min_interval, g_store_slave_file_use_link);
...
static int tracker_deal_storage_replica_chg(struct fast_task_info *pTask) {
    ...
    if ((nPkgLen <= 0) || (nPkgLen % sizeof(FDFSStorageBrief) != 0)) {
        logError(
            ...
            __LINE__, TRACKER_PROTO_CMD_STORAGE_REPLICA_CHG, pTask->client_ip,
            nPkgLen);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
    ...
    if (server_count > FDFS_MAX_SERVERS_EACH_GROUP) {
        logError(
            ...
            " exceed max: %d",
            __LINE__, pTask->client_ip, server_count,
    ...
    if (g_if_leader_self) {
        ...
        briefServers =
            (FDFSStorageBrief *)(pTask->data + sizeof(TrackerHeader));
        return tracker_mem_sync_storages(
            ((TrackerClientInfo *)pTask->arg)->pGroup, briefServers,
            server_count);
    }
}
...
static int tracker_deal_report_trunk_fid(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) != sizeof(int)) {
        logError(
            ...
            " "
            "is not correct",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_REPORT_TRUNK_FID,
            pTask->client_ip, pTask->length - (int)sizeof(TrackerHeader));
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
...
static int tracker_deal_report_trunk_free_space(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) != 8) {
        logError(
            ...
            " "
            "is not correct",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_REPORT_TRUNK_FREE,
            pTask->client_ip, pTask->length - (int)sizeof(TrackerHeader));
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
    ...
    if (trunk_free_space < 0) {
        logError(
            ...
            "%" PRId64,
            __LINE__, pTask->client_ip, trunk_free_space);
        return EINVAL;
    }
...
static int tracker_deal_notify_next_leader(struct fast_task_info *pTask) {
    ...
    *(pTask->data + pTask->length) = '\0';
    ...
    server_index = fdfs_get_tracker_leader_index_ex(
        &g_tracker_servers, leader.ip_addr, leader.port);
    if (server_index < 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "client ip: %s, leader %s:%d not exist",
            __LINE__, pTask->client_ip, leader.ip_addr, leader.port);
        return ENOENT;
    }

    if (g_if_leader_self &&
        (leader.port != g_server_port || !is_local_host_ip(leader.ip_addr))) {
    ...
    *(pTask->data + pTask->length) = '\0';
    ...
    if (leader.port == g_server_port && is_local_host_ip(leader.ip_addr)) {
        ...
        logInfo(
            "file: "__FILE__
            ", line: %d, "
            "the tracker leader is %s:%d",
            __LINE__, leader.ip_addr, leader.port);
    }

    return 0;
}

static int tracker_deal_server_get_storage_status(
    ...
    if (nPkgLen < FDFS_GROUP_NAME_MAX_LEN) {
        logError(
            ...
            "cmd=%d, client ip addr: %s, "
            "package size %d is not correct",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_GET_STATUS, pTask->client_ip,
            nPkgLen);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
    ...
    *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    if (pStorage == NULL) {
        logError(
            ...
            "storage server not exist",
            __LINE__, pTask->client_ip, group_name, ip_addr);
        pTask->length = sizeof(TrackerHeader);
        return ENOENT;
    }
...
static int tracker_deal_get_storage_id(struct fast_task_info *pTask) {
    ...
    if (nPkgLen < FDFS_GROUP_NAME_MAX_LEN) {
        logError(
            ...
            "cmd=%d, client ip addr: %s, "
            "package size %d is not correct",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_GET_SERVER_ID, pTask->client_ip,
            nPkgLen);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
...
static int tracker_deal_get_storage_group_name(struct fast_task_info *pTask) {
    ...
    if (nPkgLen < 4) {
        logError(
            ...
            "cmd=%d, client ip addr: %s, "
            "package size %d is not correct",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_GET_GROUP_NAME,
...
static int tracker_deal_fetch_storage_ids(struct fast_task_info *pTask) {
    ...
    if (nPkgLen != sizeof(int)) {
        logError(
            ...
            "package size %d is not correct, "
            "expect %d bytes",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_FETCH_STORAGE_IDS,
    ...
    for (pIdInfo = pIdsStart; pIdInfo < pIdsEnd; pIdInfo++) {
        ...
        p += sprintf(p, "%s %s %s%s\n", pIdInfo->id, pIdInfo->group_name,
                     pIdInfo->ip_addr, szPortPart);
    }
...
static int tracker_deal_storage_report_status(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) !=
        ...
        logError(
            ...
            " "
            "is not correct",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_REPORT_STATUS, pTask->client_ip,
            pTask->length - (int)sizeof(TrackerHeader));
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
...
static int tracker_deal_storage_join(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) <
        ...
        logError(
            ...
            " "
            "is not correct, expect length >= %d.",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_JOIN, pTask->client_ip,
            pTask->length - (int)sizeof(TrackerHeader),
    ...
    if ((result = fdfs_validate_group_name(joinBody.group_name)) != 0) {
        logError(
            ...
            ", line: %d, "
            "client ip: %s, invalid group_name: %s",
            __LINE__, pTask->client_ip, joinBody.group_name);
        pTask->length = sizeof(TrackerHeader);
        return result;
    }
    ...
    if (joinBody.storage_port <= 0) {
        logError(
            ...
            __LINE__, pTask->client_ip, joinBody.storage_port);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
    ...
    if (joinBody.storage_http_port < 0) {
        logError(
            ...
            __LINE__, pTask->client_ip, joinBody.storage_http_port);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
    ...
    if (joinBody.store_path_count <= 0 || joinBody.store_path_count > 256) {
        logError(
            ...
            __LINE__, pTask->client_ip, joinBody.store_path_count);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }

    joinBody.subdir_count_per_path =
        (int)buff2long(pBody->subdir_count_per_path);
    if (joinBody.subdir_count_per_path <= 0 ||
        ...
        logError(
            ...
            __LINE__, pTask->client_ip, joinBody.subdir_count_per_path);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
        ...
        *(p + FDFS_PROTO_IP_PORT_SIZE - 1) = '\0';
        if ((pSeperator = strchr(p, ':')) == NULL) {
            logError(
                ...
                "and port: %s",
                __LINE__, pTask->client_ip, p);
            pTask->length = sizeof(TrackerHeader);
            return EINVAL;
        }

        *pSeperator = '\0';
        snprintf(pTrackerServer->ip_addr, sizeof(pTrackerServer->ip_addr), "%s",
                 p);
    ...
    *(pBody->domain_name + (sizeof(pBody->domain_name) - 1)) = '\0';
    ...
    getSockIpaddr(pTask->event.fd, tracker_ip, IP_ADDRESS_SIZE);
    insert_into_local_host_ip(tracker_ip);

    result = tracker_mem_add_group_and_storage(pClientInfo, pTask->client_ip,
                                               &joinBody, true);
    if (result != 0) {
        pTask->length = sizeof(TrackerHeader);
        return result;
    ...
    pJoinBodyResp =
        (TrackerStorageJoinBodyResp *)(pTask->data + sizeof(TrackerHeader));
    ...
    pTask->length = sizeof(TrackerHeader) + sizeof(TrackerStorageJoinBodyResp);
    return 0;
}
...
static int tracker_deal_server_delete_group(struct fast_task_info *pTask) {
    ...
    if (nPkgLen != FDFS_GROUP_NAME_MAX_LEN) {
        logError(
            ...
            " is not correct, "
            "expect length: %d",
            __LINE__, TRACKER_PROTO_CMD_SERVER_DELETE_GROUP, pTask->client_ip,
...
static int tracker_deal_server_delete_storage(struct fast_task_info *pTask) {
    ...
    if (nPkgLen <= FDFS_GROUP_NAME_MAX_LEN) {
        logError(
            ...
            " is not correct, "
            "expect length > %d",
            __LINE__, TRACKER_PROTO_CMD_SERVER_DELETE_STORAGE, pTask->client_ip,
            nPkgLen, FDFS_GROUP_NAME_MAX_LEN);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
    ...
    memcpy(group_name, pTask->data + sizeof(TrackerHeader),
           FDFS_GROUP_NAME_MAX_LEN);
    group_name[FDFS_GROUP_NAME_MAX_LEN] = '\0';
    pStorageId = pTask->data + sizeof(TrackerHeader) + FDFS_GROUP_NAME_MAX_LEN;
...
static int tracker_deal_server_set_trunk_server(struct fast_task_info *pTask) {
    ...
    if (nPkgLen < FDFS_GROUP_NAME_MAX_LEN) {
        logError(
            ...
            " is not correct, "
            "expect length >= %d",
            __LINE__, TRACKER_PROTO_CMD_SERVER_SET_TRUNK_SERVER,
    ...
    *(pStorageId + FDFS_STORAGE_ID_MAX_SIZE - 1) = '\0';
    ...
    pTrunkServer = tracker_mem_set_trunk_server(pGroup, pStorageId, &result);
    if (result == 0 && pTrunkServer != NULL) {
        int nIdLen;
        nIdLen = strlen(pTrunkServer->id) + 1;
        pTask->length = sizeof(TrackerHeader) + nIdLen;
        memcpy(pTask->data + sizeof(TrackerHeader), pTrunkServer->id, nIdLen);
    } else {
        if (result == 0) {
            result = ENOENT;
        }
        ...
        logError(
            ...
            "errno: %d, error info: %s",
            __LINE__, pTask->client_ip, group_name, pStorageId, result,
            STRERROR(result));
        pTask->length = sizeof(TrackerHeader);
    }
    return result;
...
static int tracker_deal_active_test(struct fast_task_info *pTask) {
    if (pTask->length - sizeof(TrackerHeader) != 0) {
        logError(
            ...
            " is not correct, "
            "expect length 0",
            __LINE__, FDFS_PROTO_CMD_ACTIVE_TEST, pTask->client_ip,
            pTask->length - (int)sizeof(TrackerHeader));
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
...
static int tracker_deal_ping_leader(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) != 0) {
        logError(
            ...
            " is not correct, "
            "expect length 0",
            __LINE__, TRACKER_PROTO_CMD_TRACKER_PING_LEADER, pTask->client_ip,
            pTask->length - (int)sizeof(TrackerHeader));
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
    ...
    body_len =
        (FDFS_GROUP_NAME_MAX_LEN + FDFS_STORAGE_ID_MAX_SIZE) * g_groups.count;
    ...
    for (ppGroup = g_groups.sorted_groups; ppGroup < ppEnd; ppGroup++) {
        ...
        if ((*ppGroup)->pTrunkServer != NULL) {
            memcpy(p, (*ppGroup)->pTrunkServer->id, FDFS_STORAGE_ID_MAX_SIZE);
        }
...
static int tracker_unlock_by_client(struct fast_task_info *pTask) {
    if (lock_by_client_count <= 0 ||
        pTask->finish_callback == NULL) {  // already unlocked
        return 0;
    }
...
static int tracker_lock_by_client(struct fast_task_info *pTask) {
    ...
    pTask->finish_callback =
        tracker_unlock_by_client;  // make sure to release lock
    lock_by_client_count++;
...
static int tracker_deal_get_tracker_status(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) != 0) {
        logError(
            ...
            " is not correct, "
            "expect length %d",
            __LINE__, TRACKER_PROTO_CMD_TRACKER_GET_STATUS, pTask->client_ip,
            pTask->length - (int)sizeof(TrackerHeader), 0);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
...
static int tracker_deal_get_sys_files_start(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) != 0) {
        logError(
            ...
            " is not correct, "
            "expect length %d",
            __LINE__, TRACKER_PROTO_CMD_TRACKER_GET_SYS_FILES_START,
            pTask->client_ip, pTask->length - (int)sizeof(TrackerHeader), 0);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
...
static int tracker_deal_get_sys_files_end(struct fast_task_info *pTask) {
    if (pTask->length - sizeof(TrackerHeader) != 0) {
        logError(
            ...
            " is not correct, "
            "expect length %d",
            __LINE__, TRACKER_PROTO_CMD_TRACKER_GET_SYS_FILES_END,
            pTask->client_ip, pTask->length - (int)sizeof(TrackerHeader), 0);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
...
#define TRACKER_READ_BYTES_ONCE                           \
    (TRACKER_MAX_PACKAGE_SIZE - FDFS_PROTO_PKG_LEN_SIZE - \
     sizeof(TrackerHeader) - 1)
    int result;
    int index;
    ...
    if (pTask->length - sizeof(TrackerHeader) != 1 + FDFS_PROTO_PKG_LEN_SIZE) {
        logError(
            ...
            " is not correct, "
            "expect length %d",
            __LINE__, TRACKER_PROTO_CMD_TRACKER_GET_ONE_SYS_FILE,
            pTask->client_ip, pTask->length - (int)sizeof(TrackerHeader),
    ...
    if (offset < 0 || offset > file_stat.st_size) {
        logError(
            ...
            __LINE__, pTask->client_ip, offset, file_stat.st_size);
    ...
    if (read_bytes > 0) {
        bytes = read_bytes + 1;
        if ((result = getFileContentEx(full_filename, p, offset, &bytes)) !=
            0) {
            pTask->length = sizeof(TrackerHeader);
            return result;
        }
...
static int tracker_deal_storage_report_ip_changed(
    ...
    if (pTask->length - sizeof(TrackerHeader) !=
        ...
        logError(
            ...
            " is not correct, "
            "expect length = %d",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_REPORT_IP_CHANGED,
            pTask->client_ip, pTask->length - (int)sizeof(TrackerHeader),
            FDFS_GROUP_NAME_MAX_LEN + 2 * IP_ADDRESS_SIZE);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    ...
    *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    pOldIpAddr = pTask->data + sizeof(TrackerHeader) + FDFS_GROUP_NAME_MAX_LEN;
    ...
    *(pNewIpAddr + (IP_ADDRESS_SIZE - 1)) = '\0';
    ...
    if (strcmp(pNewIpAddr, pTask->client_ip) != 0) {
        logError(
            ...
            __LINE__, pTask->client_ip, group_name, pNewIpAddr,
            pTask->client_ip);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
    ...
    pTask->length = sizeof(TrackerHeader);
    return tracker_mem_storage_ip_changed(pGroup, pOldIpAddr, pNewIpAddr);
}
...
static int tracker_deal_storage_sync_notify(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) !=
        ...
        logError(
            ...
            " is not correct, "
            "expect length: %d",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_SYNC_NOTIFY, pTask->client_ip,
            pTask->length - (int)sizeof(TrackerHeader),
    ...
    if (pClientInfo->pStorage->psync_src_server == NULL) {
        memcpy(sync_src_id, pBody->src_id, FDFS_STORAGE_ID_MAX_SIZE);
        sync_src_id[FDFS_STORAGE_ID_MAX_SIZE - 1] = '\0';

        pClientInfo->pStorage->psync_src_server =
            tracker_mem_get_storage(pClientInfo->pGroup, sync_src_id);
        if (pClientInfo->pStorage->psync_src_server == NULL) {
            logError(
                "file: "__FILE__
                ", line: %d, "
                ...
                __LINE__, pTask->client_ip, sync_src_id);
            pTask->length = sizeof(TrackerHeader);
            return ENOENT;
        }
...
static int tracker_deal_server_list_group_storages(
    ...
    if (nPkgLen < FDFS_GROUP_NAME_MAX_LEN ||
        ...
        logError(
            ...
            " is not correct, "
            "expect length >= %d && <= %d",
            __LINE__, TRACKER_PROTO_CMD_SERVER_LIST_STORAGE, pTask->client_ip,
            nPkgLen, FDFS_GROUP_NAME_MAX_LEN,
            FDFS_GROUP_NAME_MAX_LEN + FDFS_STORAGE_ID_MAX_SIZE);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    ...
    *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    pDest = pStart =
        (TrackerStorageStat *)(pTask->data + sizeof(TrackerHeader));
    ppEnd = pGroup->sorted_servers + pGroup->count;
    for (ppServer = pGroup->sorted_servers; ppServer < ppEnd; ppServer++) {
        if (pStorageId != NULL && strcmp(pStorageId, (*ppServer)->id) != 0) {
            continue;
        }
        ...
        long2buff((*ppServer)->upload_priority, pDest->sz_upload_priority);
        long2buff((*ppServer)->storage_port, pDest->sz_storage_port);
        long2buff((*ppServer)->storage_http_port, pDest->sz_storage_http_port);
        long2buff((*ppServer)->store_path_count, pDest->sz_store_path_count);
    ...
    pTask->length =
        sizeof(TrackerHeader) + (pDest - pStart) * sizeof(TrackerStorageStat);
    return 0;
}
...
**/
static int tracker_deal_service_query_fetch_update(struct fast_task_info *pTask,
                                                   const byte cmd) {
    char group_name[FDFS_GROUP_NAME_MAX_LEN + 1];
    char *filename;
    char *p;
    ...
    result = tracker_mem_get_storage_by_filename(
        cmd, FDFS_DOWNLOAD_TYPE_CALL group_name, filename, filename_len,
        &pGroup, ppStoreServers, &server_count);
...
static int tracker_deal_service_query_storage(struct fast_task_info *pTask,
                                              char cmd) {
    int expect_pkg_len;
    ...
    if (pTask->length - sizeof(TrackerHeader) != expect_pkg_len) {
        logError(
            ...
            pTask->length - (int)sizeof(TrackerHeader), expect_pkg_len);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    }
    ...
    if (cmd == TRACKER_PROTO_CMD_SERVICE_QUERY_STORE_WITH_GROUP_ONE ||
        ...
        if ((*ppFoundGroup)->active_count > 0) {
            ...
            if (tracker_check_reserved_space(*ppFoundGroup)) {
                pStoreGroup = *ppFoundGroup;
                       ...
                       tracker_check_reserved_space_trunk(*ppFoundGroup)) {
                pStoreGroup = *ppFoundGroup;
            }
        }
        ...
        if (pStoreGroup == NULL) {
            ...
            if (pStoreGroup == NULL) {
                for (ppGroup = g_groups.sorted_groups; ppGroup < ppFoundGroup;
                     ppGroup++) {
                    if ((*ppGroup)->active_count == 0) {
                        continue;
                    }
            ...
            if (pStoreGroup == NULL) {
                ...
                for (ppGroup = g_groups.sorted_groups; ppGroup < ppGroupEnd;
                     ppGroup++) {
                    if ((*ppGroup)->active_count == 0) {
                        continue;
                    }
        ...
        if (!tracker_check_reserved_space(g_groups.pStoreGroup)) {
            if (!(g_if_use_trunk_file &&
                  tracker_check_reserved_space_trunk(g_groups.pStoreGroup))) {
                pTask->length = sizeof(TrackerHeader);
                return ENOSPC;
            }
        }
    ...
    avg_reserved_mb =
        g_storage_reserved_space.rs.mb / pStoreGroup->store_path_count;
    if (!tracker_check_reserved_space_path(
            pStorageServer->path_total_mbs[write_path_index],
            pStorageServer->path_free_mbs[write_path_index], avg_reserved_mb)) {
        int i;
        for (i = 0; i < pStoreGroup->store_path_count; i++) {
            if (tracker_check_reserved_space_path(
                    pStorageServer->path_total_mbs[i],
                    pStorageServer->path_free_mbs[i], avg_reserved_mb)) {
        ...
        if (i == pStoreGroup->store_path_count) {
            ...
            for (i = write_path_index; i < pStoreGroup->store_path_count; i++) {
                if (tracker_check_reserved_space_path(
                        pStorageServer->path_total_mbs[i],
...
static int tracker_deal_server_list_one_group(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) != FDFS_GROUP_NAME_MAX_LEN) {
        logError(
            ...
            " is not correct, "
            "expect length: %d",
            __LINE__, TRACKER_PROTO_CMD_SERVER_LIST_ONE_GROUP, pTask->client_ip,
            pTask->length - (int)sizeof(TrackerHeader),
            FDFS_GROUP_NAME_MAX_LEN);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
    ...
    *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    long2buff(pGroup->current_write_server, pDest->sz_current_write_server);
    ...
    long2buff(pGroup->current_trunk_file_id, pDest->sz_current_trunk_file_id);
...
static int tracker_deal_server_list_all_groups(struct fast_task_info *pTask) {
    ...
    expect_size =
        sizeof(TrackerHeader) + g_groups.count * sizeof(TrackerGroupStat);
    if (expect_size > g_min_buff_size) {
        if (expect_size <= g_max_buff_size) {
            if ((result = free_queue_set_buffer_size(pTask, expect_size)) !=
                0) {
                pTask->length = sizeof(TrackerHeader);
                return result;
            }
    ...
    for (ppGroup = g_groups.sorted_groups; ppGroup < ppEnd; ppGroup++) {
        ...
        long2buff((*ppGroup)->storage_port, pDest->sz_storage_port);
        long2buff((*ppGroup)->storage_http_port, pDest->sz_storage_http_port);
        long2buff((*ppGroup)->active_count, pDest->sz_active_count);
        long2buff((*ppGroup)->current_write_server,
                  pDest->sz_current_write_server);
        long2buff((*ppGroup)->store_path_count, pDest->sz_store_path_count);
    ...
    pTask->length =
        sizeof(TrackerHeader) + (pDest - groupStats) * sizeof(TrackerGroupStat);
...
static int tracker_deal_storage_sync_src_req(struct fast_task_info *pTask) {
    ...
    if (pTask->length - sizeof(TrackerHeader) !=
        ...
        logError(
            ...
            " is not correct, "
            "expect length: %d",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_SYNC_SRC_REQ, pTask->client_ip,
            pTask->length - (int)sizeof(TrackerHeader),
            FDFS_GROUP_NAME_MAX_LEN + FDFS_STORAGE_ID_MAX_SIZE);
        pTask->length = sizeof(TrackerHeader);
        return EINVAL;
...
static int tracker_deal_storage_sync_dest_req(struct fast_task_info *pTask) {
    ...
    do {
        ...
        pSrcStorage = tracker_get_group_sync_src_server(pClientInfo->pGroup,
        ...
        pBody =
            (TrackerStorageSyncReqBody *)(pTask->data + sizeof(TrackerHeader));
...
static void tracker_find_max_free_space_group() {
    ...
    if (tracker_check_reserved_space(*ppMaxGroup) || !g_if_use_trunk_file) {
...
static void tracker_find_min_free_space(FDFSGroupInfo *pGroup) {
    ...
    for (ppServer = pGroup->active_servers + 1; ppServer < ppServerEnd;
         ppServer++) {
...
static int tracker_deal_storage_sync_report(struct fast_task_info *pTask) {
    ...
    if (nPkgLen <= 0 || nPkgLen % (FDFS_STORAGE_ID_MAX_SIZE + 4) != 0) {
        logError(
            ...
            "cmd=%d, client ip: %s, package size " PKG_LEN_PRINTF_FORMAT
            " is not correct",
            __LINE__, TRACKER_PROTO_CMD_STORAGE_SYNC_REPORT, pTask->client_ip,
            nPkgLen);
        ...
        if (g_groups.store_server == FDFS_STORE_SERVER_ROUND_ROBIN) {
            ...
            for (p = pTask->data + sizeof(TrackerHeader); p < pEnd;
                ...
                *(src_id + (FDFS_STORAGE_ID_MAX_SIZE - 1)) = '\0';
                pSrcStorage =
                    tracker_mem_get_storage(pClientInfo->pGroup, src_id);
                if (pSrcStorage == NULL) {
                    continue;
                }
                ...
                src_index = tracker_mem_get_storage_index(pClientInfo->pGroup,
                                                          pSrcStorage);
...
static int tracker_deal_storage_df_report(struct fast_task_info *pTask) {
    ...
    pStatBuff =
        (TrackerStatReportReqBody *)(pTask->data + sizeof(TrackerHeader));
    for (i = 0; i < pClientInfo->pGroup->store_path_count; i++) {
        ...
        if (g_groups.store_path == FDFS_STORE_PATH_LOAD_BALANCE &&
            path_free_mbs[i] >
                path_free_mbs[pClientInfo->pStorage->current_write_path]) {
    ...
    do {
        ...
        if (nPkgLen != sizeof(FDFSStorageStatBuff)) {
            logError(
                ...
                __LINE__, TRACKER_PROTO_CMD_STORAGE_BEAT, pTask->client_ip,
                nPkgLen, (int)sizeof(FDFSStorageStatBuff));
            status = EINVAL;
            break;
        }

        pStatBuff =
            (FDFSStorageStatBuff *)(pTask->data + sizeof(TrackerHeader));
        ...
        pStat->total_upload_count = buff2long(pStatBuff->sz_total_upload_count);
        pStat->success_upload_count =
            buff2long(pStatBuff->sz_success_upload_count);
        pStat->total_append_count = buff2long(pStatBuff->sz_total_append_count);
        pStat->success_append_count =
            buff2long(pStatBuff->sz_success_append_count);
        pStat->total_modify_count = buff2long(pStatBuff->sz_total_modify_count);
        ...
        pStat->total_delete_count = buff2long(pStatBuff->sz_total_delete_count);
        ...
        pStat->last_source_update = buff2long(pStatBuff->sz_last_source_update);
        pStat->last_sync_update = buff2long(pStatBuff->sz_last_sync_update);
        ...
        pStat->total_upload_bytes = buff2long(pStatBuff->sz_total_upload_bytes);
        pStat->success_upload_bytes =
            buff2long(pStatBuff->sz_success_upload_bytes);
        pStat->total_append_bytes = buff2long(pStatBuff->sz_total_append_bytes);
        pStat->success_append_bytes =
            buff2long(pStatBuff->sz_success_append_bytes);
        pStat->total_modify_bytes = buff2long(pStatBuff->sz_total_modify_bytes);
...
int tracker_deal_task(struct fast_task_info *pTask) {
    ...
    switch (pHeader->cmd) {
        ...
        case TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ONE:
            result =
                tracker_deal_service_query_fetch_update(pTask, pHeader->cmd);
            break;
