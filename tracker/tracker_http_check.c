...
static void *http_check_entrance(void *arg) {
    ...
    while (g_continue_flag) {
        ...
        for (ppGroup = g_groups.groups;
             g_continue_flag && (!g_http_servers_dirty) && ppGroup < ppGroupEnd;
             ppGroup++) {
            ...
            for (ppServer = (*ppGroup)->active_servers;
                 g_continue_flag && (!g_http_servers_dirty) &&
                     ppServer < ppServerEnd;
                 ppServer++) {
                if (g_http_check_type == FDFS_HTTP_CHECK_ALIVE_TYPE_TCP) {
                    ...
                    result = connectserverbyip_nb_auto(
                        sock, (*ppServer)->ip_addr,
                        (*ppGroup)->storage_http_port, g_fdfs_connect_timeout);
                    close(sock);
                        ...
                        *((*ppGroup)->http_servers + server_count) = *ppServer;
                        ...
                        if (result != (*ppServer)->http_check_last_errno) {
                            ...
                            logError(
                                "file: "__FILE__
                                ", line: %d, %s",
                                __LINE__, (*ppServer)->http_check_error_info);
                ...
                } else  // http
                {
                    ...
                    result = get_url_content(
                        url, g_fdfs_connect_timeout, g_fdfs_network_timeout,
                    ...
                    if (result == 0) {
                        if (http_status == 200) {
                            *((*ppGroup)->http_servers + server_count) =
                                *ppServer;
                            server_count++;
                            ...
                            if ((*ppServer)->http_check_fail_count > 0) {
                                logInfo(
                                    ...
                                    "after %d times, url: %s",
                                    __LINE__,
                                    (*ppServer)->http_check_fail_count, url);
                                (*ppServer)->http_check_fail_count = 0;
                            }
                        } else {
                            if (http_status !=
                                (*ppServer)->http_check_last_status) {
                                if ((*ppServer)->http_check_fail_count > 1) {
                                    logError(
                                        "file: "__FILE__
                                        ", line: %d, "
    ...
    for (ppGroup = g_groups.groups; ppGroup < ppGroupEnd; ppGroup++) {
        ...
        for (ppServer = (*ppGroup)->all_servers; ppServer < ppServerEnd;
             ppServer++) {
            if ((*ppServer)->http_check_fail_count > 1) {
                logError(
                    "file: "__FILE__
                    ...
                    "error info: %s",
                    __LINE__, (*ppServer)->http_check_fail_count,
                    (*ppServer)->ip_addr, (*ppGroup)->storage_http_port,
                    (*ppServer)->http_check_error_info);
            }
        }
    }
...
int tracker_http_check_start() {
    ...
    if ((result = pthread_create(&http_check_tid, NULL, http_check_entrance,
                                 NULL)) != 0) {
        logCrit(
            "file: "__FILE__
            ", line: %d, "
