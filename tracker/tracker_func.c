...
static int tracker_load_store_lookup(const char *filename,
    ...
    char *pGroupName;
    g_groups.store_lookup = iniGetIntValue(NULL, "store_lookup", pItemContext,
                                           FDFS_STORE_LOOKUP_ROUND_ROBIN);
    ...
    if (pGroupName[0] == '\0') {
        logError(
            ...
            ", line: %d, "
            "conf file \"%s\", "
            "store_group is empty!",
            __LINE__, filename);
        return EINVAL;
    }

    snprintf(g_groups.store_group, sizeof(g_groups.store_group), "%s",
             pGroupName);
...
static int tracker_load_storage_id_info(const char *config_filename,
    ...
    g_use_storage_id =
        iniGetBoolValue(NULL, "use_storage_id", pItemContext, false);
    if (!g_use_storage_id) {
        return 0;
    }
...
int tracker_load_from_conf_file(const char *filename, char *bind_addr,
                                const int addr_size) {
    char *pBasePath;
    char *pBindAddr;
    char *pRunByGroup;
    ...
    do {
        ...
        g_groups.store_server = (byte)iniGetIntValue(
            NULL, "store_server", &iniContext, FDFS_STORE_SERVER_ROUND_ROBIN);
        if (!(g_groups.store_server == FDFS_STORE_SERVER_FIRST_BY_IP ||
            ...
            logWarning(
                ...
                __LINE__, g_groups.store_server, FDFS_STORE_SERVER_ROUND_ROBIN);

            g_groups.store_server = FDFS_STORE_SERVER_ROUND_ROBIN;
        }

        g_groups.download_server =
            (byte)iniGetIntValue(NULL, "download_server", &iniContext,
                                 FDFS_DOWNLOAD_SERVER_ROUND_ROBIN);
        if (!(g_groups.download_server == FDFS_DOWNLOAD_SERVER_ROUND_ROBIN ||
              g_groups.download_server == FDFS_DOWNLOAD_SERVER_SOURCE_FIRST)) {
            logWarning(
                "file: "__FILE__
                ", line: %d, "
            ...
            g_groups.download_server = FDFS_DOWNLOAD_SERVER_ROUND_ROBIN;
        }

        g_groups.store_path = (byte)iniGetIntValue(
            NULL, "store_path", &iniContext, FDFS_STORE_PATH_ROUND_ROBIN);
        ...
        if ((result = fdfs_parse_storage_reserved_space(
                 &iniContext, &g_storage_reserved_space)) != 0) {
            break;
        }
        ...
        g_check_active_interval =
            iniGetIntValue(NULL, "check_active_interval", &iniContext,
        ...
        g_storage_ip_changed_auto_adjust = iniGetBoolValue(
            NULL, "storage_ip_changed_auto_adjust", &iniContext, true);

        g_storage_sync_file_max_delay =
            iniGetIntValue(NULL, "storage_sync_file_max_delay", &iniContext,
                           DEFAULT_STORAGE_SYNC_FILE_MAX_DELAY);
        if (g_storage_sync_file_max_delay <= 0) {
            g_storage_sync_file_max_delay = DEFAULT_STORAGE_SYNC_FILE_MAX_DELAY;
        }

        g_storage_sync_file_max_time =
            iniGetIntValue(NULL, "storage_sync_file_max_time", &iniContext,
                           DEFAULT_STORAGE_SYNC_FILE_MAX_TIME);
        if (g_storage_sync_file_max_time <= 0) {
            g_storage_sync_file_max_time = DEFAULT_STORAGE_SYNC_FILE_MAX_TIME;
        }

        g_if_use_trunk_file =
            iniGetBoolValue(NULL, "use_trunk_file", &iniContext, false);

        pSlotMinSize = iniGetStrValue(NULL, "slot_min_size", &iniContext);
        if (pSlotMinSize == NULL) {
            slot_min_size = 256;
        } else if ((result = parse_bytes(pSlotMinSize, 1, &slot_min_size)) !=
                   0) {
            return result;
        }
        ...
        pTrunkFileSize = iniGetStrValue(NULL, "trunk_file_size", &iniContext);
        ...
        if (g_trunk_file_size < 4 * 1024 * 1024) {
            logWarning(
                ...
                "change to 4MB",
                __LINE__, g_trunk_file_size);
            g_trunk_file_size = 4 * 1024 * 1024;
        }

        pSlotMaxSize = iniGetStrValue(NULL, "slot_max_size", &iniContext);
        if (pSlotMaxSize == NULL) {
            slot_max_size = g_trunk_file_size / 2;
        } else if ((result = parse_bytes(pSlotMaxSize, 1, &slot_max_size)) !=
                   0) {
            return result;
        }
        ...
        if (g_slot_max_size > g_trunk_file_size / 2) {
            logWarning(
                ...
                "change to %d",
                __LINE__, g_slot_max_size, g_trunk_file_size / 2);
            g_slot_max_size = g_trunk_file_size / 2;
        }

        g_trunk_create_file_advance = iniGetBoolValue(
            NULL, "trunk_create_file_advance", &iniContext, false);
        if ((result = get_time_item_from_conf(
                 &iniContext, "trunk_create_file_time_base",
                 &g_trunk_create_file_time_base, 2, 0)) != 0) {
            return result;
        }

        g_trunk_create_file_interval = iniGetIntValue(
            NULL, "trunk_create_file_interval", &iniContext, 86400);
        pSpaceThreshold = iniGetStrValue(
            NULL, "trunk_create_file_space_threshold", &iniContext);
        if (pSpaceThreshold == NULL) {
            g_trunk_create_file_space_threshold = 0;
        } else if ((result = parse_bytes(
                        pSpaceThreshold, 1,
                        &g_trunk_create_file_space_threshold)) != 0) {
            return result;
        }
        g_trunk_compress_binlog_min_interval = iniGetIntValue(
            NULL, "trunk_compress_binlog_min_interval", &iniContext, 0);

        g_trunk_init_check_occupying = iniGetBoolValue(
            NULL, "trunk_init_check_occupying", &iniContext, false);

        g_trunk_init_reload_from_binlog = iniGetBoolValue(
            NULL, "trunk_init_reload_from_binlog", &iniContext, false);

        if ((result = tracker_load_storage_id_info(filename, &iniContext)) !=
            0) {
            return result;
        }
        ...
        g_store_slave_file_use_link = iniGetBoolValue(
            NULL, "store_slave_file_use_link", &iniContext, false);
        ...
        } else if ((result = parse_bytes(pMinBuffSize, 1, &min_buff_size)) !=
                   0) {
            return result;
        }
        g_min_buff_size = (int)min_buff_size;

        pMaxBuffSize = iniGetStrValue(NULL, "max_buff_size", &iniContext);
        if (pMaxBuffSize == NULL) {
            max_buff_size = 16 * TRACKER_MAX_PACKAGE_SIZE;
        } else if ((result = parse_bytes(pMaxBuffSize, 1, &max_buff_size)) !=
                   0) {
            return result;
        }
        ...
        g_http_check_interval =
            iniGetIntValue(NULL, "http.check_alive_interval", &iniContext, 30);

        pHttpCheckType =
            iniGetStrValue(NULL, "http.check_alive_type", &iniContext);
        if (pHttpCheckType != NULL && strcasecmp(pHttpCheckType, "http") == 0) {
        ...
        pHttpCheckUri =
            iniGetStrValue(NULL, "http.check_alive_uri", &iniContext);
        if (pHttpCheckUri == NULL) {
            *g_http_check_uri = '/';
            *(g_http_check_uri + 1) = '\0';
        } else if (*pHttpCheckUri == '/') {
            snprintf(g_http_check_uri, sizeof(g_http_check_uri), "%s",
                     pHttpCheckUri);
        } else {
            snprintf(g_http_check_uri, sizeof(g_http_check_uri), "/%s",
                     pHttpCheckUri);
        ...
        logInfo(
            ...
            g_fdfs_network_timeout, g_server_port, bind_addr, g_max_connections,
            g_accept_threads, g_work_threads, g_min_buff_size, g_max_buff_size,
            g_groups.store_lookup, g_groups.store_group, g_groups.store_server,
            g_groups.store_path,
            fdfs_storage_reserved_space_to_string(&g_storage_reserved_space,
                                                  reserved_space_str),
            g_groups.download_server, g_allow_ip_count,
            g_sync_log_buff_interval, g_check_active_interval,
            g_thread_stack_size / 1024, g_storage_ip_changed_auto_adjust,
            g_storage_sync_file_max_delay, g_storage_sync_file_max_time,
            g_if_use_trunk_file, g_slot_min_size, g_slot_max_size / FDFS_ONE_MB,
            g_trunk_file_size / FDFS_ONE_MB, g_trunk_create_file_advance,
            g_trunk_create_file_time_base.hour,
            g_trunk_create_file_time_base.minute, g_trunk_create_file_interval,
            (int)(g_trunk_create_file_space_threshold / (FDFS_ONE_MB * 1024)),
            g_trunk_init_check_occupying, g_trunk_init_reload_from_binlog,
            g_trunk_compress_binlog_min_interval, g_use_storage_id,
            g_id_type_in_filename == FDFS_ID_TYPE_SERVER_ID ? "id" : "ip",
            g_storage_id_count, g_rotate_error_log,
            g_error_log_rotate_time.hour, g_error_log_rotate_time.minute,
            g_log_context.rotate_size, g_log_file_keep_days,
            g_store_slave_file_use_link, g_use_connection_pool,
        ...
        if (!g_http_params.disabled) {
            logInfo(
                ...
                "check_active_type=%s, "
                "check_active_uri=%s",
                g_http_params.server_port, g_http_params.default_content_type,
                ...
                g_http_check_interval,
                g_http_check_type == FDFS_HTTP_CHECK_ALIVE_TYPE_TCP ? "tcp"
                                                                    : "http",
                g_http_check_uri);
        }
#endif
