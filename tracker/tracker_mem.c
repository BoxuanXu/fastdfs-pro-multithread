...
static int _tracker_mem_add_storage(FDFSGroupInfo *pGroup,
                                    ...
                                    const bool bNeedSleep, const bool bNeedLock,
                                    bool *bInserted);
...
static int tracker_mem_destroy_groups(FDFSGroups *pGroups,
                                      const bool saveFiles);

char *g_tracker_sys_filenames[TRACKER_SYS_FILE_COUNT] = {
    STORAGE_GROUPS_LIST_FILENAME_NEW, STORAGE_SERVERS_LIST_FILENAME_NEW,
    STORAGE_SYNC_TIMESTAMP_FILENAME, STORAGE_SERVERS_CHANGELOG_FILENAME};
...
static int tracker_write_to_changelog(FDFSGroupInfo *pGroup,
    ...
    len = snprintf(buff, sizeof(buff), "%d %s %s %d %s\n", (int)g_current_time,
                   pGroup->group_name, pStorage->id, pStorage->status,
                   pArg != NULL ? pArg : "");
...
static int tracker_realloc_storage_path_mbs(FDFSStorageDetail *pStorage,
                                            const int old_store_path_count,
    ...
    copy_bytes =
        (old_store_path_count < new_store_path_count ? old_store_path_count
                                                     : new_store_path_count) *
        sizeof(int64_t);
...
static int tracker_realloc_group_path_mbs(FDFSGroupInfo *pGroup,
    ...
    for (ppStorage = pGroup->all_servers; ppStorage < ppEnd; ppStorage++) {
        if ((result = tracker_realloc_storage_path_mbs(
                 *ppStorage, pGroup->store_path_count, new_store_path_count)) !=
            0) {
            return result;
        }
    }
...
static int tracker_malloc_group_path_mbs(FDFSGroupInfo *pGroup) {
    ...
    ppEnd = pGroup->all_servers + pGroup->alloc_size;
    for (ppStorage = pGroup->all_servers; ppStorage < ppEnd; ppStorage++) {
        if ((result = tracker_malloc_storage_path_mbs(
                 *ppStorage, pGroup->store_path_count)) != 0) {
            return result;
        }
    }
...
#define STORAGE_DATA_GROUP_FIELDS 4
    ...
    if ((fp = fopen(STORAGE_GROUPS_LIST_FILENAME_OLD, "r")) == NULL) {
        logError(
            ...
            "open file \"%s/%s\" fail, "
            "errno: %d, error info: %s",
            __LINE__, data_path, STORAGE_GROUPS_LIST_FILENAME_OLD, errno,
            STRERROR(errno));
    ...
    while (fgets(szLine, sizeof(szLine), fp) != NULL) {
        ...
        col_count = splitEx(szLine, STORAGE_DATA_FIELD_SEPERATOR, fields,
        ...
        if ((result = tracker_mem_add_group_ex(pGroups, &clientInfo, group_name,
                                               false, &bInserted)) != 0) {
            break;
        }
        ...
        if (col_count == STORAGE_DATA_GROUP_FIELDS - 2) {  // version < V1.12
            clientInfo.pGroup->store_path_count = 0;
            clientInfo.pGroup->subdir_count_per_path = 0;
        } else {
            clientInfo.pGroup->store_path_count = atoi(trim(fields[2]));
            clientInfo.pGroup->subdir_count_per_path = atoi(trim(fields[3]));
        }
    }
...
static int tracker_load_groups_new(FDFSGroups *pGroups, const char *data_path,
                                   FDFSStorageSync **ppTrunkServers,
    ...
    *ppTrunkServers = NULL;
    ...
    group_count = iniGetIntValue(GROUP_SECTION_NAME_GLOBAL,
                                 GROUP_ITEM_GROUP_COUNT, &iniContext, -1);
    if (group_count < 0) {
        ...
        logError(
            ...
            "in the file \"%s/%s\", "
            "item \"%s\" is not found",
            __LINE__, data_path, STORAGE_GROUPS_LIST_FILENAME_NEW,
    ...
    for (i = 1; i <= group_count; i++) {
        ...
        group_name =
            iniGetStrValue(section_name, GROUP_ITEM_GROUP_NAME, &iniContext);
        if (group_name == NULL) {
            logError(
                "file: "__FILE__
                ", line: %d, "
        ...
        pGroup->storage_port = iniGetIntValue(
            section_name, GROUP_ITEM_STORAGE_PORT, &iniContext, 0);
        pGroup->storage_http_port = iniGetIntValue(
            section_name, GROUP_ITEM_STORAGE_HTTP_PORT, &iniContext, 0);
        pGroup->store_path_count = iniGetIntValue(
            section_name, GROUP_ITEM_STORE_PATH_COUNT, &iniContext, 0);
        pGroup->subdir_count_per_path = iniGetIntValue(
            section_name, GROUP_ITEM_SUBDIR_COUNT_PER_PATH, &iniContext, 0);
        pGroup->current_trunk_file_id = iniGetIntValue(
            section_name, GROUP_ITEM_CURRENT_TRUNK_FILE_ID, &iniContext, 0);
        pValue = iniGetStrValue(section_name, GROUP_ITEM_LAST_TRUNK_SERVER,
                                &iniContext);
        if (pValue != NULL) {
            snprintf(pGroup->last_trunk_server_id,
                     sizeof(pGroup->last_trunk_server_id), "%s", pValue);
            if (g_use_storage_id &&
                (*pValue != '\0' && !fdfs_is_server_id_valid(pValue))) {
                if (tracker_mem_get_storage_id(pGroup->group_name, pValue,
                    ...
                    logWarning(
                        ...
                        " NOT exist",
                        __LINE__, pGroup->group_name, pValue);
        ...
        pValue =
            iniGetStrValue(section_name, GROUP_ITEM_TRUNK_SERVER, &iniContext);
                ...
                *ppTrunkServers = (FDFSStorageSync *)realloc(
                ...
                if (*ppTrunkServers == NULL) {
                    ...
                    logError(
                        ...
                        __LINE__,
                        (int)sizeof(FDFSStorageSync) * nStorageSyncSize);
                    break;
                }
            }
            ...
            if (g_use_storage_id && !fdfs_is_server_id_valid(pValue)) {
                if ((result = tracker_mem_get_storage_id(
                    ...
                    logError(
                        ...
                        "exist",
                        __LINE__, clientInfo.pGroup->group_name, pValue);
                    break;
                }
            }
...
static int tracker_locate_group_trunk_servers(FDFSGroups *pGroups,
                                              ...
                                              const int nTrunkServerCount,
                                              const bool bLoadFromFile) {
    FDFSGroupInfo *pGroup;
    FDFSStorageDetail *pStorage;
    ...
    for (pServer = pTrunkServers; pServer < pTrunkEnd; pServer++) {
        pGroup = tracker_mem_get_group_ex(pGroups, pServer->group_name);
        if (pGroup == NULL) {
            continue;
        }
        ...
        if (pStorage == NULL) {
            ...
            if (bLoadFromFile) {
                snprintf(buff, sizeof(buff), "in the file \"%s/data/%s\", ",
            ...
            logError(
                ...
                "does not exist",
                __LINE__, buff, pServer->group_name, pServer->id);
...
static int tracker_locate_storage_sync_server(FDFSGroups *pGroups,
    ...
    for (pSyncServer = pStorageSyncs; pSyncServer < pSyncEnd; pSyncServer++) {
        pGroup = tracker_mem_get_group_ex(pGroups, pSyncServer->group_name);
        if (pGroup == NULL) {
            continue;
        }
        ...
        pStorage->psync_src_server =
            tracker_mem_get_storage(pGroup, pSyncServer->sync_src_id);
...
#define STORAGE_DATA_SERVER_FIELDS 22
    ...
    if ((fp = fopen(STORAGE_SERVERS_LIST_FILENAME_OLD, "r")) == NULL) {
        logError(
            ...
            "open file \"%s/%s\" fail, "
            "errno: %d, error info: %s",
            __LINE__, data_path, STORAGE_SERVERS_LIST_FILENAME_OLD, errno,
            STRERROR(errno));
        return errno != 0 ? errno : ENOENT;
    }
    ...
    while (fgets(szLine, sizeof(szLine), fp) != NULL) {
        ...
        cols = splitEx(szLine, STORAGE_DATA_FIELD_SEPERATOR, fields,
        ...
        if (cols != STORAGE_DATA_SERVER_FIELDS &&
            ...
            logError(
                ...
                "%d or %d or %d or %d",
                __LINE__, data_path, STORAGE_SERVERS_LIST_FILENAME_OLD, cols,
                STORAGE_DATA_SERVER_FIELDS, STORAGE_DATA_SERVER_FIELDS - 2,
                STORAGE_DATA_SERVER_FIELDS - 4, STORAGE_DATA_SERVER_FIELDS - 5);
            result = EINVAL;
            break;
        }
        ...
        if ((clientInfo.pGroup =
                 tracker_mem_get_group_ex(pGroups, group_name)) == NULL) {
            logError(
                "file: "__FILE__
                ", line: %d, "
                "in the file \"%s/%s\", "
        ...
        if ((result = tracker_mem_add_storage(&clientInfo, NULL, ip_addr, false,
                                              false, &bInserted)) != 0) {
            break;
        }
        ...
        pStorage->sync_until_timestamp = atoi(trim_left(fields[4]));
        pStorage->stat.total_upload_count =
            strtoll(trim_left(fields[5]), NULL, 10);
        pStorage->stat.success_upload_count =
            strtoll(trim_left(fields[6]), NULL, 10);
        pStorage->stat.total_set_meta_count =
            strtoll(trim_left(fields[7]), NULL, 10);
        pStorage->stat.success_set_meta_count =
            strtoll(trim_left(fields[8]), NULL, 10);
        pStorage->stat.total_delete_count =
            strtoll(trim_left(fields[9]), NULL, 10);
        pStorage->stat.success_delete_count =
            strtoll(trim_left(fields[10]), NULL, 10);
        pStorage->stat.total_download_count =
            strtoll(trim_left(fields[11]), NULL, 10);
        pStorage->stat.success_download_count =
            strtoll(trim_left(fields[12]), NULL, 10);
        pStorage->stat.total_get_meta_count =
            strtoll(trim_left(fields[13]), NULL, 10);
        pStorage->stat.success_get_meta_count =
            strtoll(trim_left(fields[14]), NULL, 10);
        pStorage->stat.last_source_update = atoi(trim_left(fields[15]));
        pStorage->stat.last_sync_update = atoi(trim_left(fields[16]));
        if (cols > STORAGE_DATA_SERVER_FIELDS - 5) {
            pStorage->changelog_offset =
                strtoll(trim_left(fields[17]), NULL, 10);
            if (pStorage->changelog_offset < 0) {
                pStorage->changelog_offset = 0;
            }
            if (pStorage->changelog_offset > g_changelog_fsize) {
                pStorage->changelog_offset = g_changelog_fsize;
            }

            if (cols > STORAGE_DATA_SERVER_FIELDS - 4) {
                pStorage->storage_port = atoi(trim_left(fields[18]));
                pStorage->storage_http_port = atoi(trim_left(fields[19]));
                if (cols > STORAGE_DATA_SERVER_FIELDS - 2) {
                    pStorage->join_time = (time_t)atoi(trim_left(fields[20]));

                    snprintf(pStorage->version, sizeof(pStorage->version), "%s",
                             trim(fields[21]));
                }
            }
        }
        ...
        if (nStorageSyncSize <= nStorageSyncCount) {
            ...
            pStorageSyncs = (FDFSStorageSync *)realloc(
                pStorageSyncs, sizeof(FDFSStorageSync) * nStorageSyncSize);
            if (pStorageSyncs == NULL) {
                ...
                logError(
                    ...
                    ", line: %d, "
                    "realloc %d bytes fail",
                    __LINE__, (int)sizeof(FDFSStorageSync) * nStorageSyncSize);
                break;
            }
        }
...
static int tracker_load_storages_new(FDFSGroups *pGroups,
                                     const char *data_path) {
    IniContext iniContext;
    char *group_name;
    char *storage_id;
    ...
    storage_count = iniGetIntValue(STORAGE_SECTION_NAME_GLOBAL,
                                   STORAGE_ITEM_STORAGE_COUNT, &iniContext, -1);
    if (storage_count < 0) {
        iniFreeContext(&iniContext);
        logError(
            "file: "__FILE__
            ...
            "in the file \"%s/%s\", "
            "item \"%s\" is not found",
            __LINE__, data_path, STORAGE_SERVERS_LIST_FILENAME_NEW,
    ...
    for (i = 1; i <= storage_count; i++) {
        ...
        group_name =
            iniGetStrValue(section_name, STORAGE_ITEM_GROUP_NAME, &iniContext);
        if (group_name == NULL) {
            logError(
                "file: "__FILE__
                ", line: %d, "
        ...
        storage_id =
            iniGetStrValue(section_name, STORAGE_ITEM_SERVER_ID, &iniContext);

        ip_addr =
            iniGetStrValue(section_name, STORAGE_ITEM_IP_ADDR, &iniContext);
        if (ip_addr == NULL) {
            logError(
                "file: "__FILE__
                ", line: %d, "
        ...
        if ((result = tracker_mem_add_storage(&clientInfo, storage_id, ip_addr,
        ...
        pStat = &(pStorage->stat);
        pStorage->status =
            iniGetIntValue(section_name, STORAGE_ITEM_STATUS, &iniContext, 0);

        pValue =
            iniGetStrValue(section_name, STORAGE_ITEM_DOMAIN_NAME, &iniContext);
        if (pValue != NULL) {
            snprintf(pStorage->domain_name, sizeof(pStorage->domain_name), "%s",
                     pValue);
        }

        pValue =
            iniGetStrValue(section_name, STORAGE_ITEM_VERSION, &iniContext);
        if (pValue != NULL) {
            snprintf(pStorage->version, sizeof(pStorage->version), "%s",
                     pValue);
        }
        ...
        psync_src_id = iniGetStrValue(
            section_name, STORAGE_ITEM_SYNC_SRC_SERVER, &iniContext);
        if (psync_src_id == NULL) {
            psync_src_id = "";
        }

        pStorage->sync_until_timestamp = iniGetIntValue(
            section_name, STORAGE_ITEM_SYNC_UNTIL_TIMESTAMP, &iniContext, 0);
        pStorage->join_time = iniGetIntValue(
            section_name, STORAGE_ITEM_JOIN_TIME, &iniContext, 0);
        pStorage->total_mb = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_MB, &iniContext, 0);
        pStorage->free_mb = iniGetInt64Value(section_name, STORAGE_ITEM_FREE_MB,
                                             &iniContext, 0);
        pStorage->store_path_count = iniGetIntValue(
            section_name, STORAGE_ITEM_STORE_PATH_COUNT, &iniContext, 0);
        pStorage->subdir_count_per_path = iniGetIntValue(
            section_name, STORAGE_ITEM_SUBDIR_COUNT_PER_PATH, &iniContext, 0);
        pStorage->upload_priority = iniGetIntValue(
            section_name, STORAGE_ITEM_UPLOAD_PRIORITY, &iniContext, 0);
        pStorage->storage_port = iniGetIntValue(
            section_name, STORAGE_ITEM_STORAGE_PORT, &iniContext, 0);
        pStorage->storage_http_port = iniGetIntValue(
            section_name, STORAGE_ITEM_STORAGE_HTTP_PORT, &iniContext, 0);
        pStat->total_upload_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_UPLOAD_COUNT, &iniContext, 0);
        pStat->success_upload_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_UPLOAD_COUNT, &iniContext, 0);
        pStat->total_append_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_APPEND_COUNT, &iniContext, 0);
        pStat->success_append_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_APPEND_COUNT, &iniContext, 0);
        pStat->total_set_meta_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_SET_META_COUNT, &iniContext, 0);
        pStat->success_set_meta_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_SET_META_COUNT, &iniContext, 0);
        pStat->total_delete_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_DELETE_COUNT, &iniContext, 0);
        pStat->success_delete_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_DELETE_COUNT, &iniContext, 0);
        pStat->total_download_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_DOWNLOAD_COUNT, &iniContext, 0);
        pStat->success_download_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_DOWNLOAD_COUNT, &iniContext, 0);
        pStat->total_get_meta_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_GET_META_COUNT, &iniContext, 0);
        pStat->success_get_meta_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_GET_META_COUNT, &iniContext, 0);
        pStat->total_create_link_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_CREATE_LINK_COUNT, &iniContext, 0);
        pStat->success_create_link_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_CREATE_LINK_COUNT, &iniContext,
            0);
        pStat->total_delete_link_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_DELETE_LINK_COUNT, &iniContext, 0);
        pStat->success_delete_link_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_DELETE_LINK_COUNT, &iniContext,
            0);
        pStat->total_upload_bytes = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_UPLOAD_BYTES, &iniContext, 0);
        pStat->success_upload_bytes = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_UPLOAD_BYTES, &iniContext, 0);
        pStat->total_append_bytes = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_APPEND_BYTES, &iniContext, 0);
        pStat->success_append_bytes = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_APPEND_BYTES, &iniContext, 0);
        pStat->total_download_bytes = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_DOWNLOAD_BYTES, &iniContext, 0);
        pStat->success_download_bytes = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_DOWNLOAD_BYTES, &iniContext, 0);
        pStat->total_sync_in_bytes = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_SYNC_IN_BYTES, &iniContext, 0);
        pStat->success_sync_in_bytes = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_SYNC_IN_BYTES, &iniContext, 0);
        pStat->total_sync_out_bytes = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_SYNC_OUT_BYTES, &iniContext, 0);
        pStat->success_sync_out_bytes = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_SYNC_OUT_BYTES, &iniContext, 0);
        pStat->total_file_open_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_FILE_OPEN_COUNT, &iniContext, 0);
        pStat->success_file_open_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_FILE_OPEN_COUNT, &iniContext, 0);
        pStat->total_file_read_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_FILE_READ_COUNT, &iniContext, 0);
        pStat->success_file_read_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_FILE_READ_COUNT, &iniContext, 0);
        pStat->total_file_write_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_TOTAL_FILE_WRITE_COUNT, &iniContext, 0);
        pStat->success_file_write_count = iniGetInt64Value(
            section_name, STORAGE_ITEM_SUCCESS_FILE_WRITE_COUNT, &iniContext,
            0);
        pStat->last_source_update = iniGetIntValue(
            section_name, STORAGE_ITEM_LAST_SOURCE_UPDATE, &iniContext, 0);
        pStat->last_sync_update = iniGetIntValue(
            section_name, STORAGE_ITEM_LAST_SYNC_UPDATE, &iniContext, 0);
        pStat->last_synced_timestamp = iniGetIntValue(
            section_name, STORAGE_ITEM_LAST_SYNCED_TIMESTAMP, &iniContext, 0);
        pStat->last_heart_beat_time = iniGetIntValue(
            section_name, STORAGE_ITEM_LAST_HEART_BEAT_TIME, &iniContext, 0);
        pStorage->changelog_offset = iniGetInt64Value(
            section_name, STORAGE_ITEM_CHANGELOG_OFFSET, &iniContext, 0);
        ...
        snprintf(pStorageSyncs[nStorageSyncCount].sync_src_id,
                 FDFS_STORAGE_ID_MAX_SIZE, "%s", psync_src_id);
        if (g_use_storage_id && !fdfs_is_server_id_valid(psync_src_id)) {
            if ((result = tracker_mem_get_storage_id(
                     clientInfo.pGroup->group_name, psync_src_id,
                     pStorageSyncs[nStorageSyncCount].sync_src_id)) != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
                    "server id of group name: %s and "
                    ...
                    "exist",
                    __LINE__, clientInfo.pGroup->group_name, psync_src_id);
                break;
            }
        }
...
static int tracker_load_sync_timestamps(FDFSGroups *pGroups,
                                        const char *data_path) {
#define STORAGE_SYNC_TIME_MAX_FIELDS 2 + FDFS_MAX_SERVERS_EACH_GROUP
    ...
    if ((fp = fopen(STORAGE_SYNC_TIMESTAMP_FILENAME, "r")) == NULL) {
        logError(
            ...
            "open file \"%s/%s\" fail, "
            "errno: %d, error info: %s",
            __LINE__, data_path, STORAGE_SYNC_TIMESTAMP_FILENAME, errno,
            STRERROR(errno));
        return errno != 0 ? errno : ENOENT;
    }
    ...
    *previous_group_name = '\0';
    ...
    while (fgets(szLine, sizeof(szLine), fp) != NULL) {
        ...
        if ((cols = splitEx(szLine, STORAGE_DATA_FIELD_SEPERATOR, fields,
                            STORAGE_SYNC_TIME_MAX_FIELDS)) <= 2) {
            logError(
                "file: "__FILE__
                ", line: %d, "
                "the format of the file \"%s/%s\" is invalid"
                ", colums: %d <= 2",
                __LINE__, data_path, STORAGE_SYNC_TIMESTAMP_FILENAME, cols);
            result = errno != 0 ? errno : EINVAL;
            break;
        }

        snprintf(group_name, sizeof(group_name), "%s", trim(fields[0]));
        snprintf(src_storage_id, sizeof(src_storage_id), "%s", trim(fields[1]));
        if (strcmp(group_name, previous_group_name) != 0 || pGroup == NULL) {
            if ((pGroup = tracker_mem_get_group_ex(pGroups, group_name)) ==
                NULL) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
        ...
        if (g_use_storage_id && !fdfs_is_server_id_valid(src_storage_id)) {
            if ((result = tracker_mem_get_storage_id(group_name, src_storage_id,
                                                     src_storage_id)) != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
                    ...
                    "exist",
                    __LINE__, group_name, src_storage_id);
                break;
            }
        }

        if (strcmp(pGroup->all_servers[src_index]->id, src_storage_id) != 0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
                "in data file: \"%s/%s\", "
                ...
                __LINE__, data_path, STORAGE_SYNC_TIMESTAMP_FILENAME,
                group_name, src_storage_id, pGroup->all_servers[src_index]->id);
            result = errno != 0 ? errno : EINVAL;
            break;
        }
            ...
            if (pGroups->store_server == FDFS_STORE_SERVER_ROUND_ROBIN) {
                ...
                for (src_index = 0; src_index < (*ppGroup)->count;
                    ...
                    curr_synced_timestamp =
                        (*ppGroup)->last_sync_timestamps[src_index][dest_index];
                ...
                (*ppGroup)
                    ->all_servers[dest_index]
                    ->stat.last_synced_timestamp = min_synced_timestamp;
            } else {
                ...
                (*ppGroup)
                    ->all_servers[dest_index]
                    ->stat.last_synced_timestamp = max_synced_timestamp;
            }
        }
    }
...
static int tracker_load_data(FDFSGroups *pGroups) {
    ...
    if (g_if_use_trunk_file) {
        if ((result = tracker_locate_group_trunk_servers(
                 pGroups, pTrunkServers, nTrunkServerCount, true)) != 0) {
            return result;
        }
    }
...
int tracker_save_groups() {
    ...
    snprintf(trueFilename, sizeof(trueFilename), "%s/data/%s", g_fdfs_base_path,
             STORAGE_GROUPS_LIST_FILENAME_NEW);
    ...
    len = sprintf(buff,
                  ...
                  "[%s]\n"
                  "\t%s=%d\n\n",
                  GROUP_SECTION_NAME_GLOBAL, GROUP_ITEM_GROUP_COUNT,
                  g_groups.count);
    if (fc_safe_write(fd, buff, len) != len) {
        ...
        for (ppGroup = g_groups.sorted_groups; ppGroup < ppEnd; ppGroup++) {
            len = sprintf(
                ...
                "\t%s=%s\n"
                "\t%s=%s\n\n",
                (*ppGroup)->group_name, GROUP_SECTION_NAME_PREFIX,
                (int)(ppGroup - g_groups.sorted_groups) + 1,
                GROUP_ITEM_GROUP_NAME, (*ppGroup)->group_name,
                GROUP_ITEM_STORAGE_PORT, (*ppGroup)->storage_port,
                GROUP_ITEM_STORAGE_HTTP_PORT, (*ppGroup)->storage_http_port,
                GROUP_ITEM_STORE_PATH_COUNT, (*ppGroup)->store_path_count,
                GROUP_ITEM_SUBDIR_COUNT_PER_PATH,
                (*ppGroup)->subdir_count_per_path,
                GROUP_ITEM_CURRENT_TRUNK_FILE_ID,
                (*ppGroup)->current_trunk_file_id, GROUP_ITEM_TRUNK_SERVER,
                (*ppGroup)->pTrunkServer != NULL ? (*ppGroup)->pTrunkServer->id
                                                 : "",
                GROUP_ITEM_LAST_TRUNK_SERVER, (*ppGroup)->last_trunk_server_id);
    ...
    if (result == 0) {
        if (rename(tmpFilename, trueFilename) != 0) {
            logError(
                ...
                "errno: %d, error info: %s",
                __LINE__, tmpFilename, trueFilename, errno, STRERROR(errno));
            result = errno != 0 ? errno : EIO;
        }
...
int tracker_save_storages() {
    ...
    *id_buff = '\0';
    ...
    for (ppGroup = g_groups.sorted_groups;
        ...
        for (ppStorage = (*ppGroup)->all_servers; ppStorage < ppStorageEnd;
             ppStorage++) {
            pStorage = *ppStorage;
            ...
            len = sprintf(
                ...
                STORAGE_ITEM_GROUP_NAME, (*ppGroup)->group_name,
                STORAGE_ITEM_IP_ADDR, pStorage->ip_addr, STORAGE_ITEM_STATUS,
                pStorage->status, STORAGE_ITEM_VERSION, pStorage->version,
                STORAGE_ITEM_JOIN_TIME, (int)pStorage->join_time,
                STORAGE_ITEM_STORAGE_PORT, pStorage->storage_port,
                STORAGE_ITEM_STORAGE_HTTP_PORT, pStorage->storage_http_port,
                STORAGE_ITEM_DOMAIN_NAME, pStorage->domain_name,
                ...
                STORAGE_ITEM_STORE_PATH_COUNT, pStorage->store_path_count,
                STORAGE_ITEM_SUBDIR_COUNT_PER_PATH,
                pStorage->subdir_count_per_path, STORAGE_ITEM_UPLOAD_PRIORITY,
                pStorage->upload_priority, STORAGE_ITEM_TOTAL_MB,
                pStorage->total_mb, STORAGE_ITEM_FREE_MB, pStorage->free_mb,
                ...
                STORAGE_ITEM_CHANGELOG_OFFSET, pStorage->changelog_offset);
    ...
    if (result == 0) {
        len = sprintf(buff,
                      ...
                      "[%s]\n"
                      "\t%s=%d\n",
                      STORAGE_SECTION_NAME_GLOBAL, STORAGE_ITEM_STORAGE_COUNT,
                      count);
        if (fc_safe_write(fd, buff, len) != len) {
            logError(
                "file: "__FILE__
    ...
    for (ppGroup = g_groups.sorted_groups;
        ...
        for (i = 0; i < (*ppGroup)->count; i++) {
            ...
            for (k = 0; k < (*ppGroup)->count; k++) {
                ...
                len += sprintf(buff + len, "%c%d", STORAGE_DATA_FIELD_SEPERATOR,
...
static int tracker_mem_init_groups(FDFSGroups *pGroups) {
    ...
    pGroups->groups =
        (FDFSGroupInfo **)malloc(sizeof(FDFSGroupInfo *) * pGroups->alloc_size);
    if (pGroups->groups == NULL) {
        logCrit(
            "file: "__FILE__
            ", line: %d, "
            "malloc %d bytes fail!",
            __LINE__, (int)sizeof(FDFSGroupInfo *) * pGroups->alloc_size);
        return errno != 0 ? errno : ENOMEM;
    }

    memset(pGroups->groups, 0, sizeof(FDFSGroupInfo *) * pGroups->alloc_size);
    ...
    if (results == NULL) {
        *err_no = errno != 0 ? errno : ENOMEM;
        logError(
            ...
            ", line: %d, "
            "malloc %d bytes fail",
            __LINE__, (int)sizeof(int *) * alloc_size);
        return NULL;
    }
...
static void tracker_mem_free_storages(FDFSStorageDetail **servers,
                                      const int count) {
...
static int tracker_mem_init_group(FDFSGroupInfo *pGroup) {
    ...
    pGroup->count = 0;
    pGroup->all_servers = (FDFSStorageDetail **)malloc(
    ...
    if (pGroup->all_servers == NULL) {
        logError(
            ...
            ", line: %d, "
            "malloc %d bytes fail",
            __LINE__, (int)sizeof(FDFSStorageDetail *) * pGroup->alloc_size);
        return errno != 0 ? errno : ENOMEM;
    }
    ...
    for (ppServer = pGroup->all_servers; ppServer < ppServerEnd; ppServer++) {
        *ppServer = (FDFSStorageDetail *)malloc(sizeof(FDFSStorageDetail));
        if (*ppServer == NULL) {
            ...
            logError(
                ...
                ", line: %d, "
                "malloc %d bytes fail",
                __LINE__, (int)sizeof(FDFSStorageDetail));
            return errno != 0 ? errno : ENOMEM;
        }
    ...
    pGroup->sorted_servers = (FDFSStorageDetail **)malloc(
        sizeof(FDFSStorageDetail *) * pGroup->alloc_size);
    if (pGroup->sorted_servers == NULL) {
        tracker_mem_free_group(pGroup);
    ...
    pGroup->active_servers = (FDFSStorageDetail **)malloc(
        sizeof(FDFSStorageDetail *) * pGroup->alloc_size);
    if (pGroup->active_servers == NULL) {
        tracker_mem_free_group(pGroup);
    ...
    if (g_http_check_interval <= 0) {
        ...
        pGroup->http_servers = (FDFSStorageDetail **)malloc(
            sizeof(FDFSStorageDetail *) * pGroup->alloc_size);
        if (pGroup->http_servers == NULL) {
            tracker_mem_free_group(pGroup);
    ...
    pGroup->last_sync_timestamps =
        tracker_malloc_last_sync_timestamps(pGroup->alloc_size, &err_no);
    return err_no;
}
...
static int tracker_mem_realloc_groups(FDFSGroups *pGroups,
    ...
    for (ppGroup = new_groups + pGroups->count; ppGroup < ppGroupEnd;
         ppGroup++) {
        *ppGroup = (FDFSGroupInfo *)malloc(sizeof(FDFSGroupInfo));
        if (*ppGroup == NULL) {
    ...
    new_sorted_groups =
        (FDFSGroupInfo **)malloc(sizeof(FDFSGroupInfo *) * new_size);
...
FDFSStorageDetail *tracker_get_group_sync_src_server(
    FDFSGroupInfo *pGroup, FDFSStorageDetail *pDestServer) {
    FDFSStorageDetail **ppServer;
    FDFSStorageDetail **ppServerEnd;

    ppServerEnd = pGroup->active_servers + pGroup->active_count;
    for (ppServer = pGroup->active_servers; ppServer < ppServerEnd;
         ppServer++) {
...
static int tracker_mem_realloc_store_servers(FDFSGroupInfo *pGroup,
    ...
    new_servers =
        (FDFSStorageDetail **)malloc(sizeof(FDFSStorageDetail *) * new_size);
    if (new_servers == NULL) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    for (ppServer = new_servers + pGroup->count; ppServer < ppServerEnd;
         ppServer++) {
        *ppServer = (FDFSStorageDetail *)malloc(sizeof(FDFSStorageDetail));
        if (*ppServer == NULL) {
...
#ifdef WITH_HTTPD
    if (g_http_check_interval > 0) {
        new_http_servers = (FDFSStorageDetail **)malloc(
            sizeof(FDFSStorageDetail *) * new_size);
    ...
    new_last_sync_timestamps =
        tracker_malloc_last_sync_timestamps(new_size, &err_no);
    ...
    for (i = 0; i < pGroup->alloc_size; i++) {
        memcpy(new_last_sync_timestamps[i], pGroup->last_sync_timestamps[i],
    ...
    tracker_free_last_sync_timestamps(old_last_sync_timestamps, old_size);
...
static void tracker_mem_insert_into_sorted_servers(
    FDFSStorageDetail *pTargetServer, FDFSStorageDetail **sorted_servers,
    const int count) {
    FDFSStorageDetail **ppServer;
    FDFSStorageDetail **ppEnd;
...
static void tracker_mem_insert_into_sorted_groups(FDFSGroups *pGroups,
    ...
    for (ppGroup = ppEnd; ppGroup > pGroups->sorted_groups; ppGroup--) {
        if (strcmp(pTargetGroup->group_name, (*(ppGroup - 1))->group_name) >
            0) {
...
FDFSGroupInfo *tracker_mem_get_group_ex(FDFSGroups *pGroups,
    ...
    ppGroup = (FDFSGroupInfo **)bsearch(&pTargetGroups, pGroups->sorted_groups,
...
static int tracker_mem_add_group_ex(FDFSGroups *pGroups,
    ...
    do {
        ...
        *bInserted = false;
        ...
        if ((pGroups->store_lookup == FDFS_STORE_LOOKUP_SPEC_GROUP) &&
            (pGroups->pStoreGroup == NULL) &&
            (strcmp(pGroups->store_group, pGroup->group_name) == 0)) {
...
static FDFSStorageDetail *tracker_mem_get_active_storage_by_id(
    ...
    ppStorageServer = (FDFSStorageDetail **)bsearch(
        &pTargetStorage, pGroup->active_servers, pGroup->active_count,
...
static FDFSStorageDetail *tracker_mem_get_active_http_server_by_ip(
    ...
    pTargetStorage = &target_storage;
    ppStorageServer = (FDFSStorageDetail **)bsearch(
        &pTargetStorage, pGroup->http_servers, pGroup->http_server_count,
        sizeof(FDFSStorageDetail *), tracker_mem_cmp_by_storage_id);
    if (ppStorageServer != NULL) {
        return *ppStorageServer;
    } else {
...
FDFSStorageDetail *tracker_mem_get_storage(FDFSGroupInfo *pGroup,
    ...
    pTargetStorage = &target_storage;
    ppStorageServer = (FDFSStorageDetail **)bsearch(
        &pTargetStorage, pGroup->sorted_servers, pGroup->count,
        sizeof(FDFSStorageDetail *), tracker_mem_cmp_by_storage_id);
    if (ppStorageServer != NULL) {
        return *ppStorageServer;
    } else {
...
static void tracker_mem_clear_storage_fields(
    FDFSStorageDetail *pStorageServer) {
    if (pStorageServer->path_total_mbs != NULL) {
        memset(pStorageServer->path_total_mbs, 0,
               sizeof(int64_t) * pStorageServer->store_path_count);
    }

    if (pStorageServer->path_free_mbs != NULL) {
        memset(pStorageServer->path_free_mbs, 0,
               sizeof(int64_t) * pStorageServer->store_path_count);
    }
...
static int tracker_mem_remove_group(FDFSGroupInfo **groups,
                                    FDFSGroupInfo *pGroup) {
    FDFSGroupInfo **ppGroup;
    FDFSGroupInfo **ppEnd;
    FDFSGroupInfo **pp;
...
int tracker_mem_delete_storage(FDFSGroupInfo *pGroup, const char *id) {
    ...
    logDebug(
        ...
        __LINE__, pStorageServer->ip_addr, pStorageServer->storage_port,
        pGroup->group_name);
...
int tracker_mem_storage_ip_changed(FDFSGroupInfo *pGroup,
                                   const char *old_storage_ip,
                                   const char *new_storage_ip) {
    ...
    if (g_use_storage_id) {
        logError(
            ...
            "IP address",
            __LINE__, new_storage_ip);
        return EOPNOTSUPP;
    }

    pOldStorageServer = tracker_mem_get_storage(pGroup, old_storage_ip);
    if (pOldStorageServer == NULL ||
        pOldStorageServer->status == FDFS_STORAGE_STATUS_DELETED) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if (!(pNewStorageServer == NULL ||
          pNewStorageServer->status == FDFS_STORAGE_STATUS_DELETED)) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    result =
        _tracker_mem_add_storage(pGroup, &pNewStorageServer, new_storage_ip,
                                 new_storage_ip, true, true, &bInserted);
    if (result != 0) {
        return result;
    }
    ...
    snprintf(pOldStorageServer->id, sizeof(pOldStorageServer->id), "%s",
             new_storage_ip);
    snprintf(pOldStorageServer->ip_addr, sizeof(pOldStorageServer->ip_addr),
             "%s", new_storage_ip);

    snprintf(pNewStorageServer->id, sizeof(pNewStorageServer->id), "%s",
             old_storage_ip);
    snprintf(pNewStorageServer->ip_addr, sizeof(pNewStorageServer->ip_addr),
             "%s", old_storage_ip);
    ...
    // need re-sort
    qsort(pGroup->sorted_servers, pGroup->count, sizeof(FDFSStorageDetail *),
...
static int tracker_mem_add_storage(TrackerClientInfo *pClientInfo,
    ...
    pStorageServer = NULL;
    result =
        _tracker_mem_add_storage(pClientInfo->pGroup, &pStorageServer, id,
                                 ip_addr, bNeedSleep, bNeedLock, bInserted);
    if (result == 0) {
...
static int _tracker_mem_add_storage(FDFSGroupInfo *pGroup,
    ...
    if (id != NULL) {
        if (g_use_storage_id) {
            result = fdfs_check_storage_id(pGroup->group_name, id);
            if (result != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
                    ...
                    "error info: %s",
                    __LINE__, pGroup->group_name, id, ip_addr, result,
                    STRERROR(result));
                return result;
            }
        }
    ...
    do {
        ...
        *ppStorageServer = *(pGroup->all_servers + pGroup->count);
        snprintf((*ppStorageServer)->id, FDFS_STORAGE_ID_MAX_SIZE, "%s",
                 storage_id);
        memcpy((*ppStorageServer)->ip_addr, ip_addr, IP_ADDRESS_SIZE);

        tracker_mem_insert_into_sorted_servers(
            *ppStorageServer, pGroup->sorted_servers, pGroup->count);
        pGroup->count++;
        pGroup->chg_count++;
...
#define FDFS_TRIM_TIME(t, i) (t / i) * i
    ...
    pStatus->restart_interval = FDFS_TRIM_TIME(
        pStatus->restart_interval, TRACKER_SYNC_STATUS_FILE_INTERVAL);
}

static int tracker_mem_get_sys_file_piece(ConnectionInfo *pTrackerServer,
                                          const int file_index, int fd,
    ...
    *p++ = file_index;
    ...
    if ((result = tcpsenddata_nb(pTrackerServer->sock, out_buff,
                                 sizeof(out_buff), g_fdfs_network_timeout)) !=
        0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    pInBuff = in_buff;
    result = fdfs_recv_response(pTrackerServer, &pInBuff, sizeof(in_buff),
                                &in_bytes);
    if (result != 0) {
        logError(
            "file: "__FILE__
    ...
    if (in_bytes < FDFS_PROTO_PKG_LEN_SIZE) {
        logError(
            ...
            " is invalid, "
            "expect length >= %d.",
            __LINE__, pTrackerServer->ip_addr, pTrackerServer->port, in_bytes,
    ...
    *file_size = buff2long(in_buff);
    ...
    if (*file_size < 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "tracker server %s:%d, file size: %" PRId64 " < 0",
    ...
    if (write_bytes > 0 &&
        ...
        logError(
            ...
            "write to file %s fail, "
            "errno: %d, error info: %s",
            __LINE__, g_tracker_sys_filenames[file_index], errno,
            STRERROR(errno));
        return errno != 0 ? errno : EIO;
    }
...
static int tracker_mem_get_one_sys_file(ConnectionInfo *pTrackerServer,
    ...
    file_size = 0;
    while (1) {
        result = tracker_mem_get_sys_file_piece(pTrackerServer, file_index, fd,
                                                &offset, &file_size);
        if (result != 0) {
            break;
        }
...
static int tracker_mem_cmp_tracker_running_status(const void *p1,
                                                  const void *p2) {
...
static int tracker_mem_first_add_tracker_servers(
    ...
    for (pLocalTracker = servers; pLocalTracker < pLocalEnd; pLocalTracker++) {
...
static int tracker_mem_check_add_tracker_servers(
    FDFSStorageJoinBody *pJoinBody) {
    ...
    bytes =
        sizeof(ConnectionInfo) * (g_tracker_servers.server_count + add_count);
    ...
    memcpy(new_servers, g_tracker_servers.servers,
           sizeof(ConnectionInfo) * g_tracker_servers.server_count);
    ...
    for (pJoinTracker = pJoinBody->tracker_servers; pJoinTracker < pJoinEnd;
        ...
        if (pLocalTracker == pLocalEnd) {
            memcpy(pNewServer, pJoinTracker, sizeof(ConnectionInfo));
            pNewServer->sock = -1;
            pNewServer++;
        }
    }
...
static int tracker_mem_get_tracker_server(
    FDFSStorageJoinBody *pJoinBody, TrackerRunningStatus *pTrackerStatus) {
    ...
    for (i = 0; i < count; i++) {
        logDebug(
            ...
            "restart interval: %d",
            __LINE__, trackerStatus[i].pTrackerServer->ip_addr,
            trackerStatus[i].pTrackerServer->port, trackerStatus[i].if_leader,
            trackerStatus[i].running_time, trackerStatus[i].restart_interval);
    }
...
static int tracker_mem_get_sys_files_from_others(
    FDFSStorageJoinBody *pJoinBody, TrackerRunningStatus *pRunningStatus) {
    int result;
    ...
    if (pRunningStatus != NULL) {
        if (tracker_mem_cmp_tracker_running_status(pRunningStatus,
            ...
            logDebug(
                ...
                "do not need sync system files",
                __LINE__, trackerStatus.pTrackerServer->ip_addr,
                trackerStatus.pTrackerServer->port, trackerStatus.running_time,
                trackerStatus.restart_interval, pRunningStatus->running_time,
...
int tracker_mem_add_group_and_storage(TrackerClientInfo *pClientInfo,
    ...
    if (need_get_sys_files) {
        if (g_tracker_last_status.last_check_time > 0 &&
            g_up_time - g_tracker_last_status.last_check_time >
                2 * TRACKER_SYNC_STATUS_FILE_INTERVAL) { /* stop time exceeds 2
                                                            * interval */
            TrackerRunningStatus runningStatus;

            runningStatus.if_leader = false;
            tracker_calc_running_times(&runningStatus);
            result = tracker_mem_get_sys_files_from_others(pJoinBody,
                                                           &runningStatus);
            if (result != 0) {
                tracker_mem_file_unlock();
                logError(
    ...
    if ((!get_sys_files_done) && (g_groups.count == 0)) {
        if (g_groups.count == 0) {
            if ((result = tracker_mem_get_sys_files_from_others(pJoinBody,
                                                                NULL)) != 0) {
                tracker_mem_file_unlock();
                logError(
                    "file: "__FILE__
    ...
    if ((result = tracker_mem_add_group_ex(&g_groups, pClientInfo,
                                           pJoinBody->group_name, bNeedSleep,
                                           &bGroupInserted)) != 0) {
        return result;
    }
    ...
    if (g_use_storage_id) {
        pStorageIdInfo =
            fdfs_get_storage_id_by_ip(pClientInfo->pGroup->group_name, ip_addr);
        if (pStorageIdInfo == NULL) {
            logError(
                "file: "__FILE__
                ", line: %d, "
    ...
    if (pClientInfo->pGroup->storage_port == 0) {
        ...
        if (pClientInfo->pGroup->storage_port != pJoinBody->storage_port) {
            ppEnd =
                pClientInfo->pGroup->all_servers + pClientInfo->pGroup->count;
            for (ppServer = pClientInfo->pGroup->all_servers; ppServer < ppEnd;
                 ppServer++) {
                if (strcmp((*ppServer)->id, storage_id) == 0) {
                    (*ppServer)->storage_port = pJoinBody->storage_port;
                    break;
                }
            }
            ...
            for (ppServer = pClientInfo->pGroup->all_servers; ppServer < ppEnd;
                 ppServer++) {
                if ((*ppServer)->storage_port != pJoinBody->storage_port) {
                    break;
                }
            }
            if (ppServer == ppEnd)  // all servers are same, adjust
            {
                ...
                logError(
                    ...
                    pJoinBody->group_name, pClientInfo->pGroup->storage_port);
                return EINVAL;
            }
        }
    }
    ...
    if (pClientInfo->pGroup->storage_http_port == 0) {
        pClientInfo->pGroup->storage_http_port = pJoinBody->storage_http_port;
        if ((result = tracker_save_groups()) != 0) {
            return result;
        }
    } else {
        if (pClientInfo->pGroup->storage_http_port !=
            ...
            if (ppServer == ppEnd)  // all servers are same, adjust
            {
                ...
                logError(
                    ...
                    __LINE__, ip_addr, pJoinBody->storage_http_port,
        ...
        if (!pJoinBody->init_flag) {
            if (pJoinBody->status < 0 ||
                ...
                logError(
                    ...
                    __LINE__, ip_addr, pJoinBody->storage_port,
                    pJoinBody->status, pJoinBody->group_name);
                return EFAULT;
            }
        }
    }
    ...
    snprintf(pStorageServer->version, sizeof(pStorageServer->version), "%s",
             pJoinBody->version);
    snprintf(pStorageServer->domain_name, sizeof(pStorageServer->domain_name),
        ...
        if (pClientInfo->pGroup->store_path_count !=
            ...
            if (ppServer == ppEnd)  // all servers are same, adjust
            {
                if ((result = tracker_realloc_group_path_mbs(
                         pClientInfo->pGroup, pJoinBody->store_path_count)) !=
                    0) {
                    return result;
                }
                ...
                logError(
                    ...
                    __LINE__, ip_addr, pJoinBody->store_path_count,
                    pJoinBody->group_name,
                    pClientInfo->pGroup->store_path_count);
                return EINVAL;
            }
        ...
        if (pClientInfo->pGroup->subdir_count_per_path !=
            ...
            if (ppServer == ppEnd)  // all servers are same, adjust
            {
                ...
                logError(
                    ...
                    __LINE__, ip_addr, pJoinBody->subdir_count_per_path,
                    pJoinBody->group_name,
                    pClientInfo->pGroup->subdir_count_per_path);
...
int tracker_mem_sync_storages(FDFSGroupInfo *pGroup,
    ...
    do {
        ...
        for (pServer = briefServers; pServer < pEnd; pServer++) {
            ...
            memcpy(target_storage.id, pServer->id, FDFS_STORAGE_ID_MAX_SIZE);
            pTargetStorage = &target_storage;
            if ((ppFound = (FDFSStorageDetail **)bsearch(
                ...
                logWarning(
                    ...
                    __LINE__, (*ppFound)->ip_addr, (*ppFound)->storage_port,
                ...
                result = _tracker_mem_add_storage(pGroup, &pStorageServer,
                                                  pServer->id, pServer->ip_addr,
                                                  true, false, &bInserted);
                if (result != 0) {
...
static int _storage_get_trunk_binlog_size(ConnectionInfo *pStorageServer,
                                          int64_t *file_size) {
    ...
    pInBuff = in_buff;
    if ((result = fdfs_recv_response(pStorageServer, &pInBuff, sizeof(in_buff),
                                     &in_bytes)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if (in_bytes != sizeof(in_buff)) {
        logError(
            ...
            "%" PRId64 " != %d",
            __LINE__, pStorageServer->ip_addr, pStorageServer->port, in_bytes,
            (int)sizeof(in_buff));
        return EINVAL;
    }
...
static int tracker_mem_get_trunk_binlog_size(const char *storage_ip,
                                             const int port,
                                             int64_t *file_size) {
    ...
    *file_size = 0;
    ...
    logDebug(
        ...
        __LINE__, storage_server.ip_addr, storage_server.port, *file_size);
    return result;
}

static int tracker_write_to_trunk_change_log(
    FDFSGroupInfo *pGroup, FDFSStorageDetail *pLastTrunkServer) {
    char full_filename[MAX_PATH_SIZE];
    char buff[256];
    int fd;
    int len;
    ...
    len = sprintf(buff, "[%04d-%02d-%02d %02d:%02d:%02d] %s ",
                  ...
                  tm.tm_min, tm.tm_sec, pGroup->group_name);

    pLastTrunk = pLastTrunkServer;
    if (pLastTrunk == NULL && *(pGroup->last_trunk_server_id) != '\0') {
        pLastTrunk =
            tracker_mem_get_storage(pGroup, pGroup->last_trunk_server_id);
    }
    if (g_use_storage_id) {
        if (pLastTrunk == NULL) {
            ...
            len += sprintf(buff + len, " %s/%s  => ", pLastTrunk->id,
                           pLastTrunk->ip_addr);
        }
        ...
        if (pLastTrunk == NULL) {
            ...
            len += sprintf(buff + len, " %s  => ", pLastTrunk->ip_addr);
        }
...
static int tracker_set_trunk_server_and_log(
    FDFSGroupInfo *pGroup, FDFSStorageDetail *pNewTrunkServer) {
    ...
    if (pNewTrunkServer == NULL ||
        strcmp(pNewTrunkServer->id, pGroup->last_trunk_server_id) != 0) {
        int result;
...
static int tracker_mem_do_set_trunk_server(FDFSGroupInfo *pGroup,
    ...
    if (*(pGroup->last_trunk_server_id) != '\0' &&
        ...
        if ((result = fdfs_deal_no_body_cmd_ex(
                 pTrunkServer->ip_addr, pGroup->storage_port,
    ...
    logInfo(
        "file: "__FILE__
        ", line: %d, "
        "group: %s, trunk server set to %s(%s:%d)",
        __LINE__, pGroup->group_name, pGroup->pTrunkServer->id,
...
static int tracker_mem_find_trunk_server(FDFSGroupInfo *pGroup,
    ...
    result = tracker_mem_get_trunk_binlog_size(
        pStoreServer->ip_addr, pGroup->storage_port, &max_file_size);
    if (result != 0) {
        return result;
    }
    ...
    for (ppServer = pGroup->active_servers; ppServer < ppServerEnd;
        ...
        result = tracker_mem_get_trunk_binlog_size(
            (*ppServer)->ip_addr, pGroup->storage_port, &file_size);
        if (result != 0) {
            continue;
        }
...
const FDFSStorageDetail *tracker_mem_set_trunk_server(FDFSGroupInfo *pGroup,
                                                      const char *pStroageId,
                                                      int *result) {
    ...
    *result = tracker_mem_do_set_trunk_server(pGroup, pServer, true);
...
int tracker_mem_deactive_store_server(FDFSGroupInfo *pGroup,
    ...
    ppStorageServer = (FDFSStorageDetail **)bsearch(
        &pTargetServer, pGroup->active_servers, pGroup->active_count,
        sizeof(FDFSStorageDetail *), tracker_mem_cmp_by_storage_id);
    if (ppStorageServer != NULL) {
    ...
    if (ppStorageServer == NULL) {
        ...
        if (g_use_storage_id) {
            ...
            logDebug(
                ...
                __LINE__, pGroup->group_name, pTargetServer->ip_addr);
        }
    }
...
int tracker_mem_offline_store_server(FDFSGroupInfo *pGroup,
    ...
    if (g_use_storage_id) {
        logDebug(
            ...
            __LINE__, pGroup->group_name, pStorage->id, pStorage->ip_addr);
    } else {
        logDebug(
            "file: "__FILE__
            ", line: %d, "
            "storage server %s::%s offline",
            __LINE__, pGroup->group_name, pStorage->ip_addr);
    }
...
int tracker_mem_get_storage_by_filename(
    const byte cmd, FDFS_DOWNLOAD_TYPE_PARAM const char *group_name,
    ...
    *ppGroup = tracker_mem_get_group(group_name);
    ...
    } else  // file generated by version >= v1.12
    {
        ...
        base64_decode_auto(
            &g_base64_context, (char *)filename + FDFS_LOGIC_FILE_PATH_LEN,
            FDFS_FILENAME_BASE64_LENGTH, name_buff, &decoded_len);
    ...
    */
    ...
    if (cmd == TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ONE) {
        if (g_groups.download_server == FDFS_DOWNLOAD_SERVER_SOURCE_FIRST) {
            ...
            if (download_type == FDFS_DOWNLOAD_TYPE_TCP) {
                if (*storage_id != '\0') {
                    ...
                    pStoreSrcServer = tracker_mem_get_active_storage_by_ip(
                        *ppGroup, inet_ntop(AF_INET, &ip_addr, szIpAddr,
                                            sizeof(szIpAddr)));
                }
            } else {
                if (*storage_id != '\0') {
            ...
            if (*storage_id != '\0') {
                pStoreSrcServer =
                    tracker_mem_get_active_storage_by_id(*ppGroup, storage_id);
            } else {
                memset(&ip_addr, 0, sizeof(ip_addr));
                ip_addr.s_addr = storage_ip;
                pStoreSrcServer = tracker_mem_get_active_storage_by_ip(
...
// round robin
        ...
        if (download_type == FDFS_DOWNLOAD_TYPE_TCP) {
            ...
            ppStoreServers[(*server_count)++] =
                *((*ppGroup)->active_servers + read_server_index);
        } else {
            ...
            ppStoreServers[(*server_count)++] =
                *((*ppGroup)->http_servers + read_server_index);
        }
#else
        read_server_index = (*ppGroup)->current_read_server;
        if (read_server_index >= (*ppGroup)->active_count) {
        ...
        */
        ...
        do {
            if (bNormalFile) {
                current_time = g_current_time;
                if ((file_timestamp <
                     current_time - g_storage_sync_file_max_delay) ||
            ...
            if (g_groups.download_server ==
                FDFS_DOWNLOAD_SERVER_ROUND_ROBIN) {  // avoid search again
#ifdef WITH_HTTPD
                if (download_type == FDFS_DOWNLOAD_TYPE_TCP) {
                    if (*storage_id != '\0') {
                        pStoreSrcServer = tracker_mem_get_active_storage_by_id(
                ...
                } else  // http
                {
                    if (*storage_id != '\0') {
                        pStoreSrcServer =
                            tracker_mem_get_active_http_server_by_id(
                                *ppGroup, storage_id);
                    } else {
                        pStoreSrcServer =
                            tracker_mem_get_active_http_server_by_ip(*ppGroup,
                                                                     szIpAddr);
                    }
                }
#else
    ...
    } else  // TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ALL
    {
        ...
        if (bNormalFile) {
            ...
            for (ppServer = (*ppGroup)->active_servers; ppServer < ppServerEnd;
                 ppServer++) {
                if ((file_timestamp <
                     current_time - g_storage_sync_file_max_delay) ||
...
int tracker_mem_check_alive(void *arg) {
        ...
        for (ppServer = deactiveServers; ppServer < ppServerEnd; ppServer++) {
            ...
            if (g_use_storage_id) {
                logInfo(
                    ...
                    __LINE__, (*ppServer)->id, (*ppServer)->ip_addr,
                ...
                logInfo(
                    ...
                    "status change to offline!",
                    __LINE__, (*ppServer)->ip_addr, (*ppGroup)->storage_port);
            }
        }
    }
    ...
    for (ppGroup = g_groups.groups; ppGroup < ppGroupEnd; ppGroup++) {
        if ((*ppGroup)->pTrunkServer != NULL) {
            ...
            last_beat_interval =
                current_time -
                (*ppGroup)->pTrunkServer->stat.last_heart_beat_time;
            check_trunk_interval = check_trunk_times * g_check_active_interval;
            if (last_beat_interval > check_trunk_interval) {
                logInfo(
                    ...
                    "re-select trunk server",
                    __LINE__, (*ppGroup)->pTrunkServer->id,
                    (*ppGroup)->pTrunkServer->ip_addr, (*ppGroup)->storage_port,
                    last_beat_interval, check_trunk_interval);
