...
int main(int argc, char *argv[]) {
    ...
    snprintf(pidFilename, sizeof(pidFilename), "%s/data/fdfs_trackerd.pid",
             g_fdfs_base_path);
    ...
    if ((result = tracker_load_from_conf_file(conf_filename, bind_addr,
                                              sizeof(bind_addr))) != 0) {
        logCrit("exit abnormally!\n");
        log_destroy();
        return result;
    ...
    */
    ...
    INIT_SCHEDULE_ENTRY(scheduleEntries[scheduleArray.count],
                        scheduleArray.count + 1, TIME_NONE, TIME_NONE,
                        TIME_NONE, g_check_active_interval,
                        tracker_mem_check_alive, NULL);
    scheduleArray.count++;

    INIT_SCHEDULE_ENTRY(
        scheduleEntries[scheduleArray.count], scheduleArray.count + 1, 0, 0, 0,
...
static void sigDumpHandler(int sig) {
    ...
    snprintf(filename, sizeof(filename), "%s/logs/tracker_dump.log",
             g_fdfs_base_path);
