...
int fdfs_http_get_content_type_by_extname(FDFSHTTPParams *pParams,
    ...
    pHashData =
        hash_find_ex(&pParams->content_type_hash, ext_name, ext_len + 1);
    if (pHashData == NULL) {
        logWarning(
            "file: "__FILE__
            ", line: %d, "
...
int fdfs_http_params_load(IniContext *pIniContext, const char *conf_filename,
    ...
    pParams->disabled =
        iniGetBoolValue(NULL, "http.disabled", pIniContext, false);
    if (pParams->disabled) {
        return 0;
    }

    pParams->need_find_content_type =
        iniGetBoolValue(NULL, "http.need_find_content_type", pIniContext, true);

    pParams->support_multi_range =
        iniGetBoolValue(NULL, "http.multi_range.enabed", pIniContext, true);

    pParams->server_port =
        iniGetIntValue(NULL, "http.server_port", pIniContext, 80);
    ...
    pParams->anti_steal_token = iniGetBoolValue(
        NULL, "http.anti_steal.check_token", pIniContext, false);
    if (pParams->need_find_content_type || pParams->anti_steal_token ||
        pParams->support_multi_range) {
        mime_types_filename =
            iniGetStrValue(NULL, "http.mime_types_filename", pIniContext);
        ...
        if (strncasecmp(mime_types_filename, "http://", 7) != 0 &&
            *mime_types_filename != '/' &&
            ...
            if (pPathEnd == NULL) {
                snprintf(szMimeFilename, sizeof(szMimeFilename), "%s",
                         mime_types_filename);
            } else {
                int nPathLen;
                int nFilenameLen;
        ...
        default_content_type =
            iniGetStrValue(NULL, "http.default_content_type", pIniContext);
    ...
    pParams->token_ttl =
        iniGetIntValue(NULL, "http.anti_steal.token_ttl", pIniContext, 600);
    ...
    anti_steal_secret_key =
        iniGetStrValue(NULL, "http.anti_steal.secret_key", pIniContext);
    ...
    token_check_fail_filename =
        iniGetStrValue(NULL, "http.anti_steal.token_check_fail", pIniContext);
    ...
    if (!fileExists(token_check_fail_filename)) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "token_check_fail file: %s not exists",
            __LINE__, token_check_fail_filename);
        return ENOENT;
    }

    ext_name = fdfs_http_get_file_extension(
        token_check_fail_filename, strlen(token_check_fail_filename), &ext_len);
    if ((result = fdfs_http_get_content_type_by_extname(
             pParams, ext_name, ext_len, pParams->token_check_fail_content_type,
    ...
    if ((result = getFileContent(token_check_fail_filename,
                                 &pParams->token_check_fail_buff.buff,
                                 &file_size)) != 0) {
        return result;
    }
...
int fdfs_http_check_token(const BufferInfo *secret_key, const char *file_id,
    ...
    if ((result = fdfs_http_gen_token(secret_key, file_id, timestamp,
                                      true_token)) != 0) {
        return result;
    }
