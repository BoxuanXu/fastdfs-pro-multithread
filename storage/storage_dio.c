...
int storage_dio_init() {
    ...
    for (pThreadData = g_dio_thread_data; pThreadData < pDataEnd;
         pThreadData++) {
        pThreadData->count = threads_count_per_path;
        pThreadData->contexts =
            g_dio_contexts +
            (pThreadData - g_dio_thread_data) * threads_count_per_path;
        ...
        for (pContext = pThreadData->contexts; pContext < pContextEnd;
            ...
            if ((result = pthread_create(&tid, &thread_attr,
                ...
                logError(
                    ...
                    "startup threads: %d, "
                    "errno: %d, error info: %s",
                    __LINE__, g_dio_thread_count, result, STRERROR(result));
                return result;
            } else {
...
int dio_open_file(StorageFileContext *pFileContext) {
    ...
    if (pFileContext->fd < 0) {
        pFileContext->fd =
            open(pFileContext->filename, pFileContext->open_flags, 0644);
        if (pFileContext->fd < 0) {
            result = errno != 0 ? errno : EACCES;
            logError(
                "file: "__FILE__
                ...
                "open file: %s fail, "
                "errno: %d, error info: %s",
                __LINE__, pFileContext->filename, result, STRERROR(result));
        } else {
            result = 0;
        }
    ...
    if (pFileContext->offset > 0 &&
        lseek(pFileContext->fd, pFileContext->offset, SEEK_SET) < 0) {
...
int dio_read_file(struct fast_task_info *pTask) {
    ...
    do {
        ...
        /*
        logInfo("###before dio read bytes: %d, pTask->length=%d, file
        offset=%ld", \
        ...
        */
        ...
        /*
        logInfo("###after dio read bytes: %d, pTask->length=%d, file
        offset=%ld", \
                read_bytes, pTask->length, pFileContext->offset);
        */
    ...
    do {
        if (pFileContext->fd < 0) {
            if (pFileContext->extra_info.upload.before_open_callback != NULL) {
                result =
                    pFileContext->extra_info.upload.before_open_callback(pTask);
                if (result != 0) {
                    break;
                }
            }
        ...
        if (fc_safe_write(pFileContext->fd, pDataBuff, write_bytes) !=
            write_bytes) {
            result = errno != 0 ? errno : EIO;
            logError(
                "file: "__FILE__
                ...
                "errno: %d, error info: %s",
                __LINE__, pFileContext->filename, pFileContext->fd, write_bytes,
        ...
        if (pFileContext->calc_crc32) {
            pFileContext->crc32 =
                CRC32_ex(pDataBuff, write_bytes, pFileContext->crc32);
        }
        ...
        */
        ...
        if (pFileContext->offset < pFileContext->end) {
            ...
            if (pFileContext->calc_crc32) {
                pFileContext->crc32 = CRC32_FINAL(pFileContext->crc32);
            }

            if (pFileContext->calc_file_hash) {
                if (g_file_signature_method ==
                    STORAGE_FILE_SIGNATURE_METHOD_HASH) {
                    FINISH_HASH_CODES4(pFileContext->file_hash_codes)
                } else {
                    my_md5_final(
                        (unsigned char *)(pFileContext->file_hash_codes),
                        &pFileContext->md5_context);
                }
            }

            if (pFileContext->extra_info.upload.before_close_callback != NULL) {
                result = pFileContext->extra_info.upload.before_close_callback(
                    pTask);
            }
    ...
    do {
        ...
        if (ftruncate(pFileContext->fd, pFileContext->offset) != 0) {
            ...
            logError(
                ...
                ", "
                "errno: %d, error info: %s",
                __LINE__, pFileContext->filename, pFileContext->fd,
                pFileContext->offset, result, STRERROR(result));
            break;
        }

        if (pFileContext->extra_info.upload.before_close_callback != NULL) {
            result =
                pFileContext->extra_info.upload.before_close_callback(pTask);
        }
        ...
        if (pFileContext->offset < pFileContext->end) {
            if (unlink(pFileContext->filename) != 0) {
                logError(
                    ...
                    "errno: %d, error info: %s",
                    __LINE__, pTask->client_ip, pFileContext->filename, errno,
                    STRERROR(errno));
            }
        }
    }
...
int dio_check_trunk_file_when_upload(struct fast_task_info *pTask) {
    ...
    return dio_check_trunk_file_ex(
        pFileContext->fd, pFileContext->filename,
...
int dio_check_trunk_file_ex(int fd, const char *filename,
    ...
    if (memcmp(old_header, expect_header, FDFS_TRUNK_FILE_HEADER_SIZE) != 0) {
        ...
        memcpy(&newOldTrunkHeader, &srcOldTrunkHeader, sizeof(FDFSTrunkHeader));
        ...
        if (memcmp(old_header, expect_header, FDFS_TRUNK_FILE_HEADER_SIZE) !=
            0) {
            char buff[256];
            trunk_header_dump(&srcOldTrunkHeader, buff, sizeof(buff));
...
int dio_write_chunk_header(struct fast_task_info *pTask) {
    ...
    trunkHeader.alloc_size =
        pFileContext->extra_info.upload.trunk_info.file.size;
    ...
    logInfo("file: "__FILE__", line: %d, my trunk=%s, my fields=%s", __LINE__, \
            trunk_info_dump(&pFileContext->extra_info.upload.trunk_info, buff1,
    sizeof(buff1)), \
