...
static int tracker_rename_mark_files(const char *old_ip_addr,
    ...
    result =
        storage_rename_mark_file(old_ip_addr, old_port, new_ip_addr, new_port);
    result +=
        trunk_rename_mark_file(old_ip_addr, old_port, new_ip_addr, new_port);
    return result;
}
...
static void *tracker_report_thread_entrance(void *arg) {
    ...
    bServerPortChanged =
        (g_last_server_port != 0) && (g_server_port != g_last_server_port);
    ...
    while (g_continue_flag) {
        ...
        if ((result = connectserverbyip_nb(
                 pTrackerServer->sock, pTrackerServer->ip_addr,
        ...
        */
        ...
        if (tracker_report_join(pTrackerServer, tracker_index, sync_old_done) !=
            0) {
            sleep(g_heart_beat_interval);
            continue;
        }
        ...
        if (!sync_old_done) {
            ...
            if (!g_sync_old_done) {
                if (tracker_sync_dest_req(pTrackerServer) == 0) {
                    ...
                    if (storage_write_to_sync_ini_file() != 0) {
                        logCrit(
                            "file: "__FILE__
                            ", line: %d, "
        ...
        if (src_storage_status[tracker_index] != 0) {
            ...
            if (k ==
                g_tracker_group
                    .server_count) {  // src storage server already be deleted
                int my_status;
                if (tracker_get_storage_max_status(
                        &g_tracker_group, g_group_name, g_tracker_client_ip,
                        my_server_id, &my_status) == 0) {
                    tracker_sync_dest_query(pTrackerServer);
                    if (my_status < FDFS_STORAGE_STATUS_OFFLINE &&
                        g_sync_old_done) {  // need re-sync old files
        ...
        while (g_continue_flag) {
            ...
            if (current_time - last_beat_time >= g_heart_beat_interval) {
                if (tracker_heart_beat(pTrackerServer, &stat_chg_sync_count,
                                       &bServerPortChanged) != 0) {
                    break;
                }

                if (g_storage_ip_changed_auto_adjust &&
                    tracker_storage_changelog_req(pTrackerServer) != 0) {
                    break;
                }
            ...
            if (sync_time_chg_count != g_sync_change_count &&
                current_time - last_sync_report_time >= g_heart_beat_interval) {
                if (tracker_report_sync_timestamp(pTrackerServer,
                                                  &bServerPortChanged) != 0) {
                    break;
                }
    ...
    if (nContinuousFail > 0) {
        logError(
            ...
            ", errno: %d, error info: %s",
            __LINE__, pTrackerServer->ip_addr, pTrackerServer->port,
            nContinuousFail, result, STRERROR(result));
    }
...
static bool tracker_insert_into_sorted_servers(
    ...
    for (ppServer = ppEnd; ppServer > g_sorted_storages; ppServer--) {
        nCompare =
            strcmp(pInsertedServer->server.id, (*(ppServer - 1))->server.id);
        if (nCompare > 0) {
...
int tracker_sync_diff_servers(ConnectionInfo *pTrackerServer,
    ...
    if ((result = tcpsenddata_nb(pTrackerServer->sock, briefServers, out_len,
                                 g_fdfs_network_timeout)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if ((result = tcprecvdata_nb(pTrackerServer->sock, &resp, sizeof(resp),
                                 g_fdfs_network_timeout)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if (memcmp(resp.pkg_len, "\0\0\0\0\0\0\0\0", FDFS_PROTO_PKG_LEN_SIZE) !=
        0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
...
int tracker_report_storage_status(ConnectionInfo *pTrackerServer,
    ...
    if ((result =
             tcpsenddata_nb(pTrackerServer->sock, out_buff,
...
static int tracker_merge_servers(ConnectionInfo *pTrackerServer,
        ...
        if (ppFound != NULL) {
            ...
            */
            ...
            if (pServer->status == FDFS_STORAGE_STATUS_OFFLINE) {
                ...
                if (pServer->status == FDFS_STORAGE_STATUS_DELETED ||
                    pServer->status ==
                        FDFS_STORAGE_STATUS_IP_CHANGED) {  // ignore
                } else {
                    (*ppFound)->server.status = pServer->status;
                ...
                *(pServer->ip_addr + IP_ADDRESS_SIZE - 1) = '\0';
                if (is_local_host_ip(pServer->ip_addr) &&
                    ...
                    logWarning(
                        ...
                        "tracker server: %s:%d",
                        __LINE__, pServer->status, (*ppFound)->server.status,
            ...
            */
            ...
            if (g_storage_count < FDFS_MAX_SERVERS_EACH_GROUP) {
                ...
                memcpy(&(pInsertedServer->server), pServer,
                       sizeof(FDFSStorageBrief));
                if (tracker_insert_into_sorted_servers(pInsertedServer)) {
                    g_storage_count++;

                    result =
                        tracker_start_sync_threads(&(pInsertedServer->server));
                } else {
                    result = 0;
                }
            } else {
                logError(
                    ...
                    "exceeds max: %d",
                    __LINE__, pTrackerServer->ip_addr, pTrackerServer->port,
                    g_group_name, FDFS_MAX_SERVERS_EACH_GROUP);
                result = ENOSPC;
            }

            if ((res = pthread_mutex_unlock(&reporter_thread_lock)) != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
                    "call pthread_mutex_unlock fail, "
    ...
    if (g_storage_count + nDeletedCount == server_count) {
        if (pDiffServer - diffServers > 0) {
            return tracker_sync_diff_servers(pTrackerServer, diffServers,
                                             pDiffServer - diffServers);
        }
        ...
        if (fdfs_get_tracker_status(&old_leader_server, &tracker_status) == 0) {
            if (tracker_status.if_leader) {
                ...
                memcpy(&new_leader_server,
                       g_tracker_group.servers + leader_index,
                       sizeof(ConnectionInfo));
                logWarning(
                    "file: "__FILE__
                    ", line: %d, "
                    ...
                    "tracker leader",
                    __LINE__, old_leader_server.ip_addr, old_leader_server.port,
        ...
        if (fdfs_get_tracker_status(&tracker_server, &tracker_status) == 0) {
            if (tracker_status.if_leader) {
                ...
                logInfo(
                    ...
                    __LINE__, i, tracker_server.ip_addr, tracker_server.port);
                break;
            }
        }
    }
...
static int tracker_check_response(ConnectionInfo *pTrackerServer,
    ...
    int result;
    char in_buff[1 +
                 (2 + FDFS_MAX_SERVERS_EACH_GROUP) * sizeof(FDFSStorageBrief)];
    ...
    if ((nInPackLen <= 0) ||
        ((nInPackLen - 1) % sizeof(FDFSStorageBrief) != 0)) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "tracker server %s:%d, "
    ...
    if (nInPackLen > sizeof(in_buff)) {
        logError(
            ...
            " is too large, "
            "exceed max: %d",
            __LINE__, pTrackerServer->ip_addr, pTrackerServer->port, nInPackLen,
            (int)sizeof(in_buff));
        return EINVAL;
    }

    if ((result = tcprecvdata_nb(pTrackerServer->sock, in_buff, nInPackLen,
                                 g_fdfs_network_timeout)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if ((*pFlags) & FDFS_CHANGE_FLAG_TRACKER_LEADER) {
        ...
        memcpy(tracker_leader_ip, pBriefServers->ip_addr, IP_ADDRESS_SIZE - 1);
        *(tracker_leader_ip + (IP_ADDRESS_SIZE - 1)) = '\0';
        ...
        if (*tracker_leader_ip == '\0') {
            if (g_tracker_group.leader_index >= 0) {
                ...
                pTrackerLeader =
                    g_tracker_group.servers + g_tracker_group.leader_index;
                logWarning(
                    "file: "__FILE__
                    ", line: %d, "
                    "tracker server %s:%d, "
                    ...
                    __LINE__, pTrackerServer->ip_addr, pTrackerServer->port,
                    pTrackerLeader->ip_addr, pTrackerLeader->port);
            ...
            leader_index = fdfs_get_tracker_leader_index(tracker_leader_ip,
                                                         tracker_leader_port);
            if (leader_index < 0) {
                logWarning(
                    "file: "__FILE__
            ...
            if (is_local_host_ip(g_trunk_server.ip_addr) &&
                ...
                if (g_if_trunker_self) {
                    logWarning(
                        ...
                        __LINE__, g_trunk_server.ip_addr, g_trunk_server.port);
                } else {
                    logInfo(
                        "file: "__FILE__
                        ", line: %d, "
                        "I am the the trunk server %s:%d",
                        __LINE__, g_trunk_server.ip_addr, g_trunk_server.port);
    ...
    */
        ...
        if (!g_use_storage_id) {
            ...
            *bServerPortChanged = false;
            ...
            for (pStorage = pBriefServers; pStorage < pStorageEnd; pStorage++) {
                ...
                tracker_rename_mark_files(pStorage->ip_addr, g_last_server_port,
    ...
    return tracker_merge_servers(pTrackerServer, pBriefServers, server_count);
}
...
int tracker_sync_src_req(ConnectionInfo *pTrackerServer,
    ...
    if ((result = fdfs_recv_response(pTrackerServer, &pBuff,
                                     sizeof(syncReqbody), &in_bytes)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    pReader->until_timestamp = (time_t)buff2long(syncReqbody.until_timestamp);
...
static int tracker_fetch_trunk_fid(ConnectionInfo *pTrackerServer) {
    ...
    pInBuff = in_buff;
    if ((result = fdfs_recv_response(pTrackerServer, &pInBuff, sizeof(in_buff),
                                     &in_bytes)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if (trunk_fid < 0) {
        logError(
            ...
            __LINE__, pTrackerServer->ip_addr, pTrackerServer->port, trunk_fid);
        return EINVAL;
    }
...
int tracker_report_join(ConnectionInfo *pTrackerServer, const int tracker_index,
    ...
    if ((result = tcpsenddata_nb(pTrackerServer->sock, out_buff, out_len,
                                 g_fdfs_network_timeout)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    result = fdfs_recv_response(pTrackerServer, &pInBuff, sizeof(respBody),
                                &in_bytes);
...
static int tracker_report_sync_timestamp(ConnectionInfo *pTrackerServer,
                                         bool *bServerPortChanged) {
    char out_buff[sizeof(TrackerHeader) +
                  (FDFS_STORAGE_ID_MAX_SIZE + 4) * FDFS_MAX_SERVERS_EACH_GROUP];
    char *p;
    TrackerHeader *pHeader;
...
static int tracker_report_df_stat(ConnectionInfo *pTrackerServer,
                                  bool *bServerPortChanged) {
    char
        out_buff[sizeof(TrackerHeader) + sizeof(TrackerStatReportReqBody) * 16];
    char *pBuff;
    TrackerHeader *pHeader;
    ...
    if (total_len <= sizeof(out_buff)) {
        ...
        if (pBuff == NULL) {
            logError(
                ...
                "malloc %d bytes fail, "
                "errno: %d, error info: %s",
                __LINE__, total_len, errno, STRERROR(errno));
    ...
    for (i = 0; i < g_fdfs_store_paths.count; i++) {
        ...
        g_path_space_list[i].total_mb =
            ((int64_t)(sbuf.f_blocks) * sbuf.f_frsize) / FDFS_ONE_MB;
        g_path_space_list[i].free_mb =
            ((int64_t)(sbuf.f_bavail) * sbuf.f_frsize) / FDFS_ONE_MB;
    ...
    if (g_store_path_mode == FDFS_STORE_PATH_LOAD_BALANCE) {
        ...
        store_path_index = -1;
        for (i = 0; i < g_fdfs_store_paths.count; i++) {
            if (g_path_space_list[i].free_mb > g_avg_storage_reserved_mb &&
    ...
    result = tcpsenddata_nb(pTrackerServer->sock, pBuff, total_len,
...
#define FDFS_CHANGELOG_FIELDS 5
    ...
    *(pInBuff + nInPackLen) = '\0';
    ...
    while (pLineStart < pBuffEnd) {
        ...
        col_count = splitEx(szLine, ' ', cols, FDFS_CHANGELOG_FIELDS + 1);
        ...
        do {
            if (col_count != FDFS_CHANGELOG_FIELDS) {
                logError(
                    ...
                    "line content=%s",
                    __LINE__, col_count, FDFS_CHANGELOG_FIELDS, pLineStart);
                break;
            }

            pGroupName = cols[1];
            if (strcmp(pGroupName, g_group_name) !=
                0) {  // ignore other group's changelog
                break;
            }
            ...
            if (server_status == FDFS_STORAGE_STATUS_DELETED) {
                ...
                if (!g_use_storage_id) {
                    tracker_rename_mark_files(pOldStorageId, g_server_port,
                                              pNewStorageId, g_server_port);
                    if (strcmp(g_sync_src_id, pOldStorageId) == 0) {
                        snprintf(g_sync_src_id, sizeof(g_sync_src_id), "%s",
                                 pNewStorageId);
                        storage_write_to_sync_ini_file();
                    }
                }
                ...
                logError(
                    ...
                    "line content=%s",
                    __LINE__, server_status, pLineStart);
            }
        } while (0);
...
int tracker_report_thread_start() {
    ...
    report_tids =
        (pthread_t *)malloc(sizeof(pthread_t) * g_tracker_group.server_count);
    if (report_tids == NULL) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, (int)sizeof(pthread_t) * g_tracker_group.server_count,
    ...
    src_storage_status =
        (int *)malloc(sizeof(int) * g_tracker_group.server_count);
    if (src_storage_status == NULL) {
        logError(
            ...
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, (int)sizeof(int) * g_tracker_group.server_count, errno,
            STRERROR(errno));
        return errno != 0 ? errno : ENOMEM;
    }
    ...
    if (my_report_status == NULL) {
        logError(
            ...
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, (int)sizeof(signed char) * g_tracker_group.server_count,
            errno, STRERROR(errno));
        return errno != 0 ? errno : ENOMEM;
    }
    ...
    for (pTrackerServer = g_tracker_group.servers; pTrackerServer < pServerEnd;
         pTrackerServer++) {
        if ((result =
                 pthread_create(&tid, &pattr, tracker_report_thread_entrance,
                                pTrackerServer)) != 0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
