...
static int storage_do_get_group_name(ConnectionInfo *pTrackerServer) {
    ...
    if ((result = fdfs_recv_response(pTrackerServer, &pInBuff,
                                     FDFS_GROUP_NAME_MAX_LEN, &in_bytes)) !=
        0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
...
static int storage_get_group_name_from_tracker() {
    ...
    for (pTrackerServer = g_tracker_group.servers; pTrackerServer < pServerEnd;
         pTrackerServer++) {
        memcpy(&tracker_server, pTrackerServer, sizeof(ConnectionInfo));
        tracker_server.sock = -1;
        if ((pTrackerConn = tracker_connect_server(&tracker_server, &result)) ==
            NULL) {
            continue;
        }
...
static int tracker_get_my_server_id() {
    ...
    if (g_use_storage_id) {
        ...
        result =
            tracker_get_storage_id(pTrackerServer, g_group_name,
                                   g_tracker_client_ip, g_my_server_id_str);
    ...
    logInfo(
        ...
        "g_server_id_in_filename: %d",
        __LINE__, g_tracker_client_ip, g_my_server_id_str,
...
static char *get_storage_stat_filename(const void *pArg, char *full_filename) {
    ...
    snprintf(full_filename, MAX_PATH_SIZE, "%s/data/%s", g_fdfs_base_path,
...
int storage_write_to_fd(int fd, get_filename_func filename_func,
    ...
    if (ftruncate(fd, 0) != 0) {
        logError(
            ...
            __LINE__, filename_func(pArg, NULL), errno, STRERROR(errno));
...
static int storage_open_stat_file() {
    ...
    if (fileExists(full_filename)) {
        ...
        g_storage_stat.total_upload_count =
            iniGetInt64Value(NULL, STAT_ITEM_TOTAL_UPLOAD, &iniContext, 0);
        g_storage_stat.success_upload_count =
            iniGetInt64Value(NULL, STAT_ITEM_SUCCESS_UPLOAD, &iniContext, 0);
        g_storage_stat.total_append_count =
            iniGetInt64Value(NULL, STAT_ITEM_TOTAL_APPEND, &iniContext, 0);
        g_storage_stat.success_append_count =
            iniGetInt64Value(NULL, STAT_ITEM_SUCCESS_APPEND, &iniContext, 0);
        g_storage_stat.total_modify_count =
            iniGetInt64Value(NULL, STAT_ITEM_TOTAL_MODIFY, &iniContext, 0);
        g_storage_stat.success_modify_count =
            iniGetInt64Value(NULL, STAT_ITEM_SUCCESS_MODIFY, &iniContext, 0);
        g_storage_stat.total_truncate_count =
            iniGetInt64Value(NULL, STAT_ITEM_TOTAL_TRUNCATE, &iniContext, 0);
        g_storage_stat.success_truncate_count =
            iniGetInt64Value(NULL, STAT_ITEM_SUCCESS_TRUNCATE, &iniContext, 0);
        g_storage_stat.total_download_count =
            iniGetInt64Value(NULL, STAT_ITEM_TOTAL_DOWNLOAD, &iniContext, 0);
        g_storage_stat.success_download_count =
            iniGetInt64Value(NULL, STAT_ITEM_SUCCESS_DOWNLOAD, &iniContext, 0);
        g_storage_stat.last_source_update =
            iniGetIntValue(NULL, STAT_ITEM_LAST_SOURCE_UPD, &iniContext, 0);
        g_storage_stat.last_sync_update =
            iniGetIntValue(NULL, STAT_ITEM_LAST_SYNC_UPD, &iniContext, 0);
        g_storage_stat.total_set_meta_count =
            iniGetInt64Value(NULL, STAT_ITEM_TOTAL_SET_META, &iniContext, 0);
        g_storage_stat.success_set_meta_count =
            iniGetInt64Value(NULL, STAT_ITEM_SUCCESS_SET_META, &iniContext, 0);
        g_storage_stat.total_delete_count =
            iniGetInt64Value(NULL, STAT_ITEM_TOTAL_DELETE, &iniContext, 0);
        g_storage_stat.success_delete_count =
            iniGetInt64Value(NULL, STAT_ITEM_SUCCESS_DELETE, &iniContext, 0);
        g_storage_stat.total_get_meta_count =
            iniGetInt64Value(NULL, STAT_ITEM_TOTAL_GET_META, &iniContext, 0);
        g_storage_stat.success_get_meta_count =
            iniGetInt64Value(NULL, STAT_ITEM_SUCCESS_GET_META, &iniContext, 0);
        g_storage_stat.total_create_link_count =
            iniGetInt64Value(NULL, STAT_ITEM_TOTAL_CREATE_LINK, &iniContext, 0);
        g_storage_stat.success_create_link_count = iniGetInt64Value(
            NULL, STAT_ITEM_SUCCESS_CREATE_LINK, &iniContext, 0);
        g_storage_stat.total_delete_link_count =
            iniGetInt64Value(NULL, STAT_ITEM_TOTAL_DELETE_LINK, &iniContext, 0);
        g_storage_stat.success_delete_link_count = iniGetInt64Value(
            NULL, STAT_ITEM_SUCCESS_DELETE_LINK, &iniContext, 0);
        g_storage_stat.total_upload_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_TOTAL_UPLOAD_BYTES, &iniContext, 0);
        g_storage_stat.success_upload_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_SUCCESS_UPLOAD_BYTES, &iniContext, 0);
        g_storage_stat.total_append_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_TOTAL_APPEND_BYTES, &iniContext, 0);
        g_storage_stat.success_append_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_SUCCESS_APPEND_BYTES, &iniContext, 0);
        g_storage_stat.total_modify_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_TOTAL_MODIFY_BYTES, &iniContext, 0);
        g_storage_stat.success_modify_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_SUCCESS_MODIFY_BYTES, &iniContext, 0);
        g_storage_stat.total_download_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_TOTAL_DOWNLOAD_BYTES, &iniContext, 0);
        g_storage_stat.success_download_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_SUCCESS_DOWNLOAD_BYTES, &iniContext, 0);
        g_storage_stat.total_sync_in_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_TOTAL_SYNC_IN_BYTES, &iniContext, 0);
        g_storage_stat.success_sync_in_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_SUCCESS_SYNC_IN_BYTES, &iniContext, 0);
        g_storage_stat.total_sync_out_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_TOTAL_SYNC_OUT_BYTES, &iniContext, 0);
        g_storage_stat.success_sync_out_bytes = iniGetInt64Value(
            NULL, STAT_ITEM_SUCCESS_SYNC_OUT_BYTES, &iniContext, 0);
        g_storage_stat.total_file_open_count = iniGetInt64Value(
            NULL, STAT_ITEM_TOTAL_FILE_OPEN_COUNT, &iniContext, 0);
        g_storage_stat.success_file_open_count = iniGetInt64Value(
            NULL, STAT_ITEM_SUCCESS_FILE_OPEN_COUNT, &iniContext, 0);
        g_storage_stat.total_file_read_count = iniGetInt64Value(
            NULL, STAT_ITEM_TOTAL_FILE_READ_COUNT, &iniContext, 0);
        g_storage_stat.success_file_read_count = iniGetInt64Value(
            NULL, STAT_ITEM_SUCCESS_FILE_READ_COUNT, &iniContext, 0);
        g_storage_stat.total_file_write_count = iniGetInt64Value(
            NULL, STAT_ITEM_TOTAL_FILE_WRITE_COUNT, &iniContext, 0);
        g_storage_stat.success_file_write_count = iniGetInt64Value(
            NULL, STAT_ITEM_SUCCESS_FILE_WRITE_COUNT, &iniContext, 0);
        g_dist_path_index_high = iniGetIntValue(
            NULL, STAT_ITEM_DIST_PATH_INDEX_HIGH, &iniContext, 0);
        g_dist_path_index_low =
            iniGetIntValue(NULL, STAT_ITEM_DIST_PATH_INDEX_LOW, &iniContext, 0);
        g_dist_write_file_count = iniGetIntValue(
            NULL, STAT_ITEM_DIST_WRITE_FILE_COUNT, &iniContext, 0);
...
int storage_write_to_stat_file() {
    ...
    len = sprintf(
        ...
        STAT_ITEM_SUCCESS_DOWNLOAD, g_storage_stat.success_download_count,
        STAT_ITEM_TOTAL_SET_META, g_storage_stat.total_set_meta_count,
        STAT_ITEM_SUCCESS_SET_META, g_storage_stat.success_set_meta_count,
        ...
        STAT_ITEM_SUCCESS_GET_META, g_storage_stat.success_get_meta_count,
        STAT_ITEM_TOTAL_CREATE_LINK, g_storage_stat.total_create_link_count,
        STAT_ITEM_SUCCESS_CREATE_LINK, g_storage_stat.success_create_link_count,
        STAT_ITEM_TOTAL_DELETE_LINK, g_storage_stat.total_delete_link_count,
        STAT_ITEM_SUCCESS_DELETE_LINK, g_storage_stat.success_delete_link_count,
        STAT_ITEM_TOTAL_UPLOAD_BYTES, g_storage_stat.total_upload_bytes,
        STAT_ITEM_SUCCESS_UPLOAD_BYTES, g_storage_stat.success_upload_bytes,
        STAT_ITEM_TOTAL_APPEND_BYTES, g_storage_stat.total_append_bytes,
        STAT_ITEM_SUCCESS_APPEND_BYTES, g_storage_stat.success_append_bytes,
        STAT_ITEM_TOTAL_MODIFY_BYTES, g_storage_stat.total_modify_bytes,
        STAT_ITEM_SUCCESS_MODIFY_BYTES, g_storage_stat.success_modify_bytes,
        STAT_ITEM_TOTAL_DOWNLOAD_BYTES, g_storage_stat.total_download_bytes,
        STAT_ITEM_SUCCESS_DOWNLOAD_BYTES, g_storage_stat.success_download_bytes,
        STAT_ITEM_TOTAL_SYNC_IN_BYTES, g_storage_stat.total_sync_in_bytes,
        STAT_ITEM_SUCCESS_SYNC_IN_BYTES, g_storage_stat.success_sync_in_bytes,
        STAT_ITEM_TOTAL_SYNC_OUT_BYTES, g_storage_stat.total_sync_out_bytes,
        STAT_ITEM_SUCCESS_SYNC_OUT_BYTES, g_storage_stat.success_sync_out_bytes,
        STAT_ITEM_TOTAL_FILE_OPEN_COUNT, g_storage_stat.total_file_open_count,
        STAT_ITEM_SUCCESS_FILE_OPEN_COUNT,
        g_storage_stat.success_file_open_count, STAT_ITEM_TOTAL_FILE_READ_COUNT,
        g_storage_stat.total_file_read_count, STAT_ITEM_SUCCESS_FILE_READ_COUNT,
        g_storage_stat.success_file_read_count,
        STAT_ITEM_TOTAL_FILE_WRITE_COUNT, g_storage_stat.total_file_write_count,
        STAT_ITEM_SUCCESS_FILE_WRITE_COUNT,
        g_storage_stat.success_file_write_count, STAT_ITEM_LAST_SOURCE_UPD,
        (int)g_storage_stat.last_source_update, STAT_ITEM_LAST_SYNC_UPD,
        (int)g_storage_stat.last_sync_update, STAT_ITEM_DIST_PATH_INDEX_HIGH,
        g_dist_path_index_high, STAT_ITEM_DIST_PATH_INDEX_LOW,
        g_dist_path_index_low, STAT_ITEM_DIST_WRITE_FILE_COUNT,
    ...
    write_ret = storage_write_to_fd(storage_stat_fd, get_storage_stat_filename,
                                    NULL, buff, len);
...
int storage_write_to_sync_ini_file() {
    ...
    snprintf(full_filename, sizeof(full_filename), "%s/data/%s",
             g_fdfs_base_path, DATA_DIR_INITED_FILENAME);
    ...
    len = sprintf(
        ...
        INIT_ITEM_SYNC_OLD_DONE, g_sync_old_done, INIT_ITEM_SYNC_SRC_SERVER,
        g_sync_src_id, INIT_ITEM_SYNC_UNTIL_TIMESTAMP, g_sync_until_timestamp,
...
static int storage_check_and_make_data_dirs() {
    ...
    snprintf(full_filename, sizeof(full_filename), "%s/%s", data_path,
             DATA_DIR_INITED_FILENAME);
    if (fileExists(full_filename)) {
        IniContext iniContext;
        char *pValue;
        ...
        if ((result = iniLoadFromFile(full_filename, &iniContext)) != 0) {
            logError(
                ...
                "error code: %d",
                __LINE__, data_path, full_filename, result);
            return result;
        }

        pValue = iniGetStrValue(NULL, INIT_ITEM_STORAGE_JOIN_TIME, &iniContext);
        if (pValue == NULL) {
            iniFreeContext(&iniContext);
            logError(
                "file: "__FILE__
        ...
        pValue = iniGetStrValue(NULL, INIT_ITEM_SYNC_OLD_DONE, &iniContext);
        if (pValue == NULL) {
            iniFreeContext(&iniContext);
            logError(
                "file: "__FILE__
                ", line: %d, "
                "in file \"%s/%s\", item \"%s\" not exists",
                __LINE__, data_path, full_filename, INIT_ITEM_SYNC_OLD_DONE);
            return ENOENT;
        }
        g_sync_old_done = atoi(pValue);

        pValue = iniGetStrValue(NULL, INIT_ITEM_SYNC_SRC_SERVER, &iniContext);
        if (pValue == NULL) {
            iniFreeContext(&iniContext);
            logError(
                "file: "__FILE__
                ", line: %d, "
                "in file \"%s/%s\", item \"%s\" not exists",
                __LINE__, data_path, full_filename, INIT_ITEM_SYNC_SRC_SERVER);
            return ENOENT;
        }
        snprintf(g_sync_src_id, sizeof(g_sync_src_id), "%s", pValue);

        g_sync_until_timestamp = iniGetIntValue(
            NULL, INIT_ITEM_SYNC_UNTIL_TIMESTAMP, &iniContext, 0);

        pValue = iniGetStrValue(NULL, INIT_ITEM_LAST_IP_ADDRESS, &iniContext);
        if (pValue != NULL) {
            snprintf(g_last_storage_ip, sizeof(g_last_storage_ip), "%s",
                     pValue);
        }

        pValue = iniGetStrValue(NULL, INIT_ITEM_LAST_SERVER_PORT, &iniContext);
        if (pValue != NULL) {
            g_last_server_port = atoi(pValue);
        }

        pValue = iniGetStrValue(NULL, INIT_ITEM_LAST_HTTP_PORT, &iniContext);
        if (pValue != NULL) {
            g_last_http_port = atoi(pValue);
        }

        g_current_trunk_file_id = iniGetIntValue(
            NULL, INIT_ITEM_CURRENT_TRUNK_FILE_ID, &iniContext, 0);
        g_trunk_last_compress_time = iniGetIntValue(
            NULL, INIT_ITEM_TRUNK_LAST_COMPRESS_TIME, &iniContext, 0);
...
static int storage_make_data_dirs(const char *pBasePath, bool *pathCreated) {
    ...
    *pathCreated = false;
    ...
    sprintf(min_sub_path,
            FDFS_STORAGE_DATA_DIR_FORMAT "/" FDFS_STORAGE_DATA_DIR_FORMAT, 0,
            0);
    sprintf(max_sub_path,
            FDFS_STORAGE_DATA_DIR_FORMAT "/" FDFS_STORAGE_DATA_DIR_FORMAT,
        ...
        if (mkdir(dir_name, 0755) != 0) {
            if (!(errno == EEXIST && isDir(dir_name))) {
                logError(
                    ...
                    "mkdir \"%s/%s\" fail, "
                    "errno: %d, error info: %s",
                    __LINE__, data_path, dir_name, errno, STRERROR(errno));
                return errno != 0 ? errno : ENOENT;
            }
        }
            ...
            if (mkdir(sub_name, 0755) != 0) {
                if (!(errno == EEXIST && isDir(sub_name))) {
                    logError(
                        ...
                        " mkdir \"%s/%s/%s\" fail, "
                        "errno: %d, error info: %s",
                        __LINE__, data_path, dir_name, sub_name, errno,
                        STRERROR(errno));
                    return errno != 0 ? errno : ENOENT;
                }
            }
...
static int init_fsync_pthread_cond()
{
        ...
        if ((result=pthread_cond_init(&fsync_thread_cond, &thread_condattr)) !=
0)
        {
                logError("file: "__FILE__", line: %d, " \
...
#define STORAGE_ACCESS_HEADER_STR \
    "client_ip action filename status time_used_ms req_len resp_len"
...
int storage_func_init(const char *filename, char *bind_addr,
                      const int addr_size) {
    char *pBindAddr;
    char *pGroupName;
    char *pRunByGroup;
    ...
    */
    ...
    do {
        ...
        g_subdir_count_per_path =
            iniGetIntValue(NULL, "subdir_count_per_path", &iniContext,
        ...
        g_fdfs_connect_timeout = iniGetIntValue(
            NULL, "connect_timeout", &iniContext, DEFAULT_CONNECT_TIMEOUT);
        if (g_fdfs_connect_timeout <= 0) {
            g_fdfs_connect_timeout = DEFAULT_CONNECT_TIMEOUT;
        }

        g_fdfs_network_timeout = iniGetIntValue(
        ...
        g_heart_beat_interval =
            iniGetIntValue(NULL, "heart_beat_interval", &iniContext,
        ...
        g_stat_report_interval =
            iniGetIntValue(NULL, "stat_report_interval", &iniContext,
        ...
        g_client_bind_addr =
            iniGetBoolValue(NULL, "client_bind", &iniContext, true);

        result =
            fdfs_load_tracker_group_ex(&g_tracker_group, filename, &iniContext);
        if (result != 0) {
            break;
        }
        ...
        for (pServer = g_tracker_group.servers; pServer < pEnd; pServer++) {
            ...
            if (strcmp(pServer->ip_addr, "127.0.0.1") == 0) {
                logError(
                    ...
                    __LINE__, filename, pServer->ip_addr, pServer->port);
                result = EINVAL;
                break;
            }
        }
        ...
        g_sync_wait_usec = iniGetIntValue(NULL, "sync_wait_msec", &iniContext,
        ...
        g_sync_interval = iniGetIntValue(NULL, "sync_interval", &iniContext, 0);
        if (g_sync_interval < 0) {
            g_sync_interval = 0;
        }
        g_sync_interval *= 1000;

        if ((result = get_time_item_from_conf(&iniContext, "sync_start_time",
                                              &g_sync_start_time, 0, 0)) != 0) {
            break;
        }
        if ((result = get_time_item_from_conf(&iniContext, "sync_end_time",
                                              &g_sync_end_time, 23, 59)) != 0) {
            break;
        }

        g_sync_part_time =
            !((g_sync_start_time.hour == 0 && g_sync_start_time.minute == 0) &&
              (g_sync_end_time.hour == 23 && g_sync_end_time.minute == 59));

        g_max_connections = iniGetIntValue(NULL, "max_connections", &iniContext,
        ...
        g_accept_threads =
            iniGetIntValue(NULL, "accept_threads", &iniContext, 1);
        if (g_accept_threads <= 0) {
            logError(
                ...
                "value: %d <= 0!",
                __LINE__, g_accept_threads);
            result = EINVAL;
            break;
        }

        g_work_threads = iniGetIntValue(NULL, "work_threads", &iniContext,
        ...
        if (g_work_threads <= 0) {
            logError(
                ...
                "value: %d <= 0!",
                __LINE__, g_work_threads);
            result = EINVAL;
            break;
        }

        pBuffSize = iniGetStrValue(NULL, "buff_size", &iniContext);
        if (pBuffSize == NULL) {
        ...
        if (g_buff_size < 4 * 1024 ||
            g_buff_size < sizeof(TrackerHeader) + TRUNK_BINLOG_BUFFER_SIZE) {
            logError(
                "file: "__FILE__
                ", line: %d, "
                "item \"buff_size\" is too small, "
                "value: %d < %d or < %d!",
                __LINE__, g_buff_size, 4 * 1024,
        ...
        g_disk_rw_separated =
            iniGetBoolValue(NULL, "disk_rw_separated", &iniContext, true);

        g_disk_reader_threads =
            iniGetIntValue(NULL, "disk_reader_threads", &iniContext,
                           DEFAULT_DISK_READER_THREADS);
        if (g_disk_reader_threads < 0) {
            logError(
                ...
                "value: %d < 0!",
                __LINE__, g_disk_reader_threads);
            result = EINVAL;
            break;
        }

        g_disk_writer_threads =
            iniGetIntValue(NULL, "disk_writer_threads", &iniContext,
                           DEFAULT_DISK_WRITER_THREADS);
        if (g_disk_writer_threads < 0) {
            logError(
                ...
                "value: %d < 0!",
                __LINE__, g_disk_writer_threads);
            result = EINVAL;
            break;
        }
        ...
        */
        ...
        if (pRunByGroup == NULL) {
            *g_run_by_group = '\0';
            ...
            snprintf(g_run_by_group, sizeof(g_run_by_group), "%s", pRunByGroup);
        }
        if (*g_run_by_group == '\0') {
        ...
        if (pRunByUser == NULL) {
            *g_run_by_user = '\0';
            ...
            snprintf(g_run_by_user, sizeof(g_run_by_user), "%s", pRunByUser);
        }
        if (*g_run_by_user == '\0') {
        ...
        if ((result = load_allow_hosts(&iniContext, &g_allow_ip_addrs,
                                       &g_allow_ip_count)) != 0) {
            return result;
        }

        g_file_distribute_path_mode =
            iniGetIntValue(NULL, "file_distribute_path_mode", &iniContext,
                           FDFS_FILE_DIST_PATH_ROUND_ROBIN);
        g_file_distribute_rotate_count =
            iniGetIntValue(NULL, "file_distribute_rotate_count", &iniContext,
        ...
        pFsyncAfterWrittenBytes =
            iniGetStrValue(NULL, "fsync_after_written_bytes", &iniContext);
        ...
        g_sync_log_buff_interval =
            iniGetIntValue(NULL, "sync_log_buff_interval", &iniContext,
        ...
        g_sync_binlog_buff_interval =
            iniGetIntValue(NULL, "sync_binlog_buff_interval", &iniContext,
        ...
        g_write_mark_file_freq =
            iniGetIntValue(NULL, "write_mark_file_freq", &iniContext,
        ...
        g_sync_stat_file_interval =
            iniGetIntValue(NULL, "sync_stat_file_interval", &iniContext,
        ...
        pThreadStackSize =
            iniGetStrValue(NULL, "thread_stack_size", &iniContext);
        ...
        if (g_thread_stack_size < FAST_WRITE_BUFF_SIZE + 64 * 1024) {
            logError(
                ...
                "which < %d",
                __LINE__, g_thread_stack_size,
                FAST_WRITE_BUFF_SIZE + 64 * 1024);
            result = EINVAL;
            break;
        }

        g_upload_priority = iniGetIntValue(NULL, "upload_priority", &iniContext,
        ...
        g_check_file_duplicate =
            iniGetBoolValue(NULL, "check_file_duplicate", &iniContext, false);
        if (g_check_file_duplicate) {
            char *pKeyNamespace;
            char *pFileSignatureMethod;

            pFileSignatureMethod =
                iniGetStrValue(NULL, "file_signature_method", &iniContext);
            if (pFileSignatureMethod != NULL &&
                strcasecmp(pFileSignatureMethod, "md5") == 0) {
                g_file_signature_method = STORAGE_FILE_SIGNATURE_METHOD_MD5;
            } else {
            ...
            pKeyNamespace = iniGetStrValue(NULL, "key_namespace", &iniContext);
            ...
            *(g_key_namespace + g_namespace_len) = '\0';
            ...
            if ((result = fdht_load_groups(&iniContext, &g_group_array)) != 0) {
                break;
            }

            g_keep_alive =
                iniGetBoolValue(NULL, "keep_alive", &iniContext, false);
        }

        g_http_port = iniGetIntValue(NULL, "http.server_port", &iniContext, 80);
        if (g_http_port <= 0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
        ...
        pHttpDomain = iniGetStrValue(NULL, "http.domain_name", &iniContext);
        if (pHttpDomain == NULL) {
            *g_http_domain = '\0';
        } else {
            snprintf(g_http_domain, sizeof(g_http_domain), "%s", pHttpDomain);
        }

        g_use_access_log =
            iniGetBoolValue(NULL, "use_access_log", &iniContext, false);
        if (g_use_access_log) {
            ...
            result = log_set_prefix_ex(&g_access_log_context, g_fdfs_base_path,
                                       "storage_access");
            if (result != 0) {
                break;
            }
            log_set_header_callback(&g_access_log_context,
                                    storage_set_access_log_header);
        }

        g_rotate_access_log =
            iniGetBoolValue(NULL, "rotate_access_log", &iniContext, false);
        if ((result = get_time_item_from_conf(
                 &iniContext, "access_log_rotate_time",
                 &g_access_log_rotate_time, 0, 0)) != 0) {
            break;
        }

        g_rotate_error_log =
            iniGetBoolValue(NULL, "rotate_error_log", &iniContext, false);
        if ((result = get_time_item_from_conf(
                 &iniContext, "error_log_rotate_time", &g_error_log_rotate_time,
                 0, 0)) != 0) {
            break;
        }

        pRotateAccessLogSize =
            iniGetStrValue(NULL, "rotate_access_log_size", &iniContext);
        ...
        if (rotate_access_log_size > 0 &&
            ...
            logWarning(
                ...
                " is too small, "
                "change to 1 MB",
                __LINE__, rotate_access_log_size);
            rotate_access_log_size = FDFS_ONE_MB;
        }
        fdfs_set_log_rotate_size(&g_access_log_context, rotate_access_log_size);

        pRotateErrorLogSize =
            iniGetStrValue(NULL, "rotate_error_log_size", &iniContext);
        ...
        if (rotate_error_log_size > 0 && rotate_error_log_size < FDFS_ONE_MB) {
            logWarning(
                ...
                " is too small, "
                "change to 1 MB",
                __LINE__, rotate_error_log_size);
            rotate_error_log_size = FDFS_ONE_MB;
        }
        fdfs_set_log_rotate_size(&g_log_context, rotate_error_log_size);

        g_log_file_keep_days =
            iniGetIntValue(NULL, "log_file_keep_days", &iniContext, 0);

        g_file_sync_skip_invalid_record = iniGetBoolValue(
            NULL, "file_sync_skip_invalid_record", &iniContext, false);
...
#ifdef WITH_HTTPD
        {
            ...
            if ((result = fdfs_http_params_load(&iniContext, filename,
                                                &g_http_params)) != 0) {
                break;
            }

            pHttpTrunkSize =
                iniGetStrValue(NULL, "http.trunk_size", &iniContext);
        ...
        logInfo(
            "FastDFS v%d.%02d, base_path=%s, store_path_count=%d, "
            ...
            g_fdfs_version.major, g_fdfs_version.minor, g_fdfs_base_path,
            g_fdfs_store_paths.count, g_subdir_count_per_path, g_group_name,
            g_run_by_group, g_run_by_user, g_fdfs_connect_timeout,
            g_fdfs_network_timeout, g_server_port, bind_addr,
            g_client_bind_addr, g_max_connections, g_accept_threads,
            g_work_threads, g_disk_rw_separated, g_disk_reader_threads,
            g_disk_writer_threads, g_buff_size / 1024, g_heart_beat_interval,
            ...
            g_file_distribute_path_mode, g_file_distribute_rotate_count,
            g_fsync_after_written_bytes, g_sync_log_buff_interval,
            g_sync_binlog_buff_interval, g_sync_stat_file_interval,
            g_thread_stack_size / 1024, g_upload_priority, g_if_alias_prefix,
            g_check_file_duplicate,
            g_file_signature_method == STORAGE_FILE_SIGNATURE_METHOD_HASH
                ? "hash"
                : "md5",
            g_group_array.group_count, g_group_array.server_count,
            g_key_namespace, g_keep_alive, g_http_port, g_http_domain,
            g_use_access_log, g_rotate_access_log,
            g_access_log_rotate_time.hour, g_access_log_rotate_time.minute,
            g_rotate_error_log, g_error_log_rotate_time.hour,
            g_error_log_rotate_time.minute, g_access_log_context.rotate_size,
            g_log_context.rotate_size, g_log_file_keep_days,
            g_file_sync_skip_invalid_record, g_use_connection_pool,
        ...
        if (!g_http_params.disabled) {
            logInfo(
                ...
                g_http_params.server_port, g_http_trunk_size,
                g_http_params.default_content_type,
                g_http_params.anti_steal_token, g_http_params.token_ttl,
