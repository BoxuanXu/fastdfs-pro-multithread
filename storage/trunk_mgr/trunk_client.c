...
static int trunk_client_trunk_do_alloc_space(ConnectionInfo *pTrunkServer,
    ...
    snprintf(p, sizeof(out_buff) - sizeof(TrackerHeader), "%s", g_group_name);
    ...
    *p++ = pTrunkInfo->path.store_path_index;
    ...
    if ((result = tcpsenddata_nb(pTrunkServer->sock, out_buff, sizeof(out_buff),
        ...
        logError(
            ...
            __LINE__, pTrunkServer->ip_addr, pTrunkServer->port, result,
            STRERROR(result));
    ...
    p = (char *)&trunkBuff;
    if ((result = fdfs_recv_response(
             pTrunkServer, &p, sizeof(FDFSTrunkInfoBuff), &in_bytes)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if (in_bytes != sizeof(FDFSTrunkInfoBuff)) {
        logError(
            ...
            __LINE__, pTrunkServer->ip_addr, pTrunkServer->port, (int)in_bytes,
...
int trunk_client_trunk_alloc_space(const int file_size,
    ...
    if ((pTrunkServer = tracker_connect_server(&trunk_server, &result)) ==
        NULL) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "can't alloc trunk space because connect to trunk "
            "server %s:%d fail, errno: %d",
            __LINE__, trunk_server.ip_addr, trunk_server.port, result);
        return result;
    }

    result =
        trunk_client_trunk_do_alloc_space(pTrunkServer, file_size, pTrunkInfo);
...
static int trunk_client_trunk_confirm_or_free(
    ConnectionInfo *pTrunkServer, const FDFSTrunkFullInfo *pTrunkInfo,
    const int cmd, const int status) {
    TrackerHeader *pHeader;
    ...
    snprintf(out_buff + sizeof(TrackerHeader),
             sizeof(out_buff) - sizeof(TrackerHeader), "%s", g_group_name);
    ...
    if (in_bytes != 0) {
        logError(
            ...
            " is invalid, "
            "should == 0",
            __LINE__, pTrunkServer->ip_addr, pTrunkServer->port, in_bytes);
        return EINVAL;
    }
...
int trunk_client_trunk_alloc_confirm(const FDFSTrunkFullInfo *pTrunkInfo,
    ...
    result =
        trunk_client_trunk_do_alloc_confirm(pTrunkServer, pTrunkInfo, status);
