...
#define TRUNK_GET_FILENAME(file_id, filename) sprintf(filename, "%06d", file_id)
...
#define trunk_get_full_filename(pTrunkInfo, full_filename, buff_size)          \
    trunk_get_full_filename_ex(&g_fdfs_store_paths, pTrunkInfo, full_filename, \
                               buff_size)
...
#define trunk_file_get_content(pTrunkInfo, file_size, pfd, buff, buff_size)    \
    trunk_file_get_content_ex(&g_fdfs_store_paths, pTrunkInfo, file_size, pfd, \
                              buff, buff_size)
...
#define trunk_file_do_lstat_func(store_path_index, true_filename,              \
                                 filename_len, stat_func, pStat, pTrunkInfo,   \
                                 pTrunkHeader, pfd)                            \
    trunk_file_do_lstat_func_ex(&g_fdfs_store_paths, store_path_index,         \
                                true_filename, filename_len, stat_func, pStat, \
...
#define trunk_file_stat(store_path_index, true_filename, filename_len, pStat,  \
                        pTrunkInfo, pTrunkHeader)                              \
    trunk_file_stat_func(store_path_index, true_filename, filename_len,        \
                         FDFS_STAT_FUNC_STAT, pStat, pTrunkInfo, pTrunkHeader, \
                         NULL)

#define trunk_file_lstat(store_path_index, true_filename, filename_len, pStat, \
                         pTrunkInfo, pTrunkHeader)                             \
    trunk_file_do_lstat_func(store_path_index, true_filename, filename_len,    \
                             FDFS_STAT_FUNC_LSTAT, pStat, pTrunkInfo,          \
                             pTrunkHeader, NULL)
...
#define trunk_file_stat_ex1(pStorePaths, store_path_index, true_filename,  \
                            filename_len, pStat, pTrunkInfo, pTrunkHeader, \
                            pfd)                                           \
    trunk_file_stat_func_ex(pStorePaths, store_path_index, true_filename,  \
                            filename_len, FDFS_STAT_FUNC_STAT, pStat,      \
                            pTrunkInfo, pTrunkHeader, pfd)
