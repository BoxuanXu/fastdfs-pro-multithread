...
int storage_trunk_init() {
    ...
    if ((result = fast_mblock_init(&free_blocks_man, sizeof(FDFSTrunkNode),
                                   0)) != 0) {
        return result;
    }

    if ((result = fast_mblock_init(&tree_nodes_man, sizeof(FDFSTrunkSlot),
                                   0)) != 0) {
        return result;
    }

    tree_info_by_sizes =
        (AVLTreeInfo *)malloc(sizeof(AVLTreeInfo) * g_fdfs_store_paths.count);
    if (tree_info_by_sizes == NULL) {
        ...
        logError(
            ...
            __LINE__, (int)(sizeof(AVLTreeInfo) * g_fdfs_store_paths.count),
    ...
    logInfo(
        ...
        "trunk_total_free_space: %" PRId64,
        __LINE__, count, trunk_free_block_tree_node_count(),
...
static int tree_walk_callback(void *data, void *args) {
    ...
    while (pCurrent != NULL) {
        ...
        len = sprintf(pCallbackArgs->pCurrent, "%d %c %d %d %d %d %d %d\n",
                      (int)g_current_time, TRUNK_OP_TYPE_ADD_SPACE,
                      pTrunkInfo->path.store_path_index,
                      pTrunkInfo->path.sub_path_high,
                      pTrunkInfo->path.sub_path_low, pTrunkInfo->file.id,
                      pTrunkInfo->file.offset, pTrunkInfo->file.size);
        ...
        if (pCallbackArgs->pCurrent - pCallbackArgs->buff >
            ...
            if (fc_safe_write(pCallbackArgs->fd, pCallbackArgs->buff,
                ...
                logError(
                    ...
                    "errno: %d, error info: %s",
                    __LINE__, pCallbackArgs->temp_trunk_filename, result,
                    STRERROR(result));
                return result;
            }
...
static int storage_trunk_do_save() {
    ...
    if (callback_args.fd < 0) {
        ...
        logError(
            ...
            "open file %s fail, "
            "errno: %d, error info: %s",
            __LINE__, callback_args.temp_trunk_filename, result,
            STRERROR(result));
        return result;
    }
    ...
    for (i = 0; i < g_fdfs_store_paths.count; i++) {
        result = avl_tree_walk(tree_info_by_sizes + i, tree_walk_callback,
                               &callback_args);
        if (result != 0) {
            break;
        }
    ...
    if (close(callback_args.fd) != 0) {
        ...
        logError(
            ...
            "close file %s fail, "
            "errno: %d, error info: %s",
            __LINE__, callback_args.temp_trunk_filename, errno,
            STRERROR(errno));
    }
    ...
    if (rename(callback_args.temp_trunk_filename, trunk_data_filename) != 0) {
        ...
        logError(
            ...
            "errno: %d, error info: %s",
            __LINE__, callback_args.temp_trunk_filename, trunk_data_filename,
            result, STRERROR(result));
    }
...
static int storage_trunk_save() {
    ...
    return trunk_unlink_all_mark_files();  // because the binlog file be
                                           // compressed
}

static bool storage_trunk_is_space_occupied(
    ...
    if (pTrunkInfo->file.offset > 0 &&
        lseek(fd, pTrunkInfo->file.offset, SEEK_SET) < 0) {
        result = errno != 0 ? errno : EIO;
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    */
    result =
        dio_check_trunk_file_ex(fd, full_filename, pTrunkInfo->file.offset);
    close(fd);
...
static int storage_trunk_restore(const int64_t restore_offset) {
    ...
    if (restore_offset > trunk_binlog_size) {
        logWarning(
            "file: "__FILE__
            ", line: %d, "
            "restore_offset: %" PRId64 " > trunk_binlog_size: %" PRId64,
    ...
    logDebug(
        ...
        "%" PRId64
        ", need recovery binlog bytes: "
        "%" PRId64,
        __LINE__, restore_offset, trunk_binlog_size - restore_offset);

    trunk_init_reload_from_binlog = (restore_offset == 0);
    if (trunk_init_reload_from_binlog) {
        memset(&trunkNode, 0, sizeof(trunkNode));
        if ((result =
                 avl_tree_init(&tree_info_by_offset, storage_trunk_free_node,
    ...
    while (1) {
        ...
        if (result != 0) {
            if (result == ENOENT) {
                ...
                result =
                    (reader.binlog_offset >= trunk_binlog_size) ? 0 : EINVAL;
                if (result != 0) {
                    logError(
                        "file: "__FILE__
                        ", line: %d, "
                        "binlog offset: %" PRId64 " < binlog size: %" PRId64
                        ", please check the end of trunk "
                        "binlog",
                        __LINE__, reader.binlog_offset, trunk_binlog_size);
                }
            }
        ...
        if (record.trunk.path.store_path_index < 0 ||
            ...
            logError(
                "file: "__FILE__
                ", line: %d, "
                "store_path_index: %d is invalid",
                __LINE__, record.trunk.path.store_path_index);
            return EINVAL;
        }
        ...
        if (record.op_type == TRUNK_OP_TYPE_ADD_SPACE) {
            ...
            if (trunk_init_reload_from_binlog) {
                ...
                if (pMblockNode == NULL) {
                    ...
                    logError(
                        ...
                        "malloc %d bytes fail, "
                        "errno: %d, error info: %s",
                        __LINE__, (int)sizeof(FDFSTrunkNode), result,
                        STRERROR(result));
                    return result;
                }
                ...
                result = avl_tree_insert(&tree_info_by_offset, pTrunkNode);
                if (result < 0)  // error
                {
                    result *= -1;
                    logError(
                    ...
                    trunk_info_dump(&(record.trunk), buff, sizeof(buff));
                    logWarning(
                        "file: "__FILE__
                        ", line: %d"
                        ", trunk data line: "
            ...
            } else if ((result = trunk_add_space_by_trunk(&record.trunk)) !=
                       0) {
                break;
            }
            ...
            if (trunk_init_reload_from_binlog) {
                memcpy(&trunkNode.trunk, &record.trunk,
                       sizeof(FDFSTrunkFullInfo));
                if (avl_tree_delete(&tree_info_by_offset, &trunkNode) != 1) {
                    trunk_info_dump(&(record.trunk), buff, sizeof(buff));
                    logWarning(
                        "file: "__FILE__
                        ", line: %d"
                        ...
                        "trunk info: %s",
                        __LINE__, reader.binlog_offset, line_count, buff);
                }
            } else if ((result = trunk_delete_space(&record.trunk, false)) !=
                       0) {
                if (result == ENOENT) {
                    logDebug(
                        "file: "__FILE__
                        ", line: %d, "
                        "binlog offset: %" PRId64 ", trunk data line: %" PRId64,
                        __LINE__, reader.binlog_offset, line_count);
    ...
    if (unlink(trunk_mark_filename) != 0) {
        logError(
            ...
            "unlink file %s fail, "
            "errno: %d, error info: %s",
            __LINE__, trunk_mark_filename, errno, STRERROR(errno));
    }
...
int storage_delete_trunk_data_file() {
    ...
    if (result != ENOENT) {
        logError(
            ...
            "errno: %d, error info: %s",
            __LINE__, trunk_data_filename, result, STRERROR(result));
    }
    ...
    *pLineEnd = '\0';
    ...
    while (1) {
        ...
        if (pLineEnd == NULL) {
            ...
            if ((bytes = fc_safe_read(fd, buff + len, sizeof(buff) - len - 1)) <
                0) {
                result = errno != 0 ? errno : EIO;
                logError(
                    "file: "__FILE__
        ...
        ++line_count;
        *pLineEnd = '\0';
        col_count = splitEx(pLineStart, ' ', cols, TRUNK_DATA_NEW_FIELD_COUNT);
...
int trunk_free_space(const FDFSTrunkFullInfo *pTrunkInfo,
    ...
    if (pMblockNode == NULL) {
        ...
        logError(
            ...
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, (int)sizeof(FDFSTrunkNode), result, STRERROR(result));
        return result;
    }
...
static int trunk_add_free_block(FDFSTrunkNode *pNode, const bool bWriteBinLog) {
    ...
    chain = (FDFSTrunkSlot *)avl_tree_find(
        tree_info_by_sizes + pNode->trunk.path.store_path_index, &target_slot);
    if (chain == NULL) {
        ...
        if (pMblockNode == NULL) {
            ...
            logError(
                ...
                "malloc %d bytes fail, "
                "errno: %d, error info: %s",
                __LINE__, (int)sizeof(FDFSTrunkSlot), result, STRERROR(result));
            pthread_mutex_unlock(&trunk_mem_lock);
            return result;
        }
        ...
        if (avl_tree_insert(
                tree_info_by_sizes + pNode->trunk.path.store_path_index,
                chain) != 1) {
            result = errno != 0 ? errno : ENOMEM;
            logError(
                "file: "__FILE__
    ...
    if (bWriteBinLog) {
        result = trunk_mem_binlog_write(g_current_time, TRUNK_OP_TYPE_ADD_SPACE,
                                        &(pNode->trunk));
    } else {
        pthread_mutex_lock(&trunk_file_lock);
...
static int trunk_delete_space(const FDFSTrunkFullInfo *pTrunkInfo,
    ...
    pthread_mutex_lock(&trunk_mem_lock);
    pSlot = (FDFSTrunkSlot *)avl_tree_find(
        tree_info_by_sizes + pTrunkInfo->path.store_path_index, &target_slot);
    if (pSlot == NULL) {
        pthread_mutex_unlock(&trunk_mem_lock);
        logError(
            "file: "__FILE__
            ...
            __LINE__, trunk_info_dump(pTrunkInfo, buff, sizeof(buff)));
        return ENOENT;
    }
    ...
    pCurrent = pSlot->head;
    while (pCurrent != NULL &&
           memcmp(&(pCurrent->trunk), pTrunkInfo, sizeof(FDFSTrunkFullInfo)) !=
               0) {
        pPrevious = pCurrent;
        pCurrent = pCurrent->next;
    }
    ...
    if (pPrevious == NULL) {
        ...
        if (pSlot->head == NULL) {
            trunk_delete_size_tree_entry(pTrunkInfo->path.store_path_index,
                                         pSlot);
        }
    } else {
    ...
    if (bWriteBinLog) {
        result = trunk_mem_binlog_write(g_current_time, TRUNK_OP_TYPE_DEL_SPACE,
                                        &(pCurrent->trunk));
    } else {
        pthread_mutex_lock(&trunk_file_lock);
...
static int trunk_split(FDFSTrunkNode *pNode, const int size) {
    ...
    if (pNode->trunk.file.size - size < g_slot_min_size) {
        return trunk_mem_binlog_write(g_current_time, TRUNK_OP_TYPE_DEL_SPACE,
                                      &(pNode->trunk));
    }
...
static FDFSTrunkNode *trunk_create_trunk_file(const int store_path_index,
    ...
    if (pMblockNode == NULL) {
        *err_no = errno != 0 ? errno : EIO;
        logError(
            ...
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, (int)sizeof(FDFSTrunkNode), *err_no, STRERROR(*err_no));
        return NULL;
    }
    ...
    if (store_path_index >= 0) {
        ...
        if ((result = storage_get_storage_path_index(&new_store_path_index)) !=
            0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
    ...
    *err_no = trunk_mem_binlog_write(g_current_time, TRUNK_OP_TYPE_ADD_SPACE,
                                     &(pTrunkNode->trunk));
    return pTrunkNode;
}
...
int trunk_alloc_space(const int size, FDFSTrunkFullInfo *pResult) {
    ...
    pthread_mutex_lock(&trunk_mem_lock);
    while (1) {
        pSlot = (FDFSTrunkSlot *)avl_tree_find_ge(
            tree_info_by_sizes + pResult->path.store_path_index, &target_slot);
        if (pSlot == NULL) {
            break;
        }
    ...
    if (pTrunkNode != NULL) {
        if (pPreviousNode == NULL) {
            ...
            if (pSlot->head == NULL) {
                trunk_delete_size_tree_entry(pResult->path.store_path_index,
                                             pSlot);
            }
        } else {
        ...
        pTrunkNode =
            trunk_create_trunk_file(pResult->path.store_path_index, &result);
        if (pTrunkNode == NULL) {
            pthread_mutex_unlock(&trunk_mem_lock);
            return result;
        }
...
int trunk_alloc_confirm(const FDFSTrunkFullInfo *pTrunkInfo, const int status) {
    ...
    target_trunk_info.path.store_path_index = pTrunkInfo->path.store_path_index;
...
static int trunk_create_next_file(FDFSTrunkFullInfo *pTrunkInfo) {
    ...
    while (1) {
        ...
        storage_get_store_path(short_filename, filename_len, &sub_path_high,
                               &sub_path_low);
...
int trunk_check_and_init_file_ex(const char *filename,
    ...
    logWarning(
        "file: "__FILE__
        ", line: %d, "
        "file: %s, file size: %" PRId64 " < %" PRId64 ", should be resize",
...
int trunk_file_delete(const char *trunk_filename,
    ...
    while (remain_bytes > 0) {
        write_bytes = remain_bytes > sizeof(buff) ? sizeof(buff) : remain_bytes;
...
int trunk_create_trunk_file_advance(void *args) {
    ...
    if (!g_trunk_create_file_advance) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "do not need create trunk file advancely!",
            __LINE__);
        return EINVAL;
    }
    ...
    alloc_space =
        g_trunk_create_file_space_threshold - g_trunk_total_free_space;
    if (alloc_space <= 0) {
        logDebug(
            "file: "__FILE__
            ", line: %d, "
            "do not need create trunk file!",
            __LINE__);
        return 0;
    }
    ...
    if (!storage_check_reserved_space_path(
            total_mb_sum, free_mb_sum - (alloc_space / FDFS_ONE_MB),
