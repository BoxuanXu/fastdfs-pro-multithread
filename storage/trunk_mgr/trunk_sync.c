...
static int trunk_binlog_fsync_ex(const bool bNeedLock, const char *buff,
                                 int *length);
static int trunk_binlog_preread(TrunkBinLogReader *pReader);

#define trunk_binlog_fsync(bNeedLock)                               \
    trunk_binlog_fsync_ex(bNeedLock, trunk_binlog_write_cache_buff, \
...
static int trunk_binlog_open_writer(const char *binlog_filename) {
    trunk_binlog_fd =
        open(binlog_filename, O_WRONLY | O_CREAT | O_APPEND, 0644);
...
int trunk_sync_init() {
    ...
    snprintf(sync_path, sizeof(sync_path), "%s/" TRUNK_DIR_NAME, data_path);
    ...
    trunk_binlog_write_cache_buff = (char *)malloc(TRUNK_BINLOG_BUFFER_SIZE);
    if (trunk_binlog_write_cache_buff == NULL) {
        logError(
            ...
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, TRUNK_BINLOG_BUFFER_SIZE, errno, STRERROR(errno));
        ...
        if (access(binlog_filename, F_OK) == 0) {
            if (rename(binlog_filename, rollback_filename) != 0) {
                ...
                logError(
                    ...
                    "rename %s to %s fail, "
                    "errno: %d, error info: %s",
                    __LINE__, binlog_filename, rollback_filename, result,
                    STRERROR(result));
                return result;
            }
...
static int trunk_binlog_merge_file(int old_fd) {
    ...
    if (access(binlog_filename, F_OK) == 0) {
        int binlog_fd;
        if ((binlog_fd = trunk_binlog_open_read(binlog_filename, false)) < 0) {
            close(tmp_fd);
    ...
    if (rename(tmp_filename, binlog_filename) != 0) {
        ...
        logError(
            ...
            "rename %s to %s fail, "
            "errno: %d, error info: %s",
            __LINE__, tmp_filename, binlog_filename, result, STRERROR(result));
        return result;
    }
    ...
    if (access(rollback_filename, F_OK) == 0) {
        if (unlink(rollback_filename) != 0) {
            ...
            logWarning(
                ...
                ", line: %d, "
                "unlink %s fail, errno: %d, error info: %s",
                __LINE__, rollback_filename, result, STRERROR(result));
        }
    }
...
int trunk_binlog_compress_rollback() {
        ...
        if (access(rollback_filename, F_OK) == 0) {
            if (rename(rollback_filename, binlog_filename) != 0) {
                ...
                logError(
                    ...
                    "rename %s to %s fail, "
                    "errno: %d, error info: %s",
                    __LINE__, rollback_filename, binlog_filename, result,
                    STRERROR(result));
                return result;
            }
        }
    ...
    if ((rollback_fd = trunk_binlog_open_read(rollback_filename, false)) < 0) {
        return errno != 0 ? errno : ENOENT;
    }
...
static int trunk_binlog_fsync_ex(const bool bNeedLock, const char *buff,
    ...
    if (bNeedLock &&
        (result = pthread_mutex_lock(&trunk_sync_thread_lock)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "call pthread_mutex_lock fail, "
    ...
    if (*length == 0)  // ignore
    {
        ...
        logError(
            ...
            "errno: %d, error info: %s",
            __LINE__, get_trunk_binlog_filename(full_filename), trunk_binlog_fd,
        ...
        logError(
            ...
            "errno: %d, error info: %s",
            __LINE__, get_trunk_binlog_filename(full_filename), errno,
            STRERROR(errno));
    } else {
        write_ret = 0;
    }
    ...
    if (bNeedLock &&
        (result = pthread_mutex_unlock(&trunk_sync_thread_lock)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "call pthread_mutex_unlock fail, "
...
int trunk_binlog_write(const int timestamp, const char op_type,
    ...
    trunk_binlog_write_cache_len +=
        sprintf(trunk_binlog_write_cache_buff + trunk_binlog_write_cache_len,
                "%d %c %d %d %d %d %d %d\n", timestamp, op_type,
                pTrunk->path.store_path_index, pTrunk->path.sub_path_high,
                pTrunk->path.sub_path_low, pTrunk->file.id, pTrunk->file.offset,
                pTrunk->file.size);
...
int trunk_binlog_write_buffer(const char *buff, const int length) {
    ...
    // check if buff full
    if (TRUNK_BINLOG_BUFFER_SIZE - (trunk_binlog_write_cache_len + length) <
        128) {
        write_ret = trunk_binlog_fsync(false);  // sync to disk
    } else {
        write_ret = 0;
    ...
    if (write_ret == 0) {
        if (length >= TRUNK_BINLOG_BUFFER_SIZE) {
            ...
            if (write_ret == 0) {
                int len;
                len = length;
                write_ret = trunk_binlog_fsync_ex(false, buff, &len);
            }
        } else {
...
static char *trunk_get_mark_filename_by_ip_and_port(const char *ip_addr,
                                                    ...
                                                    char *full_filename,
                                                    const int filename_size) {
    snprintf(full_filename, filename_size, "%s/data/" TRUNK_DIR_NAME "/%s_%d%s",
             g_fdfs_base_path, ip_addr, port, TRUNK_SYNC_MARK_FILE_EXT);
...
char *trunk_mark_filename_by_reader(const void *pArg, char *full_filename) {
    ...
    return trunk_get_mark_filename_by_id_and_port(
        pReader->storage_id, g_server_port, full_filename, MAX_PATH_SIZE);
}
...
int trunk_reader_init(FDFSStorageBrief *pStorage, TrunkBinLogReader *pReader) {
    ...
    pReader->binlog_buff.buffer = (char *)malloc(TRUNK_BINLOG_BUFFER_SIZE);
    ...
    if (pStorage == NULL) {
        ...
        if (!bFileExist && (g_use_storage_id && pStorage != NULL)) {
            ...
            trunk_get_mark_filename_by_ip_and_port(
                pStorage->ip_addr, g_server_port, old_mark_filename,
    ...
    if ((result = trunk_open_readable_binlog(
...
static int trunk_write_to_mark_file(TrunkBinLogReader *pReader) {
    ...
    if ((result = storage_write_to_fd(pReader->mark_fd,
                                      trunk_mark_filename_by_reader, pReader,
                                      buff, len)) == 0) {
        pReader->last_binlog_offset = pReader->binlog_offset;
    }
...
static int trunk_binlog_do_line_read(TrunkBinLogReader *pReader, char *line,
...
static int trunk_binlog_read_line(TrunkBinLogReader *pReader, char *line,
                                  const int line_size, int *line_length) {
    int result;
...
int trunk_binlog_read(TrunkBinLogReader *pReader, TrunkBinLogRecord *pRecord,
                      int *record_length) {
#define COL_COUNT 8
    ...
    result = trunk_binlog_read_line(pReader, line, sizeof(line), record_length);
    if (result != 0) {
        return result;
    }
...
int trunk_rename_mark_file(const char *old_ip_addr, const int old_port,
    ...
    trunk_get_mark_filename_by_id_and_port(old_ip_addr, old_port, old_filename,
                                           sizeof(old_filename));
    if (!fileExists(old_filename)) {
        return ENOENT;
    }

    trunk_get_mark_filename_by_id_and_port(new_ip_addr, new_port, new_filename,
                                           sizeof(new_filename));
...
static int trunk_sync_data(TrunkBinLogReader *pReader,
    ...
    if ((result = tcpsenddata_nb(pStorage->sock, &header, sizeof(TrackerHeader),
    ...
    while (g_continue_flag && g_if_trunker_self &&
        ...
        if ((!g_continue_flag) || (!g_if_trunker_self) ||
            ...
            logError(
                ...
                "dest storage status: %d",
                __LINE__, g_continue_flag, g_if_trunker_self, pStorage->status);
            break;
        }
        ...
        */
        ...
        if (reader.binlog_offset == 0) {
            if ((result = fdfs_deal_no_body_cmd(
                     &storage_server,
        ...
        while (g_continue_flag &&
            ...
            if ((sync_result = trunk_sync_data(&reader, &storage_server)) !=
                0) {
                break;
            }
...
int trunk_sync_thread_start(const FDFSStorageBrief *pStorage) {
    ...
    */
    ...
    g_trunk_sync_thread_count++;
    trunk_sync_tids = (pthread_t *)realloc(
        trunk_sync_tids, sizeof(pthread_t) * g_trunk_sync_thread_count);
    if (trunk_sync_tids == NULL) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, (int)sizeof(pthread_t) * g_trunk_sync_thread_count, errno,
            STRERROR(errno));
    } else {
