...
char **storage_load_paths_from_conf_file_ex(IniContext *pItemContext,
    ...
    *path_count =
        iniGetIntValue(szSectionName, "store_path_count", pItemContext, 1);
    if (*path_count <= 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if (store_paths == NULL) {
        logError(
            ...
            __LINE__, (int)sizeof(char *) * (*path_count), errno,
            STRERROR(errno));
    ...
    for (i = 1; i < *path_count; i++) {
        ...
        if (!fileExists(pPath)) {
            logError(
                ...
                "\"%s\" can't be accessed, "
                "errno: %d, error info: %s",
                __LINE__, pPath, errno, STRERROR(errno));
...
int storage_load_paths_from_conf_file(IniContext *pItemContext) {
    ...
    if (pPath == NULL) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "conf file must have item \"base_path\"!",
            __LINE__);
        return ENOENT;
    }
    ...
    do {                                                                 \
        ...
        if (*filename_len <= FDFS_LOGIC_FILE_PATH_LEN) {                 \
            ...
            return EINVAL;                                               \
                                                                         ...
                                                                         \
        if (*logic_filename !=                                           \
            FDFS_STORAGE_STORE_PATH_PREFIX_CHAR) {/* version < V1.12 */  \
            store_path_index = 0;                                        \
...
int storage_split_filename(const char *logic_filename, int *filename_len,
...
int storage_split_filename_ex(const char *logic_filename, int *filename_len,
...
int storage_split_filename_no_check(const char *logic_filename,
                                    int *filename_len, char *true_filename,
                                    int *store_path_index) {
    SPLIT_FILENAME_BODY(logic_filename, filename_len, true_filename,
                        *store_path_index, false);
...
char *trunk_info_dump(const FDFSTrunkFullInfo *pTrunkInfo, char *buff,
    ...
    snprintf(buff, buff_size,
             ...
             pTrunkInfo->path.store_path_index, pTrunkInfo->path.sub_path_high,
...
char *trunk_header_dump(const FDFSTrunkHeader *pTrunkHeader, char *buff,
    ...
    snprintf(buff, buff_size,
             ...
             pTrunkHeader->file_size, pTrunkHeader->crc32, pTrunkHeader->mtime,
...
char *trunk_get_full_filename_ex(const FDFSStorePaths *pStorePaths,
    ...
    snprintf(full_filename, buff_size, "%s/data/" FDFS_STORAGE_DATA_DIR_FORMAT
...
void trunk_pack_header(const FDFSTrunkHeader *pTrunkHeader, char *buff) {
    *(buff + FDFS_TRUNK_FILE_FILE_TYPE_OFFSET) = pTrunkHeader->file_type;
    ...
    memcpy(buff + FDFS_TRUNK_FILE_FILE_EXT_NAME_OFFSET,
           pTrunkHeader->formatted_ext_name, FDFS_FILE_EXT_NAME_MAX_LEN + 1);
}
...
void trunk_unpack_header(const char *buff, FDFSTrunkHeader *pTrunkHeader) {
    ...
    pTrunkHeader->alloc_size =
        buff2int(buff + FDFS_TRUNK_FILE_ALLOC_SIZE_OFFSET);
    pTrunkHeader->file_size = buff2int(buff + FDFS_TRUNK_FILE_FILE_SIZE_OFFSET);
    pTrunkHeader->crc32 = buff2int(buff + FDFS_TRUNK_FILE_FILE_CRC32_OFFSET);
    pTrunkHeader->mtime = buff2int(buff + FDFS_TRUNK_FILE_FILE_MTIME_OFFSET);
    memcpy(pTrunkHeader->formatted_ext_name,
           buff + FDFS_TRUNK_FILE_FILE_EXT_NAME_OFFSET,
...
void trunk_file_info_encode(const FDFSTrunkFileInfo *pTrunkFile, char *str) {
    ...
    base64_encode_ex(&g_fdfs_base64_context, buff, sizeof(buff), str, &len,
                     false);
}
...
int trunk_file_get_content_ex(const FDFSStorePaths *pStorePaths,
                              const FDFSTrunkFullInfo *pTrunkInfo,
                              const int file_size, int *pfd, char *buff,
                              const int buff_size) {
    ...
    if (pfd != NULL) {
        ...
        if (lseek(fd, pTrunkInfo->file.offset + FDFS_TRUNK_FILE_HEADER_SIZE,
                  SEEK_SET) < 0) {
...
int trunk_file_stat_func_ex(const FDFSStorePaths *pStorePaths,
                            ...
                            const int stat_func, struct stat *pStat,
    ...
    if (!(stat_func == FDFS_STAT_FUNC_STAT &&
          IS_TRUNK_FILE_BY_ID((*pTrunkInfo)) && S_ISLNK(pStat->st_mode))) {
        return 0;
    }
    ...
    do {
        ...
        *(src_filename + src_filename_len) = '\0';
        if ((result = storage_split_filename_no_check(
        ...
        if (src_store_path_index < 0 ||
            ...
            logError(
                ...
                "invalid store path index: %d, "
                "which < 0 or >= %d",
                __LINE__, src_filename, src_store_path_index,
                pStorePaths->count);
            result = EINVAL;
            break;
        }
        ...
        result = trunk_file_do_lstat_func_ex(
            pStorePaths, src_store_path_index, src_true_filename,
            src_filename_len, stat_func, pStat, pTrunkInfo, pTrunkHeader, pfd);
    } while (0);
...
int trunk_file_do_lstat_func_ex(const FDFSStorePaths *pStorePaths,
    ...
    base64_decode_auto(&g_fdfs_base64_context,
                       (char *)true_filename + FDFS_TRUE_FILE_PATH_LEN,
    ...
    trunk_file_info_decode(
        true_filename + FDFS_TRUE_FILE_PATH_LEN + FDFS_FILENAME_BASE64_LENGTH,
        &pTrunkInfo->file);
    ...
    memcpy(pTrunkHeader->formatted_ext_name,
           true_filename + (filename_len - (FDFS_FILE_EXT_NAME_MAX_LEN + 1)),
    ...
    if (pTrunkHeader->file_type == FDFS_TRUNK_FILE_TYPE_REGULAR) {
        ...
        logError(
            ...
            ", line: %d, "
            "Invalid file type: %d",
            __LINE__, pTrunkHeader->file_type);
        return ENOENT;
    }
    ...
    fprintf(stderr, "file: "__FILE__", line: %d, true fields=%s\n", __LINE__, \
            trunk_header_dump(&trueTrunkHeader, full_filename,
    sizeof(full_filename)));
    ...
    __LINE__, \
            ...
            trunk_header_dump(pTrunkHeader, full_filename,
    sizeof(full_filename)));
    }
    */
...
int fdfs_decode_trunk_info(const int store_path_index,
    ...
    if (filename_len != FDFS_TRUNK_FILENAME_LENGTH)  // not trunk file
    {
        logWarning(
            ...
            __LINE__, filename_len, FDFS_TRUNK_FILENAME_LENGTH, true_filename);
        return EINVAL;
    }
