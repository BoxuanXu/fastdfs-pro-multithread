...
static AVLTreeInfo tree_info_by_id = {NULL, NULL,
                                      NULL};  // for unique block nodes
...
void trunk_free_block_checker_destroy() { avl_tree_destroy(&tree_info_by_id); }
...
int trunk_free_block_check_duplicate(FDFSTrunkFullInfo *pTrunkInfo) {
    ...
    */
    ...
    if (pTrunkInfo->file.offset >=
        blocks[right]->file.offset + blocks[right]->file.size) {
        return 0;
    }
    ...
    while (left <= right) {
        ...
        if (pTrunkInfo->file.offset < blocks[mid]->file.offset) {
            ...
            if (pTrunkInfo->file.offset <
                (blocks[mid]->file.offset + blocks[mid]->file.size)) {
                result = EEXIST;
                break;
            }
    ...
    if (result != 0) {
        ...
        logWarning(
            ...
            "existed trunk entry: %s",
            __LINE__, trunk_info_dump(pTrunkInfo, buff1, sizeof(buff1)),
...
static int trunk_free_block_realloc(FDFSBlockArray *pArray,
                                    const int new_alloc) {
    FDFSTrunkFullInfo **blocks;
    int result;

    blocks = (FDFSTrunkFullInfo **)realloc(
        pArray->blocks, new_alloc * sizeof(FDFSTrunkFullInfo *));
    if (blocks == NULL) {
        ...
        logError(
            ...
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, (int)(new_alloc * sizeof(FDFSTrunkFullInfo *)), result,
            STRERROR(result));
        return result;
    }
...
static int trunk_free_block_do_insert(FDFSTrunkFullInfo *pTrunkInfo,
    ...
    if (pArray->count >= pArray->alloc) {
        if ((result = trunk_free_block_realloc(
                 pArray, pArray->alloc == 0 ? TRUNK_FREE_BLOCK_ARRAY_INIT_SIZE
                                            : 2 * pArray->alloc)) != 0) {
            return result;
        }
    }
...
int trunk_free_block_insert(FDFSTrunkFullInfo *pTrunkInfo) {
    ...
    if (pTrunksById == NULL) {
        ...
        if (pTrunksById == NULL) {
            ...
            logError(
                ...
                "malloc %d bytes fail, "
                "errno: %d, error info: %s",
                __LINE__, (int)sizeof(FDFSTrunksById), result,
                STRERROR(result));
            return result;
        }
    ...
    return trunk_free_block_do_insert(pTrunkInfo, &(pTrunksById->block_array));
}
...
int trunk_free_block_delete(FDFSTrunkFullInfo *pTrunkInfo) {
    ...
    if (pTrunksById->block_array.count == 0) {
        ...
        if (avl_tree_delete(&tree_info_by_id, pTrunksById) != 1) {
            memset(&(pTrunksById->block_array), 0, sizeof(FDFSBlockArray));
            logWarning(
                "file: "__FILE__
                ", line: %d, "
    ...
    for (pp=pArray->blocks + 1; pp<ppEnd; pp++)
    {
            if ((*ppPrevious)->file.offset >= (*pp)->file.offset)
            {
                    fprintf(fp, "%d %d %d %d %d %d\n", \
                            ...
                            (*ppPrevious)->path.sub_path_high,
    (*ppPrevious)->path.sub_path_low, \
                            (*ppPrevious)->file.id, (*ppPrevious)->file.offset,
    (*ppPrevious)->file.size);

                    fprintf(fp, "%d %d %d %d %d %d\n", \
                            (*pp)->path.store_path_index, \
                            (*pp)->path.sub_path_high, (*pp)->path.sub_path_low,
    \
                            (*pp)->file.id, (*pp)->file.offset,
    (*pp)->file.size);
            }
            ppPrevious = pp;
    }
