...
static int storage_do_fetch_binlog(ConnectionInfo *pSrcStorage,
    ...
    *(out_buff + sizeof(TrackerHeader) + FDFS_GROUP_NAME_MAX_LEN) =
    ...
    if ((result = tcpsenddata_nb(pSrcStorage->sock, out_buff, sizeof(out_buff),
    ...
    if ((result = tcprecvfile(pSrcStorage->sock, full_binlog_filename, in_bytes,
                              0, g_fdfs_network_timeout, &file_bytes)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
...
static int recovery_get_src_storage_server(ConnectionInfo *pSrcStorage) {
    ...
    while (g_continue_flag) {
        result = tracker_get_storage_max_status(
        ...
        if (result == 0) {
            if (saved_storage_status == FDFS_STORAGE_STATUS_INIT) {
                logInfo(
                    ...
                    __LINE__, g_tracker_client_ip, saved_storage_status);
                return ENOENT;
            }
    ...
    while (g_continue_flag) {
        if ((pTrackerConn =
                 tracker_get_connection_r(&trackerServer, &result)) == NULL) {
            sleep(5);
            continue;
        }

        result = tracker_list_one_group(pTrackerConn, g_group_name, &groupStat);
        if (result != 0) {
        ...
        if (g_fdfs_store_paths.count > groupStat.store_path_count) {
            logInfo(
                ...
                "does not need recovery",
                __LINE__, g_fdfs_store_paths.count, groupStat.store_path_count);
        ...
        result =
            tracker_list_servers(pTrackerConn, g_group_name, NULL, storageStats,
...
static char *recovery_get_full_filename(const void *pArg, const char *filename,
                                        char *full_filename) {
    ...
    snprintf(full_filename, MAX_PATH_SIZE, "%s/data/%s", pBasePath, filename);
...
static char *recovery_get_binlog_filename(const void *pArg,
                                          char *full_filename) {
    return recovery_get_full_filename(pArg, RECOVERY_BINLOG_FILENAME,
                                      full_filename);
}

static char *recovery_get_mark_filename(const void *pArg, char *full_filename) {
    return recovery_get_full_filename(pArg, RECOVERY_MARK_FILENAME,
                                      full_filename);
}
...
static int recovery_write_to_mark_file(const char *pBasePath,
    ...
    return storage_write_to_fd(pReader->mark_fd, recovery_get_mark_filename,
                               pBasePath, buff, len);
}
...
static int recovery_init_mark_file(const char *pBasePath,
    ...
    len = sprintf(buff,
                  ...
                  "%s=%d\n",
                  MARK_ITEM_SAVED_STORAGE_STATUS, saved_storage_status,
                  MARK_ITEM_BINLOG_OFFSET, MARK_ITEM_FETCH_BINLOG_DONE,
                  fetch_binlog_done);
...
static int recovery_reader_init(const char *pBasePath,
    ...
    if ((result = iniLoadFromFile(full_mark_filename, &iniContext)) != 0) {
        logError(
            ...
            "error code: %d",
            __LINE__, full_mark_filename, result);
        return result;
    }

    if (!iniGetBoolValue(NULL, MARK_ITEM_FETCH_BINLOG_DONE, &iniContext,
                         false)) {
        ...
        logInfo(
            ...
            ", line: %d, "
            "mark file \"%s\", %s=0, "
            "need to fetch binlog again",
            __LINE__, full_mark_filename, MARK_ITEM_FETCH_BINLOG_DONE);
        return EAGAIN;
    }

    saved_storage_status =
        iniGetIntValue(NULL, MARK_ITEM_SAVED_STORAGE_STATUS, &iniContext, -1);
    if (saved_storage_status < 0) {
        ...
        logError(
            "file: "__FILE__
            ", line: %d, "
            "in mark file \"%s\", %s: %d < 0",
            __LINE__, full_mark_filename, MARK_ITEM_SAVED_STORAGE_STATUS,
            saved_storage_status);
        return EINVAL;
    }

    pReader->binlog_offset =
        iniGetInt64Value(NULL, MARK_ITEM_BINLOG_OFFSET, &iniContext, -1);
    if (pReader->binlog_offset < 0) {
        ...
        logError(
            ...
            "in mark file \"%s\", %s: "
            "%" PRId64 " < 0",
            __LINE__, full_mark_filename, MARK_ITEM_BINLOG_OFFSET,
            pReader->binlog_offset);
        return EINVAL;
    }
    ...
    if (pReader->mark_fd < 0) {
        logError(
            ...
            __LINE__, full_mark_filename, errno, STRERROR(errno));
        return errno != 0 ? errno : ENOENT;
    }

    if ((result = storage_open_readable_binlog(
             pReader, recovery_get_binlog_filename, pBasePath)) != 0) {
        return result;
    }
...
static int storage_do_recovery(const char *pBasePath,
                               StorageBinLogReader *pReader,
    ...
    while (bContinueFlag) {
        ...
        if ((pStorageConn = tracker_connect_server(pSrcStorage, &result)) ==
            NULL) {
            sleep(5);
            continue;
        }
        ...
        while (g_continue_flag) {
            ...
            if (record.op_type == STORAGE_OP_TYPE_SOURCE_CREATE_FILE ||
                ...
                if (fdfs_is_trunk_file(record.filename, record.filename_len)) {
                    ...
                    bTrunkFile = true;
                    if (fdfs_decode_trunk_info(
                            record.store_path_index, record.true_filename,
                            record.true_filename_len, &trunk_info) != 0) {
                        pReader->binlog_offset += record_length;
                        count++;
                        continue;
                    }

                    trunk_get_full_filename(&trunk_info, local_filename,
                ...
                result = storage_download_file_to_file(
                    pTrackerServer, pStorageConn, g_group_name, record.filename,
                    local_filename, &file_size);
                if (result == 0) {
                    if (!bTrunkFile) {
                        set_file_utimes(local_filename, record.timestamp);
                    }
                ...
                if ((result = storage_split_filename_ex(
                         record.filename, &record.filename_len,
                ...
                if (symlink(src_filename, local_filename) == 0) {
                    ...
                    log_it_ex(&g_log_context, log_level,
                              ...
                              "errno: %d, error info: %s",
                              __LINE__, src_filename, local_filename, result,
                              STRERROR(result));
...
static int storage_compare_trunk_id_info(void *p1, void *p2) {
    int result;
    result =
        memcmp(&(((FDFSTrunkFileIdInfo *)p1)->path),
               &(((FDFSTrunkFileIdInfo *)p2)->path), sizeof(FDFSTrunkPathInfo));
    if (result != 0) {
        return result;
    }
...
static int storage_do_split_trunk_binlog(const int store_path_index,
    ...
    pBasePath = g_fdfs_store_paths.paths[store_path_index];
    recovery_get_full_filename(pBasePath, RECOVERY_BINLOG_FILENAME ".tmp",
                               tmpFullFilename);
    ...
    if (fp == NULL) {
        ...
        logError(
            ...
            "open file: %s fail, "
            "errno: %d, error info: %s.",
            __LINE__, tmpFullFilename, result, STRERROR(result));
        return result;
    }
    ...
    while (g_continue_flag) {
        ...
        if (fdfs_is_trunk_file(record.filename, record.filename_len)) {
            if (fdfs_decode_trunk_info(store_path_index, record.true_filename,
                                       record.true_filename_len,
                                       &trunk_info) != 0) {
                continue;
            }
            ...
            pFound = (FDFSTrunkFileIdInfo *)avl_tree_find(&tree_unique_trunks,
                                                          &trunkFileId);
            if (pFound != NULL) {
                continue;
            }

            pFound = (FDFSTrunkFileIdInfo *)malloc(sizeof(FDFSTrunkFileIdInfo));
            if (pFound == NULL) {
                ...
                logError(
                    ...
                    "malloc %d bytes fail, "
                    "errno: %d, error info: %s",
                    __LINE__, (int)sizeof(FDFSTrunkFileIdInfo), result,
                    STRERROR(result));
                break;
            }

            sprintf(trunkFileId.line, "%d %c %s", (int)record.timestamp,
    ...
    if (result == 0) {
        ...
        if (tree_node_count > 0) {
            logInfo(
                "file: "__FILE__
                ", line: %d, "
                "recovering trunk file count: %d",
                __LINE__, tree_node_count);
    ...
    recovery_get_full_filename(pBasePath, RECOVERY_BINLOG_FILENAME,
                               binlogFullFilename);
    if (rename(tmpFullFilename, binlogFullFilename) != 0) {
        logError(
            ...
            "errno: %d, error info: %s",
            __LINE__, tmpFullFilename, binlogFullFilename, errno,
            STRERROR(errno));
        return errno != 0 ? errno : EPERM;
    }
...
static int storage_disk_recovery_split_trunk_binlog(
    const int store_path_index) {
    char *pBasePath;
    StorageBinLogReader reader;
    int result;
...
int storage_disk_recovery_start(const int store_path_index) {
    ...
    if ((result = storage_disk_recovery_split_trunk_binlog(store_path_index)) !=
        0) {
