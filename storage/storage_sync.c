...
static int storage_sync_copy_file(ConnectionInfo *pStorageServer,
    ...
    if ((result =
             trunk_file_stat(pRecord->store_path_index, pRecord->true_filename,
                             pRecord->true_filename_len, &stat_buf, &trunkInfo,
                             &trunkHeader)) != 0) {
        if (result == ENOENT) {
            ...
            logError(
                ...
                __LINE__, pRecord->filename, result, STRERROR(result));
            return result;
        }
    }

    need_sync_file = true;
    if (pReader->last_file_exist &&
        proto_cmd == STORAGE_PROTO_CMD_SYNC_CREATE_FILE) {
        FDFSFileInfo file_info;
        result =
            storage_query_file_info_ex(NULL, pStorageServer, g_group_name,
        ...
        if (result == 0) {
            if (file_info.file_size == stat_buf.st_size) {
                logDebug(
                    ...
                    __LINE__, pRecord->filename, pStorageServer->ip_addr,
                    pStorageServer->port);
                ...
                logWarning(
                    ...
                    " not same as mine: " OFF_PRINTF_FORMAT ", need re-sync it",
                    __LINE__, pRecord->filename, pStorageServer->ip_addr,
                    pStorageServer->port, file_info.file_size,
                    stat_buf.st_size);
    ...
    do {
        ...
        body_len = 2 * FDFS_PROTO_PKG_LEN_SIZE + 4 + FDFS_GROUP_NAME_MAX_LEN +
        ...
        if (need_sync_file && (stat_buf.st_size > 0) &&
            ((result = tcpsendfile_ex(pStorageServer->sock, full_filename,
    ...
    if (result == EEXIST) {
        if (need_sync_file &&
            pRecord->op_type == STORAGE_OP_TYPE_SOURCE_CREATE_FILE) {
            logWarning(
                "file: "__FILE__
                ", line: %d, "
...
#define FIELD_COUNT 3
    ...
    if ((result = storage_split_filename_ex(
             pRecord->filename, &pRecord->true_filename_len,
             pRecord->true_filename, &pRecord->store_path_index)) != 0) {
        return result;
    }
    ...
    if (stat_buf.st_size < start_offset + modify_length) {
        logWarning(
            "file: "__FILE__
            ", line: %d, "
            "appender file: %s 'size: %" PRId64 " < %" PRId64
            ", maybe some mistakes "
            "happened, skip sync this appender file",
    ...
    do {
        ...
        body_len = 3 * FDFS_PROTO_PKG_LEN_SIZE + 4 + FDFS_GROUP_NAME_MAX_LEN +
        ...
        if ((result = tcpsendfile_ex(
                 pStorageServer->sock, full_filename, start_offset,
                 modify_length, g_fdfs_network_timeout, &total_send_bytes)) !=
            0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
...
#define FIELD_COUNT 3
    ...
    if (stat_buf.st_size != new_file_size) {
        logDebug(
            ...
            ", line: %d, "
            "appender file: %s 'size: %" PRId64 " != %" PRId64
            ", maybe append/modify later",
            __LINE__, full_filename, stat_buf.st_size, new_file_size);
    }
...
static int storage_sync_delete_file(ConnectionInfo *pStorageServer,
    ...
    if ((result =
             tcpsenddata_nb(pStorageServer->sock, out_buff,
                            sizeof(TrackerHeader) + 4 +
...
static int storage_sync_link_file(ConnectionInfo *pStorageServer,
    ...
    TrackerHeader *pHeader;
    int result;
    char out_buff[sizeof(TrackerHeader) + 2 * FDFS_PROTO_PKG_LEN_SIZE + 4 +
                  FDFS_GROUP_NAME_MAX_LEN + 256];
    char in_buff[1];
    FDFSTrunkFullInfo trunkInfo;
    FDFSTrunkHeader trunkHeader;
    int out_body_len;
    ...
    if ((result = trunk_file_lstat_ex(pRecord->store_path_index,
                                      pRecord->true_filename,
                                      pRecord->true_filename_len, &stat_buf,
    ...
    if (pRecord->src_filename_len > 0) {
        ...
        result = trunk_file_get_content(&trunkInfo, stat_buf.st_size, &fd,
        ...
        *(src_full_filename + src_filename_len) = '\0';
        ...
        pRecord->src_filename_len =
            src_filename_len - (pSrcFilename - src_full_filename) + 4;
    ...
    long2buff(pRecord->src_filename_len,
              out_buff + sizeof(TrackerHeader) + FDFS_PROTO_PKG_LEN_SIZE);
    int2buff(pRecord->timestamp,
             out_buff + sizeof(TrackerHeader) + 2 * FDFS_PROTO_PKG_LEN_SIZE);
    sprintf(out_buff + sizeof(TrackerHeader) + 2 * FDFS_PROTO_PKG_LEN_SIZE + 4,
            "%s", g_group_name);
    memcpy(out_buff + sizeof(TrackerHeader) + 2 * FDFS_PROTO_PKG_LEN_SIZE + 4 +
    ...
    out_body_len = 2 * FDFS_PROTO_PKG_LEN_SIZE + 4 + FDFS_GROUP_NAME_MAX_LEN +
                   pRecord->filename_len + pRecord->src_filename_len;
...
static int storage_sync_data(StorageBinLogReader *pReader,
    ...
    switch (pRecord->op_type) {
        case STORAGE_OP_TYPE_SOURCE_CREATE_FILE:
            result = storage_sync_copy_file(pStorageServer, pReader, pRecord,
        ...
        case STORAGE_OP_TYPE_SOURCE_APPEND_FILE:
            result =
                storage_sync_modify_file(pStorageServer, pReader, pRecord,
                                         STORAGE_PROTO_CMD_SYNC_APPEND_FILE);
            if (result == ENOENT)  // resync appender file
            {
                result =
                    storage_sync_copy_file(pStorageServer, pReader, pRecord,
                                           STORAGE_PROTO_CMD_SYNC_UPDATE_FILE);
            }
            break;
        ...
        case STORAGE_OP_TYPE_SOURCE_TRUNCATE_FILE:
            result =
                storage_sync_truncate_file(pStorageServer, pReader, pRecord);
            break;
...
int storage_sync_init() {
    ...
    snprintf(sync_path, sizeof(sync_path), "%s/" SYNC_DIR_NAME, data_path);
    if (!fileExists(sync_path)) {
        if (mkdir(sync_path, 0755) != 0) {
            logError(
                ...
                "mkdir \"%s\" fail, "
                "errno: %d, error info: %s",
                __LINE__, sync_path, errno, STRERROR(errno));
            return errno != 0 ? errno : ENOENT;
        }
    ...
    if (binlog_write_cache_buff == NULL) {
        logError(
            ...
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, SYNC_BINLOG_WRITE_BUFF_SIZE, errno, STRERROR(errno));
        return errno != 0 ? errno : ENOMEM;
    }

    snprintf(full_filename, sizeof(full_filename), "%s/%s", sync_path,
             SYNC_BINLOG_INDEX_FILENAME);
...
static int storage_binlog_fsync(const bool bNeedLock) {
    ...
    if (binlog_write_cache_len == 0)  // ignore
    {
        ...
        logError(
            ...
            "errno: %d, error info: %s",
            __LINE__, get_writable_binlog_filename(NULL), g_binlog_fd, errno,
            STRERROR(errno));
        ...
        logError(
            ...
            "errno: %d, error info: %s",
            __LINE__, get_writable_binlog_filename(NULL), errno,
            STRERROR(errno));
        write_ret = errno != 0 ? errno : EIO;
    } else {
...
int storage_binlog_write_ex(const int timestamp, const char op_type,
    ...
    if (extra != NULL) {
        binlog_write_cache_len +=
            sprintf(binlog_write_cache_buff + binlog_write_cache_len,
...
int storage_open_readable_binlog(StorageBinLogReader *pReader,
                                 get_filename_func filename_func,
                                 const void *pArg) {
    char full_filename[MAX_PATH_SIZE];
    ...
    if (pReader->binlog_offset > 0 &&
        ...
        logError(
            ...
            __LINE__, full_filename, pReader->binlog_offset, errno,
            STRERROR(errno));
...
static char *get_mark_filename_by_id_and_port(const char *storage_id,
    ...
    if (g_use_storage_id) {
        snprintf(full_filename, filename_size, "%s/data/" SYNC_DIR_NAME "/%s%s",
                 g_fdfs_base_path, storage_id, SYNC_MARK_FILE_EXT);
    } else {
...
static char *get_mark_filename_by_ip_and_port(const char *ip_addr,
                                              ...
                                              char *full_filename,
                                              const int filename_size) {
    snprintf(full_filename, filename_size, "%s/data/" SYNC_DIR_NAME "/%s_%d%s",
             g_fdfs_base_path, ip_addr, port, SYNC_MARK_FILE_EXT);
    return full_filename;
}
...
char *get_mark_filename_by_reader(const void *pArg, char *full_filename) {
    ...
    return get_mark_filename_by_id_and_port(pReader->storage_id, g_server_port,
...
int storage_report_storage_status(const char *storage_id, const char *ip_addr,
    ...
    for (pGlobalServer = g_tracker_group.servers; pGlobalServer < pTServerEnd;
        ...
        for (i = 0; i < 3; i++) {
            ...
            if ((result = connectserverbyip_nb(
                     pTServer->sock, pTServer->ip_addr, pTServer->port,
        ...
        report_count++;
        if ((result = tracker_report_storage_status(pTServer, &briefServer)) ==
            0) {
            success_count++;
        }
...
static int storage_reader_sync_init_req(StorageBinLogReader *pReader) {
    ...
    pTrackerServers = (ConnectionInfo *)malloc(sizeof(ConnectionInfo) *
    ...
    if (pTrackerServers == NULL) {
        logError(
            ...
            "malloc %d bytes fail",
            __LINE__,
            (int)sizeof(ConnectionInfo) * g_tracker_group.server_count);
        return errno != 0 ? errno : ENOMEM;
    }
    ...
    do {
        while (g_continue_flag) {
            ...
            if ((conn_ret = connectserverbyip_nb(
                     pTServer->sock, pTServer->ip_addr, pTServer->port,
                     g_fdfs_connect_timeout)) == 0) {
                break;
            }
            ...
            logError(
                ...
                "connect to tracker server %s:%d fail, "
                "errno: %d, error info: %s",
                __LINE__, pTServer->ip_addr, pTServer->port, conn_ret,
                STRERROR(conn_ret));
...
int storage_reader_init(FDFSStorageBrief *pStorage,
    ...
    pReader->binlog_buff.buffer = (char *)malloc(STORAGE_BINLOG_BUFFER_SIZE);
    if (pReader->binlog_buff.buffer == NULL) {
        logError(
            ...
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, STORAGE_BINLOG_BUFFER_SIZE, errno, STRERROR(errno));
        return errno != 0 ? errno : ENOMEM;
    }
    ...
    if (pStorage == NULL) {
        ...
        if (!bFileExist && (g_use_storage_id && pStorage != NULL)) {
            ...
            get_mark_filename_by_ip_and_port(pStorage->ip_addr, g_server_port,
                                             old_mark_filename,
            ...
            if (fileExists(old_mark_filename)) {
                if (rename(old_mark_filename, full_filename) != 0) {
                    logError(
                        ...
                        ", errno: %d, error info: %s",
                        __LINE__, old_mark_filename, full_filename, errno,
                        STRERROR(errno));
                    return errno != 0 ? errno : EACCES;
                }
                bFileExist = true;
    ...
    if (bFileExist) {
        ...
        bNeedSyncOld =
            iniGetBoolValue(NULL, MARK_ITEM_NEED_SYNC_OLD, &iniContext, false);
        if (pStorage != NULL &&
        ...
        if (bFileExist) {
            pReader->binlog_index = iniGetIntValue(
                NULL, MARK_ITEM_BINLOG_FILE_INDEX, &iniContext, -1);
            pReader->binlog_offset = iniGetInt64Value(
                NULL, MARK_ITEM_BINLOG_FILE_OFFSET, &iniContext, -1);
            pReader->sync_old_done = iniGetBoolValue(
                NULL, MARK_ITEM_SYNC_OLD_DONE, &iniContext, false);
            pReader->until_timestamp = iniGetIntValue(
                NULL, MARK_ITEM_UNTIL_TIMESTAMP, &iniContext, -1);
            pReader->scan_row_count = iniGetInt64Value(
                NULL, MARK_ITEM_SCAN_ROW_COUNT, &iniContext, 0);
            pReader->sync_row_count = iniGetInt64Value(
                NULL, MARK_ITEM_SYNC_ROW_COUNT, &iniContext, 0);
            ...
            if (pReader->binlog_index < 0) {
                ...
                logError(
                    ...
                    "in mark file \"%s\", "
                    "binlog_index: %d < 0",
                    __LINE__, full_filename, pReader->binlog_index);
                return EINVAL;
            }
            if (pReader->binlog_offset < 0) {
                iniFreeContext(&iniContext);
    ...
    if ((result = storage_open_readable_binlog(
             pReader, get_binlog_readable_filename, pReader)) != 0) {
        return result;
    }
...
static int storage_write_to_mark_file(StorageBinLogReader *pReader) {
    ...
    if ((result =
             storage_write_to_fd(pReader->mark_fd, get_mark_filename_by_reader,
                                 pReader, buff, len)) == 0) {
        pReader->last_scan_rows = pReader->scan_row_count;
        pReader->last_sync_rows = pReader->sync_row_count;
    }
...
static int storage_binlog_preread(StorageBinLogReader *pReader) {
    ...
    if (pReader->binlog_buff.current != pReader->binlog_buff.buffer) {
        if (pReader->binlog_buff.length > 0) {
            memcpy(pReader->binlog_buff.buffer, pReader->binlog_buff.current,
    ...
    bytes_read =
        fc_safe_read(pReader->binlog_fd,
    ...
    if (bytes_read < 0) {
        logError(
            ...
            "error no: %d, error info: %s",
            __LINE__, get_binlog_readable_filename(pReader, NULL),
            pReader->binlog_offset + pReader->binlog_buff.length, errno,
            STRERROR(errno));
        return errno != 0 ? errno : EIO;
...
static int storage_binlog_do_line_read(StorageBinLogReader *pReader, char *line,
                                       const int line_size, int *line_length) {
    char *pLineEnd;
...
static int storage_binlog_read_line(StorageBinLogReader *pReader, char *line,
                                    const int line_size, int *line_length) {
    int result;
...
int storage_binlog_read(StorageBinLogReader *pReader,
    ...
    while (1) {
        result = storage_binlog_read_line(pReader, line, sizeof(line),
                                          record_length);
        if (result == 0) {
            break;
        } else if (result != ENOENT) {
    ...
    if (pRecord->filename_len > sizeof(pRecord->filename) - 1) {
        logError(
            ...
            __LINE__, get_binlog_readable_filename(pReader, NULL),
            pReader->binlog_offset, pRecord->filename_len,
            (int)sizeof(pRecord->filename) - 1);
        return EINVAL;
    }
    ...
    if (pRecord->op_type == STORAGE_OP_TYPE_SOURCE_CREATE_LINK ||
        ...
        if (p == NULL) {
            *(pRecord->src_filename) = '\0';
            ...
            pRecord->src_filename_len =
                pRecord->filename_len - (p - pRecord->filename) - 1;
            pRecord->filename_len = p - pRecord->filename;
            *p = '\0';

            memcpy(pRecord->src_filename, p + 1, pRecord->src_filename_len);
            *(pRecord->src_filename + pRecord->src_filename_len) = '\0';
        }
    } else {
        *(pRecord->src_filename) = '\0';
...
int storage_unlink_mark_file(const char *storage_id) {
    ...
    snprintf(new_filename, sizeof(new_filename), "%s.%04d%02d%02d%02d%02d%02d",
             old_filename, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
    ...
    if (rename(old_filename, new_filename) != 0) {
        logError(
            ...
            "rename file %s to %s fail"
            ", errno: %d, error info: %s",
            __LINE__, old_filename, new_filename, errno, STRERROR(errno));
        return errno != 0 ? errno : EACCES;
    }
...
int storage_rename_mark_file(const char *old_ip_addr, const int old_port,
    ...
    get_mark_filename_by_id_and_port(old_ip_addr, old_port, old_filename,
                                     sizeof(old_filename));
    if (!fileExists(old_filename)) {
        return ENOENT;
    }

    get_mark_filename_by_id_and_port(new_ip_addr, new_port, new_filename,
                                     sizeof(new_filename));
...
static void storage_sync_get_start_end_times(time_t current_time,
                                             const TimeInfo *pStartTime,
                                             const TimeInfo *pEndTime,
                                             time_t *start_time,
                                             time_t *end_time) {
    struct tm tm_time;
    // char buff[32];
    ...
    *start_time = mktime(&tm_time);
    ...
    // end time < start time
    if (pEndTime->hour < pStartTime->hour ||
        (pEndTime->hour == pStartTime->hour &&
         pEndTime->minute < pStartTime->minute)) {
...
static void *storage_sync_thread_entrance(void *arg) {
    ...
    while (g_continue_flag && pStorage->status != FDFS_STORAGE_STATUS_DELETED &&
        ...
        if (g_sync_part_time) {
            current_time = g_current_time;
            storage_sync_get_start_end_times(current_time, &g_sync_end_time,
                                             &g_sync_start_time, &start_time,
                                             &end_time);
            start_time += 60;
            end_time -= 60;
            while (g_continue_flag &&
        ...
        while (g_continue_flag &&
               ...
               pStorage->status != FDFS_STORAGE_STATUS_NONE) {
            strcpy(storage_server.ip_addr, pStorage->ip_addr);
            storage_server.sock = socket(AF_INET, SOCK_STREAM, 0);
            ...
            if ((conn_result = connectserverbyip_nb(
                     storage_server.sock, pStorage->ip_addr, g_server_port,
                     g_fdfs_connect_timeout)) == 0) {
                ...
                logInfo(
                    ...
                    "storage server %s:%d%s",
                    __LINE__, pStorage->ip_addr, g_server_port, szFailPrompt);
                nContinuousFail = 0;
                break;
            }
            ...
            if (previousCode != conn_result) {
                logError(
                    ...
                    ", errno: %d, error info: %s",
                    __LINE__, pStorage->ip_addr, g_server_port, conn_result,
                    STRERROR(conn_result));
        ...
        if (nContinuousFail > 0) {
            logError(
                ...
                __LINE__, pStorage->ip_addr, g_server_port, nContinuousFail,
        ...
        getSockIpaddr(storage_server.sock, local_ip_addr, IP_ADDRESS_SIZE);
        ...
        */
        ...
        if (pStorage->status == FDFS_STORAGE_STATUS_WAIT_SYNC) {
            pStorage->status = FDFS_STORAGE_STATUS_SYNCING;
            storage_report_storage_status(pStorage->id, pStorage->ip_addr,
                                          pStorage->status);
        }
        ...
        if (g_sync_part_time) {
            current_time = g_current_time;
            storage_sync_get_start_end_times(current_time, &g_sync_start_time,
                                             &g_sync_end_time, &start_time,
                                             &end_time);
        }
        ...
        while (g_continue_flag &&
               ...
               (pStorage->status == FDFS_STORAGE_STATUS_ACTIVE ||
                pStorage->status == FDFS_STORAGE_STATUS_SYNCING)) {
            read_result = storage_binlog_read(&reader, &record, &record_len);
                ...
                if (reader.need_sync_old && !reader.sync_old_done) {
                    ...
                    if (pStorage->status == FDFS_STORAGE_STATUS_SYNCING) {
                        ...
                        storage_report_storage_status(
                            pStorage->id, pStorage->ip_addr, pStorage->status);
                    }
                }
            ...
            if (read_result != 0) {
                if (read_result == EINVAL && g_file_sync_skip_invalid_record) {
                    logWarning(
                        ...
                        "%d, offset: %" PRId64,
                        __LINE__, reader.binlog_index, reader.binlog_offset);
                } else {
                    sleep(5);
                    break;
                }
            } else if ((sync_result = storage_sync_data(
                            &reader, &storage_server, &record)) != 0) {
                logDebug(
                    "file: "__FILE__
                    ", line: %d, "
...
int storage_sync_thread_start(const FDFSStorageBrief *pStorage) {
    ...
    if (pStorage->status == FDFS_STORAGE_STATUS_DELETED ||
        ...
        logWarning(
            ...
            "can't start sync thread!",
            __LINE__, pStorage->id, pStorage->status);
        return 0;
    }
    ...
    */
    ...
    g_storage_sync_thread_count++;
    sync_tids = (pthread_t *)realloc(
        sync_tids, sizeof(pthread_t) * g_storage_sync_thread_count);
    if (sync_tids == NULL) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, (int)sizeof(pthread_t) * g_storage_sync_thread_count,
