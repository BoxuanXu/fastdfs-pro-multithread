...
extern char g_http_domain[FDFS_DOMAIN_NAME_MAX_SIZE];  // http server domain
                                                       // name
...
extern int
    g_disk_reader_threads;  // disk reader thread count per store base path
extern int
    g_disk_writer_threads;  // disk writer thread count per store base path
...
extern bool g_sync_part_time;  // true for part time, false for all time of a
                               // day
extern int
    g_sync_log_buff_interval;  // sync log buff to disk every interval seconds
extern int g_sync_binlog_buff_interval;  // sync binlog buff to disk every
                                         // interval seconds
extern int g_write_mark_file_freq;     // write to mark file after sync N files
extern int g_sync_stat_file_interval;  // sync storage stat info to disk
                                       // interval
...
extern char
    g_sync_src_id[FDFS_STORAGE_ID_MAX_SIZE];  // the source storage server id

extern char g_group_name[FDFS_GROUP_NAME_MAX_LEN + 1];
extern char g_my_server_id_str[FDFS_STORAGE_ID_MAX_SIZE];  // my server id
                                                           // string
extern char
    g_tracker_client_ip[IP_ADDRESS_SIZE];        // storage ip as tracker client
...
extern byte
    g_id_type_in_filename;     // id type of the storage server in the filename
...
extern struct storage_nio_thread_data *
    g_nio_thread_data;  // network io thread data
extern struct storage_dio_thread_data *g_dio_thread_data;  // disk io thread
                                                           // data
