...
extern int storage_client_create_link(
    ConnectionInfo *pTrackerServer, ConnectionInfo *pStorageServer,
    const char *master_filename, const char *src_filename,
    const int src_filename_len, const char *src_file_sig,
    const int src_file_sig_len, const char *group_name, const char *prefix_name,
    const char *file_ext_name, char *remote_filename, int *filename_len);
...
#define STORAGE_STAT_FILE_FAIL_LOG(result, client_ip, type_caption, filename) \
    if (result == ENOENT) {                                                   \
        ...
        logError(                                                             \
            ...
            __LINE__, client_ip, type_caption, filename, result,              \
            STRERROR(result));                                                \
    }
...
static int storage_create_link_core(struct fast_task_info *pTask,
                                    ...
                                    int *filename_len, const bool bNeedReponse);
...
static FDFSStorageServer *get_storage_server(const char *storage_server_id) {
    ...
    ppFound = (FDFSStorageServer **)bsearch(
        &pTargetServer, g_sorted_storages, g_storage_count,
...
#define CHECK_AND_WRITE_TO_STAT_FILE1                           \
    ...
    if (pClientInfo->pSrcStorage == NULL) {                     \
        pClientInfo->pSrcStorage =                              \
            get_storage_server(pClientInfo->storage_server_id); \
    }                                                           \
    if (pClientInfo->pSrcStorage != NULL) {                     \
...
#define CHECK_AND_WRITE_TO_STAT_FILE1_WITH_BYTES(total_bytes, success_bytes, \
                                                 bytes)                      \
    pthread_mutex_lock(&stat_count_thread_lock);                             \
                                                                             \
    if (pClientInfo->pSrcStorage == NULL) {                                  \
...
#define CHECK_AND_WRITE_TO_STAT_FILE2_WITH_BYTES(                  \
    total_count, success_count, total_bytes, success_bytes, bytes) \
    pthread_mutex_lock(&stat_count_thread_lock);                   \
    total_count++;                                                 \
    success_count++;                                               \
    total_bytes += bytes;                                          \
...
#define CHECK_AND_WRITE_TO_STAT_FILE3_WITH_BYTES(                             \
    total_count, success_count, timestamp, total_bytes, success_bytes, bytes) \
    pthread_mutex_lock(&stat_count_thread_lock);                              \
    total_count++;                                                            \
    success_count++;                                                          \
    timestamp = g_current_time;                                               \
...
static void storage_log_access_log(struct fast_task_info *pTask,
    ...
    time_used =
        (tv_end.tv_sec - pClientInfo->file_context.tv_deal_start.tv_sec) *
            1000 +
        (tv_end.tv_usec - pClientInfo->file_context.tv_deal_start.tv_usec) /
            1000;
    logAccess(&g_access_log_context, &(pClientInfo->file_context.tv_deal_start),
              "%s %s %s %d %d %" PRId64
              " "
              "%" PRId64,
              pTask->client_ip, action, pClientInfo->file_context.fname2log,
              status, time_used, pClientInfo->request_length,
    ...
    do {                                                                      \
        if (g_use_access_log) {                                               \
            if (filename_len < sizeof(pClientInfo->file_context.fname2log)) { \
                memcpy(pClientInfo->file_context.fname2log, filename,         \
                       filename_len + 1);                                     \
            } else {                                                          \
                memcpy(pClientInfo->file_context.fname2log, filename,         \
                       sizeof(pClientInfo->file_context.fname2log));          \
                *(pClientInfo->file_context.fname2log +                       \
                  sizeof(pClientInfo->file_context.fname2log) - 1) = '\0';    \
            }                                                                 \
        }                                                                     \
    } while (0)
...
static bool storage_is_slave_file(const char *remote_filename,
    ...
    base64_decode_auto(&g_fdfs_base64_context,
                       (char *)remote_filename + FDFS_LOGIC_FILE_PATH_LEN,
...
static void storage_delete_file_log_error(struct fast_task_info *pTask,
    ...
    logError(
        ...
        __LINE__, pTask->client_ip, pFileContext->filename, err_no,
        STRERROR(err_no));
}
...
static void storage_sync_delete_file_log_error(struct fast_task_info *pTask,
    ...
    if (err_no == ENOENT) {
        logWarning(
            ...
            "file %s not exist, "
            "maybe delete later?",
            __LINE__, STORAGE_PROTO_CMD_SYNC_DELETE_FILE, pTask->client_ip,
...
static int storage_sync_copy_file_rename_filename(
    ...
    if (rename(pFileContext->filename, full_filename) != 0) {
        ...
        logWarning(
            ...
            "rename %s to %s fail, "
            "errno: %d, error info: %s",
            __LINE__, pFileContext->filename, full_filename, result,
            STRERROR(result));
        return result;
    }
...
static void storage_sync_copy_file_done_callback(struct fast_task_info *pTask,
    ...
    if (result != 0) {
        pthread_mutex_lock(&stat_count_thread_lock);
        g_storage_stat.total_sync_in_bytes += pClientInfo->total_offset;
        pthread_mutex_unlock(&stat_count_thread_lock);
    }
...
static void storage_sync_modify_file_done_callback(struct fast_task_info *pTask,
                                                   const int err_no) {
    StorageClientInfo *pClientInfo;
    StorageFileContext *pFileContext;
    TrackerHeader *pHeader;
    ...
    } else  // FDFS_STORAGE_FILE_OP_DISCARD
    {
        if (result == 0) {
            ...
            if (lstat(pFileContext->filename, &file_stat) != 0) {
                ...
                STORAGE_STAT_FILE_FAIL_LOG(result, pTask->client_ip, "regular",
                                           pFileContext->filename)
...
static void storage_get_metadata_done_callback(struct fast_task_info *pTask,
    ...
    STORAGE_ACCESS_LOG(pTask, ACCESS_LOG_ACTION_GET_METADATA, err_no);
    ...
    if (err_no != 0) {
        ...
        CHECK_AND_WRITE_TO_STAT_FILE2(g_storage_stat.total_get_meta_count,
...
static void storage_download_file_done_callback(struct fast_task_info *pTask,
    ...
    STORAGE_ACCESS_LOG(pTask, ACCESS_LOG_ACTION_DOWNLOAD_FILE, err_no);
...
static int storage_do_delete_meta_file(struct fast_task_info *pTask) {
        ...
        if (unlink(meta_filename) != 0) {
            if (errno != ENOENT) {
                ...
                logError(
                    ...
                    "client ip: %s, delete file %s fail,"
                    "errno: %d, error info: %s",
                    __LINE__, pTask->client_ip, meta_filename, result,
                    STRERROR(result));
                return result;
            }
        } else {
    ...
    if (g_check_file_duplicate) {
        pGroupArray =
            &((g_nio_thread_data + pClientInfo->nio_thread_index)->group_array);
        memset(&key_info_sig, 0, sizeof(key_info_sig));
        key_info_sig.namespace_len = g_namespace_len;
        memcpy(key_info_sig.szNameSpace, g_key_namespace, g_namespace_len);
        key_info_sig.obj_id_len =
            snprintf(key_info_sig.szObjectId, sizeof(key_info_sig.szObjectId),
                     "%s/%s", g_group_name, pFileContext->fname2log);
        ...
        result = fdht_get_ex1(pGroupArray, g_keep_alive, &key_info_sig,
                              FDHT_EXPIRES_NONE, &pValue, &value_len, malloc);
        if (result == 0) {
            memcpy(&key_info_fid, &key_info_sig, sizeof(FDHTKeyInfo));
            key_info_fid.obj_id_len = value_len;
            memcpy(key_info_fid.szObjectId, pValue, value_len);
            ...
            value_len = sizeof(value) - 1;
            result =
                fdht_get_ex1(pGroupArray, g_keep_alive, &key_info_fid,
                             FDHT_EXPIRES_NONE, &pValue, &value_len, malloc);
            if (result == 0) {
                memcpy(&key_info_ref, &key_info_sig, sizeof(FDHTKeyInfo));
                key_info_ref.obj_id_len = value_len;
                memcpy(key_info_ref.szObjectId, pValue, value_len);
                key_info_ref.key_len = sizeof(FDHT_KEY_NAME_REF_COUNT) - 1;
                memcpy(key_info_ref.szKey, FDHT_KEY_NAME_REF_COUNT,
                       key_info_ref.key_len);
                value_len = sizeof(value) - 1;

                result = fdht_get_ex1(pGroupArray, g_keep_alive, &key_info_ref,
                                      FDHT_EXPIRES_NONE, &pValue, &value_len,
                                      malloc);
                if (result == 0) {
                    *(pValue + value_len) = '\0';
                    ...
                    logError(
                        ...
                        "errno: %d, error info: %s",
                        __LINE__, pTask->client_ip, result, STRERROR(result));
                    return result;
                }
            } else if (result != ENOENT) {
                logError(
    ...
    if (g_check_file_duplicate) {
        ...
        value_len = sizeof(value) - 1;
        result = fdht_inc_ex(pGroupArray, g_keep_alive, &key_info_ref,
                             FDHT_EXPIRES_NEVER, -1, value, &value_len);
        if (result != 0) {
            logWarning(
                "file: "__FILE__
                ", line: %d, "
        ...
        *(key_info_ref.szObjectId + key_info_ref.obj_id_len) = '\0';
        ...
        if (pSeperator == NULL) {
            logWarning(
                ...
                ", line: %d, "
                "invalid file_id: %s",
                __LINE__, key_info_ref.szObjectId);
            return 0;
        }

        pSeperator++;
        value_len =
            key_info_ref.obj_id_len - (pSeperator - key_info_ref.szObjectId);
        memcpy(value, pSeperator, value_len + 1);
        if ((result = storage_split_filename_ex(
                 value, &value_len, true_filename, &store_path_index)) != 0) {
            return result;
        }
        if ((result = fdfs_check_data_filename(true_filename, value_len)) !=
            0) {
            return result;
        }

        if ((result = trunk_file_lstat(
                 store_path_index, true_filename, value_len, &stat_buf,
                 &(pFileContext->extra_info.upload.trunk_info),
                 &trunkHeader)) != 0) {
            STORAGE_STAT_FILE_FAIL_LOG(result, pTask->client_ip, "logic", value)
            return 0;
        }
        ...
        if ((result = storage_delete_file_auto(pFileContext)) != 0) {
            logWarning(
                ...
                __LINE__, pTask->client_ip, value, errno, STRERROR(errno));
            return 0;
        }

        storage_binlog_write(g_current_time, STORAGE_OP_TYPE_SOURCE_DELETE_FILE,
                             value);
...
static void storage_delete_fdfs_file_done_callback(struct fast_task_info *pTask,
    ...
    if (result != 0) {
        ...
        if (pFileContext->delete_flag & STORAGE_DELETE_FLAG_FILE) {
            CHECK_AND_WRITE_TO_STAT_FILE3(g_storage_stat.total_delete_count,
...
static void storage_upload_file_done_callback(struct fast_task_info *pTask,
    ...
    if (result == 0) {
        ...
        filename_len = strlen(pFileContext->fname2log);
        pClientInfo->total_length =
            sizeof(TrackerHeader) + FDFS_GROUP_NAME_MAX_LEN + filename_len;
        ...
        if (pFileContext->create_flag & STORAGE_CREATE_FLAG_FILE) {
            g_storage_stat.total_upload_count++;
            g_storage_stat.total_upload_bytes += pClientInfo->total_offset;
        }
        pthread_mutex_unlock(&stat_count_thread_lock);
...
static void storage_trunk_create_link_file_done_callback(
    ...
    if (result == 0) {
        ...
        CHECK_AND_WRITE_TO_STAT_FILE3(g_storage_stat.total_create_link_count,
    ...
    storage_set_link_file_meta(pTask, pSourceFileInfo, pFileContext->fname2log);
...
static void storage_append_file_done_callback(struct fast_task_info *pTask,
    ...
    if (err_no == 0) {
        ...
        sprintf(extra, "%" PRId64 " %" PRId64, pFileContext->start,
                pFileContext->end - pFileContext->start);
...
static void storage_do_truncate_file_done_callback(struct fast_task_info *pTask,
    ...
    if (err_no == 0) {
        ...
        sprintf(extra, "%" PRId64 " %" PRId64,
                pFileContext->end - pFileContext->start, pFileContext->offset);
        result = storage_binlog_write_ex(pFileContext->timestamp2log,
                                         pFileContext->sync_flag,
                                         pFileContext->fname2log, extra);
    } else {
    ...
    if (result == 0) {
        CHECK_AND_WRITE_TO_STAT_FILE3(g_storage_stat.total_truncate_count,
...
static void storage_set_metadata_done_callback(struct fast_task_info *pTask,
    ...
    if (result != 0) {
        ...
        CHECK_AND_WRITE_TO_STAT_FILE3(g_storage_stat.total_set_meta_count,
...
#define ALLOC_CONNECTIONS_ONCE 256
    ...
    if ((result = init_pthread_attr(&thread_attr, g_thread_stack_size)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "init_pthread_attr fail, program exit!",
            __LINE__);
        return result;
    }
    ...
    if ((result = free_queue_init_ex(
             g_max_connections, init_connections, ALLOC_CONNECTIONS_ONCE,
             g_buff_size, g_buff_size, sizeof(StorageClientInfo))) != 0) {
        return result;
    }
    ...
    for (pThreadData = g_nio_thread_data; pThreadData < pDataEnd;
        ...
        if ((result = pthread_create(&tid, &thread_attr, work_thread_entrance,
                                     pThreadData)) != 0) {
            logError(
                "file: "__FILE__
                ", line: %d, "
                ...
                "errno: %d, error info: %s",
                __LINE__, g_storage_thread_count, result, STRERROR(result));
            break;
        } else {
...
static void *accept_thread_entrance(void *arg) {
    ...
    while (g_continue_flag) {
        ...
        incomesock =
            accept(server_sock, (struct sockaddr *)&inaddr, &sockaddr_len);
        ...
        if (write(pThreadData->thread_data.pipe_fds[1], &task_addr,
            ...
            current_connections = __sync_add_and_fetch(
                &g_storage_stat.connection.current_count, 1);
...
void storage_accept_loop(int server_sock) {
    if (g_accept_threads > 1) {
        ...
        if ((result = init_pthread_attr(&thread_attr, g_thread_stack_size)) !=
            0) {
            logWarning(
                "file: "__FILE__
                ", line: %d, "
                "init_pthread_attr fail!",
                __LINE__);
        } else {
            for (i = 1; i < g_accept_threads; i++) {
                if ((result = pthread_create(
                         &tid, &thread_attr, accept_thread_entrance,
    ...
    if (g_file_distribute_path_mode == FDFS_FILE_DIST_PATH_ROUND_ROBIN) {
        ...
        *sub_path_low = g_dist_path_index_low;
        ...
        if (++g_dist_write_file_count >= g_file_distribute_rotate_count) {
            ...
            if ((result = pthread_mutex_lock(&path_index_thread_lock)) != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
                    "call pthread_mutex_lock fail, "
            ...
            if (g_dist_path_index_low >= g_subdir_count_per_path) {  // rotate
            ...
            if ((result = pthread_mutex_unlock(&path_index_thread_lock)) != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
...
static int storage_gen_filename(StorageClientInfo *pClientInfo,
    ...
    if (!pClientInfo->file_context.extra_info.upload.if_sub_path_alloced) {
        int sub_path_high;
        int sub_path_low;
        storage_get_store_path(encoded, *filename_len, &sub_path_high,
                               &sub_path_low);
    ...
    len =
        sprintf(filename, FDFS_STORAGE_DATA_DIR_FORMAT
                "/" FDFS_STORAGE_DATA_DIR_FORMAT "/",
                pTrunkInfo->path.sub_path_high, pTrunkInfo->path.sub_path_low);
...
static int storage_get_filename(StorageClientInfo *pClientInfo,
                                const int start_time, const int64_t file_size,
                                const int crc32, const char *szFormattedExt,
                                char *filename, int *filename_len,
                                char *full_filename) {
    int i;
    int result;
    ...
    store_path_index = pClientInfo->file_context.extra_info.upload.trunk_info
                           .path.store_path_index;
    for (i = 0; i < 10; i++) {
        if ((result = storage_gen_filename(
                 pClientInfo, file_size, crc32, szFormattedExt,
                 FDFS_FILE_EXT_NAME_MAX_LEN + 1, start_time, filename,
                 filename_len)) != 0) {
            return result;
        }
...
static int storage_client_create_link_wrapper(
    struct fast_task_info *pTask, const char *master_filename,
    const char *src_filename, const int src_filename_len,
    const char *src_file_sig, const int src_file_sig_len,
    const char *group_name, const char *prefix_name, const char *file_ext_name,
    ...
    if ((pTracker = tracker_get_connection_r(&trackerServer, &result)) ==
        NULL) {
        return result;
    }
    ...
    if (strcmp(group_name, g_group_name) != 0) {
        ...
        result = tracker_query_storage_update(pTracker, &storageServer,
                                              group_name, src_filename);
        if (result != 0) {
    ...
    if (bCreateDirectly) {
        ...
        *filename_len = src_filename_len;
        if ((result = storage_split_filename_ex(
                 src_filename, filename_len, sourceFileInfo.src_true_filename,
        ...
        result = storage_create_link_core(
            pTask, &sourceFileInfo, src_filename, master_filename,
        ...
        result = storage_client_create_link(
            pTracker, pStorageServer, master_filename, src_filename,
            src_filename_len, src_file_sig, src_file_sig_len, group_name,
            prefix_name, file_ext_name, remote_filename, filename_len);
...
static int storage_service_upload_file_done(struct fast_task_info *pTask) {
    ...
    *new_filename = '\0';
    ...
    if ((result = storage_get_filename(
             pClientInfo, end_time, file_size_in_name, pFileContext->crc32,
             pFileContext->extra_info.upload.formatted_ext_name, new_filename,
    ...
    if (pFileContext->extra_info.upload.file_type & _FILE_TYPE_TRUNK) {
        ...
        trunk_file_info_encode(
            &(pFileContext->extra_info.upload.trunk_info.file), trunk_buff);

        sprintf(new_fname2log + FDFS_LOGIC_FILE_PATH_LEN +
                    FDFS_FILENAME_BASE64_LENGTH,
                "%s%s", trunk_buff, new_filename + FDFS_TRUE_FILE_PATH_LEN +
                                        FDFS_FILENAME_BASE64_LENGTH);
        ...
        logError(
            ...
            "rename %s to %s fail, "
            "errno: %d, error info: %s",
            __LINE__, pFileContext->filename, new_full_filename, result,
            STRERROR(result));
    ...
    if ((pFileContext->extra_info.upload.file_type & _FILE_TYPE_SLAVE)) {
        ...
        int master_filename_len =
            strlen(pFileContext->extra_info.upload.master_filename);
        if ((result = storage_split_filename_ex(
        ...
        if ((result = fdfs_gen_slave_filename(
                 true_filename, pFileContext->extra_info.upload.prefix_name,
                 pFileContext->extra_info.upload.file_ext_name, filename,
                 &filename_len)) != 0) {
            unlink(new_full_filename);
            return result;
        }
        ...
        snprintf(pFileContext->filename, sizeof(pFileContext->filename),
                 "%s/data/%s",
                 g_fdfs_store_paths.paths[master_store_path_index], filename);
        sprintf(pFileContext->fname2log,
                "%c" FDFS_STORAGE_DATA_DIR_FORMAT "/%s",
                FDFS_STORAGE_STORE_PATH_PREFIX_CHAR, master_store_path_index,
                filename);
        ...
        if (g_store_slave_file_use_link) {
            if (symlink(new_full_filename, pFileContext->filename) != 0) {
                ...
                logError(
                    ...
                    "errno: %d, error info: %s",
                    __LINE__, new_full_filename, pFileContext->filename, result,
                    STRERROR(result));
    ...
    if (g_check_file_duplicate &&
        ...
        */
        ...
        pValue = value;
        value_len = sizeof(value) - 1;
        result = fdht_get_ex1(pGroupArray, g_keep_alive, &key_info,
                              FDHT_EXPIRES_NONE, &pValue, &value_len, malloc);
        if (result == 0) {  // exists
            char *pGroupName;
            char *pSrcFilename;
            ...
            *pSeperator = '\0';
            ...
            if ((result = storage_delete_file_auto(pFileContext)) != 0) {
                logError(
                    ...
                    "error info: %s",
                    __LINE__, ((pFileContext->extra_info.upload.file_type &
                                _FILE_TYPE_TRUNK)
                                   ? pFileContext->fname2log
                                   : pFileContext->filename),
            ...
            memset(pFileContext->extra_info.upload.group_name, 0,
                   FDFS_GROUP_NAME_MAX_LEN + 1);
            snprintf(pFileContext->extra_info.upload.group_name,
                     FDFS_GROUP_NAME_MAX_LEN + 1, "%s", pGroupName);
            result = storage_client_create_link_wrapper(
                pTask, pFileContext->extra_info.upload.master_filename,
                pSrcFilename, value_len - (pSrcFilename - value),
                key_info.szObjectId, key_info.obj_id_len, pGroupName,
            ...
            value_len = sprintf(value, "%s/%s", g_group_name, new_fname2log);
            if ((result = fdht_set_ex(pGroupArray, g_keep_alive, &key_info,
                                      FDHT_EXPIRES_NEVER, value, value_len)) !=
                0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
            ...
            if ((result = fdht_set_ex(pGroupArray, g_keep_alive, &ref_count_key,
            ...
            result = storage_client_create_link_wrapper(
                pTask, pFileContext->extra_info.upload.master_filename,
                src_filename, filename_len, szFileSig, nSigLen, g_group_name,
                pFileContext->extra_info.upload.prefix_name,
                pFileContext->extra_info.upload.file_ext_name,
                pFileContext->fname2log, &filename_len);
...
static int storage_trunk_do_create_link(
    struct fast_task_info *pTask, const int64_t file_bytes,
    const int buff_offset, FileBeforeOpenCallback before_open_callback,
    FileDealDoneCallback done_callback) {
    StorageClientInfo *pClientInfo;
    StorageFileContext *pFileContext;
    int64_t file_offset;
    ...
    file_offset =
        TRUNK_FILE_START_OFFSET(pFileContext->extra_info.upload.trunk_info);
    trunk_get_full_filename(&(pFileContext->extra_info.upload.trunk_info),
                            pFileContext->filename,
                            sizeof(pFileContext->filename));
    pFileContext->extra_info.upload.before_open_callback = before_open_callback;
    ...
    pFileContext->dio_thread_index = storage_dio_get_thread_index(
        pTask, pFileContext->extra_info.upload.trunk_info.path.store_path_index,
        pFileContext->op);
...
static int storage_trunk_create_link(struct fast_task_info *pTask,
                                     const char *src_filename,
                                     const SourceFileInfo *pSourceFileInfo,
    ...
    if ((result = trunk_client_trunk_alloc_space(TRUNK_CALC_SIZE(file_bytes),
                                                 pTrunkInfo)) != 0) {
        return result;
    }

    pTask->length = pTask->size;
    p = pTask->data + (pTask->length - sizeof(TrunkCreateLinkArg) - file_bytes);
...
static int storage_service_do_create_link(struct fast_task_info *pTask,
    ...
    if (*filename_len == 0) {
        ...
        crc32 = rand();
        if ((result =
                 storage_get_filename(pClientInfo, g_current_time, file_size,
                                      crc32, formatted_ext_name, filename,
    ...
    if (symlink(src_full_filename, full_filename) != 0) {
        ...
        logError(
            ...
            "link file %s to %s fail, "
            "errno: %d, error info: %s",
            __LINE__, src_full_filename, full_filename, result,
            STRERROR(result));
        *filename = '\0';
        *filename_len = 0;
        return result;
    ...
    *filename_len = sprintf(
        full_filename, "%c" FDFS_STORAGE_DATA_DIR_FORMAT "/%s",
        FDFS_STORAGE_STORE_PATH_PREFIX_CHAR, store_path_index, filename);
...
static int storage_set_link_file_meta(struct fast_task_info *pTask,
    ...
    key_info.obj_id_len = snprintf(
        key_info.szObjectId, sizeof(key_info.szObjectId),
        "%s/%c" FDFS_STORAGE_DATA_DIR_FORMAT "/%s", g_group_name,
        FDFS_STORAGE_STORE_PATH_PREFIX_CHAR,
        pFileContext->extra_info.upload.trunk_info.path.store_path_index,
        pSrcFileInfo->src_true_filename);
    ...
    key_info.obj_id_len =
        snprintf(key_info.szObjectId, sizeof(key_info.szObjectId), "%s/%s",
                 g_group_name, link_filename);
    key_info.key_len = sizeof(FDHT_KEY_NAME_FILE_SIG) - 1;
    memcpy(key_info.szKey, FDHT_KEY_NAME_FILE_SIG, key_info.key_len);
    if ((result = fdht_set_ex(pGroupArray, g_keep_alive, &key_info,
                              FDHT_EXPIRES_NEVER, pSrcFileInfo->src_file_sig,
    ...
    /*
    logInfo("create link, counter=%s, object_id=%s(%d), key=%s,
    file_sig_len=(%d)", \
...
static int storage_do_set_metadata(struct fast_task_info *pTask) {
    ...
    do {
        if (pFileContext->extra_info.setmeta.op_flag ==
            ...
            if ((result = storage_sort_metadata_buff(meta_buff, meta_bytes)) !=
                0) {
                break;
            }
        ...
        result =
            getFileContent(pFileContext->filename, &file_buff, &file_bytes);
        if (result == ENOENT) {
            if (meta_bytes == 0) {
                result = 0;
                break;
        ...
        old_meta_list =
            fdfs_split_metadata(file_buff, &old_meta_count, &result);
        ...
        new_meta_list =
            fdfs_split_metadata(meta_buff, &new_meta_count, &result);
        ...
        all_meta_list = (FDFSMetaData *)malloc(
            sizeof(FDFSMetaData) * (old_meta_count + new_meta_count));
        if (all_meta_list == NULL) {
            ...
            logError(
                ...
                "malloc %d bytes fail",
                __LINE__,
                (int)sizeof(FDFSMetaData) * (old_meta_count + new_meta_count));
            result = errno != 0 ? errno : ENOMEM;
            break;
        }
        ...
        all_meta_buff = fdfs_pack_metadata(
            all_meta_list, pAllMeta - all_meta_list, NULL, &all_meta_bytes);
        free(all_meta_list);
        ...
        pFileContext->sync_flag = STORAGE_OP_TYPE_SOURCE_UPDATE_FILE;
        result =
            writeToFile(pFileContext->filename, all_meta_buff, all_meta_bytes);
...
static int storage_server_set_metadata(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <=
        2 * FDFS_PROTO_PKG_LEN_SIZE + 1 + FDFS_GROUP_NAME_MAX_LEN) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            ...
            " is not correct, "
            "expect length > %d",
            __LINE__, STORAGE_PROTO_CMD_SET_METADATA, pTask->client_ip,
            nInPackLen,
            2 * FDFS_PROTO_PKG_LEN_SIZE + 1 + FDFS_GROUP_NAME_MAX_LEN);
    ...
    if (nInPackLen + sizeof(TrackerHeader) >= pTask->size) {
        logError(
            ...
            "expect length < %d",
            __LINE__, STORAGE_PROTO_CMD_SET_METADATA, pTask->client_ip,
            nInPackLen, pTask->size - (int)sizeof(TrackerHeader));
    ...
    *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    if (strcmp(group_name, g_group_name) != 0) {
        logError(
            ...
            __LINE__, pTask->client_ip, group_name, g_group_name);
        return EINVAL;
    }
    ...
    *(filename + filename_len) = '\0';
    ...
    if ((result = storage_split_filename_ex(
             filename, &true_filename_len, true_filename, &store_path_index)) !=
        0) {
        return result;
    }
    if ((result = fdfs_check_data_filename(true_filename, true_filename_len)) !=
        0) {
        return result;
    }
    ...
    *(meta_buff + meta_bytes) = '\0';
    ...
    if ((result = trunk_file_lstat(
             store_path_index, true_filename, true_filename_len, &stat_buf,
             &(pFileContext->extra_info.upload.trunk_info), &trunkHeader)) !=
        0) {
        STORAGE_STAT_FILE_FAIL_LOG(result, pTask->client_ip, "logic", filename)
        return result;
    }
    ...
    sprintf(pFileContext->fname2log, "%s%s", filename,
            FDFS_STORAGE_META_FILE_EXT);
...
static int storage_server_report_server_id(struct fast_task_info *pTask) {
    ...
    if (nInPackLen != FDFS_STORAGE_ID_MAX_SIZE) {
        logError(
            ...
            " is not correct, "
            "expect length: %d",
            __LINE__, STORAGE_PROTO_CMD_REPORT_SERVER_ID, pTask->client_ip,
            nInPackLen, FDFS_STORAGE_ID_MAX_SIZE);
        return EINVAL;
    }
...
static int storage_server_trunk_sync_binlog(struct fast_task_info *pTask) {
    ...
    if (nInPackLen == 0) {
        logError(
            ...
            __LINE__, STORAGE_PROTO_CMD_TRUNK_SYNC_BINLOG, pTask->client_ip,
            nInPackLen);
        return EINVAL;
    }
    ...
    if (!g_if_use_trunk_file) {
        logError(
            ...
            __LINE__, pTask->client_ip, STORAGE_PROTO_CMD_TRUNK_SYNC_BINLOG);
        return EINVAL;
    }
...
static int storage_server_query_file_info(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <= FDFS_GROUP_NAME_MAX_LEN) {
        logError(
            ...
            " is not correct, "
            "expect length > %d",
            __LINE__, STORAGE_PROTO_CMD_QUERY_FILE_INFO, pTask->client_ip,
    ...
    if (filename_len >= sizeof(pClientInfo->file_context.fname2log)) {
        logError(
            ...
            __LINE__, STORAGE_PROTO_CMD_QUERY_FILE_INFO, pTask->client_ip,
            filename_len, (int)sizeof(pClientInfo->file_context.fname2log));
        return EINVAL;
    }
    ...
    *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    if ((result = trunk_file_lstat(store_path_index, true_filename,
                                   true_filename_len, &file_lstat, &trunkInfo,
                                   &trunkHeader)) != 0) {
        if (result != ENOENT) {
            logError(
                "file: "__FILE__
                ...
                "errno: %d, error info: %s",
                __LINE__, pTask->client_ip, filename, result, STRERROR(result));
        } else if (!bSilence) {
            logDebug(
                "file: "__FILE__
                ", line: %d, "
    ...
    if (S_ISLNK(file_lstat.st_mode)) {
        if (IS_TRUNK_FILE_BY_ID(trunkInfo)) {
            ...
            result =
                trunk_file_get_content(&trunkInfo, file_lstat.st_size, NULL,
            ...
            if (result != 0) {
                if (!bSilence) {
                    logError(
                        ...
                        "errno: %d, error info: %s",
                        __LINE__, pTask->client_ip, true_filename, result,
                        STRERROR(result));
                }
                return result;
            }
            ...
            *(src_filename + src_filename_len) = '\0';
            if ((result = storage_split_filename_ex(
                     src_filename, &src_filename_len, src_true_filename,
                     &src_store_path_index)) != 0) {
                return result;
            }

            result = trunk_file_lstat(src_store_path_index, src_true_filename,
                                      src_filename_len, &file_stat, &trunkInfo,
                                      &trunkHeader);
            if (result != 0) {
                if (result != ENOENT) {
                    logError(
                        ...
                        __LINE__, pTask->client_ip, src_filename, result,
                        STRERROR(result));
                } else if (!bSilence) {
                    logDebug(
                        "file: "__FILE__
    ...
    base64_decode_auto(&g_fdfs_base64_context,
                       filename + FDFS_LOGIC_FILE_PATH_LEN,
                       FDFS_FILENAME_BASE64_LENGTH, decode_buff, &buff_len);
...
static int storage_server_trunk_alloc_space(struct fast_task_info *pTask) {
    ...
    if (nInPackLen != FDFS_GROUP_NAME_MAX_LEN + 5) {
        logError(
            ...
            " is not correct, "
            "expect length: %d",
            __LINE__, STORAGE_PROTO_CMD_TRUNK_ALLOC_SPACE, pTask->client_ip,
            nInPackLen, FDFS_GROUP_NAME_MAX_LEN + 5);
        return EINVAL;
    }
    ...
    *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    if (trunkInfo.path.store_path_index < 0 ||
        ...
        logError(
            ...
            "is invalid",
            __LINE__, pTask->client_ip, trunkInfo.path.store_path_index);
        return EINVAL;
    }
    ...
    pClientInfo->total_length =
        sizeof(TrackerHeader) + sizeof(FDFSTrunkInfoBuff);
    return 0;
}
...
static int storage_server_trunk_get_binlog_size(struct fast_task_info *pTask) {
    ...
    if (nInPackLen != 0) {
        logError(
            ...
            " is not correct, "
            "expect length: 0",
            __LINE__, pHeader->cmd, pTask->client_ip, nInPackLen);
        return EINVAL;
    }
    ...
    if (stat(binlog_filename, &file_stat) != 0) {
        logError(
            ...
            "errno: %d, error info: %s",
            __LINE__, pHeader->cmd, pTask->client_ip, binlog_filename, errno,
            STRERROR(errno));
        return errno != 0 ? errno : ENOENT;
    }
    ...
    pClientInfo->total_length = sizeof(TrackerHeader) + FDFS_PROTO_PKG_LEN_SIZE;
    return 0;
}

static int storage_server_trunk_truncate_binlog_file(
    struct fast_task_info *pTask) {
    StorageClientInfo *pClientInfo;
    TrackerHeader *pHeader;
    int64_t nInPackLen;
...
static int storage_server_trunk_delete_binlog_marks(
    struct fast_task_info *pTask) {
    StorageClientInfo *pClientInfo;
    TrackerHeader *pHeader;
    int64_t nInPackLen;
...
#define STORAGE_LAST_AHEAD_BYTES (2 * FDFS_PROTO_PKG_LEN_SIZE)
        ...
        if (record.op_type == STORAGE_OP_TYPE_SOURCE_CREATE_FILE ||
            ...
            if ((len = readlink(full_filename, src_filename,
                ...
                logError(
                    ...
                    "%s fail, errno: %d, error info: %s",
                    __LINE__, pTask->client_ip, full_filename, result,
                    STRERROR(result));
            ...
            *(src_filename + len) = '\0';
            if (!fileExists(src_filename)) {
                logWarning(
                    ...
                    __LINE__, pTask->client_ip, full_filename, src_filename);
                continue;
            }

            // full filename format: ${base_path}/data/filename
            pOutBuff +=
                sprintf(pOutBuff, "%d %c %s %s/%s\n", (int)record.timestamp,
                        record.op_type, record.filename, diskLogicPath,
    ...
    if (bLast) {
        pkg_len =
            pClientInfo->total_offset + pTask->length - sizeof(TrackerHeader);
        long2buff(pkg_len, pOutBuff);

        pTask->length += FDFS_PROTO_PKG_LEN_SIZE;
        pClientInfo->total_length =
            pkg_len + FDFS_PROTO_PKG_LEN_SIZE + STORAGE_LAST_AHEAD_BYTES;
    }
...
static int storage_server_do_fetch_one_path_binlog(struct fast_task_info *pTask,
                                                   const int store_path_index) {
    StorageClientInfo *pClientInfo;
    StorageFileContext *pFileContext;
    StorageBinLogReader *pReader;
    ...
    if (pReader == NULL) {
        logError(
            ...
            ", line: %d, "
            "malloc %d bytes fail, errno: %d, error info: %s",
            __LINE__, (int)sizeof(StorageBinLogReader), errno, STRERROR(errno));
        return errno != 0 ? errno : ENOMEM;
    }
    ...
    pFileContext->dio_thread_index =
        storage_dio_get_thread_index(pTask, store_path_index, pFileContext->op);
    pFileContext->extra_info.upload.trunk_info.path.store_path_index =
        store_path_index;
    pClientInfo->extra_arg = pReader;

    pClientInfo->total_length = INFINITE_FILE_SIZE + sizeof(TrackerHeader);
    pClientInfo->total_offset = 0;
...
static int storage_server_fetch_one_path_binlog(struct fast_task_info *pTask) {
    ...
    if (nInPackLen != FDFS_GROUP_NAME_MAX_LEN + 1) {
        logError(
            ...
            " is not correct, "
            "expect length = %d",
            __LINE__, STORAGE_PROTO_CMD_FETCH_ONE_PATH_BINLOG, pTask->client_ip,
    ...
    *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    return storage_server_do_fetch_one_path_binlog(pTask, store_path_index);
}
...
static int storage_upload_file(struct fast_task_info *pTask,
    ...
    if (nInPackLen < 1 + FDFS_PROTO_PKG_LEN_SIZE + FDFS_FILE_EXT_NAME_MAX_LEN) {
        logError(
            ...
            " is not correct, "
            "expect length >= %d",
            __LINE__, STORAGE_PROTO_CMD_UPLOAD_FILE, pTask->client_ip,
            nInPackLen,
    ...
    if (file_bytes < 0 ||
        ...
        logError(
            ...
            "invalid file bytes: %" PRId64 ", total body length: %" PRId64,
    ...
    *(file_ext_name + FDFS_FILE_EXT_NAME_MAX_LEN) = '\0';
    ...
    if ((result = fdfs_validate_filename(file_ext_name)) != 0) {
        logError(
            ...
            "is invalid!",
            __LINE__, pTask->client_ip, file_ext_name);
        return result;
    }
    ...
    if (bAppenderFile) {
        ...
        if (g_if_use_trunk_file &&
            trunk_check_size(TRUNK_CALC_SIZE(file_bytes))) {
    ...
    if (pFileContext->extra_info.upload.file_type & _FILE_TYPE_TRUNK) {
        ...
        if (!storage_check_reserved_space_path(
            ...
            logError(
                ...
                __LINE__, g_path_space_list[store_path_index].free_mb,
                file_bytes,
                fdfs_storage_reserved_space_to_string_ex(
                    g_storage_reserved_space.flag, g_avg_storage_reserved_mb,
                    g_path_space_list[store_path_index].total_mb,
                    g_storage_reserved_space.rs.ratio, reserved_space_str));
            return ENOSPC;
        }
        ...
        *filename = '\0';
        ...
        if ((result = storage_get_filename(
                 pClientInfo, pFileContext->extra_info.upload.start_time,
                 file_bytes, crc32,
                 pFileContext->extra_info.upload.formatted_ext_name, filename,
        ...
        pFileContext->open_flags =
            O_WRONLY | O_CREAT | O_TRUNC | g_extra_open_file_flags;
    }

    return storage_write_to_file(
        pTask, file_offset, file_bytes, p - pTask->data, dio_write_file,
        storage_upload_file_done_callback, clean_func, store_path_index);
}
...
static int storage_deal_active_test(struct fast_task_info *pTask) {
    ...
    if (nInPackLen != 0) {
        logError(
            ...
            " is not correct, "
            "expect length 0",
            __LINE__, FDFS_PROTO_CMD_ACTIVE_TEST, pTask->client_ip, nInPackLen);
        return EINVAL;
    }
...
static int storage_append_file(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <= 2 * FDFS_PROTO_PKG_LEN_SIZE) {
        logError(
            ...
            " is not correct, "
            "expect length > %d",
            __LINE__, STORAGE_PROTO_CMD_APPEND_FILE, pTask->client_ip,
    ...
    if (appender_filename_len < FDFS_LOGIC_FILE_PATH_LEN +
        ...
        logError(
            ...
            "bytes: %d",
            __LINE__, pTask->client_ip, appender_filename_len);
        return EINVAL;
    }
    ...
    *(appender_filename + appender_filename_len) = '\0';
    ...
    STORAGE_ACCESS_STRCPY_FNAME2LOG(appender_filename, appender_filename_len,
                                    pClientInfo);

    if ((result = storage_split_filename_ex(appender_filename, &filename_len,
                                            true_filename,
                                            &store_path_index)) != 0) {
        return result;
    }
    ...
    if (lstat(pFileContext->filename, &stat_buf) == 0) {
        ...
        if (result == ENOENT) {
            ...
            logError(
                ...
                ", errno: %d, error info: %s.",
                __LINE__, pTask->client_ip, pFileContext->filename, result,
                STRERROR(result));
        }
    ...
    memset(decode_buff, 0, sizeof(decode_buff));
    base64_decode_auto(&g_fdfs_base64_context,
                       pFileContext->fname2log + FDFS_LOGIC_FILE_PATH_LEN,
                       FDFS_FILENAME_BASE64_LENGTH, decode_buff, &buff_len);
    ...
    if (!IS_APPENDER_FILE(appender_file_size)) {
        logError(
            ...
            __LINE__, pTask->client_ip, appender_filename, appender_file_size);
...
static int storage_modify_file(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <= 3 * FDFS_PROTO_PKG_LEN_SIZE) {
        logError(
            ...
            " is not correct, "
            "expect length > %d",
            __LINE__, STORAGE_PROTO_CMD_MODIFY_FILE, pTask->client_ip,
    ...
    if (file_offset < 0) {
        logError(
            ...
            " is invalid, "
            "which < 0",
            __LINE__, pTask->client_ip, file_offset);
        return EINVAL;
    }
    ...
    *(appender_filename + appender_filename_len) = '\0';
    ...
    if (file_offset > stat_buf.st_size) {
        logError(
            ...
            " is invalid, which > appender file size: " OFF_PRINTF_FORMAT,
            __LINE__, pTask->client_ip, file_offset, stat_buf.st_size);
...
static int storage_do_truncate_file(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <= 2 * FDFS_PROTO_PKG_LEN_SIZE) {
        logError(
            ...
            " is not correct, "
            "expect length > %d",
            __LINE__, STORAGE_PROTO_CMD_TRUNCATE_FILE, pTask->client_ip,
            nInPackLen, 2 * FDFS_PROTO_PKG_LEN_SIZE);
        return EINVAL;
    }
    ...
    *(appender_filename + appender_filename_len) = '\0';
    ...
    if (remain_bytes == stat_buf.st_size) {
        logWarning(
            ...
            "%" PRId64 " == appender file size: " OFF_PRINTF_FORMAT
            ", skip truncate file",
            __LINE__, pTask->client_ip, remain_bytes, stat_buf.st_size);
        return 0;
    }
    if (remain_bytes > stat_buf.st_size) {
        logError(
            ...
            "%" PRId64
            " is invalid, "
            "which > appender file size: " OFF_PRINTF_FORMAT,
            __LINE__, pTask->client_ip, remain_bytes, stat_buf.st_size);
    ...
    return storage_write_to_file(
        pTask, remain_bytes, stat_buf.st_size, 0, dio_truncate_file,
        storage_do_truncate_file_done_callback, dio_truncate_finish_clean_up,
        store_path_index);
}
...
static int storage_upload_slave_file(struct fast_task_info *pTask) {
    ...
    nInPackLen = pClientInfo->total_length - sizeof(TrackerHeader);
    if (nInPackLen <= 2 * FDFS_PROTO_PKG_LEN_SIZE + FDFS_FILE_PREFIX_MAX_LEN +
        ...
        logError(
            ...
            " is not correct, "
            "expect length > %d",
            __LINE__, STORAGE_PROTO_CMD_UPLOAD_SLAVE_FILE, pTask->client_ip,
            nInPackLen, 2 * FDFS_PROTO_PKG_LEN_SIZE + FDFS_FILE_PREFIX_MAX_LEN +
                            FDFS_FILE_EXT_NAME_MAX_LEN);
        return EINVAL;
    }
    ...
    *(prefix_name + FDFS_FILE_PREFIX_MAX_LEN) = '\0';
    ...
    if ((result = fdfs_validate_filename(prefix_name)) != 0) {
        logError(
            ...
            "is invalid!",
            __LINE__, pTask->client_ip, prefix_name);
        return result;
    }
    ...
    *(master_filename + master_filename_len) = '\0';
    ...
    if ((result = storage_split_filename_ex(master_filename, &filename_len,
                                            true_filename,
                                            &store_path_index)) != 0) {
        return result;
    }
    ...
    if (!storage_check_reserved_space_path(
        ...
        logError(
            ...
            "free space: %d MB is too small, file bytes: "
            "%" PRId64 ", reserved space: %s",
            __LINE__, g_path_space_list[store_path_index].free_mb, file_bytes,
            fdfs_storage_reserved_space_to_string_ex(
                g_storage_reserved_space.flag, g_avg_storage_reserved_mb,
                g_path_space_list[store_path_index].total_mb,
                g_storage_reserved_space.rs.ratio, reserved_space_str));
    ...
    if ((result = trunk_file_lstat(
             store_path_index, true_filename, filename_len, &stat_buf,
             &(pFileContext->extra_info.upload.trunk_info), &trunkHeader)) !=
        0) {
        logError(
            "file: "__FILE__
            ...
            "errno: %d, error info: %s.",
            __LINE__, pTask->client_ip, master_filename, result,
            STRERROR(result));
        return result;
    }
    ...
    pFileContext->extra_info.upload.trunk_info.path.store_path_index =
        store_path_index;
    if ((result =
             fdfs_gen_slave_filename(true_filename, prefix_name, file_ext_name,
                                     filename, &filename_len)) != 0) {
        return result;
    }
    ...
    *filename = '\0';
    ...
    pFileContext->calc_crc32 =
        g_check_file_duplicate || g_store_slave_file_use_link;
    ...
    pFileContext->extra_info.upload.file_type =
        _FILE_TYPE_SLAVE | _FILE_TYPE_REGULAR;
    pFileContext->sync_flag = STORAGE_OP_TYPE_SOURCE_CREATE_FILE;
    pFileContext->timestamp2log = pFileContext->extra_info.upload.start_time;
    pFileContext->extra_info.upload.before_open_callback = NULL;
    pFileContext->extra_info.upload.before_close_callback = NULL;
...
static int storage_sync_copy_file(struct fast_task_info *pTask,
    ...
    if (filename_len < 0 || filename_len >= sizeof(filename)) {
        logError(
            ...
            "which < 0 or >= %d",
            __LINE__, pTask->client_ip, filename_len, (int)sizeof(filename));
        return EINVAL;
    }
    ...
    *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    if (have_file_content) {
        if (file_bytes !=
            nInPackLen - (2 * FDFS_PROTO_PKG_LEN_SIZE + 4 +
                          FDFS_GROUP_NAME_MAX_LEN + filename_len)) {
            logError(
                "file: "__FILE__
                ", line: %d, "
    ...
    *(filename + filename_len) = '\0';
    ...
    if ((result = storage_split_filename_ex(
             filename, &filename_len, true_filename, &store_path_index)) != 0) {
        return result;
    }
    if ((result = fdfs_check_data_filename(true_filename, filename_len)) != 0) {
        return result;
    ...
    pFileContext->extra_info.upload.file_type = _FILE_TYPE_REGULAR;
    if (proto_cmd == STORAGE_PROTO_CMD_SYNC_CREATE_FILE && have_file_content) {
        struct stat stat_buf;
        ...
        if ((result = trunk_file_lstat(
            ...
            if (result != ENOENT)  // accept no exist
            {
                logError(
                    ...
                    "client ip: %s, stat logic file %s fail, "
                    "errno: %d, error info: %s.",
                    __LINE__, pTask->client_ip, filename, result,
                    STRERROR(result));
                return result;
            }
            ...
            logWarning(
                ...
                "my file size: " OFF_PRINTF_FORMAT " != src file size: %" PRId64
                ", will be overwrited",
                __LINE__, pTask->client_ip, filename, stat_buf.st_size,
                file_bytes);
        } else {
            logWarning(
                "file: "__FILE__
                ...
                __LINE__, proto_cmd, pTask->client_ip, filename);
...
#define MKTEMP_MAX_COUNT 10
                    ...
                    if (g_current_time - stat_buf.st_mtime > 600) {
                        if (unlink(pFileContext->filename) != 0 &&
                            ...
                            logWarning(
                                ...
                                " fail, errno: %d, error info: %s",
                                __LINE__, pTask->client_ip,
                                pFileContext->filename, errno, STRERROR(errno));
                            continue;
                        }
                    } else {
                        logWarning(
    ...
    if (have_file_content) {
        return storage_write_to_file(
            pTask, file_offset, file_bytes, p - pTask->data, deal_func,
            storage_sync_copy_file_done_callback, clean_func, store_path_index);
    } else {
...
static int storage_sync_append_file(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <=
        ...
        logError(
            ...
            "is not correct, "
            "expect length > %d",
            __LINE__, STORAGE_PROTO_CMD_SYNC_APPEND_FILE, pTask->client_ip,
            nInPackLen,
            3 * FDFS_PROTO_PKG_LEN_SIZE + 4 + FDFS_GROUP_NAME_MAX_LEN);
        return EINVAL;
    }
    ...
    if (start_offset < 0) {
        logError(
            ...
            " is invalid, "
            "which < 0",
            __LINE__, pTask->client_ip, start_offset);
        return EINVAL;
    }
    ...
    *(group_name + FDFS_GROUP_NAME_MAX_LEN) = '\0';
    ...
    if (append_bytes !=
        nInPackLen - (3 * FDFS_PROTO_PKG_LEN_SIZE + 4 +
                      FDFS_GROUP_NAME_MAX_LEN + filename_len)) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    *(filename + filename_len) = '\0';
    ...
    if (lstat(pFileContext->filename, &stat_buf) != 0) {
        ...
        if (stat_buf.st_size >= start_offset + append_bytes) {
            logDebug(
                "file: "__FILE__
                ", line: %d, "
                "client ip: %s, file %s,  my file size: " OFF_PRINTF_FORMAT
                " >= src file size: "
            ...
            logWarning(
                ...
                ", line: %d, "
                "client ip: %s, file %s,  my file size: " OFF_PRINTF_FORMAT
                " > %" PRId64 ", but < %" PRId64 ", need be resynced",
                __LINE__, pTask->client_ip, pFileContext->filename,
                stat_buf.st_size, start_offset, start_offset + append_bytes);
        }
    ...
    if (need_write_file) {
        ...
        pFileContext->op = FDFS_STORAGE_FILE_OP_APPEND;
        pFileContext->open_flags =
            O_WRONLY | O_APPEND | g_extra_open_file_flags;

        snprintf(pFileContext->fname2log, sizeof(pFileContext->fname2log),
                 "%s %" PRId64 " %" PRId64, filename, start_offset,
                 append_bytes);
    } else {
        deal_func = dio_discard_file;
        pFileContext->op = FDFS_STORAGE_FILE_OP_DISCARD;
...
static int storage_sync_modify_file(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <=
        ...
        logError(
            ...
            "is not correct, "
            "expect length > %d",
            __LINE__, STORAGE_PROTO_CMD_SYNC_MODIFY_FILE, pTask->client_ip,
            nInPackLen,
            3 * FDFS_PROTO_PKG_LEN_SIZE + 4 + FDFS_GROUP_NAME_MAX_LEN);
        return EINVAL;
    }
...
static int storage_sync_truncate_file(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <=
        ...
        logError(
            ...
            "is not correct, "
            "expect length > %d",
            __LINE__, STORAGE_PROTO_CMD_SYNC_TRUNCATE_FILE, pTask->client_ip,
            nInPackLen,
            3 * FDFS_PROTO_PKG_LEN_SIZE + 4 + FDFS_GROUP_NAME_MAX_LEN);
        return EINVAL;
    }
    ...
    if (old_file_size < 0) {
        logError(
            ...
            ", line: %d, "
            "client ip: %s, in request pkg, "
            "start offset: %" PRId64 "is invalid, which < 0",
            __LINE__, pTask->client_ip, old_file_size);
        return EINVAL;
    }
    ...
    if (new_file_size < 0) {
        logError(
            ...
            ", line: %d, "
            "client ip: %s, in request pkg, "
            "modify file bytes: %" PRId64 " is invalid, which < 0",
            __LINE__, pTask->client_ip, new_file_size);
        return EINVAL;
    }
    ...
    *(filename + filename_len) = '\0';
    ...
    snprintf(pFileContext->fname2log, sizeof(pFileContext->fname2log),
             "%s %" PRId64 " %" PRId64, filename, old_file_size, new_file_size);
    ...
    return storage_write_to_file(
        pTask, new_file_size, old_file_size, 0, dio_truncate_file,
        storage_sync_truncate_file_done_callback, dio_truncate_finish_clean_up,
        store_path_index);
}
...
static int storage_do_sync_link_file(struct fast_task_info *pTask) {
    ...
    do {
        ...
        *(src_filename + src_filename_len) = '\0';
        ...
        if ((result = storage_split_filename_ex(
                 dest_filename, &dest_true_filename_len, dest_true_filename,
        ...
        if ((result = storage_split_filename_ex(
                 src_filename, &src_true_filename_len, src_true_filename,
                 &src_store_path_index)) != 0) {
            break;
        }
        ...
        if (trunk_file_lstat(dest_store_path_index, dest_true_filename,
            ...
            logWarning(
                ...
                "already exists, ignore it",
                __LINE__, pTask->client_ip, dest_filename);
        } else {
            FDFSTrunkFullInfo trunkInfo;
            if (trunk_file_lstat(src_store_path_index, src_true_filename,
                                 src_true_filename_len, &stat_buf, &trunkInfo,
                                 &srcTrunkHeader) != 0) {
                need_create_link = false;
                logWarning(
                    "file: "__FILE__
        ...
        if (need_create_link) {
            if (IS_TRUNK_FILE_BY_ID(
                ...
                result = storage_trunk_do_create_link(
                    pTask, src_filename_len, p - pTask->data,
                ...
                snprintf(pFileContext->filename, sizeof(pFileContext->filename),
                         "%s/data/%s",
                         g_fdfs_store_paths.paths[dest_store_path_index],
                         dest_true_filename);

                snprintf(src_full_filename, sizeof(src_full_filename),
                         "%s/data/%s",
                         g_fdfs_store_paths.paths[src_store_path_index],
                         src_true_filename);
                if (symlink(src_full_filename, pFileContext->filename) != 0) {
                    ...
                    if (result == EEXIST) {
                        ...
                        logError(
                            ...
                            "errno: %d, error info: %s",
                            __LINE__, pTask->client_ip, src_full_filename,
        ...
        snprintf(binlog_buff, sizeof(binlog_buff), "%s %s", dest_filename,
                 src_filename);
        result = storage_binlog_write(pFileContext->timestamp2log,
...
static int storage_sync_link_file(struct fast_task_info *pTask) {
    ...
    if (dest_filename_len < 0 || dest_filename_len >= sizeof(dest_filename)) {
        logError(
            ...
            "filename length: %d is invalid, "
            "which < 0 or >= %d",
            __LINE__, pTask->client_ip, dest_filename_len,
            (int)sizeof(dest_filename));
        return EINVAL;
    }
    ...
    *(dest_filename + dest_filename_len) = '\0';
    ...
    if ((result = storage_split_filename_ex(dest_filename, &dest_filename_len,
                                            dest_true_filename,
                                            &dest_store_path_index)) != 0) {
        return result;
    }
...
static int storage_server_get_metadata(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <= FDFS_GROUP_NAME_MAX_LEN) {
        logError(
            ...
            " is not correct, "
            "expect length > %d",
            __LINE__, STORAGE_PROTO_CMD_GET_METADATA, pTask->client_ip,
            nInPackLen, FDFS_GROUP_NAME_MAX_LEN);
        return EINVAL;
    }
    ...
    *(filename + filename_len) = '\0';
    ...
    if ((result = trunk_file_stat(store_path_index, true_filename, filename_len,
...
static int storage_server_download_file(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <= 16 + FDFS_GROUP_NAME_MAX_LEN) {
        logError(
            ...
            " is not correct, "
            "expect length > %d",
            __LINE__, STORAGE_PROTO_CMD_DOWNLOAD_FILE, pTask->client_ip,
    ...
    if (download_bytes < 0) {
        logError(
            ...
            "%" PRId64,
            __LINE__, pTask->client_ip, download_bytes);
        return EINVAL;
    }
    ...
    *(filename + filename_len) = '\0';
    ...
    pFileContext->fd = -1;
    result = trunk_file_stat_ex(store_path_index, true_filename, filename_len,
                                &stat_buf, &trunkInfo, &trunkHeader,
                                &pFileContext->fd);
    if (IS_TRUNK_FILE_BY_ID(trunkInfo)) {
        pthread_mutex_lock(&stat_count_thread_lock);
...
static int storage_server_delete_file(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <= FDFS_GROUP_NAME_MAX_LEN) {
        logError(
            ...
            " is not correct, "
            "expect length <= %d",
            __LINE__, STORAGE_PROTO_CMD_DELETE_FILE, pTask->client_ip,
            nInPackLen, FDFS_GROUP_NAME_MAX_LEN);
        return EINVAL;
    }
    ...
    *(filename + filename_len) = '\0';
    ...
    if ((pFileContext->extra_info.upload.file_type & _FILE_TYPE_LINK) &&
        ...
        sprintf(full_filename, "%s/data/%s",
                g_fdfs_store_paths.paths[store_path_index], true_filename);
        do {
            if ((src_filename_len = readlink(full_filename, src_filename,
                                             sizeof(src_filename))) < 0) {
                result = errno != 0 ? errno : EPERM;
                logError(
                    "file: "__FILE__
            ...
            *(src_filename + src_filename_len) = '\0';
            ...
            base_path_len = strlen(g_fdfs_store_paths.paths[store_path_index]);
            if (src_filename_len > base_path_len &&
                memcmp(src_filename, g_fdfs_store_paths.paths[store_path_index],
                       base_path_len) == 0) {
                ...
                for (i = 0; i < g_fdfs_store_paths.count; i++) {
                    ...
                    if (src_filename_len > base_path_len &&
                        memcmp(src_filename, g_fdfs_store_paths.paths[i],
                               base_path_len) == 0) {
            ...
            src_true_filename =
                src_filename + (base_path_len + (sizeof("/data/") - 1));
            snprintf(src_fname2log, sizeof(src_fname2log),
                     "%c" FDFS_STORAGE_DATA_DIR_FORMAT "/%s",
                     FDFS_STORAGE_STORE_PATH_PREFIX_CHAR, src_store_path_index,
                     src_true_filename);
...
static int storage_create_link_core(
    struct fast_task_info *pTask, SourceFileInfo *pSourceFileInfo,
    const char *src_filename, const char *master_filename,
    const int master_filename_len, const char *prefix_name,
    ...
    do {
        ...
        if (result != 0 || !S_ISREG(stat_buf.st_mode)) {
            ...
            logError(
                ...
                STRERROR(result));
            if (g_check_file_duplicate) {
                pGroupArray = &((g_nio_thread_data +
                                 pClientInfo->nio_thread_index)->group_array);
                // clean invalid entry
                memset(&key_info, 0, sizeof(key_info));
                key_info.namespace_len = g_namespace_len;
                memcpy(key_info.szNameSpace, g_key_namespace, g_namespace_len);
                ...
                key_info.obj_id_len =
                    snprintf(key_info.szObjectId, sizeof(key_info.szObjectId),
                             "%s/%s", g_group_name, src_filename);
                key_info.key_len = sizeof(FDHT_KEY_NAME_REF_COUNT) - 1;
            ...
            *filename_len = master_filename_len;
            ...
            if (master_store_path_index != store_path_index) {
                logError(
                    ...
                    "index: %d",
                    __LINE__, pTask->client_ip, master_store_path_index,
                    store_path_index);
                result = EINVAL;
                break;
            }
            ...
            if ((result = fdfs_gen_slave_filename(true_filename, prefix_name,
        ...
        if (pFileContext->extra_info.upload.file_type & _FILE_TYPE_TRUNK) {
            ...
            strcpy(pFileContext->extra_info.upload.file_ext_name,
                   file_ext_name);
            storage_format_ext_name(
                file_ext_name,
                pFileContext->extra_info.upload.formatted_ext_name);
        ...
        result = storage_service_do_create_link(
            pTask, pSourceFileInfo, stat_buf.st_size, master_filename,
            prefix_name, file_ext_name, filename, filename_len);
        if (result != 0) {
            break;
        }

        snprintf(binlog_buff, sizeof(binlog_buff), "%s %s", filename,
                 src_filename);
        result = storage_binlog_write(
            g_current_time, STORAGE_OP_TYPE_SOURCE_CREATE_LINK, binlog_buff);
    } while (0);
    ...
    do {
        if (nInPackLen <=
            ...
            logError(
                ...
                " is not correct, "
                "expect length > %d",
                __LINE__, STORAGE_PROTO_CMD_CREATE_LINK, pTask->client_ip,
                nInPackLen,
        ...
        if (sourceFileInfo.src_file_sig_len <= 0 ||
            ...
            logError(
                ...
                __LINE__, pTask->client_ip, sourceFileInfo.src_file_sig_len);
            result = EINVAL;
            break;
        }

        if (sourceFileInfo.src_file_sig_len !=
            nInPackLen -
                (3 * FDFS_PROTO_PKG_LEN_SIZE + FDFS_GROUP_NAME_MAX_LEN +
                 FDFS_FILE_PREFIX_MAX_LEN + FDFS_FILE_EXT_NAME_MAX_LEN +
        ...
        *(sourceFileInfo.src_file_sig + sourceFileInfo.src_file_sig_len) = '\0';
        ...
        if ((result = storage_split_filename_ex(
                 ...
                 sourceFileInfo.src_true_filename, &store_path_index)) != 0) {
            break;
        }
        if ((result = fdfs_check_data_filename(sourceFileInfo.src_true_filename,
                                               src_filename_len)) != 0) {
            break;
        }
        ...
        pClientInfo->file_context.extra_info.upload.trunk_info.path
            .store_path_index = store_path_index;
        result = storage_create_link_core(pTask, &sourceFileInfo, src_filename,
                                          master_filename, master_filename_len,
                                          prefix_name, file_ext_name, filename,
                                          &filename_len, true);
        if (result == STORAGE_STATUE_DEAL_FILE) {
            return 0;
        }
...
static int storage_create_link(struct fast_task_info *pTask) {
    ...
    if (nInPackLen <= 3 * FDFS_PROTO_PKG_LEN_SIZE + FDFS_GROUP_NAME_MAX_LEN +
    ...
    do {                                                                    \
        if (g_use_access_log) {                                             \
            ...
            *(pClientInfo->file_context.fname2log + 1) = '\0';              \
            ...
            gettimeofday(&(pClientInfo->file_context.tv_deal_start), NULL); \
        }                                                                   \
    } while (0)
...
int storage_deal_task(struct fast_task_info *pTask) {
    ...
    switch (pHeader->cmd) {
        case STORAGE_PROTO_CMD_DOWNLOAD_FILE:
            ...
            STORAGE_ACCESS_LOG(pTask, ACCESS_LOG_ACTION_DOWNLOAD_FILE, result);
            break;
        case STORAGE_PROTO_CMD_GET_METADATA:
            ACCESS_LOG_INIT_FIELDS();
            result = storage_server_get_metadata(pTask);
            STORAGE_ACCESS_LOG(pTask, ACCESS_LOG_ACTION_GET_METADATA, result);
            break;
