...
int fdht_recv_header(FDHTServerInfo *pServer, fdht_pkg_size_t *in_bytes) {
    ...
    if ((result = tcprecvdata_nb(pServer->sock, &resp, sizeof(resp),
    ...
    *in_bytes = buff2int(resp.pkg_len);
    if (*in_bytes < 0) {
        logError(
            ...
            "is not correct",
            __LINE__, pServer->ip_addr, pServer->port, *in_bytes);
        *in_bytes = 0;
        return EINVAL;
    }
...
int fdht_recv_response(FDHTServerInfo *pServer, char **buff,
                       const int buff_size, fdht_pkg_size_t *in_bytes) {
    int result;
    bool bMalloced;
...
int fdht_quit(FDHTServerInfo *pServer) {
    ...
    if (result != 0) {
        logError(
            ...
            "errno: %d, error info: %s",
            __LINE__, pServer->ip_addr, result, STRERROR(result));
        return result;
    }
...
int fdht_connect_server_nb(FDHTServerInfo *pServer, const int connect_timeout) {
    ...
    if ((result = connectserverbyip_nb(pServer->sock, pServer->ip_addr,
                                       pServer->port, connect_timeout)) != 0) {
        logError(
            "file: "__FILE__
            ", line: %d, "
...
int fdht_client_set(FDHTServerInfo *pServer, const char keep_alive,
                    const time_t timestamp, const time_t expires,
                    const int prot_cmd, const int key_hash_code,
                    FDHTKeyInfo *pKeyInfo, const char *pValue,
                    const int value_len) {
    int result;
...
int fdht_client_delete(FDHTServerInfo *pServer, const char keep_alive,
    ...
    int2buff(key_hash_code, pHeader->key_hash_code);
    int2buff(
        12 + pKeyInfo->namespace_len + pKeyInfo->obj_id_len + pKeyInfo->key_len,
        pHeader->pkg_len);
