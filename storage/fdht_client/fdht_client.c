...
static void fdht_proxy_extra_deal(GroupArray *pGroupArray, bool *bKeepAlive) {
    ...
    *bKeepAlive = true;
    ...
    for (group_id = 0; group_id < pGroupArray->group_count; group_id++) {
        ...
        for (ppServer = pServerArray->servers; ppServer < ppServerEnd;
             ppServer++) {
...
int fdht_client_init(const char *filename) {
    ...
    if ((result = iniLoadFromFile(filename, &iniContext)) != 0) {
        logError("load conf file \"%s\" fail, ret code: %d", filename, result);
        return result;
    }
    ...
    while (1) {
        ...
        g_fdht_connect_timeout = iniGetIntValue(
        ...
        g_fdht_network_timeout = iniGetIntValue(
        ...
        g_keep_alive = iniGetBoolValue(NULL, "keep_alive", &iniContext, false);
        ...
        logInfo(
            ...
            __LINE__, g_fdht_base_path, g_fdht_connect_timeout,
            g_fdht_network_timeout, g_keep_alive, g_group_array.use_proxy,
            szProxyPrompt, g_group_array.group_count,
...
static FDHTServerInfo *get_connection(ServerArray *pServerArray,
    ...
    for (ppServer = pServerArray->servers + server_index; ppServer < ppEnd;
         ppServer++) {
...
#define CALC_KEY_HASH_CODE(pKeyInfo, hash_key, hash_key_len, key_hash_code)  \
    ...
    if (pKeyInfo->namespace_len == 0 && pKeyInfo->obj_id_len == 0) {         \
        ...
        *(hash_key + pKeyInfo->namespace_len) = FDHT_FULL_KEY_SEPERATOR;     \
        memcpy(hash_key + pKeyInfo->namespace_len + 1, pKeyInfo->szObjectId, \
               pKeyInfo->obj_id_len);                                        \
    } else {                                                                 \
        fprintf(stderr,                                                      \
                "invalid namespace length: %d and "                          \
                "object ID length: %d\n",                                    \
...
#define CALC_OBJECT_HASH_CODE(pObjectInfo, hash_key, hash_key_len,             \
                              key_hash_code)                                   \
    ...
    *(hash_key + pObjectInfo->namespace_len) = FDHT_FULL_KEY_SEPERATOR;        \
    memcpy(hash_key + pObjectInfo->namespace_len + 1, pObjectInfo->szObjectId, \
           pObjectInfo->obj_id_len);                                           \
                                                                               \
    key_hash_code = PJWHash(hash_key, hash_key_len);                           \
    if (key_hash_code < 0) {                                                   \
        key_hash_code &= 0x7FFFFFFF;                                           \
...
int fdht_get_ex1(GroupArray *pGroupArray, const bool bKeepAlive,
    ...
    for (i = 0; i <= pGroup->count; i++) {
        pServer =
            get_readable_connection(pGroup, bKeepAlive, key_hash_code, &result);
        if (pServer == NULL) {
            return result;
        }
        ...
        do {
            ...
            if ((result = tcpsenddata_nb(pServer->sock, buff, p - buff,
                ...
                logError(
                    ...
                    pServer->ip_addr, pServer->port, result, STRERROR(result));
                break;
            }
            ...
            if (in_bytes < 4) {
                logError("server %s:%d reponse bytes: %d < 4", pServer->ip_addr,
                         pServer->port, in_bytes);
                result = EINVAL;
                break;
            }

            if ((result = tcprecvdata_nb(pServer->sock, buff, 4,
                                         g_fdht_network_timeout)) != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
                    ...
                    "errno: %d, error info: %s",
                    __LINE__, pServer->ip_addr, pServer->port, result,
                    STRERROR(result));
                break;
            }
            ...
            if (vlen != in_bytes - 4) {
                logError(
                    "server %s:%d reponse bytes: %d "
                    "is not correct, %d != %d",
                    pServer->ip_addr, pServer->port, in_bytes, vlen,
                    in_bytes - 4);
                result = EINVAL;
                break;
            }
            ...
            if ((result = tcprecvdata_nb(pServer->sock, *ppValue, *value_len,
                                         g_fdht_network_timeout)) != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
...
int fdht_batch_set_ex(GroupArray *pGroupArray, const bool bKeepAlive,
    ...
    *success_count = 0;
    ...
    for (i = 0; i <= pGroup->count; i++) {
        pServer =
            get_writable_connection(pGroup, bKeepAlive, key_hash_code, &result);
        if (pServer == NULL) {
            return result;
        }
        ...
        pkg_total_len = sizeof(FDHTProtoHeader) + 12 +
                        pObjectInfo->namespace_len + pObjectInfo->obj_id_len +
                        8 * key_count + total_key_len + total_value_len;
        ...
        do {
            ...
            if ((result = tcprecvdata_nb(pServer->sock, pBuff, in_bytes,
                                         g_fdht_network_timeout)) != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
            ...
            if (buff2int(pBuff) != key_count) {
                ...
                logError(
                    ...
                    "expect key count: %d",
                    __LINE__, pServer->ip_addr, pServer->port, buff2int(pBuff),
                    key_count);
                break;
            }
            ...
            *success_count = buff2int(pBuff + 4);
            ...
            for (pKeyValuePair = key_list; pKeyValuePair < pKeyValueEnd;
                ...
                memcpy(pKeyValuePair->szKey, p + 4, pKeyValuePair->key_len);
                p += 4 + pKeyValuePair->key_len;
    ...
    *success_count = 0;
    ...
    for (i = 0; i <= pGroup->count; i++) {
        ...
        do {
            ...
            if ((result = tcprecvdata_nb(pServer->sock, buff, in_bytes,
                                         g_fdht_network_timeout)) != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
            ...
            if (buff2int(buff) != key_count) {
                ...
                logError(
                    ...
                    "server: %s:%d, invalid key_count: %d, "
                    "expect key count: %d",
                    __LINE__, pServer->ip_addr, pServer->port, buff2int(buff),
                    key_count);
                break;
            }
            ...
            if (in_bytes > sizeof(buff)) {
                ...
                if (pInBuff == NULL) {
                    ...
                    logError(
                        ...
                        "malloc %d bytes fail, "
                        "errno: %d, error info: %s",
                        __LINE__, in_bytes, result, STRERROR(result));
                    break;
                }
            }

            if ((result = tcprecvdata_nb(pServer->sock, pInBuff, in_bytes,
                                         g_fdht_network_timeout)) != 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
                ...
                if (pKeyValuePair->pValue != NULL) {
                    if (value_len >= pKeyValuePair->value_len) {
                        *(pKeyValuePair->pValue) = '\0';
                        ...
                        memcpy(pKeyValuePair->pValue, p, value_len);
                        *(pKeyValuePair->pValue + value_len) = '\0';
                    }
                } else {
                    pKeyValuePair->pValue = (char *)malloc_func(value_len + 1);
                    if (pKeyValuePair->pValue == NULL) {
                        pKeyValuePair->value_len = 0;
                        pKeyValuePair->status = errno != 0 ? errno : ENOMEM;
                        logError(
                            "malloc %d bytes fail, "
                            "errno: %d, error info: %s",
                            value_len + 1, errno, STRERROR(errno));
...
int fdht_set_ex(GroupArray *pGroupArray, const bool bKeepAlive,
    ...
    for (i = 0; i <= pGroup->count; i++) {
        ...
        result = fdht_client_set(pServer, bKeepAlive, time(NULL), expires,
                                 FDHT_PROTO_CMD_SET, key_hash_code, pKeyInfo,
                                 pValue, value_len);
...
int fdht_inc_ex(GroupArray *pGroupArray, const bool bKeepAlive,
    ...
    for (i = 0; i <= pGroup->count; i++) {
        ...
        while (1) {
            ...
            in_buff = buff;
            if ((result = fdht_recv_response(pServer, &in_buff, sizeof(buff),
                                             &in_bytes)) != 0) {
                logError(
...
int fdht_delete_ex(GroupArray *pGroupArray, const bool bKeepAlive,
    ...
    for (i = 0; i <= pGroup->count; i++) {
        ...
        result =
            fdht_client_delete(pServer, bKeepAlive, time(NULL),
...
int fdht_connect_all_servers(GroupArray *pGroupArray, const bool bKeepAlive,
    ...
    *fail_count = 0;
    ...
    for (pServerInfo = pGroupArray->servers; pServerInfo < pServerEnd;
         pServerInfo++) {
        if ((conn_result = fdht_connect_server_nb(
                 pServerInfo, g_fdht_connect_timeout)) != 0) {
            result = conn_result;
            (*fail_count)++;
...
int fdht_stat_ex(GroupArray *pGroupArray, const bool bKeepAlive,
    ...
    for (i = 0; i < 2; i++) {
        ...
        do {
            if ((result = tcpsenddata_nb(pServer->sock, &header, sizeof(header),
                                         g_fdht_network_timeout)) != 0) {
                logError(
                    "send data to server %s:%d fail, "
                    "errno: %d, error info: %s",
            ...
            if (in_bytes >= size) {
                logError(
                    "server %s:%d reponse bytes: %d >= "
                    "buff size: %d",
                    pServer->ip_addr, pServer->port, in_bytes, size);
                result = ENOSPC;
                break;
            }
