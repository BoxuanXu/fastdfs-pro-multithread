    ...
    *ppIds = (int *)malloc(sizeof(int) * alloc_count);
    if (*ppIds == NULL) {
        logError(
            ...
            __LINE__, (int)sizeof(int) * alloc_count, errno, STRERROR(errno));
    ...
    if (pBuff == NULL) {
        logError(
            ...
            __LINE__, szIds, errno, STRERROR(errno));
        free(*ppIds);
        *ppIds = NULL;
        return errno != 0 ? errno : ENOMEM;
    }
    ...
    while (*p != '\0') {
        ...
        *(pNumStart2 + nNumLen2) = '\0';
        ...
        if (alloc_count < *id_count + (count + 1)) {
            alloc_count += count + 1;
            *ppIds = (int *)realloc(*ppIds, sizeof(int) * alloc_count);
            if (*ppIds == NULL) {
                ...
                logError(
                    ...
                    __LINE__, (int)sizeof(int) * alloc_count, result,
                    STRERROR(result));
...
int fdht_load_groups_ex(IniContext *pIniContext, GroupArray *pGroupArray,
                        const bool bLoadProxyParams) {
    IniItem *pItemInfo;
    IniItem *pItemEnd;
    int group_id;
    char item_name[32];
    ...
    pGroupArray->group_count =
        iniGetIntValue(NULL, "group_count", pIniContext, 0);
    ...
    pGroupArray->groups =
        (ServerArray *)malloc(sizeof(ServerArray) * pGroupArray->group_count);
    if (pGroupArray->groups == NULL) {
        logError(
            ...
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, (int)sizeof(ServerArray) * pGroupArray->group_count,
    ...
    if (ppServers == NULL) {
        logError(
            ...
            ", line: %d, "
            "malloc %d bytes fail, errno: %d, error info: %s",
            __LINE__, (int)sizeof(FDHTServerInfo *) * pGroupArray->group_count,
            errno, STRERROR(errno));
        return errno != 0 ? errno : ENOMEM;
    }
    ...
    pGroupArray->servers =
        (FDHTServerInfo *)malloc(sizeof(FDHTServerInfo) * alloc_server_count);
    if (pGroupArray->servers == NULL) {
        logError(
            ...
            "malloc %d bytes fail, "
            "errno: %d, error info: %s",
            __LINE__, (int)sizeof(FDHTServerInfo) * alloc_server_count, errno,
            STRERROR(errno));
        return errno != 0 ? errno : ENOMEM;
    }
    ...
    for (group_id = 0; group_id < pGroupArray->group_count; group_id++) {
        ...
        if (pServerArray->servers == NULL) {
            logError(
                ...
                "malloc %d bytes fail, "
                "errno: %d, error info: %s",
                __LINE__, (int)sizeof(FDHTServerInfo) * pServerArray->count,
                errno, STRERROR(errno));
            return errno != 0 ? errno : ENOMEM;
        }

        ppServers[group_id] = (FDHTServerInfo *)malloc(sizeof(FDHTServerInfo) *
                                                       pServerArray->count);
        ...
        for (; pItemInfo < pItemEnd; pItemInfo++) {
            ...
            if (getIpaddrByName(ip_port[0], pServerInfo->ip_addr,
                ...
                logError(
                    ...
                    "\"%s\" 's value \"%s\" is invalid, "
                    "invalid hostname: %s",
                    __LINE__, item_name, pItemInfo->value, ip_port[0]);
                return EINVAL;
            }
            ...
            if (pServerInfo->port <= 0 || pServerInfo->port > 65535) {
                logError(
                    ...
                    "\"%s\" 's value \"%s\" is invalid, "
                    "invalid port: %d",
                    __LINE__, item_name, pItemInfo->value, pServerInfo->port);
                return EINVAL;
            }

            pServerInfo->sock = -1;
            pFound = (FDHTServerInfo *)bsearch(
                pServerInfo, pGroupArray->servers, pGroupArray->server_count,
                sizeof(FDHTServerInfo), fdht_cmp_by_ip_and_port_p);
            if (pFound == NULL)  // not found
            {
                if (pGroupArray->server_count >= alloc_server_count) {
                    alloc_server_count = pGroupArray->server_count +
                                         pGroupArray->group_count + 8;
                    pGroupArray->servers = (FDHTServerInfo *)realloc(
                        pGroupArray->servers,
                        sizeof(FDHTServerInfo) * alloc_server_count);
                    if (pGroupArray->servers == NULL) {
                        logError(
                            "file: "__FILE__
                            ", "
                ...
                fdht_insert_sorted_servers(pGroupArray, pServerInfo);
    ...
    for (group_id = 0; group_id < pGroupArray->group_count; group_id++) {
        ...
        for (pServerInfo = ppServers[group_id]; pServerInfo < pServerEnd;
             pServerInfo++) {
            *ppServer = (FDHTServerInfo *)bsearch(
                pServerInfo, pGroupArray->servers, pGroupArray->server_count,
                sizeof(FDHTServerInfo), fdht_cmp_by_ip_and_port_p);
            ppServer++;
        }
        ...
        for (ppServer = pServerArray->servers + 1; ppServer < ppServerEnd;
             ppServer++) {
            if (fdht_cmp_by_ip_and_port_pp(ppServer - 1, ppServer) == 0) {
                logError(
                    "file: "__FILE__
                    ", line: %d, "
                    "group: \"%s\",  duplicate server: "
                    "%s:%d",
                    __LINE__, item_name, (*ppServer)->ip_addr,
                    (*ppServer)->port);
                return EINVAL;
            }
        }
    ...
    if (alloc_server_count > pGroupArray->server_count) {
        pGroupArray->servers = (FDHTServerInfo *)realloc(
            pGroupArray->servers,
            sizeof(FDHTServerInfo) * pGroupArray->server_count);
        if (pGroupArray->servers == NULL) {
            logError(
                "file: "__FILE__
                ", line: %d, "
                ...
                "errno: %d, error info: %s",
                __LINE__,
                (int)sizeof(FDHTServerInfo) * pGroupArray->server_count, errno,
                STRERROR(errno));
            return errno != 0 ? errno : ENOMEM;
        }
    }
    ...
    pGroupArray->use_proxy =
        iniGetBoolValue(NULL, "use_proxy", pIniContext, false);
    ...
    snprintf(pGroupArray->proxy_server.ip_addr,
             sizeof(pGroupArray->proxy_server.ip_addr), "%s", pProxyIpAddr);

    pGroupArray->proxy_server.port = iniGetIntValue(
        NULL, "proxy_port", pIniContext, FDHT_DEFAULT_PROXY_PORT);
...
int fdht_copy_group_array(GroupArray *pDestGroupArray,
    ...
    pDestGroupArray->groups = (ServerArray *)malloc(
        sizeof(ServerArray) * pDestGroupArray->group_count);
    if (pDestGroupArray->groups == NULL) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "malloc %d bytes fail, errno: %d, error info: %s",
            __LINE__, (int)sizeof(ServerArray) * pDestGroupArray->group_count,
            errno, STRERROR(errno));
        return errno != 0 ? errno : ENOMEM;
    }
    ...
    if (pDestGroupArray->servers == NULL) {
        logError(
            ...
            "malloc %d bytes fail, errno: %d, error info: %s",
            __LINE__,
            (int)sizeof(FDHTServerInfo) * pDestGroupArray->server_count, errno,
            STRERROR(errno));
    ...
    for (pServerArray = pDestGroupArray->groups; pServerArray < pArrayEnd;
        ...
        ppServerEnd = pServerArray->servers + pServerArray->count;
        for (ppServerInfo = pServerArray->servers; ppServerInfo < ppServerEnd;
             ppServerInfo++) {
    ...
    for (pServerInfo = pDestGroupArray->servers; pServerInfo < pServerEnd;
         pServerInfo++) {
