...
int main(int argc, char *argv[]) {
    ...
    if ((result = get_base_path_from_conf_file(
             conf_filename, g_fdfs_base_path, sizeof(g_fdfs_base_path))) != 0) {
        log_destroy();
        return result;
    }

    snprintf(pidFilename, sizeof(pidFilename), "%s/data/fdfs_storaged.pid",
             g_fdfs_base_path);
    ...
    if (getExeAbsoluteFilename(argv[0], g_exe_name, sizeof(g_exe_name)) ==
        NULL) {
    ...
    if ((result = storage_func_init(conf_filename, g_bind_addr,
                                    sizeof(g_bind_addr))) != 0) {
        logCrit("exit abnormally!\n");
        log_destroy();
        return result;
    }
    ...
    if ((result = storage_sync_init()) != 0) {
        logCrit(
            "file: "__FILE__
            ", line: %d, "
            "storage_sync_init fail, program exit!",
            __LINE__);
    ...
    if ((result = tracker_report_init()) != 0) {
        logCrit(
            "file: "__FILE__
            ", line: %d, "
            "tracker_report_init fail, program exit!",
            __LINE__);
        g_continue_flag = false;
        return result;
    }
    ...
    if ((result = storage_service_init()) != 0) {
        logCrit(
            "file: "__FILE__
            ", line: %d, "
            "storage_service_init fail, program exit!",
            __LINE__);
        g_continue_flag = false;
        return result;
    }
    ...
    if ((result = set_rand_seed()) != 0) {
        logCrit(
            "file: "__FILE__
            ", line: %d, "
            "set_rand_seed fail, program exit!",
            __LINE__);
        g_continue_flag = false;
        return result;
    }
    ...
    */
    ...
    INIT_SCHEDULE_ENTRY(scheduleEntries[scheduleArray.count],
                        scheduleArray.count + 1, TIME_NONE, TIME_NONE,
                        TIME_NONE, g_sync_log_buff_interval, log_sync_func,
                        &g_log_context);
    scheduleArray.count++;

    INIT_SCHEDULE_ENTRY(scheduleEntries[scheduleArray.count],
                        scheduleArray.count + 1, TIME_NONE, TIME_NONE,
                        TIME_NONE, g_sync_binlog_buff_interval,
                        fdfs_binlog_sync_func, NULL);
    scheduleArray.count++;

    INIT_SCHEDULE_ENTRY(scheduleEntries[scheduleArray.count],
                        scheduleArray.count + 1, TIME_NONE, TIME_NONE,
                        TIME_NONE, g_sync_stat_file_interval,
                        fdfs_stat_file_sync_func, NULL);
    scheduleArray.count++;

    if (g_if_use_trunk_file) {
        INIT_SCHEDULE_ENTRY(scheduleEntries[scheduleArray.count],
                            scheduleArray.count + 1, TIME_NONE, TIME_NONE,
                            TIME_NONE, 1, trunk_binlog_sync_func, NULL);
        scheduleArray.count++;
    }
    ...
    if (g_use_access_log) {
        ...
        if (g_rotate_access_log) {
            INIT_SCHEDULE_ENTRY_EX(scheduleEntries[scheduleArray.count],
                                   ...
                                   g_access_log_rotate_time, 24 * 3600,
                                   log_notify_rotate, &g_access_log_context);
            scheduleArray.count++;

            if (g_log_file_keep_days > 0) {
                log_set_keep_days(&g_access_log_context, g_log_file_keep_days);
    ...
    if ((result =
             sched_start(&scheduleArray, &schedule_tid, g_thread_stack_size,
    ...
    wait_count = 0;
    while (g_storage_thread_count != 0 || g_dio_thread_count != 0 ||
...
static void sigDumpHandler(int sig) {
    ...
    snprintf(filename, sizeof(filename), "%s/logs/storage_dump.log",
             g_fdfs_base_path);
