...
void task_finish_clean_up(struct fast_task_info *pTask) {
    ...
    if (pTask->event.timer.expires > 0) {
        fast_timer_remove(&pTask->thread_data->timer, &pTask->event.timer);
...
static int set_recv_event(struct fast_task_info *pTask) {
    ...
    if (ioevent_modify(&pTask->thread_data->ev_puller, pTask->event.fd,
                       IOEVENT_READ, pTask) != 0) {
        ...
        */
        ...
        switch (pClientInfo->stage) {
            ...
            case FDFS_STORAGE_STAGE_NIO_RECV:
                pTask->offset = 0;
                remain_bytes =
                    pClientInfo->total_length - pClientInfo->total_offset;
                ...
                if (set_recv_event(pTask) == 0) {
                    client_sock_read(pTask->event.fd, IOEVENT_READ, pTask);
                }
                result = 0;
                break;
...
static int storage_nio_init(struct fast_task_info *pTask) {
    ...
    return ioevent_set(pTask, &pThreadData->thread_data, pTask->event.fd,
                       IOEVENT_READ, client_sock_read, g_fdfs_network_timeout);
}
...
static void client_sock_read(int sock, short event, void *arg) {
    ...
    if (pClientInfo->stage != FDFS_STORAGE_STAGE_NIO_RECV) {
        if (event & IOEVENT_TIMEOUT) {
            pTask->event.timer.expires =
                g_current_time + g_fdfs_network_timeout;
            fast_timer_add(&pTask->thread_data->timer, &pTask->event.timer);
        }
    ...
    if (event & IOEVENT_TIMEOUT) {
        if (pClientInfo->total_offset == 0 && pTask->req_count > 0) {
            ...
            logError(
                ...
                __LINE__, pTask->client_ip, pTask->offset, pTask->length);
    ...
    if (event & IOEVENT_ERROR) {
        logDebug(
            ...
            "close connection",
            __LINE__, pTask->client_ip, event);
        ...
        */
        ...
        if (pClientInfo->total_length == 0)  // header
        {
            ...
            pClientInfo->total_length =
                buff2long(((TrackerHeader *)pTask->data)->pkg_len);
            if (pClientInfo->total_length < 0) {
                logError(
                    ...
                    "%" PRId64 " < 0",
                    __LINE__, pTask->client_ip, pClientInfo->total_length);
            ...
            if (pClientInfo->total_offset >= pClientInfo->total_length) {
                if (pClientInfo->total_length == sizeof(TrackerHeader) &&
                    ...
                    logDebug(
                        ...
                        __LINE__, pTask->event.fd, pTask->client_ip);
                    task_finish_clean_up(pTask);
                    return;
                }
