...
typedef struct { TrackerServerGroup *pTrackerGroup; } FDFSConfigInfo;
...
#else
#define fdfs_get_object(obj) \
    (void *)((char *)(Z_OBJ_P(obj)) - XtOffsetOf(php_fdfs_t, zo))
#endif
...
const zend_fcall_info empty_fcall_info = {0, NULL, NULL, NULL, NULL,
                                          0, NULL, NULL, 0};
#undef ZEND_BEGIN_ARG_INFO_EX
#define ZEND_BEGIN_ARG_INFO_EX(name, pass_rest_by_reference, return_reference, \
                               required_num_args)                              \
    static zend_arg_info name[] = {                                            \
        {NULL, 0, NULL, 0, 0, 0, pass_rest_by_reference, return_reference,     \
         required_num_args},
#endif
...
#define CLEAR_HASH_SOCK_FIELD(php_hash)                                        \
    {                                                                          \
        ...
        ZVAL_LONG(sock_zval, -1);                                              \
                                                                               \
        zend_hash_update_wrapper(php_hash, "sock", sizeof("sock"), &sock_zval, \
                                 sizeof(zval *), NULL);                        \
    }

// Every user visible function must have an entry in fastdfs_client_functions[].
zend_function_entry fastdfs_client_functions[] = {
    ZEND_FE(fastdfs_client_version, NULL) ZEND_FE(fastdfs_active_test, NULL)
    ...
    *meta_list = (FDFSMetaData *)malloc(sizeof(FDFSMetaData) * (*meta_count));
    if (*meta_list == NULL) {
        logError(
            ...
            __LINE__, (int)sizeof(FDFSMetaData) * (*meta_count), errno,
            STRERROR(errno));
    ...
    {
#if PHP_MAJOR_VERSION < 7
        if (zend_hash_get_current_key_ex(meta_hash, &szKey, &(key_len), &index,
...
static void php_fdfs_tracker_get_connection_impl(INTERNAL_FUNCTION_PARAMETERS,
    ...
    zend_add_assoc_stringl_ex(return_value, "ip_addr", sizeof("ip_addr"),
                              ...
                              strlen(pTrackerServer->ip_addr), 1);
...
static void php_fdfs_tracker_make_all_connections_impl(
    ...
    pContext->err_no = tracker_get_all_connections_ex(pContext->pTrackerGroup);
...
static void php_fdfs_connect_server_impl(INTERNAL_FUNCTION_PARAMETERS,
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "sl", &ip_addr,
                              &ip_len, &port) == FAILURE) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if ((pContext->err_no = conn_pool_connect_server(
             &server_info, g_fdfs_network_timeout)) == 0) {
        array_init(return_value);
        zend_add_assoc_stringl_ex(return_value, "ip_addr", sizeof("ip_addr"),
                                  ip_addr, ip_len, 1);
        zend_add_assoc_long_ex(return_value, "port", sizeof("port"), port);
        zend_add_assoc_long_ex(return_value, "sock", sizeof("sock"),
                               server_info.sock);
    } else {
        RETURN_BOOL(false);
...
static void php_fdfs_disconnect_server_impl(INTERNAL_FUNCTION_PARAMETERS,
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "a", &server_info) ==
        FAILURE) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if (zend_hash_find_wrapper(tracker_hash, "sock", sizeof("sock"), &data) ==
        FAILURE) {
        pContext->err_no = ENOENT;
        RETURN_BOOL(false);
    }
...
static int php_fdfs_get_callback_from_hash(HashTable *callback_hash,
    ...
    if (zend_hash_find_wrapper(callback_hash, "args", sizeof("args"), &data) ==
        FAILURE) {
        pCallback->args = NULL;
    } else {
...
static int php_fdfs_get_upload_callback_from_hash(
    HashTable *callback_hash, php_fdfs_upload_callback_t *pUploadCallback) {
    zval *data;
    int result;

    if ((result = php_fdfs_get_callback_from_hash(
             callback_hash, &(pUploadCallback->callback))) != 0) {
        return result;
    }
...
static int php_fdfs_get_server_from_hash(HashTable *tracker_hash,
    ...
    if (zend_hash_find_wrapper(tracker_hash, "port", sizeof("port"), &data) ==
        FAILURE) {
        logError(
            "file: "__FILE__
            ", line: %d, "
...
static void php_fastdfs_active_test_impl(INTERNAL_FUNCTION_PARAMETERS,
    ...
    if ((pContext->err_no =
             php_fdfs_get_server_from_hash(tracker_hash, &server)) != 0) {
        RETURN_BOOL(false);
    }
...
static void php_fdfs_tracker_list_groups_impl(INTERNAL_FUNCTION_PARAMETERS,
    ...
    group_nlen = 0;
    tracker_obj = NULL;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|sa", &group_name,
                              &group_nlen, &tracker_obj) == FAILURE) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "zend_parse_parameters fail!",
    ...
    if (tracker_obj == NULL) {
        pTrackerServer =
            tracker_get_connection_no_pool(pContext->pTrackerGroup);
        if (pTrackerServer == NULL) {
            pContext->err_no = ENOENT;
            RETURN_BOOL(false);
        }
    ...
    if (group_name != NULL && group_nlen > 0) {
        group_count = 1;
        result =
            tracker_list_one_group(pTrackerServer, group_name, group_stats);
    } else {
    ...
    for (pGroupStat = group_stats; pGroupStat < pGroupEnd; pGroupStat++) {
        ...
        zend_add_assoc_zval_ex(return_value, pGroupStat->group_name,
                               strlen(pGroupStat->group_name) + 1,
                               group_info_array);
        ...
        result = tracker_list_servers(
            pTrackerServer, pGroupStat->group_name, NULL, storage_infos,
        ...
        for (pStorage = storage_infos; pStorage < pStorageEnd; pStorage++) {
            ...
            zend_add_assoc_stringl_ex(server_info_array, "ip_addr",
                                      sizeof("ip_addr"), pStorage->ip_addr,
                                      strlen(pStorage->ip_addr), 1);

            zend_add_assoc_long_ex(server_info_array, "join_time",
                                   sizeof("join_time"), pStorage->join_time);

            zend_add_assoc_long_ex(server_info_array, "up_time",
                                   sizeof("up_time"), pStorage->up_time);

            zend_add_assoc_stringl_ex(
                server_info_array, "http_domain", sizeof("http_domain"),
                pStorage->domain_name, strlen(pStorage->domain_name), 1);

            zend_add_assoc_stringl_ex(server_info_array, "version",
                                      sizeof("version"), pStorage->version,
                                      strlen(pStorage->version), 1);

            zend_add_assoc_stringl_ex(
                server_info_array, "src_storage_id", sizeof("src_storage_id"),
                pStorage->src_id, strlen(pStorage->src_id), 1);

            zend_add_assoc_bool_ex(server_info_array, "if_trunk_server",
                                   sizeof("if_trunk_server"),
                                   pStorage->if_trunk_server);

            zend_add_assoc_long_ex(server_info_array, "upload_priority",
                                   sizeof("upload_priority"),
                                   pStorage->upload_priority);

            zend_add_assoc_long_ex(server_info_array, "store_path_count",
                                   sizeof("store_path_count"),
                                   pStorage->store_path_count);

            zend_add_assoc_long_ex(server_info_array, "subdir_count_per_path",
                                   sizeof("subdir_count_per_path"),
                                   pStorage->subdir_count_per_path);

            zend_add_assoc_long_ex(server_info_array, "storage_port",
                                   sizeof("storage_port"),
                                   pStorage->storage_port);

            zend_add_assoc_long_ex(server_info_array, "storage_http_port",
                                   sizeof("storage_http_port"),
                                   pStorage->storage_http_port);

            zend_add_assoc_long_ex(server_info_array, "current_write_path",
            ...
            zend_add_assoc_long_ex(server_info_array, "connection.alloc_count",
            ...
            zend_add_assoc_long_ex(server_info_array, "connection.max_count",
                                   sizeof("connection.max_count"),
                                   pStorageStat->connection.max_count);

            zend_add_assoc_long_ex(server_info_array, "total_upload_count",
                                   sizeof("total_upload_count"),
                                   pStorageStat->total_upload_count);

            zend_add_assoc_long_ex(server_info_array, "success_upload_count",
                                   sizeof("success_upload_count"),
                                   pStorageStat->success_upload_count);

            zend_add_assoc_long_ex(server_info_array, "total_append_count",
                                   sizeof("total_append_count"),
                                   pStorageStat->total_append_count);

            zend_add_assoc_long_ex(server_info_array, "success_append_count",
                                   sizeof("success_append_count"),
                                   pStorageStat->success_append_count);

            zend_add_assoc_long_ex(server_info_array, "total_modify_count",
                                   sizeof("total_modify_count"),
                                   pStorageStat->total_modify_count);

            zend_add_assoc_long_ex(server_info_array, "success_modify_count",
                                   sizeof("success_modify_count"),
                                   pStorageStat->success_modify_count);

            zend_add_assoc_long_ex(server_info_array, "total_truncate_count",
                                   sizeof("total_truncate_count"),
                                   pStorageStat->total_truncate_count);

            zend_add_assoc_long_ex(server_info_array, "success_truncate_count",
                                   sizeof("success_truncate_count"),
                                   pStorageStat->success_truncate_count);

            zend_add_assoc_long_ex(server_info_array, "total_set_meta_count",
                                   sizeof("total_set_meta_count"),
                                   pStorageStat->total_set_meta_count);

            zend_add_assoc_long_ex(server_info_array, "success_set_meta_count",
                                   sizeof("success_set_meta_count"),
                                   pStorageStat->success_set_meta_count);

            zend_add_assoc_long_ex(server_info_array, "total_delete_count",
                                   sizeof("total_delete_count"),
                                   pStorageStat->total_delete_count);

            zend_add_assoc_long_ex(server_info_array, "success_delete_count",
                                   sizeof("success_delete_count"),
                                   pStorageStat->success_delete_count);

            zend_add_assoc_long_ex(server_info_array, "total_download_count",
                                   sizeof("total_download_count"),
                                   pStorageStat->total_download_count);

            zend_add_assoc_long_ex(server_info_array, "success_download_count",
                                   sizeof("success_download_count"),
                                   pStorageStat->success_download_count);

            zend_add_assoc_long_ex(server_info_array, "total_get_meta_count",
                                   sizeof("total_get_meta_count"),
                                   pStorageStat->total_get_meta_count);

            zend_add_assoc_long_ex(server_info_array, "success_get_meta_count",
                                   sizeof("success_get_meta_count"),
                                   pStorageStat->success_get_meta_count);

            zend_add_assoc_long_ex(server_info_array, "total_create_link_count",
            ...
            zend_add_assoc_long_ex(server_info_array, "total_delete_link_count",
            ...
            zend_add_assoc_long_ex(server_info_array, "total_upload_bytes",
                                   sizeof("total_upload_bytes"),
                                   pStorageStat->total_upload_bytes);
            zend_add_assoc_long_ex(server_info_array, "success_upload_bytes",
                                   sizeof("success_upload_bytes"),
                                   pStorageStat->success_upload_bytes);
            zend_add_assoc_long_ex(server_info_array, "total_append_bytes",
                                   sizeof("total_append_bytes"),
                                   pStorageStat->total_append_bytes);
            zend_add_assoc_long_ex(server_info_array, "success_append_bytes",
                                   sizeof("success_append_bytes"),
                                   pStorageStat->success_append_bytes);
            zend_add_assoc_long_ex(server_info_array, "total_modify_bytes",
                                   sizeof("total_modify_bytes"),
                                   pStorageStat->total_modify_bytes);
            zend_add_assoc_long_ex(server_info_array, "success_modify_bytes",
                                   sizeof("success_modify_bytes"),
                                   pStorageStat->success_modify_bytes);
            zend_add_assoc_long_ex(server_info_array, "total_download_bytes",
                                   sizeof("total_download_bytes"),
                                   pStorageStat->total_download_bytes);
            zend_add_assoc_long_ex(server_info_array, "success_download_bytes",
                                   sizeof("success_download_bytes"),
                                   pStorageStat->success_download_bytes);
            zend_add_assoc_long_ex(server_info_array, "total_sync_in_bytes",
                                   sizeof("total_sync_in_bytes"),
                                   pStorageStat->total_sync_in_bytes);
            zend_add_assoc_long_ex(server_info_array, "success_sync_in_bytes",
                                   sizeof("success_sync_in_bytes"),
                                   pStorageStat->success_sync_in_bytes);
            zend_add_assoc_long_ex(server_info_array, "total_sync_out_bytes",
                                   sizeof("total_sync_out_bytes"),
                                   pStorageStat->total_sync_out_bytes);
            zend_add_assoc_long_ex(server_info_array, "success_sync_out_bytes",
                                   sizeof("success_sync_out_bytes"),
                                   pStorageStat->success_sync_out_bytes);
            zend_add_assoc_long_ex(server_info_array, "total_file_open_count",
                                   sizeof("total_file_open_count"),
                                   pStorageStat->total_file_open_count);
            zend_add_assoc_long_ex(server_info_array, "success_file_open_count",
                                   sizeof("success_file_open_count"),
                                   pStorageStat->success_file_open_count);
            zend_add_assoc_long_ex(server_info_array, "total_file_read_count",
                                   sizeof("total_file_read_count"),
                                   pStorageStat->total_file_read_count);
            zend_add_assoc_long_ex(server_info_array, "success_file_read_count",
                                   sizeof("success_file_read_count"),
                                   pStorageStat->success_file_read_count);
            zend_add_assoc_long_ex(server_info_array, "total_file_write_count",
            ...
            zend_add_assoc_long_ex(server_info_array, "last_heart_beat_time",
                                   sizeof("last_heart_beat_time"),
                                   pStorageStat->last_heart_beat_time);

            zend_add_assoc_long_ex(server_info_array, "last_source_update",
                                   sizeof("last_source_update"),
                                   pStorageStat->last_source_update);
            zend_add_assoc_long_ex(server_info_array, "last_sync_update",
                                   sizeof("last_sync_update"),
                                   pStorageStat->last_sync_update);
            zend_add_assoc_long_ex(server_info_array, "last_synced_timestamp",
...
static void php_fdfs_tracker_query_storage_store_impl(
    ...
    if (group_name != NULL && group_nlen > 0) {
        ...
        result = tracker_query_storage_store_with_group(
            pTrackerServer, new_group_name, &storage_server, &store_path_index);
    } else {
        *new_group_name = '\0';
        result = tracker_query_storage_store_without_group(
            pTrackerServer, &storage_server, new_group_name, &store_path_index);
    }
    ...
    zend_add_assoc_long_ex(return_value, "store_path_index",
                           sizeof("store_path_index"), store_path_index);
}
...
static void php_fdfs_tracker_query_storage_store_list_impl(
    ...
    if (group_name != NULL && group_nlen > 0) {
        snprintf(new_group_name, sizeof(new_group_name), "%s", group_name);
        result = tracker_query_storage_store_list_with_group(
            pTrackerServer, new_group_name, storage_servers,
            FDFS_MAX_SERVERS_EACH_GROUP, &storage_count, &store_path_index);
    } else {
        *new_group_name = '\0';
        result = tracker_query_storage_store_list_without_group(
            pTrackerServer, storage_servers, FDFS_MAX_SERVERS_EACH_GROUP,
            &storage_count, new_group_name, &store_path_index);
    }
...
static void php_fdfs_tracker_do_query_storage_impl(INTERNAL_FUNCTION_PARAMETERS,
                                                   FDFSPhpContext *pContext,
                                                   const byte cmd,
                                                   const bool bFileId) {
    int argc;
    ...
    if (argc < min_param_count || argc > max_param_count) {
        logError(
            ...
            "count: %d < %d or > %d",
            __LINE__, argc, min_param_count, max_param_count);
        pContext->err_no = EINVAL;
        RETURN_BOOL(false);
    }
    ...
    if (bFileId) {
        ...
        if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|a", &file_id,
                                  &file_id_len, &tracker_obj) == FAILURE) {
            logError(
                "file: "__FILE__
                ", line: %d, "
                "zend_parse_parameters fail!",
        ...
        *pSeperator = '\0';
        ...
        remote_filename = pSeperator + 1;
                                     ...
                                     &filename_len, &tracker_obj) == FAILURE) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "zend_parse_parameters fail!",
    ...
    result = tracker_do_query_storage(pTrackerServer, &storage_server, cmd,
...
static void php_fdfs_tracker_delete_storage_impl(INTERNAL_FUNCTION_PARAMETERS,
                                                 FDFSPhpContext *pContext) {
    int argc;
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss", &group_name,
                              &group_name_len, &storage_ip,
                              &storage_ip_len) == FAILURE) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "zend_parse_parameters fail!",
    ...
    pContext->err_no =
        tracker_delete_storage(pContext->pTrackerGroup, group_name, storage_ip);
    if (pContext->err_no == 0) {
        RETURN_BOOL(true);
    } else {
        RETURN_BOOL(false);
...
static void php_fdfs_storage_delete_file_impl(INTERNAL_FUNCTION_PARAMETERS,
                                              FDFSPhpContext *pContext,
                                              const bool bFileId) {
    int argc;
    char *group_name;
    ...
    if (bFileId) {
        ...
        if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|aa", &file_id,
                                  &file_id_len, &tracker_obj,
                                  &storage_obj) == FAILURE) {
            logError(
                "file: "__FILE__
                ", line: %d, "
    ...
    result = storage_delete_file(pTrackerServer, pStorageServer, group_name,
                                 remote_filename);
    if (tracker_hash != NULL && pTrackerServer->sock != saved_tracker_sock) {
        CLEAR_HASH_SOCK_FIELD(tracker_hash)
    }
    if (pStorageServer != NULL && pStorageServer->sock != saved_storage_sock) {
...
static void php_fdfs_storage_truncate_file_impl(INTERNAL_FUNCTION_PARAMETERS,
                                                FDFSPhpContext *pContext,
                                                const bool bFileId) {
    int argc;
    char *group_name;
    ...
    if (bFileId) {
        ...
        if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|laa", &file_id,
                                  &file_id_len, &truncated_file_size,
        ...
        *pSeperator = '\0';
        group_name = new_file_id;
        remote_filename = pSeperator + 1;
    } else if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss|laa",
                                     &group_name, &group_nlen, &remote_filename,
                                     &filename_len, &truncated_file_size,
                                     &tracker_obj, &storage_obj) == FAILURE) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    result = storage_truncate_file(pTrackerServer, pStorageServer, group_name,
                                   remote_filename, truncated_file_size);
    if (tracker_hash != NULL && pTrackerServer->sock != saved_tracker_sock) {
        CLEAR_HASH_SOCK_FIELD(tracker_hash)
    }
    if (pStorageServer != NULL && pStorageServer->sock != saved_storage_sock) {
...
static void php_fdfs_storage_download_file_to_callback_impl(
    ...
    if (bFileId) {
        ...
        if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "sa|llaa",
                                  &file_id, &file_id_len, &download_callback,
                                  &file_offset, &download_bytes, &tracker_obj,
                                  &storage_obj) == FAILURE) {
            logError(
                "file: "__FILE__
                ", line: %d, "
        ...
        *pSeperator = '\0';
        ...
        remote_filename = pSeperator + 1;
                                     ...
                                     &group_name, &group_nlen, &remote_filename,
                                     &filename_len, &download_callback,
    ...
    result = php_fdfs_get_callback_from_hash(callback_hash, &php_callback);
    if (result != 0) {
        if (tracker_obj == NULL) {
            conn_pool_disconnect_server(pTrackerServer);
        }
    ...
    result = storage_download_file_ex(
        ...
        file_offset, download_bytes, php_fdfs_download_callback,
...
static void php_fdfs_storage_download_file_to_buff_impl(
    ...
    if (bFileId) {
        ...
        if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s|llaa", &file_id,
                                  &file_id_len, &file_offset, &download_bytes,
                                  &tracker_obj, &storage_obj) == FAILURE) {
            logError(
                "file: "__FILE__
                ", line: %d, "
        ...
        *pSeperator = '\0';
        group_name = new_file_id;
        remote_filename = pSeperator + 1;
    } else if (zend_parse_parameters(
                   ZEND_NUM_ARGS() TSRMLS_CC, "ss|llaa", &group_name,
                   &group_nlen, &remote_filename, &filename_len, &file_offset,
    ...
    result = storage_do_download_file_ex(
        pTrackerServer, pStorageServer, FDFS_DOWNLOAD_TO_BUFF, group_name,
        remote_filename, file_offset, download_bytes, &file_buff, NULL,
        &file_size);
    if (tracker_hash != NULL && pTrackerServer->sock != saved_tracker_sock) {
        CLEAR_HASH_SOCK_FIELD(tracker_hash)
    }
...
static void php_fdfs_storage_download_file_to_file_impl(
    ...
    if (bFileId) {
        ...
        if (zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "ss|llaa", &file_id, &file_id_len,
        ...
        *pSeperator = '\0';
        ...
        remote_filename = pSeperator + 1;
                   ...
                   ZEND_NUM_ARGS() TSRMLS_CC, "sss|llaa", &group_name,
                   &group_nlen, &remote_filename, &remote_file_nlen,
                   &local_filename, &local_file_nlen, &file_offset,
                   &download_bytes, &tracker_obj, &storage_obj) == FAILURE) {
        logError(
            "file: "__FILE__
    ...
    result = storage_do_download_file_ex(
        pTrackerServer, pStorageServer, FDFS_DOWNLOAD_TO_FILE, group_name,
        remote_filename, file_offset, download_bytes, &local_filename, NULL,
        &file_size);
    if (tracker_hash != NULL && pTrackerServer->sock != saved_tracker_sock) {
        CLEAR_HASH_SOCK_FIELD(tracker_hash)
    }
...
static void php_fdfs_storage_get_metadata_impl(INTERNAL_FUNCTION_PARAMETERS,
                                               FDFSPhpContext *pContext,
                                               const bool bFileId) {
    int argc;
    char *group_name;
    ...
    result = storage_get_metadata(pTrackerServer, pStorageServer, group_name,
                                  remote_filename, &meta_list, &meta_count);
    if (tracker_hash != NULL && pTrackerServer->sock != saved_tracker_sock) {
        CLEAR_HASH_SOCK_FIELD(tracker_hash)
    }
    if (pStorageServer != NULL && pStorageServer->sock != saved_storage_sock) {
    ...
    if (meta_list != NULL) {
        ...
        for (pMetaData = meta_list; pMetaData < pMetaEnd; pMetaData++) {
            zend_add_assoc_stringl_ex(
                return_value, pMetaData->name, strlen(pMetaData->name) + 1,
                pMetaData->value, strlen(pMetaData->value), 1);
        }
...
static void php_fdfs_storage_file_exist_impl(INTERNAL_FUNCTION_PARAMETERS,
                                             FDFSPhpContext *pContext,
                                             const bool bFileId) {
    int argc;
    char *group_name;
    ...
    result = storage_file_exist(pTrackerServer, pStorageServer, group_name,
                                remote_filename);
    if (tracker_hash != NULL && pTrackerServer->sock != saved_tracker_sock) {
        CLEAR_HASH_SOCK_FIELD(tracker_hash)
    }
...
static void php_fdfs_tracker_query_storage_list_impl(
    ...
    result = tracker_query_storage_list(
        pTrackerServer, storage_servers, FDFS_MAX_SERVERS_EACH_GROUP,
...
static int php_fdfs_upload_callback(void *arg, const int64_t file_size,
                                    int sock) {
    ...
    if (zend_call_user_function_wrapper(EG(function_table), NULL,
        ...
        logError(
            "file: "__FILE__
            ", line: %d, "
            "call callback function: %s fail",
            __LINE__, Z_STRVAL_P(pUploadCallback->callback.func_name));
        return EINVAL;
    }
...
static int php_fdfs_download_callback(void *arg, const int64_t file_size,
    ...
    args[2] = &zdata;
    if (zend_call_user_function_wrapper(EG(function_table), NULL,
                                        pCallback->func_name, &ret, 3,
                                        args TSRMLS_CC) == FAILURE) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "call callback function: %s fail",
            __LINE__, Z_STRVAL_P(pCallback->func_name));
        return EINVAL;
    }
...
static void php_fdfs_storage_upload_file_impl(INTERNAL_FUNCTION_PARAMETERS,
    ...
    if (upload_type == FDFS_UPLOAD_BY_CALLBACK) {
        result = zend_parse_parameters(
            ZEND_NUM_ARGS() TSRMLS_CC, "a|zazaa", &callback_obj, &ext_name_obj,
            &metadata_obj, &group_name_obj, &tracker_obj, &storage_obj);
    } else {
        result = zend_parse_parameters(
            ZEND_NUM_ARGS() TSRMLS_CC, "s|zazaa", &local_filename,
            &filename_len, &ext_name_obj, &metadata_obj, &group_name_obj,
    ...
    *remote_filename = '\0';
    ...
    if (metadata_obj == NULL) {
        ...
        result = fastdfs_convert_metadata_to_array(metadata_obj, &meta_list,
                                                   &meta_count);
        if (result != 0) {
            pContext->err_no = result;
            RETURN_BOOL(false);
    ...
    if (upload_type == FDFS_UPLOAD_BY_FILE) {
        result = storage_upload_by_filename_ex(
            pTrackerServer, pStorageServer, store_path_index, cmd,
            local_filename, file_ext_name, meta_list, meta_count, group_name,
            remote_filename);
        ...
        result = storage_do_upload_file(
            pTrackerServer, pStorageServer, store_path_index, cmd,
            FDFS_UPLOAD_BY_BUFF, buff, NULL, buff_len, NULL, NULL,
    ...
    } else  // by callback
    {
        ...
        result = storage_upload_by_callback_ex(
            pTrackerServer, pStorageServer, store_path_index, cmd,
            php_fdfs_upload_callback, (void *)&php_callback,
            php_callback.file_size, file_ext_name, meta_list, meta_count,
            group_name, remote_filename);
    }
    ...
    if (bFileId) {
        ...
        zend_add_assoc_stringl_ex(return_value, "group_name",
                                  ...
                                  strlen(group_name), 1);
        zend_add_assoc_stringl_ex(return_value, "filename", sizeof("filename"),
                                  remote_filename, strlen(remote_filename), 1);
    }
}
...
static void php_fdfs_storage_upload_slave_file_impl(
    ...
    if (bFileId) {
        ...
        if (upload_type == FDFS_UPLOAD_BY_CALLBACK) {
            result = zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "ass|zaaa", &callback_obj,
                &master_file_id, &master_file_id_len, &prefix_name,
                &prefix_name_len, &ext_name_obj, &metadata_obj, &tracker_obj,
                &storage_obj);
        } else {
            result = zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "sss|zaaa", &local_filename,
                &filename_len, &master_file_id, &master_file_id_len,
                &prefix_name, &prefix_name_len, &ext_name_obj, &metadata_obj,
                &tracker_obj, &storage_obj);
        }
        if (result == FAILURE) {
            logError(
        ...
        *pSeperator = '\0';
        ...
        if (upload_type == FDFS_UPLOAD_BY_CALLBACK) {
            result = zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "asss|zaaa", &callback_obj,
                &group_name, &group_name_len, &master_filename,
                &master_filename_len, &prefix_name, &prefix_name_len,
                &ext_name_obj, &metadata_obj, &tracker_obj, &storage_obj);
        } else {
            result = zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "ssss|zaaa", &local_filename,
                &filename_len, &group_name, &group_name_len, &master_filename,
                &master_filename_len, &prefix_name, &prefix_name_len,
                &ext_name_obj, &metadata_obj, &tracker_obj, &storage_obj);
        }
        if (result == FAILURE) {
    ...
    *remote_filename = '\0';
    ...
    snprintf(new_group_name, sizeof(new_group_name), "%s", group_name);
    if (upload_type == FDFS_UPLOAD_BY_FILE) {
        result = storage_upload_slave_by_filename(
            pTrackerServer, pStorageServer, local_filename, master_filename,
            prefix_name, file_ext_name, meta_list, meta_count, new_group_name,
            remote_filename);
    } else if (upload_type == FDFS_UPLOAD_BY_BUFF) {
        result = storage_upload_slave_by_filebuff(
            pTrackerServer, pStorageServer, local_filename, filename_len,
            master_filename, prefix_name, file_ext_name, meta_list, meta_count,
    ...
    } else  // by callback
    {
        ...
        result = storage_upload_slave_by_callback(
            pTrackerServer, pStorageServer, php_fdfs_upload_callback,
            (void *)&php_callback, php_callback.file_size, master_filename,
            prefix_name, file_ext_name, meta_list, meta_count, new_group_name,
            remote_filename);
    }
...
*/
static void php_fdfs_storage_append_file_impl(INTERNAL_FUNCTION_PARAMETERS,
                                              FDFSPhpContext *pContext,
                                              const byte upload_type,
                                              const bool bFileId) {
    int result;
    ...
    if (bFileId) {
        ...
        if (upload_type == FDFS_UPLOAD_BY_CALLBACK) {
            result = zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "as|aa",
                                           &callback_obj, &appender_file_id,
                                           &appender_file_id_len, &tracker_obj,
                                           &storage_obj);
        } else {
            result = zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "ss|aa", &local_filename,
                &filename_len, &appender_file_id, &appender_file_id_len,
                &tracker_obj, &storage_obj);
        }
        if (result == FAILURE) {
            logError(
        ...
        *pSeperator = '\0';
        ...
        if (upload_type == FDFS_UPLOAD_BY_CALLBACK) {
            result = zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "ass|aa", &callback_obj, &group_name,
                &group_name_len, &appender_filename, &appender_filename_len,
                &tracker_obj, &storage_obj);
        } else {
            result = zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "sss|aa", &local_filename,
                &filename_len, &group_name, &group_name_len, &appender_filename,
                &appender_filename_len, &tracker_obj, &storage_obj);
        }
        if (result == FAILURE) {
            logError(
                "file: "__FILE__
    ...
    if (upload_type == FDFS_UPLOAD_BY_FILE) {
        result = storage_append_by_filename(pTrackerServer, pStorageServer,
                                            local_filename, group_name,
                                            appender_filename);
    } else if (upload_type == FDFS_UPLOAD_BY_BUFF) {
        ...
        result = storage_append_by_callback(
            pTrackerServer, pStorageServer, php_fdfs_upload_callback,
            (void *)&php_callback, php_callback.file_size, group_name,
            appender_filename);
    }
...
return true for success, false for error
*/
static void php_fdfs_storage_modify_file_impl(INTERNAL_FUNCTION_PARAMETERS,
                                              FDFSPhpContext *pContext,
                                              const byte upload_type,
                                              const bool bFileId) {
    int result;
    ...
    if (bFileId) {
        ...
        if (upload_type == FDFS_UPLOAD_BY_CALLBACK) {
            result = zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "als|aa", &callback_obj,
                &file_offset, &appender_file_id, &appender_file_id_len,
                &tracker_obj, &storage_obj);
        } else {
            result = zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "sls|aa", &local_filename,
        ...
        *pSeperator = '\0';
        ...
        if (upload_type == FDFS_UPLOAD_BY_CALLBACK) {
            result = zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "alss|aa", &callback_obj,
                &file_offset, &group_name, &group_name_len, &appender_filename,
                &appender_filename_len, &tracker_obj, &storage_obj);
        } else {
            result = zend_parse_parameters(
                ZEND_NUM_ARGS() TSRMLS_CC, "slss|aa", &local_filename,
                &filename_len, &file_offset, &group_name, &group_name_len,
    ...
    if (upload_type == FDFS_UPLOAD_BY_FILE) {
        result = storage_modify_by_filename(pTrackerServer, pStorageServer,
                                            local_filename, file_offset,
                                            group_name, appender_filename);
    } else if (upload_type == FDFS_UPLOAD_BY_BUFF) {
        result = storage_modify_by_filebuff(
            pTrackerServer, pStorageServer, local_filename, file_offset,
            filename_len, group_name, appender_filename);
    } else {
        HashTable *callback_hash;
        php_fdfs_upload_callback_t php_callback;
        ...
        result = storage_modify_by_callback(
            pTrackerServer, pStorageServer, php_fdfs_upload_callback,
            (void *)&php_callback, file_offset, php_callback.file_size,
            group_name, appender_filename);
    }
...
static void php_fdfs_storage_set_metadata_impl(INTERNAL_FUNCTION_PARAMETERS,
    ...
    if (bFileId) {
        ...
        if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "sa|saa", &file_id,
                                  &file_id_len, &metadata_obj, &op_type_str,
                                  &op_type_len, &tracker_obj,
                                  &storage_obj) == FAILURE) {
            logError(
                "file: "__FILE__
                ", line: %d, "
    ...
    if (op_type_str == NULL) {
        ...
        op_type = STORAGE_SET_METADATA_FLAG_MERGE;
    } else if (TO_UPPERCASE(*op_type_str) ==
        ...
        logError(
            ...
            ", line: %d, "
            "invalid op_type: %s!",
            __LINE__, op_type_str);
        pContext->err_no = EINVAL;
        RETURN_BOOL(false);
    }

    result =
        storage_set_metadata(pTrackerServer, pStorageServer, group_name,
                             remote_filename, meta_list, meta_count, op_type);
    if (tracker_hash != NULL && pTrackerServer->sock != saved_tracker_sock) {
        CLEAR_HASH_SOCK_FIELD(tracker_hash)
    }
    if (pStorageServer != NULL && pStorageServer->sock != saved_storage_sock) {
...
static void php_fdfs_http_gen_token_impl(INTERNAL_FUNCTION_PARAMETERS,
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "sl", &file_id,
                              &file_id_len, &ts) == FAILURE) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "zend_parse_parameters fail!",
    ...
    result =
        fdfs_http_gen_token(&g_anti_steal_secret_key, file_id, (int)ts, token);
    pContext->err_no = result;
    if (result != 0) {
        RETURN_BOOL(false);
    }
...
static void php_fdfs_send_data_impl(INTERNAL_FUNCTION_PARAMETERS,
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ls", &sock, &buff,
                              &buff_len) == FAILURE) {
        logError(
            "file: "__FILE__
            ", line: %d, "
    ...
    if ((pContext->err_no = tcpsenddata_nb(sock, buff, buff_len,
...
static void php_fdfs_get_file_info_impl(INTERNAL_FUNCTION_PARAMETERS,
    ...
    if (bFileId) {
        ...
        if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s", &file_id,
                                  &file_id_len) == FAILURE) {
            logError(
                "file: "__FILE__
                ", line: %d, "
    ...
    result =
        fdfs_get_file_info_ex(group_name, remote_filename, true, &file_info);
    pContext->err_no = result;
    if (result != 0) {
        RETURN_BOOL(false);
    }
    ...
    array_init(return_value);
    zend_add_assoc_long_ex(return_value, "source_id", sizeof("source_id"),
                           file_info.source_id);
    zend_add_assoc_long_ex(return_value, "create_timestamp",
                           sizeof("create_timestamp"),
                           file_info.create_timestamp);
    zend_add_assoc_long_ex(return_value, "file_size", sizeof("file_size"),
                           (long)file_info.file_size);
    zend_add_assoc_stringl_ex(
        return_value, "source_ip_addr", sizeof("source_ip_addr"),
        file_info.source_ip_addr, strlen(file_info.source_ip_addr), 1);
    zend_add_assoc_long_ex(return_value, "crc32", sizeof("crc32"),
                           file_info.crc32);
}
...
static void php_fdfs_gen_slave_filename_impl(INTERNAL_FUNCTION_PARAMETERS,
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss|z",
                              ...
                              &prefix_name, &prefix_name_len,
                              &ext_name_obj) == FAILURE) {
        logError(
            "file: "__FILE__
            ", line: %d, "
            "zend_parse_parameters fail!",
    ...
    result = fdfs_gen_slave_filename(master_filename, prefix_name,
...
ZEND_FUNCTION(fastdfs_tracker_make_all_connections) {
    php_fdfs_tracker_make_all_connections_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                               &php_context);
}
...
*/
ZEND_FUNCTION(fastdfs_get_last_error_no) { RETURN_LONG(php_context.err_no); }
...
ZEND_FUNCTION(fastdfs_client_version) {
    ...
    len = sprintf(szVersion, "%d.%02d", g_fdfs_version.major,
                  g_fdfs_version.minor);
...
ZEND_FUNCTION(fastdfs_tracker_query_storage_store) {
    php_fdfs_tracker_query_storage_store_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                              &php_context);
}
...
ZEND_FUNCTION(fastdfs_tracker_query_storage_list) {
    php_fdfs_tracker_query_storage_list_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                             &php_context, false);
}
...
ZEND_FUNCTION(fastdfs_tracker_delete_storage) {
    php_fdfs_tracker_delete_storage_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                         &php_context);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_by_filename) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_FILE, FDFS_UPLOAD_BY_FILE, false);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_by_filename1) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_FILE, FDFS_UPLOAD_BY_FILE, true);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_by_filebuff) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_FILE, FDFS_UPLOAD_BY_BUFF, false);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_by_filebuff1) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_FILE, FDFS_UPLOAD_BY_BUFF, true);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_by_callback) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_FILE, FDFS_UPLOAD_BY_CALLBACK, false);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_by_callback1) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_FILE, FDFS_UPLOAD_BY_CALLBACK, true);
}
...
ZEND_FUNCTION(fastdfs_storage_append_by_callback) {
    php_fdfs_storage_append_file_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                      &php_context, FDFS_UPLOAD_BY_CALLBACK,
                                      false);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_appender_by_filename) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_APPENDER_FILE, FDFS_UPLOAD_BY_FILE, false);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_appender_by_filename1) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_APPENDER_FILE, FDFS_UPLOAD_BY_FILE, true);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_appender_by_filebuff) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_APPENDER_FILE, FDFS_UPLOAD_BY_BUFF, false);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_appender_by_filebuff1) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_APPENDER_FILE, FDFS_UPLOAD_BY_BUFF, true);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_appender_by_callback) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_APPENDER_FILE, FDFS_UPLOAD_BY_CALLBACK, false);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_appender_by_callback1) {
    php_fdfs_storage_upload_file_impl(
        INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
        STORAGE_PROTO_CMD_UPLOAD_APPENDER_FILE, FDFS_UPLOAD_BY_CALLBACK, true);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_slave_by_filename) {
    php_fdfs_storage_upload_slave_file_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                            &php_context, FDFS_UPLOAD_BY_FILE,
                                            false);
}
...
ZEND_FUNCTION(fastdfs_storage_upload_slave_by_filebuff) {
    php_fdfs_storage_upload_slave_file_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                            &php_context, FDFS_UPLOAD_BY_BUFF,
                                            false);
}
...
ZEND_FUNCTION(fastdfs_storage_set_metadata) {
    php_fdfs_storage_set_metadata_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                       &php_context, false);
}
...
ZEND_FUNCTION(fastdfs_storage_get_metadata) {
    php_fdfs_storage_get_metadata_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                       &php_context, false);
}
...
ZEND_FUNCTION(fastdfs_storage_file_exist) {
    php_fdfs_storage_file_exist_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                     &php_context, false);
}
...
ZEND_FUNCTION(fastdfs_http_gen_token) {
    php_fdfs_http_gen_token_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                 &php_context);
}
...
ZEND_FUNCTION(fastdfs_get_file_info) {
    php_fdfs_get_file_info_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context,
                                false);
}
...
ZEND_FUNCTION(fastdfs_send_data) {
    php_fdfs_send_data_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU, &php_context);
}
...
ZEND_FUNCTION(fastdfs_gen_slave_filename) {
    php_fdfs_gen_slave_filename_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                     &php_context);
}
...
static PHP_METHOD(FastDFS, __construct) {
    ...
    config_index = 0;
    bMultiThread = false;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "|lb", &config_index,
    ...
    if (bMultiThread) {
        i_obj->context.pTrackerGroup =
            (TrackerServerGroup *)emalloc(sizeof(TrackerServerGroup));
        if (i_obj->context.pTrackerGroup == NULL) {
            logError(
                "file: "__FILE__
                ", line: %d, "
...
PHP_METHOD(FastDFS, storage_append_by_filename) {
    ...
    i_obj = (php_fdfs_t *)fdfs_get_object(object);
    php_fdfs_storage_append_file_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                      &(i_obj->context), FDFS_UPLOAD_BY_FILE,
                                      false);
}
...
PHP_METHOD(FastDFS, storage_append_by_filebuff) {
    ...
    i_obj = (php_fdfs_t *)fdfs_get_object(object);
    php_fdfs_storage_append_file_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                      &(i_obj->context), FDFS_UPLOAD_BY_BUFF,
                                      false);
}
...
PHP_METHOD(FastDFS, get_file_info) {
    ...
    i_obj = (php_fdfs_t *)fdfs_get_object(object);
    php_fdfs_get_file_info_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                                &(i_obj->context), false);
}
...
PHP_METHOD(FastDFS, send_data) {
    ...
    i_obj = (php_fdfs_t *)fdfs_get_object(object);
    php_fdfs_send_data_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU,
                            &(i_obj->context));
}
...
static zend_function_entry fdfs_class_methods[] = {
    ...
    FDFS_ME(tracker_close_all_connections,
    ...
    FDFS_ME(tracker_query_storage_store_list,
            arginfo_tracker_query_storage_store_list)
    ...
    FDFS_ME(tracker_query_storage_update1,
    ...
    FDFS_ME(storage_upload_appender_by_filename1,
            arginfo_storage_upload_appender_by_filename1)
    FDFS_ME(storage_upload_appender_by_filebuff,
            arginfo_storage_upload_appender_by_filebuff)
    FDFS_ME(storage_upload_appender_by_filebuff1,
            arginfo_storage_upload_appender_by_filebuff1)
    FDFS_ME(storage_upload_appender_by_callback,
            arginfo_storage_upload_appender_by_callback)
    FDFS_ME(storage_upload_appender_by_callback1,
            arginfo_storage_upload_appender_by_callback1)
    FDFS_ME(storage_upload_slave_by_filename,
            arginfo_storage_upload_slave_by_filename)
    FDFS_ME(storage_upload_slave_by_filename1,
            arginfo_storage_upload_slave_by_filename1)
    FDFS_ME(storage_upload_slave_by_filebuff,
            arginfo_storage_upload_slave_by_filebuff)
    FDFS_ME(storage_upload_slave_by_filebuff1,
            arginfo_storage_upload_slave_by_filebuff1)
    FDFS_ME(storage_upload_slave_by_callback,
            arginfo_storage_upload_slave_by_callback)
    FDFS_ME(storage_upload_slave_by_callback1,
            arginfo_storage_upload_slave_by_callback1)
    ...
    FDFS_ME(storage_truncate_file1, arginfo_storage_truncate_file1) FDFS_ME(
        storage_download_file_to_buff, arginfo_storage_download_file_to_buff)
    FDFS_ME(storage_download_file_to_buff1,
            arginfo_storage_download_file_to_buff1)
    FDFS_ME(storage_download_file_to_file,
            arginfo_storage_download_file_to_file)
    FDFS_ME(storage_download_file_to_file1,
            arginfo_storage_download_file_to_file1)
    FDFS_ME(storage_download_file_to_callback,
            arginfo_storage_download_file_to_callback)
    FDFS_ME(storage_download_file_to_callback1,
            arginfo_storage_download_file_to_callback1)
    ...
    FDFS_ME(close, arginfo_close){NULL, NULL, NULL}};
#undef FDFS_ME
/* }}} */
...
static void php_fdfs_free_storage(zend_object *object) {
    php_fdfs_t *i_obj =
        (php_fdfs_t *)((char *)(object) - XtOffsetOf(php_fdfs_t, zo));
    zend_object_std_dtor(&i_obj->zo TSRMLS_CC);
    php_fdfs_destroy(i_obj TSRMLS_CC);
}
#endif
...
zend_object_value php_fdfs_new(zend_class_entry *ce TSRMLS_DC) {
    ...
    retval.handle = zend_objects_store_put(
        i_obj, (zend_objects_store_dtor_t)zend_objects_destroy_object,
...
zend_object *php_fdfs_new(zend_class_entry *ce) {
    ...
    i_obj = (php_fdfs_t *)ecalloc(
        1, sizeof(php_fdfs_t) + zend_object_properties_size(ce));
...
PHP_FASTDFS_API zend_class_entry *php_fdfs_get_ce(void) { return fdfs_ce; }
...
#define ITEM_NAME_ANTI_STEAL_SECRET_KEY \
    "fastdfs_client.http.anti_steal_secret_key"
#define ITEM_NAME_USE_CONN_POOL "fastdfs_client.use_connection_pool"
#define ITEM_NAME_CONN_POOL_MAX_IDLE_TIME \
    "fastdfs_client.connection_pool_max_idle_time"
    ...
    if (zend_get_configuration_directive_wrapper(ITEM_NAME_BASE_PATH,
        ...
        fprintf(stderr,
                ...
                "\"%s\", set to %s!",
                __LINE__, ITEM_NAME_BASE_PATH, g_fdfs_base_path);
    } else {
    ...
    if (!fileExists(g_fdfs_base_path)) {
        logError("\"%s\" can't be accessed, error info: %s", g_fdfs_base_path,
                 STRERROR(errno));
    ...
    if (zend_get_configuration_directive_wrapper(
            ITEM_NAME_CONNECT_TIMEOUT, sizeof(ITEM_NAME_CONNECT_TIMEOUT),
    ...
    if (zend_get_configuration_directive_wrapper(
            ITEM_NAME_NETWORK_TIMEOUT, sizeof(ITEM_NAME_NETWORK_TIMEOUT),
    ...
    if (zend_get_configuration_directive_wrapper(
            ITEM_NAME_ANTI_STEAL_SECRET_KEY,
    ...
    config_list =
        (FDFSConfigInfo *)malloc(sizeof(FDFSConfigInfo) * config_count);
    if (config_list == NULL) {
        fprintf(stderr,
                "file: "__FILE__
                ", line: %d, "
    ...
    for (pConfigInfo = config_list; pConfigInfo < pConfigEnd; pConfigInfo++) {
        ...
        if (zend_get_configuration_directive_wrapper(
                szItemName, nItemLen + 1, &conf_filename) != SUCCESS) {
            ...
            if (zend_get_configuration_directive_wrapper(
                    ITEM_NAME_CONF_FILE, sizeof(ITEM_NAME_CONF_FILE),
        ...
        if (pConfigInfo == config_list)  // first config file
        {
            ...
            pConfigInfo->pTrackerGroup =
                (TrackerServerGroup *)malloc(sizeof(TrackerServerGroup));
        ...
        if ((result = fdfs_load_tracker_group(
                 pConfigInfo->pTrackerGroup, Z_STRVAL_P(conf_filename))) != 0) {
            return result;
        }
    }

    if (zend_get_configuration_directive_wrapper(
            ITEM_NAME_USE_CONN_POOL, sizeof(ITEM_NAME_USE_CONN_POOL),
            &use_conn_pool) == SUCCESS) {
        ...
        if (strcasecmp(use_conn_pool_str, "yes") == 0 ||
            ...
            if (zend_get_configuration_directive_wrapper(
                ...
                if (g_connection_pool_max_idle_time <= 0) {
                    logError(
                        ...
                        "is invalid!",
                        __LINE__, ITEM_NAME_CONN_POOL_MAX_IDLE_TIME,
                        g_connection_pool_max_idle_time);
                    return EINVAL;
                }
            } else {
            ...
            result = conn_pool_init(&g_connection_pool, g_fdfs_connect_timeout,
    ...
    logDebug(
        "base_path=%s, connect_timeout=%d, network_timeout=%d, "
        ...
        (int)strlen(pAntiStealSecretKey), config_count,
        g_tracker_group.server_count, g_use_connection_pool,
        g_connection_pool_max_idle_time);
...
PHP_MINIT_FUNCTION(fastdfs_client) {
...
#if PHP_MAJOR_VERSION < 7
    fdfs_exception_ce = zend_register_internal_class_ex(
        &ce, php_fdfs_get_exception_base(0 TSRMLS_CC), NULL TSRMLS_CC);
#else
    fdfs_exception_ce = zend_register_internal_class_ex(
        &ce, php_fdfs_get_exception_base(0 TSRMLS_CC));
#endif
    ...
    REGISTER_LONG_CONSTANT("FDFS_STORAGE_STATUS_INIT", FDFS_STORAGE_STATUS_INIT,
                           CONST_CS | CONST_PERSISTENT);
    ...
    REGISTER_LONG_CONSTANT("FDFS_STORAGE_STATUS_ACTIVE",
                           ...
                           CONST_CS | CONST_PERSISTENT);
    REGISTER_LONG_CONSTANT("FDFS_STORAGE_STATUS_NONE", FDFS_STORAGE_STATUS_NONE,
                           CONST_CS | CONST_PERSISTENT);
...
PHP_MSHUTDOWN_FUNCTION(fastdfs_client) {
    ...
    if (config_list != NULL) {
        ...
        for (pConfigInfo = config_list; pConfigInfo < pConfigEnd;
             pConfigInfo++) {
            if (pConfigInfo->pTrackerGroup != NULL) {
                tracker_close_all_connections_ex(pConfigInfo->pTrackerGroup);
            }
        }
    }
...
PHP_RINIT_FUNCTION(fastdfs_client) { return SUCCESS; }

PHP_RSHUTDOWN_FUNCTION(fastdfs_client) {
    fprintf(stderr,
            "request shut down. file: "__FILE__
            ", line: %d\n",
            __LINE__);
    return SUCCESS;
}
